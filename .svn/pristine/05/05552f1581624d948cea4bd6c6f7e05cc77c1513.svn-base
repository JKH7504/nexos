 ...
 *  프로그램ID         : RIM05050E0
 *  프로그램명         : 반입시리얼관리[B2C]
 *  프로그램설명       : 반입시리얼관리[B2C] 화면 Javascript
    ...
    switch (id) {
        ...
        case "DELIVERY_CD":
            $NP.onDeliveryChange(val, {
                ...
                title: $NC.getDisplayMsg("JS.RIM05050E0.028", "온라인몰 검색"),
                columnTitle: [
                    "온라인몰코드",
                    "온라인몰명"
                ],
                errorMessage: $NC.getDisplayMsg("JS.RIM05050E0.029", "등록되어 있지 않은 온라인몰입니다.")
            });
            return;
        case "INBOUND_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIM05050E0.001", "반입일자를 정확히 입력하십시오."));
            break;
        case "INBOUND_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIM05050E0.002", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "INBOUND_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIM05050E0.003", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function onInquiry(isSave) {
    ...
    if (isSave !== true && $NC.isGridModified(G_GRDT1SUB)) {
        if (!confirm($NC.getDisplayMsg("JS.RIM05050E0.004", "반입 시리얼번호 등록 작업 중 입니다.\n재조회 처리하시겠습니까?\n\n※ 현재 작업 중인 데이터는 저장되지 않습니다."))) {
            setFocusScan();
            return;
        }
    }
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.RIM05050E0.005", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.RIM05050E0.006", "사업부코드를 입력하십시오."));
    ...
    if ($NC.getTabActiveIndex("#ctrMasterView") == 0) {
        ...
        if ($NC.isNull(INBOUND_DATE)) {
            alert($NC.getDisplayMsg("JS.RIM05050E0.007", "반입일자를 입력하십시오."));
        ...
        // 데이터 조회
        $NC.serviceCall("/RIM05050E0/getDataSet.do", $NC.getGridParams(G_GRDT1MASTER), onGetT1Master);
    ...
    // 반입시리얼 내역 탭
        ...
        if ($NC.isNull(INBOUND_DATE1)) {
            alert($NC.getDisplayMsg("JS.RIM05050E0.008", "시작일자를 입력하십시오."));
        ...
        if ($NC.isNull(INBOUND_DATE2)) {
            alert($NC.getDisplayMsg("JS.RIM05050E0.009", "종료일자를 입력하십시오."));
            $NC.setFocus("#dtpQInbound_Date2");
            return;
        }

        if (INBOUND_DATE1 > INBOUND_DATE2) {
            alert($NC.getDisplayMsg("JS.RIM05050E0.010", "반입일자 검색 범위 오류입니다."));
            $NC.setFocus("#dtpQInbound_Date1");
            return;
        }
        ...
        // 데이터 조회
        $NC.serviceCall("/RIM05050E0/getDataSet.do", $NC.getGridParams(G_GRDT2MASTER), onGetT2Master);
    }
}
...
function _Save() {
    ...
    if (G_GRDT1DETAIL.data.getLength() == 0 || $NC.isNull(G_GRDT1DETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.RIM05050E0.011", "시리얼번호 저장할 상품을 먼저 선택하십시오."));
    ...
    if (G_GRDT1SUB.data.getItems().length == 0) {
        alert($NC.getDisplayMsg("JS.RIM05050E0.012", "저장할 데이터가 없습니다."));
        setFocusScan();
        return;
    }
    ...
    if (G_GRDT1SUB.data.getLength() > 0) {
        ...
        if (refRowData.CONFIRM_QTY < SUM_CONFIRM_QTY) {
            alert($NC.getDisplayMsg("JS.RIM05050E0.013", "반입수량을 초과해서 시리얼번호를 등록할 수 없습니다."));
            setFocusScan();
            return;
        }
    }
    ...
    $NC.serviceCall("/RIM05050E0/save.do", {
        P_MASTER_PARAMS: {
...
function _Delete() {
    ...
    if (G_GRDT1SUB.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.RIM05050E0.014", "삭제할 데이터가 없습니다."));
        setFocusScan();
        return;
    }
...
function tabOnBeforeActivate(event, ui) {
    ...
    if ($NC.isGridModified(G_GRDT1SUB)) {
        alert($NC.getDisplayMsg("JS.RIM05050E0.015", "반입 시리얼번호 등록 작업 중 입니다.\n\n작업완료(저장) 후 이동하십시오."));
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "RIM05050E0.RS_T1_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options,
        onBeforeSort: function(args) {
            if ($NC.isGridModified(G_GRDT1SUB)) {
                alert($NC.getDisplayMsg("JS.RIM05050E0.016", "반입 시리얼번호 등록 작업 중 입니다.\n\n작업완료(저장) 후 정렬하십시오."));
                return false;
            }
            return true;
        }
...
function grdT1MasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIM05050E0/getDataSet.do", $NC.getGridParams(G_GRDT1DETAIL), onGetT1Detail);
...
function grdT1DetailInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Detail", {
        ...
        queryId: "RIM05050E0.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options,
        onBeforeSort: function(args) {
            if ($NC.isGridModified(G_GRDT1SUB)) {
...
function grdT1DetailOnAfterScroll(e, args) {
    ...
    if (rowData.SERIAL_IN_YN == $ND.C_NO) {
        ...
        // 데이터 조회
        $NC.serviceCall("/RIM05050E0/getDataSet.do", $NC.getGridParams(G_GRDT1SUB), onGetT1Sub);
    }
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "RIM05050E0.RS_T2_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options,
...
function showDeliveryPopup() {
    ...
    $NP.showDeliveryPopup({
        title: $NC.getDisplayMsg("JS.LOM05050E0.028", "온라인몰 검색"),
        columnTitle: [
            "온라인몰코드",
            "온라인몰명"
        ],
...
function getInboundState(params, onSuccess) {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIM05050E0/getData.do", {
...
function onScanItem($view, scanVal) {
    ...
    if (G_GRDT1DETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.RIM05050E0.017", "데이터 조회 후 처리하십시오."));
        setFocusScan($view);
        return;
    }
    ...
    if (searchRow == -1) {
        alert($NC.getDisplayMsg("JS.RIM05050E0.018", "해당 전표에 존재하지 않는 상품입니다."));
        setFocusScan($view);
        return;
    }
    ...
    if (rowData.SERIAL_IN_YN != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.RIM05050E0.019", "시리얼번호 입력 대상 상품이 아닙니다."));
        setFocusScan($view);
        return;
    }
...
function onScanSerial($view, scanVal) {
    ...
    if (G_GRDT1DETAIL.data.getLength() == 0 || $NC.isNull(G_GRDT1DETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.RIM05050E0.020", "시리얼번호를 입력할 상품을 먼저 선택하십시오."));
        setFocusScan("#edtItem_Bar_Cd");
        return;
    }
    ...
    if (rowCount > 0) {
        ...
        if (refRowData.CONFIRM_QTY <= SUM_CONFIRM_QTY) {
            alert($NC.getDisplayMsg("JS.RIM05050E0.021", "반입수량과 동일하게 시리얼번호가 등록되었습니다.\n\n다른 시리얼번호를 등록하려면 삭제 후 등록하십시오."));
            setFocusScan($view);
            return;
        }
    }
    ...
    if (searchRow != -1) {
        ...
        alert($NC.getDisplayMsg("JS.RIM05050E0.022", "이미 입력된 시리얼 번호 입니다."));
        setFocusScan($view);
        return;
    }
    ...
    $NC.serviceCallAndWait("/RIM05050E0/getDataSet.do", {
        P_QUERY_ID: "RIM05050E0.RS_T1_REF1",
        P_QUERY_PARAMS: {
            P_CENTER_CD: refRowData.CENTER_CD,
            P_BU_CD: refRowData.BU_CD,
            P_INBOUND_DATE: refRowData.INBOUND_DATE,
    ...
    if ((dsSerial || []).length == 0) {
        alert($NC.getDisplayMsg("JS.RIM05050E0.023", "시리얼 입출고 정보를 가져오지 못했습니다."));
        setFocusScan($view);
        return;
    }
    rowData = dsSerial[0] || {};
    ...
    if (rowData.OUTBOUND_QTY == 0) {
        if (!confirm($NC.getDisplayMsg("JS.RIM05050E0.024", "[시리얼번호: " + ITEM_SERIAL + "] 출고이력이 없는 시리얼번호입니다.\n\n해당 시리얼번호를 추가하시겠습니까?", ITEM_SERIAL))) {
            setFocusScan($view);
            return;
        }
    }
...
function showSerialHistoryOverlay(e) {
    ...
    if (G_GRDT1DETAIL.data.getLength() == 0 || $NC.isNull(G_GRDT1DETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.RIM05050E0.025", "시리얼 입출고 이력 조회할 상품을 선택하십시오."));
        return;
    }
    ...
    $NC.showOverlay("#ctrSerialHistoryOverlay", {
        title: $NC.getDisplayMsg("JS.RIM05050E0.026", "시리얼 입출고 이력"),
        fullscreen: false,
        width: 650,
        height: 400,
        onComplete: function() {
...
function onOverlayScanSerial($view, scanVal) {
    ...
    // 이력 데이터 조회
    $NC.serviceCallAndWait("/RIM05050E0/getDataSet.do", $NC.getGridParams(window.G_GRDSERIALHISTORY), function(ajaxData) {
        $NC.setInitGridData(window.G_GRDSERIALHISTORY, ajaxData);
        if (!$NC.setInitGridAfterOpen(window.G_GRDSERIALHISTORY)) {
            alert($NC.getDisplayMsg("JS.RIM05050E0.027", "시리얼 입출고 이력이 존재하지 않습니다."));
        }
        $NC.setFocus("#edtL1_Item_Serial");
    }, function(ajaxData) {
        $NC.onError(ajaxData);
