 ...
 *  프로그램ID         : LCC02021P1
 *  프로그램명         : 상품변환등록 팝업 (의류)
 *  프로그램설명       : 상품변환등록 팝업 (의류) 화면 Javascript
    ...
    switch (id) {
        ...
        case "ZONE_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM121)) {
                alert($NC.getDisplayMsg("JS.LCC02021P1.001", "로케이션 존코드 길이(" + $NC.G_VAR.policyVal.CM121 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BANK_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM122)) {
                alert($NC.getDisplayMsg("JS.LCC02021P1.002", "로케이션 행코드 길이(" + $NC.G_VAR.policyVal.CM122 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BAY_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM123)) {
                alert($NC.getDisplayMsg("JS.LCC02021P1.003", "로케이션 열코드 길이(" + $NC.G_VAR.policyVal.CM123 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "LEV_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM124)) {
                alert($NC.getDisplayMsg("JS.LCC02021P1.004", "로케이션 단코드 길이(" + $NC.G_VAR.policyVal.CM124 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "STOCK_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02021P1.005", "입고일자를 정확히 입력하십시오."));
            }
            break;
        case "VALID_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02021P1.006", "유통기한를 정확히 입력하십시오."));
            }
            break;
    }
}
...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/LCC02020E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
}
...
function _New() {
    ...
    if (checkedRows.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.008", "추가할 재고내역 데이터를 선택하십시오."));
        return;
    }
    ...
    try {
        for (var rIndex = 0, rCount = checkedRows.length; rIndex < rCount; rIndex++) {
            ...
            newRowData = {
                ...
                LINK_ITEM_SPEC: "",
                LINK_ITEM_LOT: $ND.C_BASE_ITEM_LOT,
                LINK_QTY_IN_BOX: 1,
    ...
    if (addedCount == 0) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.009", "선택한 상품은 이미 추가된 재고내역입니다."));
        return;
    }
...
function _Save() {
    ...
    if (G_GRDSUB.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.010", "저장할 데이터가 없습니다."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.011", "물류센터를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.012", "사업부를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.013", "먼저 입출고구분을 선택하십시오."));
        $NC.setFocus("#cboInout_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ETC_DATE)) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.014", "먼저 입출고일자를 입력하십시오."));
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_C) {
        if (dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC02021P1.015", "저장할 지시내역 정보가 없습니다."));
            return;
        }
    } else {
        if ($NC.G_VAR.masterData.CRUD != $ND.C_DV_CRUD_U && dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC02021P1.016", "수정 후 저장하십시오."));
            return;
        }
    }
...
function _Delete() {
    ...
    if (G_GRDSUB.data.getLength() == 0 || $NC.isNull(G_GRDSUB.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.017", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        case "LOCATION_CD":
            ...
            return;
        case "ETC_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LCC02021P1.018", "입출고일자를 정확히 입력하십시오."));
        ...
        case "LINK_VALID_DATE":
            ...
            if ($NC.isNotNull(rowData.LINK_VALID_DATE)) {
                if (!$NC.isDate(rowData.LINK_VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.LCC02021P1.019", "사용기한을 정확히 입력하십시오."));
...
function validateSubData(row, rowData) {
    ...
    if ($NC.isNull(rowData.CONFIRM_QTY) || Number(rowData.CONFIRM_QTY) <= 0) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.020", "변환수량에 0보다 큰 수량을 입력하십시오."));
    ...
    if (rowData.CRUD == $ND.C_DV_CRUD_C && Number(rowData.PSTOCK_QTY) < Number(rowData.CONFIRM_QTY)) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.021", "재고수량 보다 많은 수량을 입력하실 수 없습니다."));
        $NC.setFocusGrid(G_GRDSUB, row, "CONFIRM_QTY", true);
        return false;
    }
    if (rowData.CRUD == $ND.C_DV_CRUD_C && $NC.isNull(rowData.LINK_LOCATION_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.022", "로케이션을 입력하십시오."));
        $NC.setFocusGrid(G_GRDSUB, row, "LINK_LOCATION_CD", true);
        return false;
    }
    if (rowData.CRUD == $ND.C_DV_CRUD_C && $NC.isNull(rowData.LINK_ITEM_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.023", "변환상품을 입력하십시오."));
        $NC.setFocusGrid(G_GRDSUB, row, "LINK_ITEM_CD", true);
        return false;
    }
    if (rowData.CRUD == $ND.C_DV_CRUD_C && $NC.isNull(rowData.LINK_ITEM_LOT)) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.024", "변환상품의 LOT번호를 입력하십시오."));
        $NC.setFocusGrid(G_GRDSUB, row, "LINK_ITEM_LOT", true);
        return false;
    }

    if ($NC.isNull(rowData.ETC_DIV)) {
        alert($NC.getDisplayMsg("JS.LCC02021P1.025", "사유구분을 선택하십시오."));
