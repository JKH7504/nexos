...
function onProcessPreBT() {
    ...
    if ($NC.isNotNull(DELIVERY_BATCH_F)) {
        if (!confirm($NC.getDisplayMsg("JS.LOM02010E1.040", "출고등록 일괄 취소 처리하시겠습니까?\n운송차수(" + DELIVERY_BATCH_F + ")", DELIVERY_BATCH_F))) {
            return;
        }
    } else {
        if (!confirm($NC.getDisplayMsg("JS.LOM02010E1.041", "출고등록 일괄 취소 처리하시겠습니까?"))) {
            return;
        }
    }
...
function onProcessNxtBT() {
    ...
    if (G_GRDMASTERBT.data.getItems().length == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.042", "일괄등록 처리할 대상이 없습니다."));
        return;
    }
    if ($NC.G_VAR.PROCESS_YN != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.027", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록 후 처리하십시오."));
    ...
    if ($NC.isNotNull(DELIVERY_BATCH_F)) {
        if (!confirm($NC.getDisplayMsg("JS.LOM02010E1.045", "출고등록 일괄처리하시겠습니까?\n운송차수(" + DELIVERY_BATCH_F + ")", DELIVERY_BATCH_F))) {
            return;
        }
    } else {
        if (!confirm($NC.getDisplayMsg("JS.LOM02010E1.046", "출고등록 일괄처리하시겠습니까?"))) {
            return;
        }
    }
    ...
    // 개별등록
    var queryParams = {
        P_QUERY_ID: "LOM02010E1.RS_T1B_SUB1",
        P_QUERY_PARAMS: {
            P_CENTER_CD: CENTER_CD,
            P_BU_CD: BU_CD,
    ...
    if (dsResult.length == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E1_BT.XXX", "출고등록 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterBTInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterBT", {
        ...
        queryId: "LOM02010E1.RS_T1B_MASTER",
        sortCol: "ITEM_CD",
        gridOptions: options,
...
function grdDetailBTInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailBT", {
        ...
        queryId: "LOM02010E1.RS_T1B_DETAIL",
        sortCol: "ORDER_DATE",
        gridOptions: options
    });
    ...
    switch (G_GRDDETAILBT.view.getColumnId(args.cell)) {
        case "ADJUST_QTY":
            ...
            if (Number(rowData.ORDER_QTY) < Number(rowData.ADJUST_QTY)) {
                alert($NC.getDisplayMsg("JS.LOM02010E1.047", "조정수량이 예정수량보다 클 수 없습니다."));
                ...
                alert($NC.getDisplayMsg("JS.LOM02010E1.048", "출고가능량을 초과해서 조정할 수 없습니다."));
...
function grdDetailBTOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        ...
        if (ORDER_QTY > ADJUST_QTY && $NC.isNull(rowData.SHORTAGE_DIV)) {
            alert($NC.getDisplayMsg("JS.LOM02010E1.049", "미출고사유를 선택하십시오."));
...
function setAdjustQtyInfo(withPStock) {
    ...
    $NC.serviceCallAndWait("/LOM02010E0/getData.do", {
        ...
        if (oMsg != $ND.C_OK) {
            alert($NC.getDisplayMsg("JS.LOM02010E1.050", "출고가능량 정보를 가져오지 못했습니다.\n데이터를 다시 선택하십시오."));
...
function onEntryBatchSuccess(ajaxData) {
    ...
    alert($NC.getDisplayMsg("JS.LOM02010E1.051", "정상적으로 처리 되었습니다."));
    window.setMasterSummaryInfo();
    window._Inquiry();
}
...
function showOutwaitOverlay(e) {
    ...
    if (G_GRDMASTERBT.data.getLength() == 0 || $NC.isNull(G_GRDMASTERBT.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.052", "출고대기량을 조회할 상품을 선택하십시오."));
        return;
    }
    ...
    if (G_GRDOUTWAIT.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.053", "출고대기 내역이 존재하지 않습니다."));
        return;
    }

    $NC.showOverlay("#divOutwaitOverlay", {
        title: $NC.getDisplayMsg("JS.LOM02010E1.054", "출고대기 내역"),
        fullscreen: false,
        width: 700,
        height: 300,
        onComplete: function() {
