...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BU_CD":
            ...
            return;
        case "OUTBOUND_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB08020Q1.001", "검색 시작일자를 정확히 입력하십시오."));
            setOutboundBatchCombo();
            break;
        case "OUTBOUND_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB08020Q1.002", "검색 종료일자를 정확히 입력하십시오."));
            setOutboundBatchCombo();
            break;
        case "RDELIVERY_CD":
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOB08020Q1.003", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(OUTBOUND_DATE1)) {
        alert($NC.getDisplayMsg("JS.LOB08020Q1.004", "검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQOutbound_Date1");
        return;
    }
    var OUTBOUND_DATE2 = $NC.getValue("#dtpQOutbound_Date2");
    if ($NC.isNull(OUTBOUND_DATE2)) {
        alert($NC.getDisplayMsg("JS.LOB08020Q1.005", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQOutbound_Date2");
        return;
    }
    if (OUTBOUND_DATE1 > OUTBOUND_DATE2) {
        alert($NC.getDisplayMsg("JS.LOB08020Q1.006", "출고일자 검색 범위 오류입니다."));
...
function _Print(reportInfo) {
    ...
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LOB08020Q1.007", "사업부를 입력하십시오."));
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "LOB08020Q1.RS_T1_MASTER",
...
function grdT1DetailInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Detail", {
        ...
        queryId: "LOB08020Q1.RS_T1_DETAIL",
        sortCol: "ITEM_CD",
        gridOptions: options
    });
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "LOB08020Q1.RS_T2_MASTER",
        sortCol: "WB_NO",
        gridOptions: options
    });
...
function btnWBDeliveryInfoOnClick() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOB08020Q1.008", "조회 후 처리하십시오."));
        return;
    }

    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.SEND_STATE != "50") {
        alert($NC.getDisplayMsg("JS.LOB08020Q1.009", "아직 택배사로 송신되지 않은 운송장입니다. 배송조회할 수 없습니다."));
        return;
    }

    if ($NC.isNull(rowData.WB_NO)) {
        alert($NC.getDisplayMsg("JS.LOB08020Q1.010", "운송장번호가 없는 데이터입니다. 배송조회할 수 없습니다."));
        return;
    }

    $NC.showProgramSubPopup({
        PROGRAM_ID: "LOB08021P1",
        PROGRAM_NM: $NC.getDisplayMsg("JS.LOB08021P1.001", "택배사 배송조회"),
        url: "lo/LOB08021P1.html",
        width: 1100,
        height: 600,
        G_PARAMETER: {
            P_CENTER_CD: rowData.CENTER_CD,
...
function setUserProgramPermission() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        $NC.setEnable("#btnWBDeliveryInfo", G_GRDT1MASTER.data.getLength() > 0);
    }
}
