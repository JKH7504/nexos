...
function _New() {
    ...
    if ($NC.isNull(PROCESSING_QTY) || PROCESSING_QTY < 0) {
        alert($NC.getDisplayMsg("JS.LCC02091P0.018", "먼저 가공작업수량에 0이상의 값을 입력하십시오."));
...
function _Save() {
    ...
    if ($NC.isNull($NC.getValue("#edtProcessing_Qty")) || Number($NC.getValue("#edtProcessing_Qty")) < 0) {
        alert($NC.getDisplayMsg("JS.LCC02091P0.009", "가공작업수량에 0 이상의 숫자를 입력하십시오."));
        $NC.setFocus("#edtProcessing_Qty");
        return;
    }

    if ($NC.isNull($NC.getValue("#edtPro_Confirm_Qty")) || Number($NC.getValue("#edtPro_Confirm_Qty")) < 0) {
        alert($NC.getDisplayMsg("JS.LCC02091P0.019", "가공작업확정수량에 0 이상의 숫자를 입력하십시오."));
        $NC.setFocus("#edtPro_Confirm_Qty");
        return;
    }
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        ...
        case "MAT_CONFIRM_QTY":
            if (Number(rowData.MAT_CONFIRM_QTY) < 0) {
                alert($NC.getDisplayMsg("JS.LCC02091P0.020", "확정수량은 0보다 작을 수 없습니다."));
...
function grdDetailOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        ...
        if ($NC.isNull(rowData.MAT_CONFIRM_QTY)) {
            alert($NC.getDisplayMsg("JS.LCC02091P0.021", "자재확정수량을 입력하십시오."));
