...
function _Initialize() {
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.ROC01011P1.001", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.ROC01011P1.002", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.ROC01011P1.003", "저장"));
}
...
function _New() {
    ...
    if ($NC.isNull(VENDOR_CD)) {
        alert($NC.getDisplayMsg("JS.ROC01011P1.004", "먼저 공급처 코드를 입력하십시오."));
        $NC.setFocus("#edtVendor_Cd");
        return;
    }
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.ROC01011P1.006", "물류센터가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.ROC01011P1.007", "사업부가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDER_DATE)) {
        alert($NC.getDisplayMsg("JS.ROC01011P1.008", "먼저 예정일자를 입력하십시오."));
        $NC.setFocus("#dtpOrder_Date");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.VENDOR_CD)) {
        alert($NC.getDisplayMsg("JS.ROC01011P1.009", "먼저 공급처 코드를 입력하십시오."));
        $NC.setFocus("#edtDelivery_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.ROC01011P1.010", "먼저 반출구분을 선택하십시오."));
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.ROC01011P1.005", "저장할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.ROC01011P1.011", "저장하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.ROC01011P1.012", "수정 후 저장하십시오."));
        return;
    }
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.ROC01011P1.013", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        case "INOUT_CD":
            ...
            break;
        case "ORDER_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.ROC01011P1.014", "예정일자를 정확히 입력하십시오."));
...
function grdDetailOnGetColumns() {
    ...
    // 정책에 따른 컬럼 표시
    if ($NC.G_VAR.G_PARAMETER.P_POLICY_LS250 == "2" && $NC.G_VAR.G_PARAMETER.P_POLICY_RO250 == "2") {
        $NC.setGridColumn(columns, {
            id: "VALID_DATE",
            field: "VALID_DATE",
            name: "유통기한",
...
function grdDetailOnCellChange(e, args) {
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        ...
        case "ORDER_QTY":
            if (Number(rowData.ORDER_QTY) < 1) {
                alert($NC.getDisplayMsg("JS.ROC01011P1.015", "예정수량은 1보다 작을 수 없습니다."));
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.ROC01011P1.016", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                } else {
...
function grdDetailOnCheckPStock(row, isCheckOnly) {
    ...
    $NC.serviceCallAndWait("/ROC01010E0/getData.do", {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.ROC01011P1.022", "출고가능량 정보를 가져오지 못했습니다."));
            return;
        }
        var oMsg = $NC.getOutMessage(resultData);
        if (oMsg != $ND.C_OK) {
...
function btnStockAddItemOnClick() {
    ...
    if (dsStock.length == 0) {
        alert($NC.getDisplayMsg("JS.ROC01011P0.023", "예정등록할 재고를 선택하십시오."));
        return;
    }
