...
function _Print(reportInfo) {
    ...
    switch ($NC.G_VAR.activeView.PROCESS_CD) {
        ...
        case $ND.C_PROCESS_DELIVERY:
            ...
            // PAPER_LOM03 - 토탈피킹지시서
            // PAPER_LOM04 - 분배지시서
            // PAPER_LOM05 - 토탈지시서(원패킹)
            // PAPER_LOM08 - 토탈지시서(원패킹제외)
            if (reportInfo.REPORT_CD == "PAPER_LOM01" //
                || reportInfo.REPORT_CD == "PAPER_LOM03" //
                || reportInfo.REPORT_CD == "PAPER_LOM04" //
                || reportInfo.REPORT_CD == "PAPER_LOM05" //
                || reportInfo.REPORT_CD == "PAPER_LOM08") {
...
function setReportInfo() {
    ...
    $NC.setProgramReportInfo({
        ...
        if ($NC.G_VAR.activeView.PROCESS_CD == $ND.C_PROCESS_ENTRY) {
            ...
            // PAPER_LOM05 - 토탈지시서(원패킹)
            // PAPER_LOM08 - 토탈지시서(원패킹제외)
            return reportInfo.REPORT_CD != "PAPER_LOM01" //
                && reportInfo.REPORT_CD != "PAPER_LOM03" //
                && reportInfo.REPORT_CD != "PAPER_LOM04" //
                && reportInfo.REPORT_CD != "PAPER_LOM05" //
                && reportInfo.REPORT_CD != "PAPER_LOM08";
...
function setErpBatchCombo(processCd) {
    ...
    $NC.setInitCombo("/LOM02010E0/getDataSet.do", {
        P_QUERY_ID: "LOM02010E0.RS_SUB5",
        P_QUERY_PARAMS: {
            P_CENTER_CD: $NC.getValue("#cboQCenter_Cd"),
            P_BU_CD: $NC.getValue("#edtQBu_Cd"),
            P_ORDER_DATE1: ORDER_DATE1,
