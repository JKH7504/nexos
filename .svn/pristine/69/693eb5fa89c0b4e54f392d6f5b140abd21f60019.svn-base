...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOM04050E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
}
...
function _Save() {
    ...
    if (OUTBOUND_BATCH == $NC.G_VAR.G_PARAMETER.P_OUTBOUND_BATCH) {// 작업차수와 결품차수가 동일할때
        alert($NC.getDisplayMsg("JS.LOM04051P0.002", "현재 작업중인 차수와 동일한 차수 입니다."));
        return;
    }

    $NC.serviceCall("/LOM04050E0/callLOProcDistributeReBatch.do", {
        P_CENTER_CD: $NC.G_VAR.G_PARAMETER.P_CENTER_CD,
        P_BU_CD: $NC.G_VAR.G_PARAMETER.P_BU_CD,
        P_OUTBOUND_DATE: $NC.G_VAR.G_PARAMETER.P_OUTBOUND_DATE,
        P_OUTBOUND_BATCH: $NC.G_VAR.G_PARAMETER.P_OUTBOUND_BATCH,
...
function onSave(ajaxData) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOM04050E0/getDataSet.do", $NC.getGridParams(G_GRDSUB), onGetSub);
...
function setOrderCnt() {
    // 데이터 조회
    $NC.serviceCallAndWait("/LOM04050E0/getData.do", {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LOM04051P0.003", "작업이 완료된 차수 입니다."));
            return;
        }
...
function btnPrintOnClick() {
    var reportIndex = $NC.getSearchArray($NC.G_VAR.printOptions, {
        searchKey: "REPORT_CD",
        searchVal: "PAPER_LOM09" // PAPER_LOM09 - 선반번호변경지시서(결품차수)
    });
    if (reportIndex == -1) {
        alert($NC.getDisplayMsg("JS.LOM04051P0.XXX", "출력할 레포트 정보를 검색할 수 없습니다. 관리자에게 문의하십시오."));
        return;
    }
    var reportInfo = $NC.G_VAR.printOptions[reportIndex];

    if (G_GRDSUB.data.getLength() == 0 || $NC.isNull(G_GRDSUB.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOM04051P0.XXX", "출력 할 데이터가 없습니다."));
        return;
    }
    var rowData = G_GRDSUB.data.getItem(G_GRDSUB.lastRow);

    if (!confirm($NC.getDisplayMsg("JS.LOM04051P0.XXX", "출력 하시겠습니까?"))) {
        return;
    }
