 ...
 *  프로그램ID         : CMC08021P0
 *  프로그램명         : 포장박스복사 팝업
 *  프로그램설명       : 포장박스복사 팝업 화면 Javascript
...
function _Save() {
    ...
    if ($NC.isNull(FROM_BU_CD)) {
        alert($NC.getDisplayMsg("JS.CMC08021P0.002", "기준 사업부를 입력하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.CMC08021P0.003", "대상 사업부를 입력하십시오."));
        $NC.setFocus("#edtBu_Cd");
        return;
    }

    if (FROM_BU_CD == BU_CD) {
        alert($NC.getDisplayMsg("JS.CMC08021P0.004", "동일 사업부로 포장박스정보를 복사할 수 없습니다. \n다시 입력하십시오."));
        $NC.setFocus("#edtBu_Cd");
        return;
    }

    $NC.serviceCall("/CMC08020E0/callBoxCopy.do", {
