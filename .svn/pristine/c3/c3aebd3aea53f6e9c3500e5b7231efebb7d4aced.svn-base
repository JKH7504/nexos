...
function _Initialize() {
    ...
    $NC.setEnable("#btnCopyBuPolicy", false);

    $("#btnQBu_Cd").click(showUserBuPopup);
    $("#btnCopyBuPolicy").click(btnCopyBuPolicyOnClick);
        ...
        case "SELECT_YN":
            ...
            if (refRowData.SELECT_YN == $ND.C_YES) {
                ...
                $NC.G_VAR.recommendParams = {
                    P_POLICY_DIV: "3", // 사업부정책관리 화면은 3으로 입력
...
function _Inquiry() {
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.CSC04020E0.001", "사업부를 입력하십시오."));
    ...
    // 데이터 조회
    $NC.serviceCall("/CSC04020E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
    ...
    // 데이터 조회
    $NC.serviceCall("/CSC04020E0/getDataSet.do", $NC.getGridParams(G_GRDSUB), onGetSub);
}
...
function _Save() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.CSC04020E0.002", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (selectedCount > 1) {
        alert($NC.getDisplayMsg("JS.CSC04020E0.004", "정책 적용값은 하나만 선택하여야 합니다."));
        return;
    }

    var dsMaster = dsD.concat(dsC);
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.CSC04020E0.003", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/CSC04020E0/save.do", {
...
function grdMasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/CSC04020E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
 ...
 * 사업부정책복사 버튼 호출.
 */
function btnCopyBuPolicyOnClick() {
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "CSC04021P0",
        PROGRAM_NM: $NC.getDisplayMsg("JS.CSC04021P0.001", "사업부정책 복사"),
        url: "cs/CSC04021P0.html",
        width: 610,
        height: 210,
        resizeable: false,
        G_PARAMETER: {
