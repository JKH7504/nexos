...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "RDELIVERY_CD":
            ...
            return;
        case "OUTBOUND_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB05010Q0.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "OUTBOUND_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB05010Q0.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOB05010Q0.003", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LOB05010Q0.004", "사업부를 입력하십시오."));
    ...
    if ($NC.isNull(OUTBOUND_DATE1)) {
        alert($NC.getDisplayMsg("JS.LOB05010Q0.005", "검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQOutbound_Date1");
        return;
    }
    var OUTBOUND_DATE2 = $NC.getValue("#dtpQOutbound_Date2");
    if ($NC.isNull(OUTBOUND_DATE2)) {
        alert($NC.getDisplayMsg("JS.LOB05010Q0.006", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQOutbound_Date2");
        return;
    }

    var INOUT_CD = $NC.getValue("#cboQInout_Cd");
    var MISSED_DIV = $NC.getValue("#cboQMissed_Div");
    ...
    // 데이터 조회
    $NC.serviceCall("/LOB05010Q0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
