...
function onProcessPreC() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOM02010E1.055", "출고지시 취소 처리하시겠습니까?\n출고차수(" + OUTBOUND_BATCH_F + ")", OUTBOUND_BATCH_F))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.056", "출고지시 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.057", "선택한 데이터 중 출고지시 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtC() {
    ...
    if (DIRECTIONS_DIV == "2" && !batchInfo.isTotal) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.058", "[신규] 또는 [토탈출고] 차수를 선택하고 출고지시 처리하십시오."));
    ...
    if (DIRECTIONS_DIV == "1") {
        if (!confirm($NC.getDisplayMsg("JS.LOM02010E1.059", "개별 출고지시 처리하시겠습니까?"))) {
            return;
        }
    } else {
        if (!confirm($NC.getDisplayMsg("JS.LOM02010E1.060", "출고지시 처리하시겠습니까?\n출고차수(" + OUTBOUND_BATCH_F + ")", OUTBOUND_BATCH_F))) {
            return;
        }
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.061", "출고지시 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.062", "선택한 데이터 중 출고지시 처리 가능한 데이터가 없습니다."));
        return;
    }
        ...
        // 기존 차수에 추가
            ...
            $NC.serviceCallAndWait("/LOM02010E0/getDataSet.do", {
                ...
                if ($NC.isNull(dsResult) || dsResult.length == 0) {
                    alert($NC.getDisplayMsg("JS.LOM02010E1.063", "기존 출고차수에 추가 가능여부를 확인하지 못했습니다. 다시 처리하십시오."));
                    return;
                }

                if (dsResult[0].EQUIP_STATE > "01") {
                    alert($NC.getDisplayMsg("JS.LOM02010E1.064", "출고차수(" + OUTBOUND_BATCH_F + ")는 이미 출하장비로 전송되어 해당 차수로 추가 출고지시할 수 없습니다.",
                        OUTBOUND_BATCH_F));
                    return;
                }

                if (EQUIP_DIV != dsResult[0].EQUIP_DIV) {
                    alert($NC.getDisplayMsg("JS.LOM02010E1.065", "해당 차수는 다른 출하장비(" + dsResult[0].EQUIP_DIV_D + ")로 할당되어 추가 출고지시할 수 없습니다.",
...
function grdMasterCInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterC", {
        ...
        queryId: "LOM02010E1.RS_T2_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
...
function grdDetailCInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailC", {
        ...
        queryId: "LOM02010E1.RS_T2_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function btnBillCntOnClick() {
    ...
    var BILL_CNT = Number($NC.getValue("#edtBill_Cnt", "0"));
    if (BILL_CNT <= 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.023", "1이상의 전표수를 입력하십시오."));
        $NC.setFocus("#edtBill_Cnt");
        return;
    }
    var dsTarget = G_GRDMASTERC.data.getItems();
    var rCount = dsTarget.length;
    if (rCount <= 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.024", "선택할 수 있는 전표가 없습니다."));
        return;
    }
    var rowData;
    for (var rIndex = 0; rIndex < rCount; rIndex++) {
...
function btnEquipSendOnClick() {
    ...
    // 신규차수
    if (batchInfo.isNew) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.067", "[출하장비전송] 처리할 출고차수를 선택하십시오."));
        return;
    }
    // 개별차수
    else if (!batchInfo.isTotal) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.068", "[토탈출고] 차수를 선택하고 [출하장비전송] 처리하십시오."));
        return;
    }

    var EQUIP_ALLOC_DIV = $NC.getValue("#cboEquip_Alloc_Div");
    if ($NC.isNull(EQUIP_ALLOC_DIV)) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.068", "할당구분을 선택하십시오."));
        $NC.setFocus("#cboEquip_Alloc_Div");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOM02010E1.069", "출하장비전송 처리 하시겠습니까?"))) {
        return;
    }
    ...
    $NC.serviceCall("/LOM02010E0/callSendFwLoBatch.do", {
        P_CENTER_CD: CENTER_CD,
        P_BU_CD: BU_CD,
        P_OUTBOUND_DATE: OUTBOUND_DATE,
        ...
        alert($NC.getDisplayMsg("JS.LOM02010E1.070", "[출하장비전송] 처리가 완료되었습니다."));
        window._Inquiry();
    });
}
...
function btnEquipSendCancelOnClick() {
    ...
    // 신규차수
    if (batchInfo.isNew) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.071", "[출하장비전송취소] 처리할 출고차수를 선택하십시오."));
        return;
    }
    // 개별차수
    else if (!batchInfo.isTotal) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.072", "[토탈출고] 차수를 선택하고 [출하장비전송취소] 처리하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOM02010E1.073", "출하장비전송취소 처리 하시겠습니까?"))) {
        return;
    }
    ...
    $NC.serviceCall("/LOM02010E0/callSendBwLoBatch.do", {
        P_CENTER_CD: CENTER_CD,
        P_BU_CD: BU_CD,
        P_OUTBOUND_DATE: OUTBOUND_DATE,
        P_OUTBOUND_BATCH: batchInfo.OUTBOUND_BATCH,
        ...
        alert($NC.getDisplayMsg("JS.LOM02010E1.070", "[출하장비전송취소] 처리가 완료되었습니다."));
        window._Inquiry();
    });
}
