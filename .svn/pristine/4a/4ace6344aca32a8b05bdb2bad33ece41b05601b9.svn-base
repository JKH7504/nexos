...
function _Initialize() {
    ...
    $("#btnAttachFileUpload").click(btnAttachFileUploadOnClick); // 파일첨부 버튼 클릭
    $("#btnAttachFileDelete").click(btnAttachFileDeleteOnClick); // 파일삭제 버튼 클릭
    $("#btnAttachFileDownload").click(btnAttachFileDownloadOnClick); // 파일첨부 다운로드 버튼 클릭
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            return;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB01010E0.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB01010E0.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
        case "DELIVERY_CD":
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.003", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.004", "사업부를 입력하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE1)) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.005", "검색 시작일자를 입력하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE2)) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.006", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQOrder_Date2");
        return;
    }

    if (ORDER_DATE1 > ORDER_DATE2) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.007", "출고예정일자 범위 입력오류입니다."));
        $NC.setFocus("#dtpQOrder_Date1");
        return;
    }
    ...
    // 데이터 조회
    $NC.serviceCall("/LOB01010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _New() {
    ...
    if ($NC.G_VAR.policyVal.LO110 != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.008", "출고예정정보 신규등록/수정이 불가능한 사업부입니다."));
        return;
    }
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "LOB01011P0",
        PROGRAM_NM: $NC.getDisplayMsg("JS.LOB01011P0.001", "출고예정등록/수정"),
        url: "lo/LOB01011P0.html",
        width: 1024,
        height: 600,
        G_PARAMETER: {
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.009", "삭제할 데이터가 없습니다."));
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    if ($NC.G_VAR.stateFWBW.CANCEL != rowData.OUTBOUND_STATE) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.010", "삭제가 불가능한 전표입니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOB01010E0.011", "예정전표를 삭제하시겠습니까?"))) {
        return;
    }

    // 삭제처리 호출
    $NC.serviceCall("/LOB01010E0/callLOOrderDelete.do", {
...
function grdMasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOB01010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
...
function grdMasterOnDblClick(e, args) {
    ...
    if (!permission.canSave) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.012", "해당 프로그램의 저장권한이 없습니다."));
        return;
    }
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LOB01010E0.013", "출고진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        ...
        if (refRowData.OUTBOUND_STATE != resultData.O_OUTBOUND_STATE) {
            alert($NC.getDisplayMsg("JS.LOB01010E0.014", "[진행상태 : " + resultData.O_OUTBOUND_STATE + "] 데이터가 변경되었습니다.\n다시 조회 후 데이터를 확인하십시오.",
        ...
        $NC.serviceCallAndWait("/LOB01010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail, function(subAjaxData) {
        ...
        if (dsDetail.length == 0) {
            alert($NC.getDisplayMsg("JS.LOB01010E0.015", "등록/수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function getOutboundState(params, onSuccess) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOB01010E0/getData.do", {
...
function _OnGridCheckBoxFormatterClick(e, view, args) {
    ...
    switch (columnId) {
        case "HOLD_YN":
            ...
            if (HOLD_YN == $ND.C_YES) {
                if (!confirm($NC.getDisplayMsg("JS.LOB01010E0.016", "예정일자: " + rowData.ORDER_DATE + ", 예정번호: " + rowData.ORDER_NO
                ...
                if (!confirm($NC.getDisplayMsg("JS.LOB01010E0.017", "예정일자: " + rowData.ORDER_DATE + ", 예정번호: " + rowData.ORDER_NO
            ...
            // 보류처리 호출
            $NC.serviceCall("/LOB01010E0/callLOOrderHold.do", {
                P_CENTER_CD: rowData.CENTER_CD,
                P_BU_CD: rowData.BU_CD,
                P_ORDER_DATE: rowData.ORDER_DATE,
                P_ORDER_NO: rowData.ORDER_NO,
...
function btnOrderClosingOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.018", "종결처리할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOB01010E0.019", "종결처리 하시겠습니까?"))) {
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    // 종결처리 호출
    $NC.serviceCall("/LOB01010E0/callLOOrderClosing.do", {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_ORDER_DATE: rowData.ORDER_DATE,
        P_ORDER_NO: rowData.ORDER_NO,
...
function btnAttachFileUploadOnClick(e) {

    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.020", "조회 후 처리하십시오."));
        return;
    }

    if ($NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.021", "파일첨부할 전표를 선택하십시오."));
        return;
    }

    if (!$NC.getProgramPermission().canSave) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.022", "해당 프로그램의 저장권한이 없습니다."));
        return;
    }
    ...
    if ($NC.isNotNull(rowData.ATTACH_FILE_NM)) {
        if (!confirm($NC.getDisplayMsg("JS.LOB01010E0.023", "첨부파일명: " + rowData.ATTACH_FILE_NM //
            + "\n\n이미 첨부 파일이 추가되어 있습니다. 변경하시겠습니까?", rowData.ATTACH_FILE_NM))) {
            return;
        }
    }

    $NC.showUploadFilePopup(null, function(view, fileName) {
        var uploadFileNm = fileName.substring(fileName.lastIndexOf("\\") + 1);
        if (uploadFileNm.length > 150) {
            alert($NC.getDisplayMsg("JS.LOB01010E0.024", "첨부파일명: " + uploadFileNm //
                + "\n\n파일명이 너무 깁니다. 파일명 변경 후 처리하십시오.", uploadFileNm));
            return;
        }
        $NC.fileUpload("/LOB01010E0/attachmentFileUpload.do", {
            P_CENTER_CD: rowData.CENTER_CD,
            P_BU_CD: rowData.BU_CD,
            P_ORDER_DATE: rowData.ORDER_DATE,
            P_ORDER_NO: rowData.ORDER_NO,
...
function btnAttachFileDeleteOnClick(e) {
    ...
    if ($NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.025", "첨부 파일 삭제할 전표를 선택하십시오."));
        return;
    }
    ...
    if ($NC.isNull(rowData.ATTACH_FILE_NM)) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.026", "첨부 파일이 등록되어 있지 않은 전표입니다."));
        return;
    }
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOB01010E0.027", "첨부 파일을 삭제하시겠습니까?"))) {
        return;
    }

    $NC.serviceCall("/LOB01010E0/attachmentFileDelete.do", {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_ORDER_DATE: rowData.ORDER_DATE,
        P_ORDER_NO: rowData.ORDER_NO,
        P_OUTBOUND_DATE: null,
        P_OUTBOUND_NO: null,
        P_ATTACH_FILE_NM: null,
        P_USER_ID: $NC.G_USERINFO.USER_ID
    }, onAttachFileSuccess);
}

function btnAttachFileDownloadOnClick(e) {
    ...
    if ($NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOB01010E0.028", "파일첨부할 전표를 선택하십시오."));
        return;
    }
    ...
    $NC.fileDownload("/LOB01010E0/attachmentFileDownload.do", {
        P_ATTACH_FILE_NM: rowData.ATTACH_FILE_NM,
        P_USER_ID: $NC.G_USERINFO.USER_ID
    });
}

function onAttachFileSuccess(ajaxData) {
...
function setUserProgramPermission() {
    ...
    $NC.setEnable("#btnOrderClosing", canSave && enable);
    $NC.setEnable("#btnAttachFileUpload", canSave && enable);
    $NC.setEnable("#btnAttachFileDelete", canSave && enable);
    $NC.setEnable("#btnAttachFileDownload", enable);
}
