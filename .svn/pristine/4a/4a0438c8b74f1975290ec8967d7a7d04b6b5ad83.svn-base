 ...
 * <pre>
 *  ==================================================================================================================================================
 *  프로그램ID         : PDA_LIC02320E0
 *  프로그램명         : PDA 입고검수[멀티상품매핑]
 *  프로그램설명       : PDA 입고검수[멀티상품매핑] 화면 Javascript
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.lastPalletId)) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "파렛트ID를 먼저 스캔하십시오."));
        setFocusScan();
        return;
    }

    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "처리할 상품 내역이 없습니다."));
        setFocusScan();
        return;
    }
    ...
    if (searchRows.length > 0) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "유통기한필수대상 상품입니다. 유통기한을 입력하십시오."));
    ...
    if (searchRows.length > 0) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "제조번호필수대상 상품입니다. 제조번호를 입력하십시오."));
        $NC.setGridSelectRow(G_GRDMASTER, searchRows[0]);
        // Scroll에서 데이터 표시 후 포커스 이동하도록 딜레이 처리
        setTimeout(function() {
    ...
    if ($NC.G_VAR.policyVal.LI530 != "2") {
        if (!confirm($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "검수 처리하시겠습니까?"))) {
            setFocusScan();
            return;
        }
    }
    ...
    $NC.serviceCall("/PDA_LIC02320E0/callLIProcInspectT2.do", {
...
function grdMasterInitialize() {
    ...
    $NC.setInitGridObject("#grdMaster", {
        ...
        queryId: "PDA_LIC02320E0.RS_MASTER",
        sortCol: "ITEM_CD",
        gridOptions: options
    });
...
function grdMasterOnAfterScroll(e, args) {
    ...
    if ($NC.G_VAR.policyVal.CM490 == $ND.C_YES) {
        var searchRow = $NC.getGridSearchRows(G_GRDMASTER, {
            compareFn: function(resultData) {
                return resultData.SERIAL_IN_DIV == $ND.C_POLICY_VAL_2 && resultData.SERIAL_QTY != resultData.INBOUND_QTY;
            }
        });
        if ($NC.isNotEmpty(searchRow)) {
            alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "입고일련번호관리대상 상품이 " + searchRow.length + "건이 존재합니다.\n[입고시리얼관리]화면에서 작업 후 진행하십시오."));
            _Cancel();
            return;
        }
    }
    ...
    switch (args.col) {
        case "VALID_DATE":
            if ($NC.isNotNull(args.val)) {
                $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.PDA_LIC02320E0.009", "유통기한을 정확히 입력하십시오."), "N");
            }
        ...
        case "CONFIRM_BOX":
            ...
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "검수BOX 수량을 정확히 입력하십시오."));
        ...
        case "CONFIRM_EA":
            changeQty = $NC.toNumber(args.val, -1);
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "검수EA 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
        ...
        case "CONFIRM_QTY":
            changeQty = $NC.toNumber(args.val, -1);
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "검수 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
...
function grdMasterOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONFIRM_QTY)) {
            alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "검수 수량을 입력하십시오."));
...
function onGetMaster(ajaxData) {
    ...
    if (dsResult.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "파렛트ID 정보가 없습니다."));
        _Cancel();
        return;
    }
    // 기본 체크, 첫번째 ROW 데이터로
    ...
    if (resultData.INBOUND_STATE != $ND.C_STATE_DIRECTIONS) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "입고진행상태: [" + resultData.INBOUND_STATE + "] 검수가능한 상태가 아닙니다.", resultData.INBOUND_STATE));
        _Cancel();
        return;
    }
    if (resultData.CENTER_CD != $NC.G_USERINFO.CENTER_CD) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "물류센터: [" + resultData.CENTER_CD + "] 설정된 물류센터와 다른 물류센터의 파렛트ID입니다.", resultData.BU_CD));
        _Cancel();
        return;
    }
    if (resultData.BU_CD != $NC.G_USERINFO.BU_CD) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "사업부: [" + resultData.BU_CD + "] 설정된 사업부와 다른 사업부의 파렛트ID입니다.", resultData.BU_CD));
        _Cancel();
        return;
    }
    if (resultData.INBOUND_DATE != $NC.G_USERINFO.WORK_DATE) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "입고일자: [" + resultData.INBOUND_DATE + "] 설정된 작업일자와 다른 입고일자의 파렛트ID입니다.",
    ...
    if (resultData.INSPECT_YN == $ND.C_YES) {
        if (!confirm($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "검수 처리된 파렛트ID입니다.\n재검수 처리하시겠습니까?"))) {
            _Cancel();
            return;
        }
    }
...
function onGetDetail(ajaxData) {
    ...
    if ($NC.isEmpty(resultData)) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "상품바코드 존재여부를 확인하지 못했습니다. 다시 스캔하십시오."));
    ...
    if (searchRow == -1) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "파렛트ID에 존재하지 않는 상품입니다."));
        _Cancel($ND.C_CLEAR_TYPE_DETAIL);
        return;
    }
...
function onPalletScan($scan, scanVal) {
    ...
    // 데이터 조회
    $NC.serviceCall("/PDA_LIC02320E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster, onPalletError);
}
...
function onSave(ajaxData) {
    ...
    var title = $NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "확인"), //
    message = $NC.getDisplayMsg("JS.PDA_LIC02320E0.XXX", "정상 처리되었습니다."), //
    buttons = {};
