...
function _Inquiry() {
    ...
    if ($NC.isNull(CUST_CD)) {
        alert($NC.getDisplayMsg("JS.CMC08010E0.001", "고객사를 입력하십시오."));
    ...
    var DEPT_NM = $NC.getValue("#edtQDept_Nm", true);
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.CMC08010E0.002", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    for (var rIndex = 0, rCount = G_GRDMASTER.data.getLength(); rIndex < rCount; rIndex++) {
        ...
        dsMaster.push({
            ...
            P_DEPT_FULL_NM: rowData.DEPT_FULL_NM,
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.CMC08010E0.003", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/CMC08010E0/save.do", {
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.CMC08010E0.004", "삭제할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.CMC08010E0.005", "부서를 삭제 하시겠습니까?"))) {
        return;
    }
...
function grdMasterOnNewRecord(args) {
    ...
    $NC.setFocusGrid(G_GRDMASTER, args.row, "DEPT_CD", true);
}
...
function grdMasterOnCellChange(e, args) {
    ...
    switch (G_GRDMASTER.view.getColumnId(args.cell)) {
        case "DEPT_NM":
            if ($NC.isNull(rowData.DEPT_FULL_NM)) {
                rowData.DEPT_FULL_NM = rowData.DEPT_NM;
            }
            break;
        case "OPEN_DATE":
            if ($NC.isNotNull(rowData.OPEN_DATE)) {
                if (!$NC.isDate(rowData.OPEN_DATE)) {
                    alert($NC.getDisplayMsg("JS.CMC08010E0.006", "거래일자를 정확히 입력하십시오."));
                    rowData.OPEN_DATE = "";
        ...
        case "CLOSE_DATE":
            if ($NC.isNotNull(rowData.CLOSE_DATE)) {
                if (!$NC.isDate(rowData.CLOSE_DATE)) {
                    alert($NC.getDisplayMsg("JS.CMC08010E0.007", "종료일자를 정확히 입력하십시오."));
                    rowData.CLOSE_DATE = "";
                    $NC.setFocusGrid(G_GRDMASTER, args.row, args.cell, true);
                } else {
...
function grdMasterOnBeforePost(row) {
    ...
    if (!$NC.isGridValidPostRow(G_GRDMASTER, row, "DEPT_CD")) {
        return true;
    }

    var rowData = G_GRDMASTER.data.getItem(row);
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.DEPT_CD)) {
            alert($NC.getDisplayMsg("JS.CMC08010E0.008", "부서코드를 입력하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "DEPT_CD", true);
            return false;
        }
        if ($NC.isNull(rowData.DEPT_NM)) {
            alert($NC.getDisplayMsg("JS.CMC08010E0.009", "부서명을 입력하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "DEPT_NM", true);
            return false;
        }
        if ($NC.isNull(rowData.DEPT_FULL_NM)) {
            rowData.DEPT_FULL_NM = rowData.DEPT_NM;
        ...
        if ($NC.isNull(rowData.DEAL_DIV)) {
            alert($NC.getDisplayMsg("JS.CMC08010E0.010", "거래구분을 선택하십시오."));
        ...
        if ($NC.isNotNull(rowData.OPEN_DATE) && $NC.isNotNull(rowData.CLOSE_DATE)) {
            if (rowData.CLOSE_DATE < rowData.OPEN_DATE) {
                alert($NC.getDisplayMsg("JS.CMC08010E0.011", "거래일자와 종료일자의 범위 입력오류입니다."));
...
function onGetMaster(ajaxData) {
    ...
    $NC.setInitGridAfterOpen(G_GRDMASTER, "DEPT_CD", true);
