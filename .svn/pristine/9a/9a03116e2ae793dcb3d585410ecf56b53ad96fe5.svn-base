...
function onProcessPreE() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOM02010E0.103", "배송완료 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E0.104", "배송완료 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E0.105", "선택한 데이터 중 배송완료 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtE() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOM02010E0.106", "배송완료 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDDETAILE)) {
        alert($NC.getDisplayMsg("JS.LOM02010E0.107", "데이터가 수정되었습니다. 저장 후 배송완료 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E0.108", "배송완료 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E0.109", "선택한 데이터 중 배송완료처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterEInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterE", {
        ...
        queryId: "LOM02010E0.RS_T4_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
...
function grdDetailEInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailE", {
        ...
        queryId: "LOM02010E0.RS_T4_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdDetailEOnCellChange(e, args) {
    ...
    switch (G_GRDDETAILE.view.getColumnId(args.cell)) {
        case "DELIVERY_QTY":
            ...
            if (DELIVERY_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LOM02010E0.110", "배송수량에 0보다 작은값을 입력할 수 없습니다."));
                ...
                alert($NC.getDisplayMsg("JS.LOM02010E0.111", "배송수량이 확정수량을 초과할 수 없습니다."));
                rowData.DELIVERY_QTY = args.oldItem.DELIVERY_QTY;
                rowData.MISSED_QTY = args.oldItem.MISSED_QTY;
                $NC.setFocusGrid(G_GRDDETAILE, args.row, args.cell, true);
                break;
