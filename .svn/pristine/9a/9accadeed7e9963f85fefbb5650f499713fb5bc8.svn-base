...
function _Initialize() {
    ...
    $NC.serviceCall("/WC/getMultiDataSet.do", {
        P_SERVICE_PARAMS: [
            ...
            {
                P_RESULT_ID: "O_WC_POP_CMCODE_INOUT_CD_E",
                P_QUERY_ID: "WC.POP_CMCODE",
                P_QUERY_PARAMS: {
                    P_COMMON_GRP: "INOUT_CD",
                    P_COMMON_CD: $ND.C_ALL,
        ...
        $NC.setInitComboData({
            ...
            nameField: "COMMON_NM",
            fullNameField: "COMMON_CD_F",
            data: $NC.toArray(multipleData.O_WC_POP_CMCODE_INOUT_CD_E),
            addAll: true
        });
        // 조회조건 - 출고구분 세팅
        $NC.setInitComboData({
            ...
            nameField: "COMMON_NM",
            fullNameField: "COMMON_CD_F",
            data: $NC.toArray(multipleData.O_WC_POP_CMCODE_INOUT_CD_D),
            addAll: true
        });
    });
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            if ($NC.isNull(BU_CD)) {
                alert($NC.getDisplayMsg("JS.LCC01010E0.027", "사업부를 먼저 선택 하십시오."));
            ...
            return;
        case "ETC_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC01010E0.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ETC_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC01010E0.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
        case "INOUT_DIVA":
            $("#cboQInout_Cd").show();
            $("#cboQInout_CdE").hide();
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC01010E0.003", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC01010E0.004", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var ETC_DATE1 = $NC.getValue("#dtpQEtc_Date1");
    if ($NC.isNull(ETC_DATE1)) {
        alert($NC.getDisplayMsg("JS.LCC01010E0.005", "검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQEtc_Date1");
        return;
    }
    var ETC_DATE2 = $NC.getValue("#dtpQEtc_Date2");
    if ($NC.isNull(ETC_DATE2)) {
        alert($NC.getDisplayMsg("JS.LCC01010E0.006", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQEtc_Date2");
        return;
    }
    if (ETC_DATE1 > ETC_DATE2) {
        alert($NC.getDisplayMsg("JS.LCC01010E0.007", "입출고일자 검색 범위 오류입니다."));
        $NC.setFocus("#dtpQEtc_Date1");
        return;
    }
    var INOUT_DIV = $NC.getValueRadioGroup("rgbQInout_Div");
    var INOUT_CD = $NC.getValue("#cboQInout_Cd" + INOUT_DIV);
    if ($NC.isNull(INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LCC01010E0.008", "입출고구분을 선택하십시오."));
        $NC.setFocus("#cboQInout_Cd" + INOUT_DIV);
        return;
    }
    if ($NC.isNotNull(INOUT_DIV) && INOUT_CD == $ND.C_ALL) {
        INOUT_CD = INOUT_DIV + INOUT_CD;
    }
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        // 데이터 조회
        $NC.serviceCall("/LCC01010E0/getDataSet.do", $NC.getGridParams(G_GRDT1MASTER), onGetT1Master);
    }
    // 내역 조회
    else {
        ...
        // 데이터 조회
        $NC.serviceCall("/LCC01010E0/getDataSet.do", $NC.getGridParams(G_GRDT2MASTER), onGetT2Master);
    }
}
...
function _New() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        $NC.showProgramSubPopup({
            PROGRAM_ID: "LCC01011P0",
            PROGRAM_NM: $NC.getDisplayMsg("JS.LCC01010E0.009", "기타입출고 등록"),
            url: "lc/LCC01011P0.html",
            width: 1024,
            height: 600,
            G_PARAMETER: {
...
function _Save() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LCC01010E0.010", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC01010E0.011", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LCC01010E0/updateRemark.do", {
...
function _Delete() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
            alert($NC.getDisplayMsg("JS.LCC01010E0.012", "삭제할 데이터가 없습니다."));
            return;
        }

        var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
        if (rowData.CONFIRM_YN == $ND.C_YES) {
            alert($NC.getDisplayMsg("JS.LCC01010E0.013", "취소처리 후 삭제 하십시오."));
            return;
        }

        if (rowData.INOUT_SUB_CD == "EZ" || rowData.INOUT_SUB_CD == "DZ") {
            alert($NC.getDisplayMsg("JS.LCC01010E0.014", "입출고구분 : " + rowData.INOUT_NM + " 전표는 삭제 할 수 없습니다.", rowData.INOUT_NM));
            return;
        }

        if (!confirm($NC.getDisplayMsg("JS.LCC01010E0.015", "삭제 하시겠습니까?"))) {
            return;
        }

        $NC.serviceCall("/LCC01010E0/callLCProcessing.do", {
            P_PROCESS_DIV: "DEL",
            P_CENTER_CD: rowData.CENTER_CD,
            P_BU_CD: rowData.BU_CD,
            P_ETC_DATE: rowData.ETC_DATE,
...
function _Print(reportInfo) {
    ...
    if ($NC.isNull(rowData)) {
        alert($NC.getDisplayMsg("JS.LCC01010E0.016", "출력할 데이터를 먼저 선택하십시오."));
        return;
    }
    ...
    switch (reportInfo.REPORT_CD) {
        ...
        case "LABEL_CMC03":
            if (rowData.INOUT_SUB_CD.startsWith("D")) {
                alert($NC.getDisplayMsg("JS.LCC01010E0.017", "기타출고 전표입니다. \n기타입고 데이터를 선택하시오."));
                return;
            }
            // 선택 데이터 가져오기
            // checkedData = {};
        ...
        case "LABEL_CMC04":
            if (rowData.INOUT_SUB_CD.startsWith("D")) {
                alert($NC.getDisplayMsg("JS.LCC01010E0.018", "기타출고 전표입니다. \n기타입고 데이터를 선택하시오."));
                return;
            }
            // 선택 데이터 가져오기
            // checkedData = {};
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "LCC01010E0.RS_T1_MASTER",
        sortCol: "ETC_DATE",
        gridOptions: options,
        canDblClick: true
    });
...
function grdT1MasterOnDblClick(e, args) {
    ...
    if (refRowData.CONFIRM_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC01010E0.018", "확정 된 전표입니다."));
        return;
    }

    // 조회후 상태가 바뀌었는지 한번더 상태 체크
    $NC.serviceCall("/LCC01010E0/getConfirmYn.do", {
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LCC01010E0.019", "확정여부를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        var oMsg = $NC.getOutMessage(resultData);
        if (oMsg != $ND.C_OK) {
        ...
        if (resultData.O_CONFIRM_YN != $ND.C_NO) {
            alert($NC.getDisplayMsg("JS.LCC01010E0.020", "이미 확정처리된 데이터입니다.\n다시 조회 후 데이터를 확인하십시오."));
            return;
        }
        ...
        // 데이터 조회 - 하단그리드
        $NC.serviceCallAndWait("/LCC01010E0/getDataSet.do", {
            P_QUERY_ID: "LCC01010E0.RS_SUB2",
            P_QUERY_PARAMS: {
                P_CENTER_CD: refRowData.CENTER_CD,
                P_BU_CD: refRowData.BU_CD,
                P_ETC_DATE: refRowData.ETC_DATE,
        ...
        if (dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC01010E0.021", "수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function grdT1MasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LCC01010E0/getDataSet.do", $NC.getGridParams(G_GRDT1DETAIL), onGetT1Detail);
...
/**
 * 기타입출고 등록 탭 : 상태가 등록인 경우만 그리드 편집 가능
 * 
 * @param e
 * @param args
 * @returns {Boolean}
...
function grdT1DetailInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Detail", {
        ...
        queryId: "LCC01010E0.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "LCC01010E0.RS_T2_MASTER",
        sortCol: "ETC_DATE",
        gridOptions: options
    });
...
function onProcessPre() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC01010E0.022", "조회 후 처리하십시오."));
        return;
    }

    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.CONFIRM_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LCC01010E0.023", "확정된 전표만 취소 할 수 있습니다."));
        return;
    }

    if (rowData.INOUT_SUB_CD == "EZ" || rowData.INOUT_SUB_CD == "DZ") {
        alert($NC.getDisplayMsg("JS.LCC01010E0.024", "입출고구분 : " + rowData.INOUT_NM + " 전표는 취소 할 수 없습니다.", rowData.INOUT_NM));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC01010E0.025", "취소 처리 하시겠습니까?"))) {
        return;
    }
...
function onProcessNxt() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LCC01010E0.026", "확정 처리 하시겠습니까?"))) {
        return;
    }
