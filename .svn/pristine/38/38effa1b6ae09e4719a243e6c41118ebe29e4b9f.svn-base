 ...
 *  프로그램ID         : RIM02011P0
 *  프로그램명         : 온라인반입등록 팝업
 *  프로그램설명       : 온라인반입등록 팝업 화면 Javascript
...
function _Initialize() {
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.RIM02011P0.001", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.RIM02011P0.002", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.RIM02011P0.003", "저장"));
}
...
function _OnLoaded() {
    ...
    // 예정 -> 등록, 등록 수정
        ...
        $NC.setValue("#edtBox_Seq", dsMaster.BOX_SEQ);
        ...
        $NC.setValue("#edtReturn_Cost", dsMaster.RETURN_COST);
...
function _New() {
    ...
    if ($NC.isNull(DELIVERY_CD)) {
        alert($NC.getDisplayMsg("JS.RIM02011P0.004", "먼저 온라인몰 코드를 입력하십시오."));
        $NC.setFocus("#edtDelivery_Cd");
        return;
    }
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.RIM02011P0.006", "물류센터가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.RIM02011P0.007", "사업부가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.INBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.RIM02011P0.008", "먼저 반입일자를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.RIM02011P0.009", "먼저 반입구분을 선택하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_NM)) {
        alert($NC.getDisplayMsg("JS.RIM02011P0.010", "먼저 주문자명을 입력하십시오."));
        $NC.setFocus("#edtOrderer_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_HP)) {
        alert($NC.getDisplayMsg("JS.RIM02011P0.011", "먼저 주문자 휴대폰번호를 입력하십시오."));
        $NC.setFocus("#edtOrderer_Hp");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_NM)) {
        alert($NC.getDisplayMsg("JS.RIM02011P0.012", "먼저 수령자명을 입력하십시오."));
        $NC.setFocus("#edtShipper_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_HP)) {
        alert($NC.getDisplayMsg("JS.RIM02011P0.013", "먼저 수령자 휴대폰번호를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_ZIP_CD) || $NC.isNull($NC.G_VAR.masterData.SHIPPER_ADDR_BASIC)) {
        alert($NC.getDisplayMsg("JS.RIM02011P0.014", "먼저 수령자 주소를 입력하십시오."));
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.RIM02011P0.005", "저장할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.RIM02011P0.015", "저장하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.RIM02011P0.016", "수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/RIM02010E0/save.do", {
        P_DS_MASTER: {
            P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
            P_BU_CD: $NC.G_VAR.masterData.BU_CD,
            P_INBOUND_DATE: $NC.G_VAR.masterData.INBOUND_DATE,
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.RIM02011P0.017", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        ...
        case "INOUT_CD":
            ...
            break;
        case "INBOUND_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.RIM02011P0.018", "반입일자를 정확히 입력하십시오."));
        ...
        case "REMARK1":
            ...
            break;
        case "BOX_SEQ":
            $NC.G_VAR.masterData.BOX_SEQ = args.val;
            break;
        case "PALLET_ID_CNT":
            $NC.G_VAR.masterData.PALLET_ID_CNT = args.val;
            break;
        case "RETURN_COST":
            var RETURN_COST = $NC.getValue("#edtReturn_Cost");

            if ((!Number(RETURN_COST) && RETURN_COST != 0) || RETURN_COST < 0) {
                $NC.setValue("#edtReturn_Cost", 0);
                $NC.G_VAR.masterData.RETURN_COST = 0;
                $NC.setFocus("#edtReturn_Cost");
                return;
            }
            $NC.G_VAR.masterData.RETURN_COST = args.val;
            break;
        case "ORDERER_CD":
            $NC.G_VAR.masterData.ORDERER_CD = args.val;
            break;
        case "ORDERER_NM":
            $NC.G_VAR.masterData.ORDERER_NM = args.val;
            if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_NM)) {
            ...
            break;
        case "ORDERER_TEL":
            $NC.G_VAR.masterData.ORDERER_TEL = args.val;
            ...
            break;
        case "ORDERER_HP":
            $NC.G_VAR.masterData.ORDERER_HP = args.val;
            if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_HP)) {
...
function grdDetailOnBeforeEditCell(e, args) {
    ...
    switch (args.column.id) {
        ...
        case "ITEM_LOT":
            return isNew;
        case "ITEM_STATE_F":
            // [B2C]상품상태 변경 기준 - 상품상태 변경 가능
            return $NC.G_VAR.G_PARAMETER.P_POLICY_RI251 == $ND.C_YES || $NC.G_VAR.G_PARAMETER.P_POLICY_RI251 == $ND.C_NO && isNew;
        case "VALID_DATE":
        case "BATCH_NO":
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        ...
        case "ENTRY_QTY":
            ...
            if (ENTRY_QTY < CHECK_QTY) {
                alert($NC.getDisplayMsg("JS.RIM02011P0.019", "등록수량이 " + CHECK_QTY + "보다 작을 수 없습니다.", CHECK_QTY));
            ...
            break;
        case "ENTRY_BOX":
            var ENTRY_BOX = Number(rowData.ENTRY_BOX) || 0;
            if (ENTRY_BOX < 0) {
                alert($NC.getDisplayMsg("JS.RIM02011P0.020", "등록BOX는 0보다 작을 수 없습니다."));
                rowData.ENTRY_BOX = args.oldItem.ENTRY_BOX;
                $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                break;
            }
            rowData.ENTRY_BOX = ENTRY_BOX;
            rowData = grdDetailOnCalc(rowData, undefined, columnId);
            // 등록수량이 CHECK_QTY보다 작은 수량 등록 불가능
            if (Number(rowData.ENTRY_QTY) < CHECK_QTY) {
                alert($NC.getDisplayMsg("JS.RIM02011P0.019", "등록수량이 " + CHECK_QTY + "보다 작을 수 없습니다.", CHECK_QTY));
                rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                rowData.ENTRY_BOX = args.oldItem.ENTRY_BOX;
            ...
            break;
        case "ENTRY_EA":
            var ENTRY_EA = Number(rowData.ENTRY_EA) || 0;
            if (ENTRY_EA < 0) {
                alert($NC.getDisplayMsg("JS.RIM02011P0.021", "등록EA는 0보다 작을 수 없습니다."));
                rowData.ENTRY_EA = args.oldItem.ENTRY_EA;
                $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                break;
            }
            rowData.ENTRY_EA = ENTRY_EA;
            rowData = grdDetailOnCalc(rowData, undefined, columnId);
            // 등록수량이 CHECK_QTY보다 작은 수량 등록 불가능
            if (Number(rowData.ENTRY_QTY) < CHECK_QTY) {
                alert($NC.getDisplayMsg("JS.RIM02011P0.019", "등록수량이 " + CHECK_QTY + "보다 작을 수 없습니다.", CHECK_QTY));
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.RIM02011P0.022", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                } else {
...
function setUserProgramPermission() {
    ...
    if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ORDER || $NC.G_VAR.ORDER_YN == $ND.C_YES) {
        ...
        // YN - 추가만 가능
        // YY - 추가/삭제 가능
        var POLICY_RI222 = $NC.G_VAR.G_PARAMETER.P_POLICY_RI222.match(/N|Y/gi);
        if ($NC.isNull(POLICY_RI222)) {
            POLICY_RI222 = [
                $ND.C_NO,
                $ND.C_NO
            ];
        ...
        $NC.setEnable("#btnEntryNew", POLICY_RI222[0] == $ND.C_YES && permission.canSave);
        $NC.setEnable("#btnEntryDelete", POLICY_RI222[1] == $ND.C_YES && permission.canDelete);
