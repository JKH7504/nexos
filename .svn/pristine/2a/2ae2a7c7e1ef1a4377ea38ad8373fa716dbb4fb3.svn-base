...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "DELIVERY_CD":
            ...
            return;
        case "INBOUND_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIC05090E0.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "INBOUND_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIC05090E0.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.RIC05090E0.003", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.RIC05090E0.004", "사업부를 입력하십시오."));
    ...
    if ($NC.isNull(INBOUND_DATE1)) {
        alert($NC.getDisplayMsg("JS.RIC05090E0.005", "검색 시작일자를 입력하십시오."));
    ...
    if ($NC.isNull(INBOUND_DATE2)) {
        alert($NC.getDisplayMsg("JS.RIC05090E0.006", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQInbound_Date2");
        return;
    }

    if (INBOUND_DATE1 > INBOUND_DATE2) {
        alert($NC.getDisplayMsg("JS.RIC05090E0.007", "반입일자 범위 입력오류입니다."));
        $NC.setFocus("#dtpQInbound_Date1");
        return;
    }
    ...
    if ($NC.isNull(EXTRA_WORK_DIV1) && $NC.isNull(EXTRA_WORK_DIV2)) {
        alert($NC.getDisplayMsg("JS.RIC05090E0.008", "작업구분을 선택하십시오."));
    ...
    // 데이터 조회
    $NC.serviceCall("/RIC05090E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || G_GRDDETAIL.data.getLength() == 0 || G_GRDSUB.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.RIC05090E0.009", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (dsMaster.length == 0 && dsSub.length == 0) {
        alert($NC.getDisplayMsg("JS.RIC05090E0.010", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/RIC05090E0/save.do", {
...
function grdMasterOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.BILL_PLT_QTY)) {
            alert($NC.getDisplayMsg("JS.RIC05090E0.011", "정산PLT를 정확히 입력하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "BILL_PLT_QTY", true);
            return false;
        }
        if ($NC.isNull(rowData.BILL_BOX_QTY)) {
            alert($NC.getDisplayMsg("JS.RIC05090E0.012", "정산BOX를 정확히 입력하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "BILL_BOX_QTY", true);
            return false;
        }
        if ($NC.isNull(rowData.BILL_EA_QTY)) {
            alert($NC.getDisplayMsg("JS.RIC05090E0.013", "정산EA를 정확히 입력하십시오."));
    ...
    switch (G_GRDMASTER.view.getColumnId(args.cell)) {
        case "BILL_PLT_QTY":
            if (Number(rowData.BILL_PLT_QTY) < 0) {
                alert($NC.getDisplayMsg("JS.RIC05090E0.014", "정산PLT에는 0보다 작은값을 입력할 수 없습니다."));
        ...
        case "BILL_BOX_QTY":
            if (Number(rowData.BILL_BOX_QTY) < 0) {
                alert($NC.getDisplayMsg("JS.RIC05090E0.015", "정산BOX에는 0보다 작은값을 입력할 수 없습니다."));
        ...
        case "BILL_EA_QTY":
            if (Number(rowData.BILL_EA_QTY) < 0) {
                alert($NC.getDisplayMsg("JS.RIC05090E0.016", "정산EA에는 0보다 작은값을 입력할 수 없습니다."));
...
function grdSubOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.EXTRA_WORK_QTY)) {
            alert($NC.getDisplayMsg("JS.RIC05090E0.017", "추가작업수량을 정확히 입력하십시오."));
    ...
    switch (G_GRDSUB.view.getColumnId(args.cell)) {
        case "EXTRA_WORK_QTY":
            ...
            if (Number(rowData.EXTRA_WORK_QTY) < 0) {
                alert($NC.getDisplayMsg("JS.RIC05090E0.018", "추가작업수량에는 0보다 작은값을 입력할 수 없습니다."));
                rowData.EXTRA_WORK_QTY = 0;
...
function grdMasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIC05090E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
    ...
    // 데이터 조회
    $NC.serviceCall("/RIC05090E0/getDataSet.do", $NC.getGridParams(G_GRDSUB), onGetSub);
