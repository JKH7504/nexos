...
function _Initialize() {
    ...
    $NC.setInitComboData({
        ...
        data: [
            {
                COMMON_CD: "0",
                COMMON_NM: $NC.getDisplayMsg("JS.LCC04010E1.036", "전체")
            },
            {
                COMMON_CD: "1",
                COMMON_NM: $NC.getDisplayMsg("JS.LCC04010E1.037", "차이분")
            }
        ],
        onComplete: function() {
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            if ($NC.isNull(BU_CD)) {
                alert($NC.getDisplayMsg("JS.LCC04010E1.038", "사업부를 먼저 선택 하십시오."));
            ...
            return;
        case "INVEST_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC04010E1.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "INVEST_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC04010E1.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.003", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.004", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var INVEST_DATE1 = $NC.getValue("#dtpQInvest_Date1");
    if ($NC.isNull(INVEST_DATE1)) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.005", "검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQInvest_Date1");
        return;
    }
    var INVEST_DATE2 = $NC.getValue("#dtpQInvest_Date2");
    if ($NC.isNull(INVEST_DATE2)) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.006", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQInvest_Date2");
        return;
    }
    if (INVEST_DATE1 > INVEST_DATE2) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.007", "실사일자 검색 범위 오류입니다."));
        $NC.setFocus("#dtpQInvest_Date1");
        return;
    }
    ...
    if ($NC.isNull(VIEW_DIV)) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.008", "차이분여부를 선택하십시오."));
    ...
    if ($NC.isNull(GAP_DIV)) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.009", "실사차이사유구분을 선택 입력하십시오."));
...
function _New() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        $NC.showProgramSubPopup({
            PROGRAM_ID: "LCC04011P1",
            PROGRAM_NM: $NC.getDisplayMsg("JS.LCC04010E1.010", "재고실사등록/수정"),
            url: "lc/LCC04011P1.html",
            width: 1024,
            height: 610,
            G_PARAMETER: {
...
function _Save() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.011", "저장할 데이터가 없습니다."));
        return;
    }

    var refRowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (refRowData.REFLECT_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.012", "이미 재고에 반영된 데이터에 대하여 수정할 수 없습니다."));
        return;
    }
    if (refRowData.CONFIRM_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.013", "이미 확정된 데이터에 대하여 수정할 수 없습니다."));
        return;
    }
    ...
    var dsDetail = [];
    var dsTarget = G_GRDT1DETAIL.data.getItems();
    var rowData;
    ...
    if (dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.014", "수정 후 저장하십시오."));
        return;
    }
...
function _Delete() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.015", "삭제할 데이터가 없습니다."));
        return;
    }

    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.REFLECT_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.016", "이미 재고에 반영된 데이터에 대하여 삭제할 수 없습니다."));
        return;
    }
    if (rowData.CONFIRM_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.017", "이미 확정된 데이터 입니다. \n취소처리 후 삭제 하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC04010E1.018", "삭제 하시겠습니까?"))) {
        return;
    }
    ...
    switch (reportInfo.REPORT_CD) {
        ...
        case "PAPER_LCC09":
            if (rowData.CONFIRM_YN == $ND.C_NO) {
                alert($NC.getDisplayMsg("JS.LCC04010E1.019", "[" + reportInfo.REPORT_NM + "]재고실사 확정된 내역을 선택하십시오.", reportInfo.REPORT_NM));
                return;
            }
            // 선택 데이터 가져오기
            // checkedData = {};
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "LCC04010E1.RS_T1_MASTER",
        sortCol: "INVEST_DATE",
        gridOptions: options,
        canDblClick: true
    });
...
function grdT1MasterOnDblClick(e, args) {
    ...
    if ($NC.G_VAR.policyVal.EM191 == "1") {
        alert($NC.getDisplayMsg("JS.LCC04010E1.020", "[RFID]시스템연동 정책에 의해 팝업 조회 불가능 합니다."));
        return;
    }
    ...
    if (refRowData.REFLECT_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.021", "이미 재고에 반영된 데이터 입니다."));
        return;
    }
    if (refRowData.CONFIRM_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.022", "이미 확정처리된 전표입니다."));
        return;
    }
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LCC04010E1.023", "확정여부를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        var oMsg = $NC.getOutMessage(resultData);
        if (oMsg != $ND.C_OK) {
        ...
        if (resultData.O_CONFIRM_YN != $ND.C_NO) {
            alert($NC.getDisplayMsg("JS.LCC04010E1.024", "이미 확정처리된 전표입니다.\n다시 조회 후 데이터를 확인하십시오."));
            return;
        }
        ...
        if (dsDetail.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC04010E1.025", "수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function grdT1DetailInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Detail", {
        ...
        queryId: "LCC04010E1.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "LCC04010E1.RS_T2_MASTER",
        sortCol: "INVEST_DATE",
        gridOptions: options
    });
...
function grdT3MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT3Master", {
        ...
        queryId: "LCC04010E1.RS_T3_MASTER",
        sortCol: "INVEST_DATE",
        gridOptions: options
    });
...
function onProcessPre() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.026", "처리할 데이터가 없습니다.\n 조회 후 처리하십시오."));
        return;
    }

    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.CONFIRM_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.027", "확정된 데이터에 대하여 취소 할 수 있습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC04010E1.028", "취소를 하시겠습니까?"))) {
        return;
    }
...
function onProcessNxt() {
    ...
    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.CONFIRM_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.029", "확정 된 데이터입니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC04010E1.030", "확정을 하시겠습니까?"))) {
        return;
    }
...
function btnApplyResultOnClick() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.031", "조회 후 처리하십시오."));
        return;
    }

    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.CONFIRM_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.032", "확정된 전표만 재고반영 처리할 수 있습니다."));
        return;
    }
    if (rowData.REFLECT_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC04010E1.033", "이미 재고에 반영된 된 데이터입니다."));
        return;
    }
    if (rowData.DATA_CREATE_DIV == "1" && $NC.G_VAR.policyVal.LS210 == "2") {
        alert($NC.getDisplayMsg("JS.LCC04010E1.034", "유통기한/제조배치가 없이 상품별로 생성된 재고실사 데이터입니다.\n해당 데이터는 재고반영 처리할 수 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC04010E1.035", "재고반영을 하시겠습니까?"))) {
        return;
    }
