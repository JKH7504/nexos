...
function onProcessPreC() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E1.047", "입고지시 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.048", "입고지시 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.049", "선택한 데이터 중 입고지시 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtC() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E1.050", "입고지시 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBC)) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.051", "데이터가 수정되었습니다. 저장 후 입고지시 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.052", "입고지시 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.053", "선택한 데이터 중 입고지시 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function btnNewPalletIdCOnClick() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E1.054", "새 파렛트ID를 생성하시겠습니까?"))) {
        return;
    }
...
function onGetNewPalletIdC(ajaxData) {
    ...
    $NC.serviceCall("/LIC02010E0/getDataSet.do", {
        P_QUERY_ID: "LIC02010E1.RS_T2_SUB1",
        P_QUERY_PARAMS: {
            P_CENTER_CD: rowData.CENTER_CD,
            P_BU_CD: rowData.BU_CD,
            P_INBOUND_DATE: rowData.INBOUND_DATE,
...
function btnNewDirectionsCOnClick() {
    ...
    if (G_GRDPALLETIDC.data.getLength() == 0 || $NC.isNull(G_GRDPALLETIDC.lastRow)) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.055", "신규 파렛트ID를 먼저 생성 후 처리하십시오."));
        return;
    }
    ...
    if (rowData.ENTRY_QTY == rowData.DIRECTIONS_QTY) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.056", "추가하려는 상품은 이미 등록수량만큼 지시 처리되었습니다."));
        return;
    }
    ...
    if (searchIndex != -1) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.057", "추가하려는 상품은 해당 파렛트ID에 이미 추가되어 있습니다."));
...
function btnDeleteDirectionsCOnClick() {
    ...
    if (G_GRDSUBC.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.058", "삭제하려는 데이터를 선택하십시오."));
        return;
    }
...
function grdMasterCInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterC", {
        ...
        queryId: "LIC02010E1.RS_T2_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options
    });
...
function grdDetailCInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailC", {
        ...
        queryId: "LIC02010E1.RS_T2_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdSubCOnSetColumns() {
    ...
    $NC.setGridColumnOptions(G_GRDSUBC, {
        ENTRY_QTY: {
            editor: $NC.G_VAR.policyVal.LI230 == "2" || $NC.G_VAR.policyVal.LI230 == "3" ? Slick.Editors.Number : null
        }
    });

    $NC.setGridColumns(G_GRDSUBC, [ // 숨김컬럼 세팅
        $NC.G_VAR.policyVal.LS220 == "1" ? "PALLET_ID" : "",
        $NC.G_VAR.policyVal.LI230 == "1" ? "LINE_NO,ITEM_CD,ITEM_BAR_CD,ITEM_NM,ITEM_SPEC,BRAND_NM,ITEM_STATE_F,FLOCATION_CD,ITEM_LOT,CONS_QTY" : "",
        $NC.G_VAR.policyVal.LS210 != "2" ? "VALID_DATE,BATCH_NO" : ""
    ]);
}
...
function grdSubCOnCellChange(e, args) {
    ...
    switch (G_GRDSUBC.view.getColumnId(args.cell)) {
        case "ENTRY_QTY":
            ...
            if (ENTRY_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LIC02010E1.059", "지시수량이 0보다 작을 수 없습니다."));
            ...
            if (searchIndex == -1) {
                alert($NC.getDisplayMsg("JS.LIC02010E1.060", "상세정보에서 해당 상품을 검색하지 못해 지시수량을 반영하지 못했습니다."));
                rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                rowData.ENTRY_BOX = args.oldItem.ENTRY_BOX;
                rowData.ENTRY_EA = args.oldItem.ENTRY_EA;
                rowData.ENTRY_WEIGHT = args.oldItem.ENTRY_WEIGHT;
            ...
            if (D_ENTRY_QTY < S0_ENTRY_QTY) {
                alert($NC.getDisplayMsg("JS.LIC02010E1.061", "지시수량이 등록수량보다 클 수 없습니다."));
                rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                rowData.ENTRY_BOX = args.oldItem.ENTRY_BOX;
                rowData.ENTRY_EA = args.oldItem.ENTRY_EA;
                rowData.ENTRY_WEIGHT = args.oldItem.ENTRY_WEIGHT;
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.LIC02010E1.062", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDSUBC, args.row, args.cell, true);
                } else {
...
function grdSubCOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.ENTRY_QTY)) {
            alert($NC.getDisplayMsg("JS.LIC02010E1.063", "지시수량을 입력하십시오."));
...
function grdSubCOnBeforeEditCell(e, args) {
    ...
    switch (args.column.id) {
        case "ENTRY_QTY":
            return $NC.G_VAR.policyVal.LI230 == "2" || $NC.G_VAR.policyVal.LI230 == "3";
        case "VALID_DATE":
        case "BATCH_NO":
...
function onGetMasterC(ajaxData) {
    ...
    if ($NC.setInitGridAfterOpen(G_GRDMASTERC, "INBOUND_NO", true)) {
        // 마스터 총수량 세팅
        var rowData = G_GRDMASTERC.data.getItem(0);
        $NC.setValue("#edtTotal_Order_QtyC", $NC.getDisplayNumber(rowData.TOTAL_ORDER_QTY));
        $NC.setValue("#edtTotal_Entry_QtyC", $NC.getDisplayNumber(rowData.TOTAL_ENTRY_QTY));
        $NC.setValue("#edtTotal_Vendor_CntC", $NC.getDisplayNumber(rowData.TOTAL_VENDOR_CNT));
    } else {
        // 디테일 초기화
        $NC.clearGridData(G_GRDDETAILC);
        // 파렛트ID 초기화
        ...
        $NC.clearGridData(G_GRDSUBC);
        // 마스터 총수량 초기화
        $NC.setValue("#edtTotal_Order_QtyC");
        $NC.setValue("#edtTotal_Entry_QtyC");
        $NC.setValue("#edtTotal_Vendor_CntC");
    }
