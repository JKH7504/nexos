...
function _NewB() {
    ...
    if ($NC.G_VAR.policyVal.RO210 != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.016", "반출등록 신규처리가 불가능한 사업부입니다."));
        return;
    }
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "ROC02011P0",
        PROGRAM_NM: $NC.getDisplayMsg("JS.ROC02011P0.001", "반출등록/수정"),
        url: "ro/ROC02011P0.html",
        width: 1024,
        height: 600,
        G_PARAMETER: {
...
function onProcessPreB() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.ROC02010E0.019", "반출등록 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.020", "반출등록 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.021", "선택한 데이터 중 반출등록 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtB() {
    ...
    if ($NC.isNull(OUTBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.022", "출고일자를 먼저 입력하십시오."));
        $NC.setFocus("#dtpOutbound_DateB");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.ROC02010E0.023", "반출등록 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.024", "반출등록 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.025", "선택한 데이터 중 반출등록 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterBInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterB", {
        ...
        queryId: "ROC02010E0.RS_T1_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options,
        canDblClick: true
    });
...
function grdMasterBOnDblClick(e, args) {
    ...
    if (!permission.canSave) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.026", "해당 프로그램의 저장권한이 없습니다."));
        return;
    }
    ...
    if (refRowData.OUTBOUND_STATE == $ND.C_STATE_CONFIRM) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.027", "반출작업에서 확정 된 전표는 수정할 수 없습니다."));
        return;
    }
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.ROC02010E0.028", "반출진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        ...
        if (refRowData.OUTBOUND_STATE != resultData.O_OUTBOUND_STATE) {
            alert($NC.getDisplayMsg("JS.ROC02010E0.029", "[진행상태 : " + resultData.O_OUTBOUND_STATE + "] 데이터가 변경되었습니다.\n다시 조회 후 데이터를 확인하십시오.",
        ...
        if (dsDetail.length == 0) {
            alert($NC.getDisplayMsg("JS.ROC02010E0.030", "등록/수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function grdDetailBInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailB", {
        ...
        queryId: "ROC02010E0.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
