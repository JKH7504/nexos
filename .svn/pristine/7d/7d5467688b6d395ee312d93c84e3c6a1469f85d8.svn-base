...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            return;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.ROC01010E1.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.ROC01010E1.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
        case "VENDOR_CD":
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.ROC01010E1.003", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.ROC01010E1.004", "사업부를 입력하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE1)) {
        alert($NC.getDisplayMsg("JS.ROC01010E1.005", "검색 시작일자를 입력하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE2)) {
        alert($NC.getDisplayMsg("JS.ROC01010E1.006", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQOrder_Date2");
        return;
    }

    if (ORDER_DATE1 > ORDER_DATE2) {
        alert($NC.getDisplayMsg("JS.ROC01010E1.007", "출고예정일자 범위 입력오류입니다."));
        $NC.setFocus("#dtpQOrder_Date1");
        return;
    }
...
function _New() {
    ...
    if ($NC.G_VAR.policyVal.RO110 != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.ROC01010E1.008", "출고예정정보 신규등록/수정이 불가능한 사업부입니다."));
        return;
    }
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "ROC01011P1",
        PROGRAM_NM: $NC.getDisplayMsg("JS.ROC01011P1.001", "반출예정등록/수정"),
        url: "ro/ROC01011P1.html",
        width: 1024,
        height: 600,
        G_PARAMETER: {
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.ROC01010E1.009", "삭제할 데이터가 없습니다."));
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    if ($NC.G_VAR.stateFWBW.CANCEL != rowData.OUTBOUND_STATE) {
        alert($NC.getDisplayMsg("JS.ROC01010E1.010", "삭제가 불가능한 전표입니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.ROC01010E1.011", "예정전표를 삭제하시겠습니까?"))) {
        return;
    }
...
function grdMasterOnDblClick(e, args) {
    ...
    if (!permission.canSave) {
        alert($NC.getDisplayMsg("JS.ROC01010E1.012", "해당 프로그램의 저장권한이 없습니다."));
        return;
    }
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.ROC01010E1.013", "반출진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        ...
        if (refRowData.OUTBOUND_STATE != resultData.O_OUTBOUND_STATE) {
            alert($NC.getDisplayMsg("JS.ROC01010E1.014", "[진행상태 : " + resultData.O_OUTBOUND_STATE + "] 데이터가 변경되었습니다.\n다시 조회 후 데이터를 확인하십시오.",
        ...
        if (dsDetail.length == 0) {
            alert($NC.getDisplayMsg("JS.ROC01010E1.015", "등록/수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function btnOrderClosingOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.ROC01010E1.016", "종결처리할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.ROC01010E1.017", "종결처리 하시겠습니까?"))) {
        return;
    }
