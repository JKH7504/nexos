...
function _SaveE() {
    ...
    if (!$NC.isGridValidLastRow(G_GRDMASTERE)) {
        return;
    }
    ...
    if (refRowData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
    ...
    if (refRowData.CRUD != $ND.C_DV_CRUD_R || dsDetail.length > 0) {
        $NC.serviceCall("/LOB02010E0/saveDelivery.do", {
            P_DS_MASTER: {
                ...
                P_DELIVERY_DATE: refRowData.DELIVERY_DATE,
                P_RETRIEVE_YN: refRowData.RETRIEVE_YN,
                P_RETRIEVE_USER_ID: refRowData.RETRIEVE_USER_ID,
                P_RETRIEVE_DATETIME: refRowData.RETRIEVE_DATETIME,
...
function onProcessPreE() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOB02010E0.092", "배송완료 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.093", "배송완료 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.094", "선택한 데이터 중 배송완료 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtE() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOB02010E0.095", "배송완료 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDDETAILE)) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.096", "데이터가 수정되었습니다. 저장 후 배송완료 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.097", "배송완료 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.098", "선택한 데이터 중 배송완료 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterEInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterE", {
        ...
        queryId: "LOB02010E0.RS_T4_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
    ...
    G_GRDMASTERE.view.onBeforeEditCell.subscribe(grdMasterEOnBeforeEditCell);
    G_GRDMASTERE.view.onCellChange.subscribe(grdMasterEOnCellChange);
...
/**
 * 배송완료처리 탭 : 상단그리드 편집불가능
 * 
 * @param e
 * @param args
 * @returns {Boolean}
 */
function grdMasterEOnBeforeEditCell(e, args) {

    var rowData = args.item;
    switch (args.column.id) {
        case "DELIVERY_DATE":
            return rowData.OUTBOUND_STATE < $ND.C_STATE_DELIVERY; // 배송 완료 전 수정 가능
        default:
            return false;
    }
}

function grdMasterEOnCellChange(e, args) {

    var rowData = args.item;

    // 마지막 선택 Row 수정 데이터 반영 및 상태 변경
    $NC.setGridApplyChange(G_GRDMASTERE, rowData);
}
...
function grdDetailEInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailE", {
        ...
        queryId: "LOB02010E0.RS_T4_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdDetailEOnBeforeEditCell(e, args) {
    ...
    var refRowData = G_GRDMASTERE.data.getItem(G_GRDMASTERE.lastRow);
    if (refRowData.INOUT_SUB_CD == $ND.C_INOUT_GRP_D2) {// 입출고그룹 "D2 - 수송출고" 일 경우 배송수량 수정 불가
        return false;
    }
...
function grdDetailEOnCellChange(e, args) {
    ...
    switch (G_GRDDETAILE.view.getColumnId(args.cell)) {
        case "DELIVERY_QTY":
            ...
            if (DELIVERY_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LOB02010E0.099", "배송수량에 0보다 작은값을 입력할 수 없습니다."));
                ...
                alert($NC.getDisplayMsg("JS.LOB02010E0.100", "배송수량이 확정수량을 초과할 수 없습니다."));
                rowData.DELIVERY_QTY = args.oldItem.DELIVERY_QTY;
                rowData.MISSED_QTY = args.oldItem.MISSED_QTY;
                $NC.setFocusGrid(G_GRDDETAILE, args.row, args.cell, true);
                break;
