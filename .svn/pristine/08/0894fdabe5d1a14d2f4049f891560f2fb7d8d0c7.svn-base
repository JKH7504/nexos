 ...
 *  ==================================================================================================================================================
 *  프로그램ID         : LOM02011P1
 *  프로그램명         : 출고등록 팝업[B2C] (의류)
 *  프로그램설명       : 출고등록 팝업[B2C] (의류) 화면 Javascript
...
function _Initialize() {
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.LOM02011P1.XXX", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.LOM02011P1.XXX", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.LOM02011P1.XXX", "저장"));
}
...
function _OnLoaded() {
    ...
    // 예정 -> 등록, 등록 수정
        ...
        $NC.G_VAR.masterData = {
            ...
            DISTRIBUTE_DIV: dsMaster.DISTRIBUTE_DIV,
        ...
        try {
            for (rIndex = 0, rCount = dsDetail.length; rIndex < rCount; rIndex++) {
                ...
                if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ORDER) { // 예정
                    ...
                    rowData.BU_DATE = dsMaster.BU_DATE;
                    rowData.BU_NO = dsMaster.BU_NO;
                    rowData.ORG_BU_NO = dsMaster.ORG_BU_NO;
                    rowData.BU_DATETIME = dsMaster.BU_DATETIME;
                }
                // 등록
                else {
                ...
                newRowData = {
                    ...
                    MARGIN_RATE: rowData.MARGIN_RATE,
...
function _New() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.DELIVERY_CD)) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.002", "먼저 온라인몰 코드를 입력하십시오."));
        $NC.setFocus("#edtDelivery_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_NM)) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.003", "먼저 주문자명을 입력하십시오."));
        $NC.setFocus("#edtOrderer_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_HP)) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.004", "먼저 주문자 휴대폰번호를 입력하십시오."));
        $NC.setFocus("#edtOrderer_Hp");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_NM)) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.005", "먼저 수령자명을 입력하십시오."));
        $NC.setFocus("#edtShipper_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_HP)) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.006", "먼저 수령자 휴대폰번호를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_ZIP_CD) || $NC.isNull($NC.G_VAR.masterData.SHIPPER_ADDR_BASIC)) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.007", "먼저 수령자 주소를 입력하십시오."));
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.009", "물류센터를 확인할 수 없습니다. 취소 후 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.010", "사업부를 확인할 수 없습니다. 취소 후 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.OUTBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.011", "먼저 출고일자를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.012", "먼저 출고구분을 선택하십시오."));
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.008", "저장할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOM02011P1.013", "저장하시겠습니까?"))) {
        return;
    }
    ...
    for (rIndex = 0, rCount = G_GRDDETAIL.data.getLength(); rIndex < rCount; rIndex++) {
        ...
        if (rowData.SHORTAGE_YN == $ND.C_YES) {
            alert($NC.getDisplayMsg("JS.LOM02011P1.014", "출고가능량을 초과한 상품이 존재합니다\n조정 후 저장하십시오."));
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.015", "데이터 수정 후 저장하십시오."));
        return;
    }
    ...
    $NC.serviceCall("/LOM02010E0/save.do", {
        ...
        P_DS_MASTER: {
            ...
            P_DISTRIBUTE_DIV: $NC.G_VAR.masterData.DISTRIBUTE_DIV,
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.016", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        ...
        case "ORDER_DIV":
            ...
            break;
        case "OUTBOUND_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LOM02011P1.017", "출고일자를 정확히 입력하십시오."));
        ...
        case "DELIVERY_CD":
            $NP.onDeliveryChange(args.val, {
                ...
                title: $NC.getDisplayMsg("JS.LOM02011P1.018", "온라인몰 검색"),
                columnTitle: [
                    "온라인몰코드",
                    "온라인몰명"
                ],
                errorMessage: $NC.getDisplayMsg("JS.LOM02011P1.019", "등록되어 있지 않은 온라인몰입니다.")
            });
            return;
        case "DELIVERY_BATCH":
...
function grdDetailOnCellChange(e, args) {
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        ...
        case "ENTRY_QTY":
            ...
            if (ENTRY_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LOM02011P1.020", "등록수량이 0보다 작을 수 없습니다."));
            ...
            if (Number(rowData.ORDER_LINE_NO) > 0 && $NC.getValue("#cboInout_Cd") != "D20") {
                if (Number(rowData.ORDER_QTY) < ENTRY_QTY) {
                    alert($NC.getDisplayMsg("JS.LOM02011P1.021", "등록수량이 예정수량을 초과할 수 없습니다."));
                    rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                    break;
                }
...
function grdDetailOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        ...
        if (Number(rowData.ORDER_QTY) > Number(rowData.ENTRY_QTY) && $NC.isNull(rowData.SHORTAGE_DIV)) {
            alert($NC.getDisplayMsg("JS.LOM02011P1.026", "미출고사유를 선택하십시오."));
...
function grdDetailOnCheckPStock(row, isCheckOnly) {
    ...
    $NC.serviceCallAndWait("/LOM02010E0/getData.do", {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LOM02011P1.027", "출고가능량 정보를 가져오지 못했습니다."));
            return;
        }
        var oMsg = $NC.getOutMessage(resultData);
        if (oMsg != $ND.C_OK) {
...
function showOutwaitOverlay(e) {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.028", "출고대기 내역을 확인할 상품을 선택하십시오."));
        return;
    }
    ...
    if (G_GRDOUTWAIT.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.029", "출고대기 내역이 존재하지 않습니다."));
        return;
    }

    $NC.showOverlay("#divOutwaitOverlay", {
        title: $NC.getDisplayMsg("JS.LOM02011P1.030", "출고대기 내역"),
        fullscreen: false,
        width: 700,
        height: 300,
        onComplete: function() {
...
function btnStockAddItemOnClick() {
    ...
    if (dsStock.length == 0) {
        alert($NC.getDisplayMsg("JS.LOM02011P1.031", "출고등록할 재고를 선택하십시오."));
        return;
    }
