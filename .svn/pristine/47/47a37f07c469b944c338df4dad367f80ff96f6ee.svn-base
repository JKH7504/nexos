...
function _InquiryC(args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOF02010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTERC), onGetMasterC);
}
...
function onProcessPreC() {
    ...
    if (rCount == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.026", "조회 후 처리하십시오."));
        return;
    }
    // if ($NC.G_VAR.PROCESS_YN != $ND.C_YES) {
    // alert($NC.getDisplayMsg("JS.LOF02010E0.027", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록 후 처리하십시오."));
    // return;
    // }

    var OUTBOUND_BATCH_F = $NC.getValueCombo("#cboQOutbound_BatchC", "F");
    if (!confirm($NC.getDisplayMsg("JS.LOF02010E0.040", "출고지시 취소 처리하시겠습니까?\n출고차수(" + OUTBOUND_BATCH_F + ")", OUTBOUND_BATCH_F))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.041", "출고지시 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.042", "선택한 데이터 중 출고지시 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtC() {
    ...
    if (DIRECTIONS_DIV == "2" && !batchInfo.isTotal) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.043", "[신규] 또는 [토탈출고] 차수를 선택하고 출고지시 처리하십시오."));
    ...
    if (DIRECTIONS_DIV == "1") {
        if (!confirm($NC.getDisplayMsg("JS.LOF02010E0.044", "개별 출고지시 처리하시겠습니까?"))) {
            return;
        }
    } else {
        if (!confirm($NC.getDisplayMsg("JS.LOF02010E0.045", "출고지시 처리하시겠습니까?\n출고차수(" + OUTBOUND_BATCH_F + ")", OUTBOUND_BATCH_F))) {
            return;
        }
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.046", "출고지시 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.047", "선택한 데이터 중 출고지시 처리 가능한 데이터가 없습니다."));
        return;
    }
    ...
    if (DIRECTIONS_DIV == "2") {
        ...
        // 기존 차수에 추가
            ...
            // 기존 출고차수에 추가 가능한지 체크
            $NC.serviceCallAndWait("/LOF02010E0/getDataSet.do", {
                P_QUERY_ID: "LOF02010E0.RS_SUB3",
                P_QUERY_PARAMS: {
                ...
                if ($NC.isNull(dsResult) || dsResult.length == 0) {
                    alert($NC.getDisplayMsg("JS.LOF02010E0.048", "기존 출고차수에 추가 가능여부를 확인하지 못했습니다. 다시 처리하십시오."));
                    return;
                }

                if (dsResult[0].EQUIP_STATE > "01") {
                    alert($NC.getDisplayMsg("JS.LOF02010E0.049", "출고차수(" + OUTBOUND_BATCH_F + ")는 이미 출하장비로 전송되어 해당 차수로 추가 출고지시할 수 없습니다.",
                        OUTBOUND_BATCH_F));
                    return;
                }

                if (EQUIP_DIV != dsResult[0].EQUIP_DIV) {
                    alert($NC.getDisplayMsg("JS.LOF02010E0.050", "해당 차수는 다른 출하장비(" + dsResult[0].EQUIP_DIV_D + ")로 할당되어 추가 출고지시할 수 없습니다.",
                        dsResult[0].EQUIP_DIV_D));
                    return;
                }
    ...
    if (OUTBOUND_BATCH == $ND.C_BASE_BATCH_NO) {
        $NC.serviceCallAndWait("/LOF02010E0/getOutboundBatch.do", {
            P_CENTER_CD: CENTER_CD,
            P_BU_CD: BU_CD,
            P_OUTBOUND_DATE: OUTBOUND_DATE,
...
function grdMasterCInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterC", {
        ...
        queryId: "LOF02010E0.RS_T2_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
...
function grdMasterCOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOF02010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAILC), onGetDetailC);
...
function grdDetailCInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailC", {
        ...
        queryId: "LOF02010E0.RS_T2_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function btnBillCntOnClick() {
    ...
    if (billCount <= 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.051", "1이상의 전표수를 입력하십시오."));
...
function btnEquipSendOnClick() {
    ...
    // 신규차수
    if (batchInfo.isNew) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.052", "[출하장비전송] 처리할 출고차수를 선택하십시오."));
        return;
    }
    // 개별차수
    else if (!batchInfo.isTotal) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.053", "[토탈출고] 차수를 선택하고 [출하장비전송] 처리하십시오."));
        return;
    }

    var EQUIP_DIV = $NC.getValue("#cboEquip_Div");
    if ($NC.isNull(EQUIP_DIV)) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.054", "출하장비구분을 선택하십시오."));
    ...
    if ($NC.isNull(EQUIP_ALLOC_DIV)) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.055", "할당구분을 선택하십시오."));
    ...
    $NC.serviceCall("/LOB02010E0/getDataSet.do", {
        ...
        if ($NC.isNull(dsResult)) {
            alert($NC.getDisplayMsg("JS.LOF02010E0.056", "소터투입 불가 상품 체크에 오류가 발생 했습니다."));
            return;
        }
        ...
        if (!confirm($NC.getDisplayMsg("JS.LOF02010E0.057", message + "출하장비전송 처리 하시겠습니까?", message))) {
            return;
        }
        ...
        $NC.serviceCall("/LOB02010E0/callSendFwLoBatch.do", {
            ...
            alert($NC.getDisplayMsg("JS.LOF02010E0.058", "[출하장비전송] 처리가 완료되었습니다."));
            window._Inquiry();
        });
    });
}
...
function btnEquipSendCancelOnClick() {
    ...
    // 신규차수
    if (batchInfo.isNew) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.059", "[출하장비전송취소] 처리할 출고차수를 선택하십시오."));
        return;
    }
    // 개별차수
    else if (!batchInfo.isTotal) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.060", "[토탈출고] 차수를 선택하고 [출하장비전송취소] 처리하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOF02010E0.061", "출하장비전송취소 처리 하시겠습니까?"))) {
        return;
    }
    ...
    $NC.serviceCall("/LOB02010E0/callSendBwLoBatch.do", {
        ...
        alert($NC.getDisplayMsg("JS.LOF02010E0.062", "[출하장비전송취소] 처리가 완료되었습니다."));
        window._Inquiry();
    });
}
...
function btnOrderGroupOnClick() {
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "LOF02012P0",
        PROGRAM_NM: $NC.getDisplayMsg("JS.JS.LOF02012P0.001", "주문구성선택"),
        url: "lo/LOF02012P0.html",
        width: 900,
        height: 500,
        G_PARAMETER: {
            P_CENTER_CD: CENTER_CD,
            ...
            P_ITEM_CNT1: ITEM_CNT1,
            P_ITEM_CNT2: ITEM_CNT2,
            P_TOT_ITEM_QTY1: TOT_ITEM_QTY1,
            P_TOT_ITEM_QTY2: TOT_ITEM_QTY2
        },
        onOk: function(resultInfo) {
            ...
            for (var i = 0; i < resultInfo.length; i++) {
                for (rIndex = 0; rIndex < rCount; rIndex++) {
                    ...
                    if (resultInfo[i].LIST_ITEM_CD == rowData.LIST_ITEM_CD) {
                        rowData.CHECK_YN = $ND.C_YES;
                        G_GRDMASTERC.data.updateItem(rowData.id, rowData);
                    }
                }
