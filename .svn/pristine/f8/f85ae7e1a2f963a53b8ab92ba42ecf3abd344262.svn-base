...
function _Initialize() {
    ...
    $NC.setGlobalVar({
        autoResizeView: function() {
            ...
            // 활성 탭에 따라 리사이즈 그리드 변경
            var activeTab = $NC.getTabActiveIndex("#ctrMasterView");
            if (activeTab == 0) {
                resizeView.grids = [
                    "#grdT1Master",
                    "#grdT1Detail"
    ...
    // $NC.setVisibleTab("#ctrMasterView", false, 1);
    ...
    $NC.setProgramReportInfo({
        ...
        // TAB별로 구분
        if (reportInfo.PROGRAM_SUB_CD == "T1") {
            $NC.G_VAR.printOptionT1.push(reportInfo);
        } else if (reportInfo.PROGRAM_SUB_CD == "T2") {
            $NC.G_VAR.printOptionT2.push(reportInfo);
        }
        if (reportInfo.PROGRAM_SUB_CD == "*") {
            $NC.G_VAR.printOptionT1.push(reportInfo);
            $NC.G_VAR.printOptionT2.push(reportInfo);
        }
        return true;
    ...
    $NC.setTooltip("#btnMoveProcessNxt", $NC.getDisplayMsg("JS.COMMON.040", "확정 처리"));
}
...
function _OnLoaded() {
    ...
    $NC.setInitSplitter("#ctrT1TabSheetView", "h", 300);
}
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            if ($NC.isNull(BU_CD)) {
                alert($NC.getDisplayMsg("JS.LCC02040E0.030", "사업부를 먼저 선택 하십시오."));
            ...
            return;
        case "ETC_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02040E0.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ETC_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02040E0.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.003", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.004", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var ETC_DATE1 = $NC.getValue("#dtpQEtc_Date1");
    if ($NC.isNull(ETC_DATE1)) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.005", "검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQEtc_Date1");
        return;
    }
    var ETC_DATE2 = $NC.getValue("#dtpQEtc_Date2");
    if ($NC.isNull(ETC_DATE2)) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.006", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQEtc_Date2");
        return;
    }
    if (ETC_DATE1 > ETC_DATE2) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.007", "입출고일자 검색 범위 오류입니다."));
        $NC.setFocus("#dtpQEtc_Date1");
        return;
    }
    ...
    if (activeTab == 0) {
        ...
        // 데이터 조회
        $NC.serviceCall("/LCC02040E0/getDataSet.do", $NC.getGridParams(G_GRDT1MASTER), onGetT1Master);
    }
    // 재고이동 화면
    else if (activeTab == 1) {
        // 조회시 전역 변수 값 초기화
        ...
        // 데이터 조회
        $NC.serviceCall("/LCC02040E0/getDataSet.do", $NC.getGridParams(G_GRDT2MASTER), onGetT2Master);
    }
    // 내역 조회
    else {
        ...
        // 데이터 조회
        $NC.serviceCall("/LCC02040E0/getDataSet.do", $NC.getGridParams(G_GRDT3MASTER), onGetT3Master);
    }
}
...
function _New() {
    ...
    if ($NC.getTabActiveIndex("#ctrMasterView") == 0) {
        ...
        $NC.showProgramSubPopup({
            PROGRAM_ID: "LCC02041P0",
            PROGRAM_NM: $NC.getDisplayMsg("JS.LCC02040E0.008", "세트상품 등록"),
            url: "lc/LCC02041P0.html",
            width: 1024,
            height: 600,
            G_PARAMETER: {
...
function _Save() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.031", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.032", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LCC02040E0/updateRemark.do", {
...
function _Delete() {
    ...
    var activeTab = $NC.getTabActiveIndex("#ctrMasterView"), rowData;
    // 등록 화면
    if (activeTab == 0) {

        if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
            alert($NC.getDisplayMsg("JS.LCC02040E0.009", "삭제할 데이터가 없습니다."));
            return;
        }

        rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
        if (rowData.CONFIRM_YN == $ND.C_YES) {
            alert($NC.getDisplayMsg("JS.LCC02040E0.010", "이미 확정한 데이터는 삭제할수 없습니다."));
            return;
        }

        if (rowData.ETC_STATE == $ND.C_STATE_ORDER) {
            alert($NC.getDisplayMsg("JS.LCC02040E0.011", "예정 데이터는 삭제할 수 없습니다."));
            return;
        }

        if (!confirm($NC.getDisplayMsg("JS.LCC02040E0.012", "삭제 하시겠습니까?"))) {
            return;
        }

        $NC.serviceCall("/LCC02040E0/callLCProcessing.do", {
            P_PROCESS_DIV: "DEL",
            P_CENTER_CD: rowData.CENTER_CD,
            P_BU_CD: rowData.BU_CD,
            P_ETC_DATE: rowData.ETC_DATE,
    ...
    // 재고이동 화면
        ...
        rowData = G_GRDT2MASTER.data.getItem(G_GRDT2MASTER.lastRow);
        if (rowData.CONFIRM_YN == $ND.C_YES) {
            alert($NC.getDisplayMsg("JS.LCC02040E0.010", "이미 확정한 데이터는 삭제할수 없습니다."));
            return;
        }

        if ($NC.isNull(rowData.MOVE_NO)) {
            alert($NC.getDisplayMsg("JS.LCC02040E0.013", "재고이동 데이터를 생성하지 않은 데이터입니다."));
            return;
        }
        ...
        $NC.serviceCall("/LCC02040E0/callLCBWSetMoveEntry.do", {
            P_CENTER_CD: rowData.CENTER_CD,
            P_BU_CD: rowData.BU_CD,
...
function _Print(reportInfo) {
    ...
    if (activeTab == 0) {
        ...
        if ($NC.isNull(rowData)) {
            alert($NC.getDisplayMsg("JS.LCC02040E0.014", "출력할 데이터를 먼저 선택하십시오."));
            return;
        }

        if (rowData.ETC_STATE == $ND.C_STATE_ORDER) {
            alert($NC.getDisplayMsg("JS.LCC02040E0.015", "예정 데이터입니다. 등록 처리를 먼저하십시오."));
            return;
        }
    }
    // 재고이동
        ...
        if ($NC.isNull(rowData.MOVE_NO)) {
            alert($NC.getDisplayMsg("JS.LCC02040E0.016", "재고이동 등록하지 않은 예정 데이터입니다.\n재고이동 등록 처리를 먼저하십시오."));
            return;
        }
    }
...
function onSave(ajaxData) {
    ...
    var activeTab = $NC.getTabActiveIndex("#ctrMasterView"), lastKeyVal;
    // 세트변환 등록
    if (activeTab == 0) {
        lastKeyVal = $NC.getGridLastKeyVal(G_GRDT1MASTER, {
            selectKey: [
                "ETC_DATE",
                "ETC_NO",
                "ORDER_DATE",
    ...
    // 재고이동 등록
        ...
        lastKeyVal = $NC.getGridLastKeyVal(G_GRDT2MASTER, {
            selectKey: [
                "ORDER_DATE",
                "ORDER_NO"
            ]
...
function tabOnActivate(event, ui) {
    ...
    var activeTab = $NC.getTabActiveIndex("#ctrMasterView"), activeTabSheet;
    // Active시 Resize 호출
    if (activeTab == 2) {
        $NC.onGlobalResize();
    } else {
        activeTabSheet = activeTab == 0 ? "#ctrT1TabSheetView" : "#ctrT2TabSheetView";
        // 스플리터가 초기화가 되어 있으면 _OnResize 호출
        if ($NC.isSplitter(activeTabSheet)) {
            // 스필리터를 통한 _OnResize 호출
            $(activeTabSheet).trigger("resize");
        } else {
            // 스플리터 초기화
            $NC.setInitSplitter(activeTabSheet, "h", 300);
        }
    }
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "LCC02040E0.RS_T1_MASTER",
        sortCol: "ETC_DATE",
        gridOptions: options,
        canDblClick: true
    });
...
function grdT1MasterOnDblClick(e, args) {
    ...
    if (refRowData.ETC_STATE != $ND.C_STATE_ORDER) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.017", "예정 상태의 전표만 처리할 수 있습니다."));
        return;
    }
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LCC02040E0.018", "진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        var oMsg = $NC.getOutMessage(resultData);
        if (oMsg != $ND.C_OK) {
        ...
        if (refRowData.ETC_STATE != resultData.O_ETC_STATE) {
            alert($NC.getDisplayMsg("JS.LCC02040E0.019", "[진행상태 : " + resultData.O_ETC_STATE + "] 데이터가 변경되었습니다.\n다시 조회 후 데이터를 확인하십시오.",
        ...
        // 작업구분 -> 1, 세트구성, 2, 세트해체
        // 세트 상품 데이터가 필요하기 때문에 작업구분에 따라 조회
        var ORDER_DATE, ORDER_NO;
        if (refRowData.INOUT_WORK_DIV == "1") {
            ORDER_DATE = refRowData.LINK_ORDER_DATE;
            ORDER_NO = refRowData.LINK_ORDER_NO;
        } else {
            ORDER_DATE = refRowData.ORDER_DATE;
            ORDER_NO = refRowData.ORDER_NO;
        }
        $NC.serviceCallAndWait("/LCC02040E0/getDataSet.do", {
            P_QUERY_ID: "LCC02040E0.RS_SUB4",
            P_QUERY_PARAMS: {
                P_CENTER_CD: refRowData.CENTER_CD,
                P_BU_CD: refRowData.BU_CD,
        ...
        if (dsDetail.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC02040E0.020", "등록할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function grdT1MasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LCC02040E0/getDataSet.do", $NC.getGridParams(G_GRDT1DETAIL), onGetT1Detail);
...
/**
 * 세트변환 등록 탭 : 상태가 등록인 경우만 그리드 편집 가능
 * 
 * @param e
 * @param args
 * @returns {Boolean}
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "LCC02040E0.RS_T2_MASTER",
        sortCol: "ORDER_DATE",
        gridOptions: options,
        canDblClick: true
    });
...
function grdT2MasterOnDblClick(e, args) {
    ...
    var refRowData = G_GRDT2MASTER.data.getItem(args.row);
    if ($NC.isNull(refRowData)) {
        return;
    }

    // 등록한 전표는 수정불가
    if ($NC.isNotNull(refRowData.MOVE_NO)) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.021", "이미 재고이동 등록되어 있습니다."));
        return;
    }
    ...
    function(ajaxData) {
        ...
        $NC.showProgramSubPopup({
            PROGRAM_ID: "LCC02042P0",
            PROGRAM_NM: $NC.getDisplayMsg("JS.LCC02040E0.022", "세트변환 재고이동 등록"),
            url: "lc/LCC02042P0.html",
            width: 1024,
            height: 600,
            G_PARAMETER: {
                P_PROCESS_CD: $ND.C_PROCESS_ORDER,
                ...
                P_BU_CD: BU_CD,
                P_BU_NM: BU_NM,
                P_MOVE_DATE: refRowData.ORDER_DATE,
                P_MOVE_NO: refRowData.ORDER_NO,
                P_MASTER_DS: refRowData,
                P_DETAIL_DS: dsDetail,
                P_SUB_DS: null,
                P_PERMISSION: permission
...
function grdT2MasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LCC02040E0/getDataSet.do", $NC.getGridParams(G_GRDT2DETAIL), onGetT2Detail);
...
function grdT2DetailInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Detail", {
        ...
        queryId: "LCC02040E0.RS_T2_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdT3MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT3Master", {
        ...
        queryId: "LCC02040E0.RS_T3_MASTER",
        sortCol: "ETC_DATE",
        gridOptions: options
    });
...
function onGetT2Detail(ajaxData) {
    ...
    $NC.setInitGridAfterOpen(G_GRDT2DETAIL, "LINE_NO");
}
...
function setUserProgramPermission() {
    ...
    if (permission.canConfirm) {
        $("#btnMoveProcessNxt").click(onMoveProcessNxt);
    }
    $NC.setEnable("#btnMoveProcessNxt", permission.canConfirm);
...
function onProcessPre() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.023", "조회 후 처리하십시오."));
        return;
    }

    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.CONFIRM_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.024", "확정된 전표만 취소 할 수 있습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC02040E0.025", "취소 처리 하시겠습니까?"))) {
        return;
    }
...
function onProcessNxt() {
    ...
    if (rowData.CONFIRM_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.026", "이미 확정 처리한 데이터입니다."));
        return;
    }

    if (rowData.ETC_STATE == $ND.C_STATE_ORDER) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.027", "예정 데이터입니다. 등록 처리를 먼저 하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC02040E0.028", "확정 처리 하시겠습니까?"))) {
        return;
    }
...
function onMoveProcessNxt() {
    ...
    if ($NC.isNull(rowData.MOVE_NO)) {
        alert($NC.getDisplayMsg("JS.LCC02040E0.029", "재고이동 등록되지 않은 데이터입니다. 등록 처리를 먼저 하십시오."));
        return;
    }
    ...
    $NC.serviceCall("/LCC02040E0/callLCFWMoveConfirm.do", {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_MOVE_DATE: rowData.MOVE_DATE,
        P_MOVE_NO: rowData.MOVE_NO,
    ...
    if (activeTab == 0) {
        if ($NC.isNotNull(G_GRDT1MASTER.queryParams)) {
            ...
            if (G_GRDT1MASTER.data.getLength() > 0) {
                ...
                $NC.G_VAR.printOptions = $NC.G_VAR.printOptionT1; // 레포트
            }
        }
    }
    // 재고이동 등록 탭
        ...
        if ($NC.isNotNull(G_GRDT2MASTER.queryParams)) {
            ...
            if (G_GRDT2MASTER.data.getLength() > 0) {
                $NC.G_VAR.buttons._print = "1";
                $NC.G_VAR.printOptions = $NC.G_VAR.printOptionT2; // 레포트
            }
        }
    }
...
function getEtcState(params, onSuccess) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LCC02040E0/getData.do", {
