...
/**
 * Class: ROC02010E0DAOImpl<br>
 * Description: ROC02010E0 DAO (Data Access Object)<br>
 ...
 *          </pre>
 */
@Repository("ROC02010E0DAO")
public class ROC02010E0DAOImpl extends DaoSupport implements ROC02010E0DAO {

    // private final Logger logger = LoggerFactory.getLogger(ROC02010E0DAOImpl.class);
    final String PROGRAM_ID                 = "ROC02010E0";

    final String TABLE_NM_RO020NM           = "RO020NM";
    final String INSERT_ID_RO020NM          = PROGRAM_ID + ".INSERT_" + TABLE_NM_RO020NM;
    final String UPDATE_ID_RO020NM          = PROGRAM_ID + ".UPDATE_" + TABLE_NM_RO020NM;
    final String DELETE_ID_RO020NM          = PROGRAM_ID + ".DELETE_" + TABLE_NM_RO020NM;

    final String TABLE_NM_RO020ND           = "RO020ND";
    final String INSERT_ID_RO020ND          = PROGRAM_ID + ".INSERT_" + TABLE_NM_RO020ND;
    final String UPDATE_ID_RO020ND          = PROGRAM_ID + ".UPDATE_" + TABLE_NM_RO020ND;
    final String DELETE_ID_RO020ND          = PROGRAM_ID + ".DELETE_" + TABLE_NM_RO020ND;

    final String TABLE_NM_RO030NM           = "RO030NM";
    final String UPDATE_ID_RO030NM          = PROGRAM_ID + ".UPDATE_" + TABLE_NM_RO030NM;

    final String SP_ID_RO_PROCESSING        = "RO_PROCESSING";
    final String SP_ID_RO_020NM_GETNO       = "WT.RO_020NM_GETNO";
    final String SP_ID_RO_020ND_GETNO       = "WT.RO_020ND_GETNO";
    final String SP_ID_RO_OUTBOUND_STATE    = "WF.GET_RO_OUTBOUND_STATE";
    final String SP_ID_RO_POLICY_ENTRY_INIT = "RO_POLICY_ENTRY_INIT";
    final String SP_ID_RO_020ND_QTY_UPDATE  = "RO_020ND_QTY_UPDATE";
    final String SP_ID_GET_RO_PSTOCK_QTY    = "WF.GET_RO_PSTOCK_QTY";
    ...
    private void checkingPStockQty(Map<String, Object> rowData, int rownum) {
        ...
        Map<String, Object> resultMap = callProcedure(SP_ID_GET_RO_PSTOCK_QTY, callParams);
        ...
        if (sumEntryQty > pstockQty) {
            String errMsg = NexosMessage.getDisplayMsg("JAVA.ROC02010E0.003",
    ...
    public void save(Map<String, Object> params) throws Exception {
        ...
        if (Consts.PROCESS_ORDER.equals(PROCESS_CD) || Consts.PROCESS_ENTRY_CREATE.equals(PROCESS_CD)) {
            ...
            if (dsCount < 1) {
                throw new RuntimeException(NexosMessage.getDisplayMsg("JAVA.ROC02010E0.001", "반출등록 상세내역이 존재하지 않습니다."));
            }
            ...
            Map<String, Object> resultMap = callProcedure(SP_ID_RO_020NM_GETNO, callParams);
            String oMsg = Util.getOutMessage(resultMap);
            if (!Consts.OK.equals(oMsg)) {
                throw new RuntimeException(oMsg);
            }
            ...
            // 마스터 생성, CRUD 체크 안함
            insertSql(INSERT_ID_RO020NM, masterRowData);
        }
        // BU: 수정 처리
        else {
            // 반출순번 채번
            ...
            Map<String, Object> resultMap = callProcedure(SP_ID_RO_020ND_GETNO, callParams);
            String oMsg = Util.getOutMessage(resultMap);
            if (!Consts.OK.equals(oMsg)) {
                throw new RuntimeException(oMsg);
            }
            ...
            if (Consts.DV_CRUD_U.equals(masterRowData.get(Consts.PK_CRUD))) {
                updateSql(UPDATE_ID_RO020NM, masterRowData);
            }
        }
        ...
        if (dsCount > 0) {
            ...
            Map<String, Object> resultMap = callProcedure(SP_ID_RO_POLICY_ENTRY_INIT, callParams);
            String oMsg = Util.getOutMessage(resultMap);
            if (!Consts.OK.equals(oMsg)) {
                throw new RuntimeException(oMsg);
            }
            ...
            for (int i = 0; i < dsCount; i++) {
                ...
                if (Consts.DV_CRUD_C.equals(rowCrud)) {
                    ...
                    insertSql(INSERT_ID_RO020ND, rowData);
                } else if (Consts.DV_CRUD_U.equals(rowCrud)) {
                    updateSql(UPDATE_ID_RO020ND, rowData);
                } else if (Consts.DV_CRUD_D.equals(rowCrud)) {
                    deleteSql(DELETE_ID_RO020ND, rowData);
                }
            }
            ...
            resultMap = callProcedure(SP_ID_RO_PROCESSING, callParams);
        ...
        // 신규등록 혹은 수정등록의 후처리
        // this.closingCarplanCreate(dsMaster, userId);
    }
    ...
    public void saveConfirm(Map<String, Object> params) throws Exception {
        ...
        for (int i = 0; i < dsCount; i++) {
            ...
            if (Consts.DV_CRUD_U.equals(rowCrud)) {
                updateSql(UPDATE_ID_RO030NM, rowData);
                ...
                Map<String, Object> resultMap = callProcedure(SP_ID_RO_020ND_QTY_UPDATE, callParams);
                String oMsg = Util.getOutMessage(resultMap);
                if (!Consts.OK.equals(oMsg)) {
                    throw new RuntimeException(oMsg);
                }
