...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "RECV_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.EDR06010E0.020", "수신 일자를 정확히 입력하십시오."));
            break;
        case "RECV_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.EDR06010E0.001", "수신 시작일자를 정확히 입력하십시오."));
            break;
        case "RECV_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.EDR06010E0.002", "수신 종료일자를 정확히 입력하십시오."));
            break;
    }
    // 화면클리어
    onChangingCondition();
...
function _Inquiry() {
    ...
    if ($NC.isNull(BU_NM)) {
        alert($NC.getDisplayMsg("JS.EDR06010E0.003", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var DEFINE_NO = $NC.getValueCombo("#cboQDefine_No");
    if ($NC.isNull(DEFINE_NO)) {
        alert($NC.getDisplayMsg("JS.EDR06010E0.004", "수신정의를 선택하십시오."));
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        if ($NC.isNull(RECV_DATE1)) {
            alert($NC.getDisplayMsg("JS.EDR06010E0.005", "수신 시작일자를 입력하십시오."));
            $NC.setFocus("#dtpQRecv_Date1");
            return;
        }
        var RECV_DATE2 = $NC.getValue("#dtpQRecv_Date2");
        if ($NC.isNull(RECV_DATE2)) {
            alert($NC.getDisplayMsg("JS.EDR06010E0.006", "수신 종료일자를 입력하십시오."));
        ...
        // 데이터 조회
        $NC.serviceCall("/EDR06010E0/getDataSet.do", $NC.getGridParams(G_GRDT1MASTER), onGetT1Master);
    } else {
        var RECV_DATE = $NC.getValue("#dtpQRecv_Date");
        if ($NC.isNull(RECV_DATE)) {
            alert($NC.getDisplayMsg("JS.EDR06010E0.021", "수신 일자를 입력하십시오."));
        ...
        // 데이터 조회
        $NC.serviceCall("/EDR06010E0/getDataSet.do", $NC.getGridParams(G_GRDT2MASTER), onGetT2Master);
    }
}
...
function _Delete() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.EDR06010E0.007", "삭제할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.EDR06010E0.008", "선택한 수신처리내역을 삭제하시겠습니까?"))) {
        return;
    }

    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    // ER_PROCESSING 호출
    $NC.serviceCall("/EDR06010E0/callERROOrderDelete.do", {
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "EDR06010E0.RS_T1_MASTER",
        sortCol: "RECV_DATE",
        gridOptions: options
    });
...
function grdT1MasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/EDR06010E0/getDataSet.do", $NC.getGridParams(G_GRDT1DETAIL), onGetT1Detail);
...
function grdT1DetailInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Detail", {
        ...
        queryId: "EDR06010E0.RS_T1_DETAIL",
        sortCol: "RECV_SEQ",
        gridOptions: options
    });
...
function grdT1DetailOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/EDR06010E0/getDataSet.do", $NC.getGridParams(G_GRDT1SUB), onGetT1Sub);
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "EDR06010E0.RS_T2_MASTER",
        sortCol: "RECV_NO",
        gridOptions: options
    });
...
function onGetRecvClose(ajaxData) {
    ...
    alert($NC.getDisplayMsg("JS.EDR06010E0.009", "정상적으로 종결처리 되었습니다."));
}
...
function onGetDelete(ajaxData) {
    ...
    _Inquiry();
    G_GRDT1MASTER.lastKeyVal = lastKeyVal;
    alert($NC.getDisplayMsg("JS.EDR06010E0.010", "정상적으로 삭제처리 되었습니다."));
}
...
function btnRecvOnClick(e) {
    ...
    if (!$NC.getProgramPermission().canSave) {
        alert($NC.getDisplayMsg("JS.EDR06010E0.011", "해당 프로그램의 저장권한이 없습니다."));
        return;
    }
    ...
    if ($NC.isNull(DATA_DIV)) {
        alert($NC.getDisplayMsg("JS.EDR06010E0.012", "수신정의를 먼저 선택하십시오."));
        return;
    }
    var DEFINE_NO = $NC.getValueCombo("#cboQDefine_No");
    ...
    if (DATA_DIV.startsWith("3") && $NC.isNull(REMOTE_DIV)) {
        ...
        switch (DATA_DIV) {
            ...
            case $ND.C_DATA_DIV_JSON:
                ...
                break;
            default:
                alert($NC.getDisplayMsg("JS.EDR06010E0.013", "자료구분 값이 잘못 지정되었습니다."));
                return;
        }
        $NC.showUploadFilePopup(accept, function(view, fileName) {
            ...
            if (DATA_DIV == $ND.C_DATA_DIV_EXCEL) {
                if (fileExt != "xlsx" && fileExt != "xls") {
                    alert($NC.getDisplayMsg("JS.EDR06010E0.014", "엑셀 파일(*.xls, *.xlsx) 파일을 선택하십시오."));
                    return;
                }
            } else if (DATA_DIV == $ND.C_DATA_DIV_TEXT) {
                if (fileExt != "txt") {
                    alert($NC.getDisplayMsg("JS.EDR06010E0.015", "텍스트 파일(*.txt) 파일을 선택하십시오."));
                    return;
                }
            } else if (DATA_DIV == $ND.C_DATA_DIV_XML) {
                if (fileExt != "xml") {
                    alert($NC.getDisplayMsg("JS.EDR06010E0.016", "XML 파일(*.xml) 파일을 선택하십시오."));
                    return;
                }
            } else if (DATA_DIV == $ND.C_DATA_DIV_JSON) {
                if (fileExt != "json") {
                    alert($NC.getDisplayMsg("JS.EDR06010E0.017", "JSON 파일(*.json) 파일을 선택하십시오."));
                    return;
                }
            } else {
                alert($NC.getDisplayMsg("JS.EDR06010E0.013", "자료구분 값이 잘못 지정되었습니다."));
            ...
            $NC.fileUpload("/EDR06010E0/recvFile.do", {
                P_BU_CD: BU_CD,
        ...
        $NC.serviceCall("/EDR06010E0/recvProcessing.do", {
            P_BU_CD: BU_CD,
            P_EDI_DIV: $NC.G_VAR.EDI_DIV,
            P_DEFINE_NO: DEFINE_NO,
            P_RECV_DATE: null,
...
function btnErrorProcessAndColseOnClick(e) {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.EDR06010E0.018", "처리할 대상을 선택하십시오."));
        return;
    }
    ...
    if (Number(rowData.RECV_CNT0) + Number(rowData.RECV_CNT1) + Number(rowData.RECV_CNT2) //
        ...
        alert($NC.getDisplayMsg("JS.EDR06010E0.019", "[오류/종결]처리 가능한 데이터를 선택하십시오."));
        return;
    }
...
function onGetDefineNo() {
    ...
    $NC.setInitCombo("/EDR06010E0/getDataSet.do", {
