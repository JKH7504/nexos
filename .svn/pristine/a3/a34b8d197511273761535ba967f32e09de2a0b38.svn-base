...
function onProcessPreD() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.074", "출고확정 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.075", "출고확정취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.076", "선택한 데이터 중 출고확정취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtD() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.077", "출고확정 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBD)) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.078", "데이터가 수정되었습니다. 저장 후 출고확정 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.079", "출고확정 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.080", "선택한 데이터 중 확정처리 가능한 데이터가 없습니다."));
        return;
    }
    if ($NC.G_VAR.policyVal.LO490 == $ND.C_YES && noInspectCount > 0) {
        if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.081", "검수 완료 되지 않은 전표가 존재합니다. \n확정 처리하시겠습니까?"))) {
            return;
        }
    }
...
function grdMasterDInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterD", {
        ...
        queryId: "LOB02010E1.RS_T3_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
...
function grdMasterDOnAfterScroll(e, args) {
    ...
    // 직송처리/취소 버튼 활성화
    $NC.setEnable("#btnSetDirectYn");
    // 직송여부 값에 따라 button value toggle
    $NC.setValue("#btnSetDirectYn", rowData.DIRECT_YN == $ND.C_YES ? "직송취소" : "직송처리");
...
function grdDetailDInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailD", {
        ...
        queryId: "LOB02010E1.RS_T3_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdSubDOnCellChange(e, args) {
    ...
    switch (G_GRDSUBD.view.getColumnId(args.cell)) {
        case "CONFIRM_QTY":
            ...
            if (CONFIRM_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LOB02010E1.082", "확정수량이 0보다 작을 수 없습니다."));
            ...
            if (Number(rowData.ENTRY_QTY) < CONFIRM_QTY) {
                alert($NC.getDisplayMsg("JS.LOB02010E1.083", "확정수량이 등록수량을 초과할 수 없습니다."));
                rowData.CONFIRM_QTY = args.oldItem.CONFIRM_QTY;
                rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
                rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
                rowData.CONFIRM_WEIGHT = args.oldItem.CONFIRM_WEIGHT;
...
function grdSubDOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONFIRM_QTY)) {
            alert($NC.getDisplayMsg("JS.LOB02010E1.084", "확정수량을 입력하십시오."));
...
function onGetMasterD(ajaxData) {
    ...
    if ($NC.setInitGridAfterOpen(G_GRDMASTERD, "OUTBOUND_NO", true)) {
        ...
        $NC.setValue("#edtTotal_Rdelivery_CntD", $NC.getDisplayNumber(rowData.TOTAL_RDELIVERY_CNT));
    } else {
        // 디테일 초기화
        $NC.clearGridData(G_GRDDETAILD);
        // 출고지시 초기화
...
function btnSetDirectYnOnClick() {
    ...
    if (rowData.OUTBOUND_STATE != PROCESS_STATE) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.085", "출고확정 후 처리하십시오."));
        return;
    }

    if (rowData.DIRECT_YN == $ND.C_YES) {
        if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.086", "직송취소로 변경 하시겠습니까?"))) {
            return;
        }
    } else {
        if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.087", "직송처리로 변경 하시겠습니까?"))) {
            return;
        }
    }
