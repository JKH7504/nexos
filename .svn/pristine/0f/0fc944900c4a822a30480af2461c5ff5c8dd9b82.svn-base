 ...
 *  프로그램ID         : LIC05021P0
 *  프로그램명         : 입고중량등록 팝업
 *  프로그램설명       : 입고중량등록 팝업 화면 Javascript
...
function _Initialize() {
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.LIC05021P0.001", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.LIC05021P0.002", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.LIC05021P0.003", "저장"));
}
...
function _OnLoaded() {
    ...
    $NC.G_VAR.masterData = {
        ...
        INBOUND_DATE: $NC.G_VAR.G_PARAMETER.P_INBOUND_DATE,
        INBOUND_NO: $NC.G_VAR.G_PARAMETER.P_INBOUND_NO,
        LINE_NO: $NC.G_VAR.G_PARAMETER.P_LINE_NO,
        INBOUND_STATE: $NC.G_VAR.G_PARAMETER.P_INBOUND_STATE,
    ...
    try {
        ...
        for (var rIndex = 0, rCount = dsDetail.length; rIndex < rCount; rIndex++) {
            ...
            newRowData = {
                ...
                ENTRY_WEIGHT: rowData.ENTRY_QTY * rowData.ITEM_WEIGHT,
...
function _Save() {
    ...
    if (G_GRDDETAIL.data.getItems().length == 0) {
        alert($NC.getDisplayMsg("JS.LIC05021P0.001", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (G_GRDDETAIL.data.getLength() > 0) {
        ...
        if (CONFIRM_QTY != SUM_ENTRY_QTY) {
            alert($NC.getDisplayMsg("JS.LIC05021P0.002", "전표의 확정수량만큼 입력되지 않았습니다."));
            return;
        }
    }
    ...
    $NC.serviceCall("/LIC05020E0/save.do", {
        P_DS_MASTER: {
            P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
            P_BU_CD: $NC.G_VAR.masterData.BU_CD,
            P_INBOUND_DATE: $NC.G_VAR.masterData.INBOUND_DATE,
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LIC05021P0.003", "삭제할 데이터가 없습니다."));
        return;
    }
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        case "ITEM_WEIGHT":
            ...
            if (Number(rowData.ITEM_WEIGHT) <= 0) {
                alert($NC.getDisplayMsg("JS.LIC05021P0.004", "상품중량은 0보다 작을 수 없습니다."));
            ...
            // 중량이 키값이기 때문에 중복 체크
                ...
                if ($NC.getGridSearchRows(G_GRDDETAIL, {
                    ...
                    alert($NC.getDisplayMsg("JS.LIC05021P0.005", "입력한 상품중량과 동일한 상품중량이 이미 등록되어 있습니다."));
                    rowData.ITEM_WEIGHT = args.oldItem.ITEM_WEIGHT;
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                }
            }
            rowData.ENTRY_WEIGHT = rowData.ENTRY_QTY * rowData.ITEM_WEIGHT;
            break;
        case "ENTRY_QTY":
            // 등록수량 체크
            if (Number(rowData.ENTRY_QTY) < 1) {
                alert($NC.getDisplayMsg("JS.LIC05021P0.006", "등록수량은 1보다 작을 수 없습니다."));
            ...
            // 등록수량 합계 체크.
                ...
                if (CONFIRM_QTY < SUM_ENTRY_QTY) {
                    alert($NC.getDisplayMsg("JS.LIC05021P0.007", "중량 등록수량이 확정수량을 초과할 수 없습니다."));
                    rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                }
            }
...
function grdDetailOnBeforePost(row) {
    ...
    if (rowData.ITEM_WEIGHT <= 0) {
        alert($NC.getDisplayMsg("JS.LIC05021P0.008", "상품중량을 입력하십시오."));
        $NC.setFocusGrid(G_GRDDETAIL, row, "ITEM_WEIGHT", true);
        return false;
    }
    if (rowData.ENTRY_QTY <= 0) {
        alert($NC.getDisplayMsg("JS.LIC05021P0.009", "등록수량을 입력하십시오."));
