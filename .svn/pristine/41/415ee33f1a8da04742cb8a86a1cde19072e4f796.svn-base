...
function _NewB() {
    ...
    if ($NC.G_VAR.policyVal.LI210 != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.024", "입고등록 신규처리가 불가능한 사업부입니다."));
        return;
    }
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "LIC02011P0",
        PROGRAM_NM: $NC.getDisplayMsg("JS.LIC02010E0.025", "입고등록/수정"),
        url: "li/LIC02011P0.html",
        width: 1024,
        height: 600,
        G_PARAMETER: {
            ...
            P_POLICY_LI250: $NC.G_VAR.policyVal.LI250,
...
function onProcessPreB() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E0.028", "입고등록 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.029", "입고등록 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.030", "선택한 데이터 중 입고등록 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtB() {
    ...
    if ($NC.isNull(INBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.031", "입고일자를 먼저 입력하십시오."));
        $NC.setFocus("#dtpInbound_DateB");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LIC02010E0.032", "입고등록 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.033", "입고등록 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.034", "선택한 데이터 중 입고등록 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterBInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterB", {
        ...
        queryId: "LIC02010E0.RS_T1_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options,
        canDblClick: true
    });
...
function grdMasterBOnDblClick(e, args) {
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LIC02010E0.035", "입고진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        ...
        if (refRowData.INBOUND_STATE != resultData.O_INBOUND_STATE) {
            alert($NC.getDisplayMsg("JS.LIC02010E0.036", "[진행상태 : " + resultData.O_INBOUND_STATE + "] 데이터가 변경되었습니다.\n다시 조회 후 데이터를 확인하십시오.",
        ...
        if (dsDetail.length == 0) {
            alert($NC.getDisplayMsg("JS.LIC02010E0.037", "등록/수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function grdDetailBInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailB", {
        ...
        queryId: "LIC02010E0.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
