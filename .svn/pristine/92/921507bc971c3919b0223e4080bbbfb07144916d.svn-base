 ...
 *  프로그램ID         : EDS09040E0
 *  프로그램명         : [송신]택배송장[한덱스]
 *  프로그램설명       : [송신]택배송장[한덱스] 화면 Javascript
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        case "BU_CD":
            ...
            return;
        case "SEND_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.EDS09040E0.001", "송신 시작일자를 정확히 입력하십시오."));
            break;
        case "SEND_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.EDS09040E0.002", "송신 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _OnInputChange(e, view, val) {
    ...
    switch (id) {
        case "INOUT_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.EDS09040E0.003", "송신 출고일자를 정확히 입력하십시오."));
            break;
    }
}
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.004", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.005", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var SEND_DATE1 = $NC.getValue("#dtpQSend_Date1");
    if ($NC.isNull(SEND_DATE1)) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.006", "송신 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQSend_Date1");
        return;
    }
    var SEND_DATE2 = $NC.getValue("#dtpQSend_Date2");
    if ($NC.isNull(SEND_DATE2)) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.007", "송신 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQSend_Date2");
        return;
    }
    var DEFINE_NO = $NC.getValueCombo("#cboQDefine_No");
    if ($NC.isNull(DEFINE_NO)) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.008", "송신정의를 선택하십시오."));
    ...
    // 데이터 조회 - 미송신건수
    $NC.serviceCall("/EDS09040E0/getDataSet.do", {
    ...
    // 데이터 조회
    $NC.serviceCall("/EDS09040E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function grdMasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/EDS09040E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL1), onGetDetail1);
...
function grdDetail1OnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/EDS09040E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL2), onGetDetail2);
...
function btnSendOnClick(e) {
    ...
    if (!$NC.getProgramPermission().canSave) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.009", "해당 프로그램의 저장권한이 없습니다."));
        return;
    }
    ...
    if ($NC.isNull(DATA_DIV)) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.010", "송신정의를 먼저 선택하십시오."));
        return;
    }
    var DEFINE_NO = $NC.getValueCombo("#cboQDefine_No");
    var INOUT_DATE = $NC.getValue("#dtpInout_Date");
    if ($NC.isNull(INOUT_DATE)) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.011", "송신 출고일자를 입력하십시오."));
    ...
    // ES_PROCESSING 호출
    $NC.serviceCall("/EDS09040E0/sendProcessing.do", {
        P_BU_CD: BU_CD,
        P_EDI_DIV: $NC.G_VAR.EDI_DIV,
        P_DEFINE_NO: DEFINE_NO,
        P_SEND_DATE: null,
...
function btnErrorProcessOnClick(e) {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.012", "처리할 대상을 선택하십시오."));
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    if (Number(rowData.SEND_CNT0) + Number(rowData.SEND_CNT1) + Number(rowData.SEND_CNT2) + Number(rowData.SEND_CNT3) == 0) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.013", "정상 송신 데이터입니다. 오류 데이터를 선택하십시오."));
        return;
    }
...
function btnDownloadFileOnClick(e) {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.014", "파일 다운로드할 대상을 선택하십시오."));
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    if (Number(rowData.SEND_CNT0) + Number(rowData.SEND_CNT1) + Number(rowData.SEND_CNT2) + Number(rowData.SEND_CNT3) > 0) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.015", "정상적으로 처리되지 않은 송신 데이터가 존재합니다."));
        return;
    }

    var DATA_DIV = $NC.getValue("#cboQDefine_No");
    if (!DATA_DIV.startsWith("3")) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.016", "송신정의가 파일 형식으로 지정된 경우만 파일로 다운로드할 수 있습니다."));
        return;
    }

    $NC.fileDownload("/EDS09040E0/sendFileDownload.do", {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_EDI_DIV: rowData.EDI_DIV,
        P_DEFINE_NO: rowData.DEFINE_NO,
...
function btnShowPopupOnClick(e) {
    ...
    if ($NC.G_VAR.nonSendCnt == 0) {
        alert($NC.getDisplayMsg("JS.EDS09040E0.017", "미송신 데이터가 한건도 없습니다."));
        return;
    }

    $NC.showProgramSubPopup({
        PROGRAM_ID: "EDS09041P0",
        PROGRAM_NM: $NC.getDisplayMsg("JS.EDS09041P0.001", "미송신내역"),
        url: "ed/EDS09041P0.html",
        width: 1000,
        height: 600,
        G_PARAMETER: {
            P_BU_CD: BU_CD,
...
function onGetDefineNo() {
    ...
    $NC.setInitCombo("/EDS09040E0/getDataSet.do", {
...
function setValueNonSendCnt(nonSendCnt) {
    ...
    if ($NC.isNull(nonSendCnt) || nonSendCnt < 1) {
        $NC.setValue("#lblNon_Send_Cnt", $NC.getDisplayMsg("JS.EDS09040E0.018", "미송신건수 : -"));
    } else {
        $NC.setValue("#lblNon_Send_Cnt", $NC.getDisplayMsg("JS.EDS09040E0.019", "미송신건수 : " + nonSendCnt + "건", nonSendCnt));
    }
