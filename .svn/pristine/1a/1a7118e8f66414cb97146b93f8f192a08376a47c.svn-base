...
function onProcessPreC() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E2.040", "입고지시 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.041", "입고지시 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.042", "선택한 데이터 중 입고지시 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtC() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E2.043", "입고지시 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBC)) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.044", "데이터가 수정되었습니다. 저장 후 입고지시 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.045", "입고지시 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.046", "선택한 데이터 중 입고지시 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function btnNewPalletIdCOnClick() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E2.047", "새 파렛트ID를 생성하시겠습니까?"))) {
        return;
    }
...
function btnNewDirectionsCOnClick() {
    ...
    if (G_GRDPALLETIDC.data.getLength() == 0 || $NC.isNull(G_GRDPALLETIDC.lastRow)) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.048", "신규 파렛트ID를 먼저 생성 후 처리하십시오."));
        return;
    }
    ...
    if (rowData.ENTRY_QTY == rowData.DIRECTIONS_QTY) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.049", "추가하려는 상품은 이미 등록수량만큼 지시 처리되었습니다."));
        return;
    }
    ...
    if (searchIndex != -1) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.050", "추가하려는 상품은 해당 파렛트ID에 이미 추가되어 있습니다."));
...
function btnDeleteDirectionsCOnClick() {
    ...
    if (G_GRDSUBC.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.051", "삭제하려는 데이터를 선택하십시오."));
        return;
    }
...
function grdMasterCInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterC", {
        ...
        queryId: "LIC02010E2.RS_T2_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options
    });
...
function grdDetailCInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailC", {
        ...
        queryId: "LIC02010E2.RS_T2_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdSubCOnSetColumns() {
    ...
    $NC.setGridColumns(G_GRDSUBC, [ // 숨김컬럼 세팅
        ...
        $NC.G_VAR.policyVal.LI230 == "1" ? "LINE_NO,ITEM_CD,ITEM_BAR_CD,ITEM_NM,ITEM_SPEC,BRAND_NM,ITEM_STATE_F,FLOCATION_CD,ITEM_LOT" : "",
        $NC.G_VAR.policyVal.LS210 != "2" ? "VALID_DATE,BATCH_NO" : ""
    ]);
}
...
function grdSubCOnCellChange(e, args) {
    ...
    switch (G_GRDSUBC.view.getColumnId(args.cell)) {
        case "ENTRY_QTY":
            ...
            if (ENTRY_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LIC02010E2.052", "지시수량이 0보다 작을 수 없습니다."));
            ...
            if (searchIndex == -1) {
                alert($NC.getDisplayMsg("JS.LIC02010E2.053", "상세정보에서 해당 상품을 검색하지 못해 지시수량을 반영하지 못했습니다."));
                rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                rowData.ENTRY_BOX = args.oldItem.ENTRY_BOX;
                rowData.ENTRY_EA = args.oldItem.ENTRY_EA;
                rowData.ENTRY_WEIGHT = args.oldItem.ENTRY_WEIGHT;
            ...
            if (D_ENTRY_QTY < S0_ENTRY_QTY) {
                alert($NC.getDisplayMsg("JS.LIC02010E2.054", "지시수량이 등록수량보다 클 수 없습니다."));
                rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                rowData.ENTRY_BOX = args.oldItem.ENTRY_BOX;
                rowData.ENTRY_EA = args.oldItem.ENTRY_EA;
                rowData.ENTRY_WEIGHT = args.oldItem.ENTRY_WEIGHT;
...
function grdSubCOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.ENTRY_QTY)) {
            alert($NC.getDisplayMsg("JS.LIC02010E2.056", "지시수량을 입력하십시오."));
