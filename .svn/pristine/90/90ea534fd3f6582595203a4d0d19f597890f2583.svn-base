 ...
 *  프로그램ID         : LCC02031P1
 *  프로그램명         : 사업부이관등록 팝업 (의류)
 *  프로그램설명       : 사업부이관등록 팝업 (의류) 화면 Javascript
        ...
        $NC.setInitComboData({
            ...
            onComplete: function() {
                if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ENTRY_CREATE) {
                    ...
                    $NC.setValue("#cboTag_Div", $NC.G_VAR.G_PARAMETER.P_MASTER_DS.TAG_DIV);
                }
            }
        });
    });
    ...
    switch (id) {
        ...
        case "ZONE_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM121)) {
                alert($NC.getDisplayMsg("JS.LCC02031P1.001", "로케이션 존코드 길이(" + $NC.G_VAR.policyVal.CM121 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BANK_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM122)) {
                alert($NC.getDisplayMsg("JS.LCC02031P1.002", "로케이션 행코드 길이(" + $NC.G_VAR.policyVal.CM122 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BAY_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM123)) {
                alert($NC.getDisplayMsg("JS.LCC02031P1.003", "로케이션 열코드 길이(" + $NC.G_VAR.policyVal.CM123 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "LEV_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM124)) {
                alert($NC.getDisplayMsg("JS.LCC02031P1.004", "로케이션 단코드 길이(" + $NC.G_VAR.policyVal.CM124 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "STOCK_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02031P1.005", "입고일자를 정확히 입력하십시오."));
            }
            break;
        case "VALID_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02031P1.006", "유통기한을 정확히 입력하십시오."));
            }
            break;
    }
}
...
function _New() {
    ...
    if (checkedRows.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.008", "추가할 재고내역 데이터를 선택하십시오."));
        return;
    }
    ...
    if (addedCount == 0) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.009", "선택한 상품은 이미 추가된 재고내역입니다."));
        return;
    }
...
function _Save() {
    ...
    if (G_GRDSUB.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.010", "저장할 데이터가 없습니다."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.011", "물류센터를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD_OUT)) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.012", "출고사업부를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD_IN)) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.013", "입고사업부를 입력하십시오."));
        $NC.setFocus("#edtIn_Bu_Cd");
        return;
    }

    if ($NC.G_VAR.masterData.BU_CD == $NC.G_VAR.masterData.LINK_BU_CD) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.014", "출고사업부와 입고사업부가 같습니다. 입고사업부를 변경하십시오."));
        $NC.setFocus("#edtIn_Bu_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.015", "먼저 입출고구분을 선택하십시오."));
        $NC.setFocus("#cboInout_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ETC_DATE)) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.016", "먼저 입출고일자를 입력하십시오."));
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_C) {
        if (dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC02031P1.017", "저장할 지시내역 정보가 없습니다."));
            return;
        }
    } else {
        if ($NC.G_VAR.masterData.CRUD != $ND.C_DV_CRUD_U && dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC02031P1.018", "수정 후 저장하십시오."));
            return;
        }
    }
...
function _Delete() {
    ...
    if (G_GRDSUB.data.getLength() == 0 || $NC.isNull(G_GRDSUB.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.019", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        case "LOCATION_CD":
            ...
            return;
        case "ETC_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LCC02031P1.020", "입출고일자를 정확히 입력하십시오."));
...
function grdSubInitialize() {
    ...
    $NC.setInitGridObject("#grdSub", {
        ...
        queryId: "LCC02030E1.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options,
...
function validateSubData(row, rowData) {
    ...
    if ($NC.isNull(rowData.CONFIRM_QTY) || Number(rowData.CONFIRM_QTY) <= 0) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.021", "출고수량에 0보다 큰 수량을 입력하십시오."));
    ...
    if (rowData.CRUD == $ND.C_DV_CRUD_C && Number(rowData.PSTOCK_QTY) < Number(rowData.CONFIRM_QTY)) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.022", "재고수량 보다 많은 수량을 입력하실 수 없습니다."));
        $NC.setFocusGrid(G_GRDSUB, row, "CONFIRM_QTY", true);
        return false;
    }
    if (rowData.CRUD == $ND.C_DV_CRUD_C && $NC.isNull(rowData.LINK_LOCATION_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.023", "이동로케이션을 입력하십시오."));
        $NC.setFocusGrid(G_GRDSUB, row, "LINK_LOCATION_CD", true);
        return false;
    }
    if ($NC.isNull(rowData.ETC_DIV)) {
        alert($NC.getDisplayMsg("JS.LCC02031P1.024", "사유구분을 선택하십시오."));
