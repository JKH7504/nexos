 ...
 *  프로그램ID         : LOM06010Q0
 *  프로그램명         : 적재파렛트출력및내역
 *  프로그램설명       : 적재파렛트출력및내역 화면 Javascript
...
function _OnLoaded() {
    ...
    // 스플리터 초기화
    $NC.setInitSplitter("#divMasterView", "v", $NC.G_OFFSET.leftViewWidth, 250, 300);
}
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        case "CENTER_CD":
            break;
        case "SHIP_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOM06010Q0.001", "검색 적재일자를 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOM06010Q0.002", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(SHIP_DATE)) {
        alert($NC.getDisplayMsg("JS.LOM06010Q0.003", "적재일자를 입력하십시오."));
    ...
    // 데이터 조회
    $NC.serviceCall("/LOM06010Q0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
    ...
    $NC.serviceCall("/LOM06010Q0/save.do", {
...
function grdMasterOnAfterScroll(e, args) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_C && rowData.CRUD != $ND.C_DV_CRUD_N) {
        ...
        // 디테일 조회
        $NC.serviceCall("/LOM06010Q0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
    }
...
function onGetMaster(ajaxData) {
    ...
    if (!$NC.setInitGridAfterOpen(G_GRDMASTER, "SHIP_ID", true)) {
        // 디테일 초기화
        $NC.setInitGridVar(G_GRDDETAIL);
        onGetDetail({
            data: null
    ...
    $NC.G_VAR.buttons._print = G_GRDMASTER.data.getLength() > 0 ? "1" : "0";
...
function onGetDetail(ajaxData) {
    ...
    $NC.setInitGridAfterOpen(G_GRDDETAIL, "OUTBOUND_NO");
}
...
function onBtnShipIdGen() {
    ...
    if ($NC.isNull(SHIP_ID_CNT)) {
        alert($NC.getDisplayMsg("JS.LOM06010Q0.004", "적재파렛트 발행매수를 입력하십시오."));
        $NC.setFocus("#edtShip_Id_Cnt");
        return;
    }

    $NC.serviceCall("/LOM06010Q0/callShipIdGetNo.do", {
        P_CENTER_CD: CENTER_CD,
        P_CREATE_DATE: "",
...
function doPrint() {
    ...
    if (reportIndex == -1) {
        alert($NC.getDisplayMsg("JS.LOM06010Q0.005", "출력할 레포트 정보를 검색할 수 없습니다. 관리자에게 문의하십시오."));
        return;
    }
    ...
    if ($.browser.msie) {
        if ($NC.isNull($NC.G_USERINFO.PRINT_SHIP_ID)) {
            alert($NC.getDisplayMsg("JS.LOM06010Q0.006", "설정하신 프린터가 없습니다.\n\n자동출력프린터를 먼저 등록하십시오."));
            return;
        }
