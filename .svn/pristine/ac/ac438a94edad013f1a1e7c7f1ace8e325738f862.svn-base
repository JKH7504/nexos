 ...
 *  프로그램ID         : LCC02031P0
 *  프로그램명         : 사업부이관등록 팝업
 *  프로그램설명       : 사업부이관등록 팝업 화면 Javascript
...
function _Initialize() {
    ...
    $("#btnQIn_Bu_Cd").click(showUserBuPopup);
...
function _OnLoaded() {
    ...
    // 예정 -> 등록, 등록 수정
        ...
        $NC.setValue("#edtIn_Bu_Cd", dsMaster.LINK_BU_CD);
        $NC.setValue("#edtIn_Bu_Nm", dsMaster.LINK_BU_NM);
        ...
        $NC.setEnable("#edtIn_Bu_Cd", false);
        $NC.setEnable("#btnQIn_Bu_Cd", false);
    ...
    switch (id) {
        ...
        case "ZONE_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM121)) {
                alert($NC.getDisplayMsg("JS.LCC02031P0.001", "로케이션 존코드 길이(" + $NC.G_VAR.policyVal.CM121 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BANK_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM122)) {
                alert($NC.getDisplayMsg("JS.LCC02031P0.002", "로케이션 행코드 길이(" + $NC.G_VAR.policyVal.CM122 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BAY_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM123)) {
                alert($NC.getDisplayMsg("JS.LCC02031P0.003", "로케이션 열코드 길이(" + $NC.G_VAR.policyVal.CM123 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "LEV_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM124)) {
                alert($NC.getDisplayMsg("JS.LCC02031P0.004", "로케이션 단코드 길이(" + $NC.G_VAR.policyVal.CM124 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "STOCK_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02031P0.005", "입고일자를 정확히 입력하십시오."));
            }
            break;
        case "VALID_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02031P0.006", "유통기한를 정확히 입력하십시오."));
            }
            break;
    }
}
...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/LCC02030E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
}
...
function _New() {
    ...
    if (checkedRows.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.008", "추가할 재고내역 데이터를 선택하십시오."));
        return;
    }
    ...
    try {
        for (var rIndex = 0, rCount = checkedRows.length; rIndex < rCount; rIndex++) {
            ...
            newRowData = {
                ...
                CONFIRM_BOX: rowData.PSTOCK_BOX,
                CONFIRM_EA: rowData.PSTOCK_EA,
    ...
    if (addedCount == 0) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.009", "선택한 상품은 이미 추가된 재고내역입니다."));
        return;
    }
...
function _Save() {
    ...
    if (G_GRDSUB.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.010", "저장할 데이터가 없습니다."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.011", "물류센터를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.012", "출고사업부를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.LINK_BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.013", "입고사업부를 입력하십시오."));
        $NC.setFocus("#edtIn_Bu_Cd");
        return;
    }

    if ($NC.G_VAR.masterData.BU_CD == $NC.G_VAR.masterData.LINK_BU_CD) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.014", "출고사업부와 입고사업부가 같습니다. 입고사업부를 변경하십시오."));
        $NC.setFocus("#edtIn_Bu_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.015", "먼저 입출고구분을 선택하십시오."));
        $NC.setFocus("#cboInout_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ETC_DATE)) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.016", "먼저 입출고일자를 입력하십시오."));
    ...
    for (var rIndex = 0, rCount = dsTarget.length; rIndex < rCount; rIndex++) {
        ...
        dsSub.push({
            ...
            P_LINK_CONFIRM_QTY: rowData.LINK_CONFIRM_QTY,
            ...
            P_LINK_LINE_NO: rowData.LINK_LINE_NO,
            P_CRUD: rowData.CRUD
        });
    }
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_C) {
        if (dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC02031P0.017", "저장할 지시내역 정보가 없습니다."));
            return;
        }
    } else {
        if ($NC.G_VAR.masterData.CRUD != $ND.C_DV_CRUD_U && dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC02031P0.018", "수정 후 저장하십시오."));
            return;
        }
    }

    $NC.serviceCall("/LCC02030E0/save.do", {
        P_DS_MASTER: {
            P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
            P_BU_CD: $NC.G_VAR.masterData.BU_CD,
            P_ETC_DATE: $NC.G_VAR.masterData.ETC_DATE,
...
function _Delete() {
    ...
    if (G_GRDSUB.data.getLength() == 0 || $NC.isNull(G_GRDSUB.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.019", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        case "LOCATION_CD":
            ...
            return;
        case "ETC_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LCC02031P0.020", "입출고일자를 정확히 입력하십시오."));
    ...
    $NC.setGridColumn(columns, {
        ...
        editorOptions: $NC.getGridComboEditorOptions("/WC/getDataSet.do", {
            ...
            P_QUERY_PARAMS: {
                ...
                P_COMMON_CD: $ND.C_ALL,
                P_ATTR05_CD: $ND.C_YES, // Y - 사업부이관사유여부
                P_VIEW_DIV: "1" // 1:등록팝업, 2:조회팝업
            }
        }, {
            codeField: "ETC_DIV",
...
function grdSubInitialize() {
    ...
    $NC.setInitGridObject("#grdSub", {
        ...
        queryId: "LCC02030E0.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options,
...
function validateSubData(row, rowData) {
    ...
    if ($NC.isNull(rowData.CONFIRM_QTY) || Number(rowData.CONFIRM_QTY) <= 0) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.021", "출고수량에 0보다 큰 수량을 입력하십시오."));
    ...
    if (rowData.CRUD == $ND.C_DV_CRUD_C && Number(rowData.PSTOCK_QTY) < Number(rowData.CONFIRM_QTY)) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.022", "재고수량 보다 많은 수량을 입력하실 수 없습니다."));
        $NC.setFocusGrid(G_GRDSUB, row, "CONFIRM_QTY", true);
        return false;
    }
    if (rowData.CRUD == $ND.C_DV_CRUD_C && $NC.isNull(rowData.LINK_LOCATION_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.023", "이동로케이션을 입력하십시오."));
        $NC.setFocusGrid(G_GRDSUB, row, "LINK_LOCATION_CD", true);
        return false;
    }
    if ($NC.isNull(rowData.ETC_DIV)) {
        alert($NC.getDisplayMsg("JS.LCC02031P0.024", "사유구분을 선택하십시오."));
...
function onUserBuPopup(resultInfo) {
    ...
    if ($NC.isNotNull(resultInfo)) {
        $NC.setValue("#edtIn_Bu_Cd", resultInfo.BU_CD);
        $NC.setValue("#edtIn_Bu_Nm", resultInfo.BU_NM);
        $NC.G_VAR.masterData.LINK_BU_CD = resultInfo.BU_CD;
    } else {
        $NC.setValue("#edtIn_Bu_Cd");
        $NC.setValue("#edtIn_Bu_Nm");
        $NC.setFocus("#edtIn_Bu_Cd", true);
        $NC.G_VAR.masterData.LINK_BU_CD = "";
    }
