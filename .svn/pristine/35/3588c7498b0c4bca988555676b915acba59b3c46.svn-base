 ...
 * <pre>
 *  ==================================================================================================================================================
 *  프로그램ID         : PDA_LCC03030E0
 *  프로그램명         : PDA 재고이동(멀티)
 *  프로그램설명       : PDA 재고이동(멀티) 화면 Javascript
...
function _Initialize() {
    ...
    // 처리버튼 클릭 -> _Save로 처리
    $("#btnSave").click(_Save);
    $("#btnL1_Select").click(btnLocationSelect);
...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/PDA_LCC03030E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster, onLocationError);
}
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.lastPalletId)) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "파렛트ID를 먼저 스캔하십시오."));
        setFocusScan();
        return;
    }
    ...
    var searchRows = $NC.getGridSearchRows(G_GRDMASTER, {
        compareFn: function(rowData) {
            // 재고이동 대상 중 이동로케이션 미입력의 경우
            return rowData.CONFIRM_QTY > 0 && $NC.isNull(rowData.MOVE_LOCATION_CD);
        }
    });
    if (searchRows.length > 0) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "이동로케이션을 스캔하지 않은 상품이 존재합니다. 이동로케이션을 스캔하십시오."));
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "처리할 상품 내역이 없습니다."));
        return;
    }
    ...
    var checkedRows = $NC.getGridSearchRows(G_GRDMASTER, {
        compareFn: function(rowData) {
            return rowData.ITEM_STATE == "L" && rowData.CONFIRM_QTY > 0 && rowData.CONFIRM_QTY != rowData.STOCK_QTY;
        }
    });
    if (checkedRows.length > 0) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "라벨링상태의 상품은 부분이동이 불가능합니다."));
        $NC.setGridSelectRow(G_GRDMASTER, checkedRows[0]);
        return;
    }
    ...
    if (checkedData.checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "처리할 상품 내역이 없습니다. 재고이동할 상품을 선택하고 이동수량을 입력하십시오."));
        setFocusScan();
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "재고이동 처리하시겠습니까?"))) {
        setFocusScan();
        return;
    }

    var refRowData = G_GRDMASTER.data.getItem(0);
    $NC.serviceCall("/PDA_LCC03030E0/callLCProcMove.do", {
...
function grdMasterInitialize() {
    ...
    $NC.setInitGridObject("#grdMaster", {
        ...
        queryId: "PDA_LCC03030E0.RS_MASTER",
        sortCol: "ITEM_CD",
        gridOptions: options
    });
...
function grdMasterOnCellChange(e, args) {
    ...
    switch (args.col) {
        case "CONFIRM_BOX":
            ...
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "이동BOX 수량을 정확히 입력하십시오."));
            ...
            if (rowData.STOCK_QTY < confirmQty) {
                alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "이동수량이 재고수량을 초과할 수 없습니다."));
                $NC.setFocus(args.view);
                return;
            }
        ...
        case "CONFIRM_EA":
            changeQty = $NC.toNumber(args.val, -1);
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "이동EA 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
        ...
        case "CONFIRM_QTY":
            changeQty = $NC.toNumber(args.val, -1);
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "이동 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
...
function grdMasterOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONFIRM_QTY)) {
            alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "검수 수량을 입력하십시오."));
            $NC.setGridSelectRow(G_GRDMASTER, row);
            setFocusScan("#edtConfirm_Qty");
            return false;
        }
        if (Number(rowData.CONFIRM_QTY) < 0) {
            alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "검수 수량을 정확히 입력하십시오."));
            $NC.setGridSelectRow(G_GRDMASTER, row);
            setFocusScan("#edtConfirm_Qty");
            return false;
        }
...
function setDetailInputValue(rowData) {
    ...
    $NC.setValue("#edtConfirm_Ea", rowData["CONFIRM_EA"]);
    $NC.setValue("#edtConfirm_Qty", rowData["CONFIRM_QTY"]);
    $NC.setValue("#edtMove_Location_Cd", rowData["MOVE_LOCATION_CD"]);
...
function onGetMaster(ajaxData) {
    ...
    if (dsResult.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "해당 로케이션에 재고가 존재하지 않습니다."));
...
function onGetDetail(ajaxData) {
    ...
    if ($NC.isEmpty(resultData)) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "상품바코드 존재여부를 확인하지 못했습니다. 다시 스캔하십시오."));
        _Cancel($ND.C_CLEAR_TYPE_DETAIL);
        return;
    }
    ...
    if (searchRow == -1) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "로케이션에 존재하지 않는 상품입니다."));
        _Cancel($ND.C_CLEAR_TYPE_DETAIL);
        return;
    }
...
function onGetPallet(ajaxData) {

    var dsResult = $NC.toArray(ajaxData);
    if (dsResult.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "파렛트ID 존재여부를 확인하지 못했습니다. 다시 스캔하십시오."));
        _Cancel();
        return;
    } else if (dsResult.length == 1) {
        $NC.G_VAR.lastLocationCd = dsResult[0].LOCATION_CD;
        $NC.setValue("#edtLocation_Cd", dsResult[0].LOCATION_CD);
        _Inquiry();
        return;
    } else {
        SubInquiry();
...
function onGetSub(ajaxData) {
    ...
    if (!$NC.isVisible("#ctrSubOverlay")) {
        $NC.showOverlay("#ctrSubOverlay", {
            onComplete: function() {
                $NC.onGlobalResize();
                $NC.setValue("#edtL1_Pallet_Id", $NC.G_VAR.lastPalletId);
            }
        });
    }
}

function onGetMoveLocation(ajaxData) {

    var resultData = $NC.toObject(ajaxData);
    // 리턴 값이 없으면 다시
    if ($NC.isEmpty(resultData)) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "로케이션 존재여부를 확인하지 못했습니다. 다시 스캔하십시오."));
    ...
    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    rowData.MOVE_LOCATION_CD = $NC.G_VAR.lastMoveLocationCd;
    $NC.setGridApplyChange(G_GRDMASTER, rowData);
...
function onPalletScan($scan, scanVal) {
    ...
    $NC.serviceCall("/PDA_LCC03030E0/getDataSet.do", {
        P_QUERY_ID: "PDA_LCC03030E0.RS_SUB",
        P_QUERY_PARAMS: {
            P_CENTER_CD: $NC.G_USERINFO.CENTER_CD,
            P_BU_CD: $NC.G_USERINFO.BU_CD,
            P_PALLET_ID: $NC.G_VAR.lastPalletId
        }
    }, onGetPallet, onPalletError);
}
...
function onMoveLocationScan($scan, scanVal) {
    ...
    }, onGetMoveLocation, onMoveLocationError);
}
...
function onSave(ajaxData) {
    ...
    var title = $NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "확인"), //
    message = $NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "정상 처리되었습니다."), //
    buttons = {};
...
function onMoveLocationError(ajaxData) {
...
function btnAllOnClick() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.PDA_LCC03030E0.XXX", "전체 상품의 이동수량을 입력 처리하시겠습니까? "))) {
        return;
    }
