 ...
 * <pre>
 *  ==================================================================================================================================================
 *  프로그램ID         : PDA_LSC01010Q1
 *  프로그램명         : PDA 상품별 재고조회
 *  프로그램설명       : PDA 상품별 재고조회 화면(의류) Javascript
...
function grdMasterInitialize() {
    ...
    $NC.setInitGridObject("#grdMaster", {
        ...
        queryId: "PDA_LSC01010Q1.RS_MASTER",
        sortCol: "ITEM_CD",
        gridOptions: options
    });
...
function setDetailInputValue(rowData) {
    ...
    $NC.setValue("#edtZone_Cd", rowData["ZONE_CD_F"]);
    $NC.setValue("#edtItem_State", rowData["ITEM_STATE_F"]);
    $NC.setValue("#edtQty_In_Box", rowData["QTY_IN_BOX"]);
    $NC.setValue("#edtPStock_Qty", rowData["PSTOCK_QTY"]);
    $NC.setValue("#edtStock_Box", $NC.getDisplayNumber(rowData["STOCK_BOX"]));
    $NC.setValue("#edtStock_Ea", $NC.getDisplayNumber(rowData["STOCK_EA"]));
    $NC.setValue("#edtStock_Qty", $NC.getDisplayNumber(rowData["STOCK_QTY"]));
    // $NC.setValue("#edtTot_Stock_Box", $NC.getDisplayNumber(rowData["TOT_STOCK_BOX"]));
    // $NC.setValue("#edtTot_Stock_Ea", $NC.getDisplayNumber(rowData["TOT_STOCK_EA"]));
    $NC.setValue("#edtTot_Stock_Qty", $NC.getDisplayNumber(rowData["TOT_STOCK_QTY"]));
}
...
function onGetMaster(ajaxData) {
    ...
    if (dsResult.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LSC01010Q1.XXX", "상품의 재고가 존재하지 않습니다."));
        _Cancel();
        return;
    }
...
function onItemScan($scan, scanVal) {
    ...
    // 데이터 조회
    $NC.serviceCall("/PDA_LSC01010Q1/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster, onItemError);
}
