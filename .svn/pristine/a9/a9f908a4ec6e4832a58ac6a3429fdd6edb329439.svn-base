...
function onProcessPreE() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.RIM02010E1.064", "반입적치취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E1.065", "반입적치취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E1.066", "선택한 데이터 중 반입적치취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtE() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.RIM02010E1.067", "반입적치 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBE)) {
        alert($NC.getDisplayMsg("JS.RIM02010E1.068", "데이터가 수정되었습니다. 저장 후 반입적치 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E1.069", "반입적치 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E1.070", "선택한 데이터 중 반입적치처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterEInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterE", {
        ...
        queryId: "RIM02010E1.RS_T4_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options
    });
...
function grdDetailEInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailE", {
        ...
        queryId: "RIM02010E1.RS_T4_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdSubEOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.PUTAWAY_LOCATION_CD)) {
            alert($NC.getDisplayMsg("JS.RIM02010E1.071", "적치 로케이션을 지정하십시오."));
