 ...
 * <pre>
 *  ==================================================================================================================================================
 *  프로그램ID         : PDA_LOC02320E0
 *  프로그램명         : PDA 출고피킹(지정LOT)
 *  프로그램설명       : PDA 출고피킹(지정LOT)
...
function _OnInputKeyUp(e, view) {
    ...
    switch (id) {
        ...
        case "ITEM_LOT":
            ...
            onItemLotScan(view, $NC.getValue(view));
            break;
    }
}
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.lastPaperNo)) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "지시서를 먼저 스캔하십시오."));
        setFocusScan();
        return;
    }

    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "처리할 상품 내역이 없습니다."));
        setFocusScan();
        return;
    }
    ...
    if (checkedData.checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "피킹작업 후 처리하십시오.\n(변경사항 없음)"));
        setFocusScan();
        return;
    }
    ...
    if (sumVals.ENTRY_QTY == sumVals.PICK_QTY) {
        if (!confirm($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "피킹완료 처리하시겠습니까?"))) {
            setFocusScan();
            return;
        }
    } else {
        if (!confirm($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "지시수량과 피킹수량이 동일하지 않습니다.\n피킹완료 처리하시겠습니까?"))) {
            setFocusScan();
            return;
        }
    }
    ...
    $NC.serviceCall("/PDA_LOC02320E0/callLOProcPick.do", {
...
function onContainerFocus(e) {
    ...
    if (!$view.is(":focus")) {
        // 지시서 스캔하여 정상 조회되어 있으면 상품LOT에 포커
        if ($NC.isNull($NC.G_VAR.lastPaperNo)) {
            setFocusScan();
        } else {
            setFocusScan("#edtItem_Lot");
...
function grdMasterInitialize() {
    ...
    $NC.setInitGridObject("#grdMaster", {
        ...
        queryId: "PDA_LOC02320E0.RS_MASTER",
        sortCol: "ITEM_CD",
        gridOptions: options
    });
...
function grdMasterOnCellChange(e, args) {
    ...
    switch (args.col) {
        case "PICK_BOX":
            ...
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "피킹BOX 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
            pickQty = $NC.getBQty(changeQty, rowData.PICK_EA, rowData.QTY_IN_BOX);
            if (rowData.ENTRY_QTY < pickQty) {
                alert($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "피킹수량이 지시수량을 초과할 수 없습니다."));
                $NC.setFocus(args.view);
                return;
            }
        ...
        case "PICK_EA":
            changeQty = $NC.toNumber(args.val, -1);
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "피킹EA 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
        ...
        case "PICK_QTY":
            changeQty = $NC.toNumber(args.val, -1);
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "피킹 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
...
function grdMasterOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.PICK_QTY)) {
            alert($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "피킹 수량을 입력하십시오."));
...
function setDetailInputValue(rowData) {
    ...
    // Row 데이터로 에디터 세팅
    $NC.G_VAR.lastItemLot = rowData["ITEM_LOT"];
    ...
    // 입력 활성/비활성
    $NC.setEnableGroup("#ctrDetailView", $NC.isNotNull($NC.G_VAR.lastItemLot));
...
function onGetMaster(ajaxData) {
    ...
    if (dsResult.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "출고지시 정보가 없습니다."));
        _Cancel();
        return;
    }
    ...
    if (resultData.OUTBOUND_STATE != $ND.C_STATE_DIRECTIONS) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "출고지시 상태가 아닙니다. 확인 후 작업하십시오."));
        _Cancel();
        return;
    }
...
function onGetDetail() {
    ...
    var searchRows = $NC.getGridSearchRows(G_GRDMASTER, {
        searchKey: "ITEM_LOT",
        searchVal: $NC.G_VAR.lastItemLot
    });

    if (searchRows.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "피킹지시서에 존재하지 않는 지정LOT입니다."));
    ...
    // 해당 상품LOT 피킹 완료면, 메시지 표시 후 첫번째 상품 선택
    if (targetRow == -1) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "해당 지정LOT은 피킹이 완료되었습니다."));
...
function onPaperScan($scan, scanVal) {
    ...
    // 데이터 조회
    $NC.serviceCall("/PDA_LOC02320E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster, onPaperError);
}

function onItemLotScan($scan, scanVal) {
...
function onSave(ajaxData) {
    ...
    var title = $NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "확인"), //
    message = $NC.getDisplayMsg("JS.PDA_LOC02320E0.XXX", "정상 처리되었습니다."), //
    buttons = {};
...
function onItemLotError(ajaxData) {
...
function onPickYnApply() {
    ...
    if ($NC.isNull($NC.G_VAR.lastItemLot)) {
        setFocusScan();
        return;
    }
