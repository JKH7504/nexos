 ...
 *  ==================================================================================================================================================
 *  프로그램ID         : EDS01011P0
 *  프로그램명         : 인터페이스송신정의등록 팝업
 *  프로그램설명       : 인터페이스송신정의등록 팝업 화면 Javascript
...
function _Initialize() {
    ...
    $("#btnAddAllColumns").click(btnAddAllColumnsOnClick); // 송신항목 전체추가 버튼

    $("#btnCycleNew").click(btnCycleNewOnClick); // 특정송신구주기추가
    $("#btnCycleDelete").click(btnCycleDeleteOnClick); // 특정송신구주기추가
...
function _SetResizeOffset() {
    ...
    $NC.G_OFFSET.fixedSubGridHeight = 118; // 118; // 94; // 70; // 24
}
        ...
        try {
            for (rIndex = 0, rCount = dsDetail.length; rIndex < rCount; rIndex++) {
                ...
                newRowData = {
                    ...
                    XLS_COLUMN_TITLE_NM: rowData.XLS_COLUMN_TITLE_NM,
    ...
    $NC.serviceCall("/WC/getMultiDataSet.do", {
        ...
        $NC.setInitComboData({
            ...
            onComplete: function() {
                ...
                setRemoteParamMapTooltip($NC.G_VAR.masterData.REMOTE_ACTION_TYPE);
            }
        });
    });
}
...
function _New() {
    ...
    if ($NC.isNull(EDI_DIV)) {
        alert($NC.getDisplayMsg("JS.EDS01011P0.004", "송신구분을 선택하십시오."));
    ...
    if ($NC.isNull(DEFINE_NO)) {
        alert($NC.getDisplayMsg("JS.EDS01011P0.005", "정의번호를 입력하십시오."));
    ...
    if ($NC.isNull(DEFINE_NM)) {
        alert($NC.getDisplayMsg("JS.EDS01011P0.006", "정의명칭을 입력하십시오."));
    ...
    if ($NC.isNull(DATA_DIV)) {
        alert($NC.getDisplayMsg("JS.EDS01011P0.007", "송신처리구분을 선택하십시오."));
...
function _Save() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.EDS01011P0.002", "저장할 데이터가 없습니다."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.EDS01011P0.003", "사업부를 입력하십시오."));
        return;
    }
    ...
    switch (dataDiv) {
        ...
        case $ND.C_DATA_DIV_DBLINK:
            if ($NC.isNull($NC.G_VAR.masterData.LINK_TABLE_NM)) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.008", "[DBLink]테이블명을 입력하십시오."));
        ...
        case $ND.C_DATA_DIV_DBCONNECT:
            if ($NC.isNull($NC.G_VAR.masterData.REMOTE_ACTION_TYPE)) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.009", "[DBConnect]원격액션타입을 선택하십시오."));
                $NC.setFocus("#cboRemote_Action_Type");
                return;
            }
            // 원격액션타입이 TABLE<INSERT>일 경우 TABLE명 입력 체크
            if ($NC.G_VAR.masterData.REMOTE_ACTION_TYPE == "2" && $NC.isNull($NC.G_VAR.masterData.LINK_TABLE_NM)) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.010", "[DBConnect]테이블명을 입력하십시오."));
                $NC.setFocus("#edtLink_Table_Nm");
                return;
            }
            // 원격액션타입이 PROCEDURE<CALL>, SCRIPT<CALL>일 경우 원격파라메터매핑 입력 체크
            if (($NC.G_VAR.masterData.REMOTE_ACTION_TYPE == "3" || $NC.G_VAR.masterData.REMOTE_ACTION_TYPE == "4")
                && $NC.isNull($NC.G_VAR.masterData.REMOTE_PARAM_MAP)) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.011", "[DBConnect]원격파라메터매핑 값을 입력하십시오."));
                $NC.setFocus("#edtRemote_Param_Map");
                return;
            }
            break;
        // EXCEL
        case $ND.C_DATA_DIV_EXCEL:
            ...
            // alert($NC.getDisplayMsg("JS.EDS01011P0.012", "[EXCEL]데이터첫행을 입력하십시오."));
        ...
        case $ND.C_DATA_DIV_TEXT:
            if ($NC.isNull($NC.G_VAR.masterData.TXT_DELIMETER_YN)) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.013", "[TEXT]구분자사용여부를 선택하십시오."));
            ...
            if ($NC.G_VAR.masterData.TXT_DELIMETER_YN == $ND.C_YES && $NC.isNull($NC.G_VAR.masterData.TXT_COL_DELIMETER)) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.014", "[TEXT]구분자를 입력하십시오."));
        ...
        case $ND.C_DATA_DIV_XML:
            if ($NC.isNull($NC.G_VAR.masterData.XML_TAG_ROOT)) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.015", "[XML]루트태그를 입력하십시오."));
        ...
        case $ND.C_DATA_DIV_SAP:
            if ($NC.isNull($NC.G_VAR.masterData.SAP_FUNCTION_NM)) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.016", "[SAP]펑션명을 입력하십시오."));
    ...
    if (isIncorrectRemote) {
        alert($NC.getDisplayMsg("JS.EDS01011P0.017", "원격송수신구분이 설정할 수 없는 값으로 지정되었습니다.\n원격송수신구분을 다시 설정하십시오."));
    ...
    switch ($NC.getValue("#cboData_Cycle_Div")) {
        ...
        case "1":
            if (G_GRDSUB.data.getLength() == 0) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.018", "수행주기 데이터가 없습니다."));
                return;
            }
        ...
        case "2":
            ...
            if ($NC.isNull(REPEAT_TIME)) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.019", "수행주기를 초단위로 입력하십시오."));
                $NC.setFocus("#edtRepeat_Time");
                return;
            }
            if (Number(REPEAT_TIME) < 60) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.020", "수행주기는 60초(1분) 이상으로 지정하십시오."));
                $NC.setFocus("#edtRepeat_Time");
                return;
            }
            ...
            // 지정시각에 반복
                ...
                if ($NC.isNull(START_TIME)) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.021", "시작시각을 입력하십시오."));
                    $NC.setFocus("#edtStart_Time");
                    return;
                }
                if ($NC.isNull(END_TIME)) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.022", "종료시각을 입력하십시오."));
                ...
                if (isNaN(dsCheckTime[0]) || isNaN(dsCheckTime[1])) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.023", "시작시각을 시분(00:00)형식으로 정확히 입력하십시오."));
                ...
                if (hh < 0 || hh > 23) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.024", "시간을 정확히 입력하십시오."));
                    $NC.setFocus("#edtStart_Time", true);
                    return;
                }
                mm = parseInt(dsCheckTime[1], 10);
                if (mm < 0 || mm > 59) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.025", "분을 정확히 입력하십시오."));
                    $NC.setFocus("#edtStart_Time", true);
                    return;
                }
                ...
                if (isNaN(dsCheckTime[0]) || isNaN(dsCheckTime[1])) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.026", "종료시각을 시분(00:00)형식으로 정확히 입력하십시오."));
                ...
                if (START_TIME > END_TIME) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.027", "송신주기 시작,종료시각 범위 입력오류입니다."));
                    $NC.setFocus("#edtEnd_Time");
                    return;
                }
    ...
    var isDBParamMap = isDBConn && ($NC.G_VAR.masterData.REMOTE_ACTION_TYPE == "3" || $NC.G_VAR.masterData.REMOTE_ACTION_TYPE == "4");
    ...
    for (rIndex = 0, rCount = dsTarget.length; rIndex < rCount; rIndex++) {
        ...
        dsDetail.push({
            ...
            P_XLS_COLUMN_TITLE_NM: isEXCEL ? rowData.XLS_COLUMN_TITLE_NM : null,
    ...
    if (dsDetail.length == 0) {
        if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R) {
            alert($NC.getDisplayMsg("JS.EDS01011P0.028", "데이터 수정 후 저장하십시오."));
            return;
        } else if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_C) {
            alert($NC.getDisplayMsg("JS.EDS01011P0.056", "상세내역 추가 후 저장하십시오."));
            return;
        }
    }
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.EDS01011P0.029", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        ...
        case "XLS_FIRST_ROW":
            if (isNaN(args.val)) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.030", "숫자만 입력 가능합니다."));
        ...
        case "REMOTE_DIV":
            ...
            if ($NC.isNotNull(args.val)) {
                ...
                if ($NC.G_VAR.masterData.DATA_DIV == $ND.C_DATA_DIV_EXCEL && (args.val.startsWith("2") || args.val.startsWith("4"))) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.031", "[EXCEL]일 경우 웹서비스 또는 SAP 형식은 선택할 수 없습니다."));
                    args.val = "";
                    $NC.setValue(args.view);
                }
                // DATA_DIV가 파일 형식일 경우 DB형식이면 에러 처리
                    ...
                    alert($NC.getDisplayMsg("JS.EDS01011P0.032", "데이터베이스 또는 SAP 형식은 선택할 수 없습니다."));
                    args.val = "";
                    $NC.setValue(args.view);
                }
                // DATA_DIV가 DBConnect 형식일 경우 DB 형식이 아니면 에러 처리
                    ...
                    alert($NC.getDisplayMsg("JS.EDS01011P0.033", "데이터베이스 접속 형식을 선택하십시오."));
                    args.val = "";
                    $NC.setValue(args.view);
                }
                // DATA_DIV가 SAP일 경우 SAP 형식이 아니면 에러 처리
                else if ($NC.G_VAR.masterData.DATA_DIV == $ND.C_DATA_DIV_SAP && !args.val.startsWith("4")) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.034", "[SAP]일 경우 SAP 형식을 선택하십시오."));
                    args.val = "";
                    $NC.setValue(args.view);
                }
            }
        ...
        case "WEBSERVICE_AUTH_DIV":
            if (args.val == "2") {
                ...
                if ($NC.G_VAR.masterData.WEBSERVICE_DIV == "20") {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.035", "SOAP 방식 인증은 BASIC 인증만 처리 가능합니다."));
                    args.val = "";
                    $NC.setValue(args.view);
                }
            }
        ...
        case "AUTO_EXEC_YN":
            if (args.val == $ND.C_YES) {
                if ($NC.G_VAR.masterData.REMOTE_DIV == "20") {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.036", "원격송수신구분[20 - Local WebService]은 자동수행 할 수 없습니다."));
                    args.val = $ND.C_NO;
                    $NC.setValue(args.view, args.val);
                } else if ($NC.G_VAR.masterData.DATA_DIV == $ND.C_DATA_DIV_DBCONNECT && $NC.isNull($NC.G_VAR.masterData.REMOTE_DIV)) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.037", "원격송수신 정보를 먼저 지정하십시오."));
                    args.val = $ND.C_NO;
                    $NC.setValue(args.view, args.val);
                }
            }
        ...
        case "END_TIME":
            if ($NC.isNotNull(args.val)) {
                ...
                if (isNaN(dsCheckTime[0]) || isNaN(dsCheckTime[1])) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.057", "시각을 시분(00:00)형식으로 정확히 입력하십시오."));
...
function setDataDivChange(dataDiv, initialize) {
    ...
    var isDBConn = remoteDiv.startsWith("3");
    var isDBParamMap = isDBConn && (remoteActionType == "3" || remoteActionType == "4");
    ...
    switch (dataDiv) {
        ...
        case $ND.C_DATA_DIV_DBCONNECT:
            ...
            setEnableWithVisible("#edtRemote_Param_Map", isDBParamMap);
            // FTP, SFTP
            setEnableWithVisible("#chkRemote_Passive_Yn", false);
            setEnableWithVisible("#edtRemote_Charset", false);
            setEnableWithVisible("#edtRemote_Dir", false);
        ...
        case $ND.C_DATA_DIV_EXCEL:
            ...
            setEnableWithVisible("#edtPrefix_File_Nm");
...
function setRemoteActionTypeChange(remoteActionType) {
    ...
    var isDBParamMap = remoteActionType == "3" || remoteActionType == "4";
    ...
    // SQLServer, Oracle, MySQL
    setEnableWithVisible("#edtRemote_Param_Map", isDBParamMap);
    setRemoteParamMapTooltip(remoteActionType);
...
function grdDetailOnNewRecord(args) {
    ...
    $NC.setFocusGrid(G_GRDDETAIL, args.row, "COLUMN_ID", true);
}
...
function grdDetailOnCellChange(e, args) {
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        case "IF_CODE_GRP":
            if ($NC.isNotNull($NC.G_VAR.masterData.PKG_NM) && $NC.isNotNull(rowData.IF_CODE_GRP)) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.038", "패키지를 호출할 경우 변환코드그룹을 지정할 수 없습니다."));
                rowData.IF_CODE_GRP = "";
                rowData.IF_CODE_GRP_D = "";
                $NC.setFocusGrid(G_GRDDETAIL, args.row, "IF_CODE_GRP", true);
            } else {
        ...
        case "TXT_POSITION":
            if ($NC.isNotNull(rowData.TXT_POSITION)) {
                if (isNaN(rowData.TXT_POSITION)) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.039", "텍스트시작위치에 숫자 값을 입력하십시오."));
                    rowData.TXT_POSITION = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, "TXT_POSITION", true);
                } else if (Number(rowData.TXT_POSITION) < 1) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.040", "텍스트시작위치에 0보다 큰 숫자를 입력하십시오."));
                    rowData.TXT_POSITION = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, "TXT_POSITION", true);
                }
            }
        ...
        case "TXT_LENGTH":
            if ($NC.isNotNull(rowData.TXT_LENGTH)) {
                if (isNaN(rowData.TXT_LENGTH)) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.041", "텍스트컬럼길이에 숫자 값을 입력하십시오."));
                    rowData.TXT_LENGTH = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, "TXT_LENGTH", true);
                } else if (Number(rowData.TXT_LENGTH) < 1) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.042", "텍스트컬럼길이 0보다 큰 숫자를 입력하십시오."));
                    rowData.TXT_LENGTH = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, "TXT_LENGTH", true);
                }
            }
            break;
        case "DATE_FORMAT_DIV_F":
            if ($NC.isNotNull($NC.G_VAR.masterData.PKG_NM) && $NC.isNotNull(rowData.DATE_FORMAT_DIV)) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.043", "패키지를 호출할 경우 날짜포맷구분을 지정할 수 없습니다."));
                rowData.DATE_FORMAT_DIV = "";
                rowData.DATE_FORMAT_DIV_F = "";
                $NC.setFocusGrid(G_GRDDETAIL, args.row, "DATE_FORMAT_DIV_F", true);
            }
            break;
    }
...
function grdDetailOnBeforePost(row) {
    ...
    if (!$NC.isGridValidPostRow(G_GRDDETAIL, row, "COLUMN_NM")) {
        return true;
    }

    var rowData = G_GRDDETAIL.data.getItem(row);
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.COLUMN_ID)) {
            alert($NC.getDisplayMsg("JS.EDS01011P0.044", "컬럼ID를 입력하십시오."));
            $NC.setFocusGrid(G_GRDDETAIL, row, "COLUMN_ID", true);
            return false;
        }

        if ($NC.isNull(rowData.DATA_NULL_YN)) {
            alert($NC.getDisplayMsg("JS.EDS01011P0.045", "널허용여부를 선택하십시오."));
        ...
        // alert($NC.getDisplayMsg("JS.EDS01011P0.046", "날짜포맷구분을 선택하십시오."));
...
function grdDetailOnPopup(e, args) {
    ...
    switch (args.column.id) {
        case "IF_CODE_GRP":
            if ($NC.isNotNull($NC.G_VAR.masterData.PKG_NM)) {
                alert($NC.getDisplayMsg("JS.EDS01011P0.038", "패키지를 호출할 경우 변환코드그룹을 지정할 수 없습니다."));
                rowData.IF_CODE_GRP = "";
                rowData.IF_CODE_GRP_D = "";
        ...
        case "REPEAT_TIME":
            if ($NC.isNotNull(rowData.REPEAT_TIME)) {
                ...
                if (isNaN(checkTime[0]) || isNaN(checkTime[1])) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.047", "특정송신주기가 시분(00:00)형식으로 정확히 입력하십시오."));
                    rowData.REPEAT_TIME = "";
                ...
                if (hh < 0 || hh > 23) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.048", "특정송신주기가 시간을 정확히 입력하십시오."));
                    rowData.REPEAT_TIME = "";
                    $NC.setFocusGrid(G_GRDSUB, args.row, "REPEAT_TIME", true);
                    break;
                }
                ...
                if (mm < 0 || mm > 59) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.049", "특정송신주기가 분을 정확히 입력하십시오."));
                    rowData.REPEAT_TIME = "";
                    $NC.setFocusGrid(G_GRDSUB, args.row, "REPEAT_TIME", true);
                    break;
                }
                ...
                if (searchRows.length > 1) {
                    alert($NC.getDisplayMsg("JS.EDS01011P0.050", "이미 등록된 [특정송신주기] 입니다."));
                    rowData.REPEAT_TIME = "";
                    $NC.setFocusGrid(G_GRDSUB, args.row, "REPEAT_TIME", true);
                    break;
                }
...
function grdSubOnBeforePost(row) {
    ...
    if ($NC.isNull(rowData.REPEAT_TIME)) {
        alert($NC.getDisplayMsg("JS.EDS01011P0.051", "수행주기를 입력하십시오."));
...
function btnAddAllColumnsOnClick() {
    ...
    var REMOTE_DIV = $NC.getValue("#cboRemote_Div");
    var PKG_NM = $NC.getValue("#edtPkg_Nm");

    // 데이터 조회
    $NC.serviceCall("/EDS01010E0/getDataSet.do", {
        P_QUERY_ID: "EDS01010E0.RS_SUB1",
        P_QUERY_PARAMS: {
            P_EDI_DIV: EDI_DIV,
            P_REMOTE_DIV: REMOTE_DIV,
            P_PKG_NM: PKG_NM
        }
    }, onGetSub1);
}
...
function onGetSub1(ajaxData) {
    ...
    if (rCount == 0) {
        alert($NC.getDisplayMsg("JS.EDS01011P0.052", "송신항목이 없습니다."));
        return;
    }

    var rowData, newRowData, rIndex, searchIndex, dynamicColIndex = rCount;
    // 삭제된 항목 제거
    for (rIndex = G_GRDDETAIL.data.getLength() - 1; rIndex >= 0; rIndex--) {
        rowData = G_GRDDETAIL.data.getItem(rIndex);

        // 동적 컬럼은 제외
        if ((rowData.COLUMN_NM || "").startsWith("<")) {
            continue;
        }
    ...
    for (rIndex = 0, rCount = G_GRDDETAIL.data.getLength(); rIndex < rCount; rIndex++) {
        ...
        dynamicColIndex++;
        rowData.COLUMN_ID = dynamicColIndex;
        $NC.setGridApplyChange(G_GRDDETAIL, rowData);
    }
...
function setTooltip() {
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnCycleNew", $NC.getDisplayMsg("JS.EDS01011P0.053", "신규"));
    $NC.setTooltip("#btnCycleDelete", $NC.getDisplayMsg("JS.EDS01011P0.054", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.EDS01011P0.055", "저장"));

    $NC.setTooltip("#edtLink_Where_Text", "송신 대상에 적용되는 추가조건절\n" //
        + "ALIAS는 대상 테이블명으로 입력\n" //
        + "EX) 입고확정: LI020NM, LI020ND  출고확정: LO020NM, LO020ND");

    $NC.setTooltip("#edtTxt_Col_Delimeter", "컬럼구분자를 사용할 경우 컬럼 설정\n" //
        + "[TXT]시작위치: 1부터 순서대로 입력\n" //
        + "[TXT]컬럼길이: 값의 최대 길이 입력, [최대 길이 < 값의 길이] 최대 길이로 자름");

    $NC.setTooltip("#edtXml_Tag_Root", "전체 데이터 입력 태그 설정\n" //
        + "마스터,마스터/디테일 구조:  #MASTER_DATA#\n\n" //
        + "동적 값 입력시 SQL#[SQL_TEXT]# 형식으로 입력, SQL_TEXT 값을 SELECT해서 입력 함");

    $NC.setTooltip("#edtXml_Tag_Bunch", "마스터 데이터 입력 태그 설정(태그명: #MASTER_DATA#)\n" //
        + "마스터 구조:  #컬럼명#\n마스터/디테일 구조: 마스터 데이터 > #컬럼명#, 디테일 데이터 > #DETAIL_DATA#\n\n" //
        + "동적 값 입력시 SQL#[SQL_TEXT]# 형식으로 입력, SQL_TEXT 값을 SELECT해서 입력 함");

    $NC.setTooltip("#edtXml_Tag_Sub_Bunch", "디테일 데이터 입력 태그 설정(태그명: #DETAIL_DATA#)\n" //
        + "마스터/디테일 구조일 경우 지정\n디테일 데이터:  #컬럼명#\n\n" //
        + "동적 값 입력시 SQL#[SQL_TEXT]# 형식으로 입력, SQL_TEXT 값을 SELECT해서 입력 함");
    ...
    $NC.setTooltip("#edtJson_Tag_Root", "전체 데이터 입력 태그 설정\n"//
        + "마스터,마스터/디테일 구조:  #MASTER_DATA#\n\n" //
        + "동적 값 입력시 SQL#[SQL_TEXT]# 형식으로 입력, SQL_TEXT 값을 SELECT해서 입력 함");

    $NC.setTooltip("#edtJson_Tag_Bunch", "마스터 데이터 입력 태그 설정(태그명: #MASTER_DATA#)\n"
        + "마스터 구조:  #컬럼명#\n마스터/디테일 구조: 마스터 데이터 > #컬럼명#, 디테일 데이터 > #DETAIL_DATA#\n\n" //
        + "동적 값 입력시 SQL#[SQL_TEXT]# 형식으로 입력, SQL_TEXT 값을 SELECT해서 입력 함");

    $NC.setTooltip("#edtJson_Tag_Sub_Bunch", "디테일 데이터 입력 태그 설정(태그명: #DETAIL_DATA#)\n" //
        + "마스터/디테일 구조일 경우 지정\n디테일 데이터:  #컬럼명#\n\n" //
        + "동적 값 입력시 SQL#[SQL_TEXT]# 형식으로 입력, SQL_TEXT 값을 SELECT해서 입력 함");
    ...
    $NC.setTooltip("#edtPkg_Param_Map", "호출 파라메터 매핑 설정\n상대파라메터=패키지파라메터명\nEX) centerCd=P_CENTER_CD");
    ...
    $NC.setTooltip("#edtWebService_Url", "웹서비스URL 설정\n" //
        + "URL에 #컬럼명# 입력시 해당 데이터로 변경 처리");

    $NC.setTooltip("#edtWebService_Header_Val", "웹서비스 헤더 설정\n" //
        + "Http 헤더에 입력할 값을 Key=Value 형식으로 입력");

    $NC.setTooltip("#edtEdi_Dir", "자료구분이 파일 형태일 경우 파일 생성 위치\n" //
        + "기본값: 송신파일루트\\송수신구분\n\n" //
        + "로컬 특정 위치에 파일 생성시 원격송수신구분을 제거하고 <절대경로> 형식으로 입력");

    $NC.setTooltip("#edtPrefix_File_Nm", "자료구분이 파일 형태일 경우 파일명 앞에 붙일 값\n" //
        + "기본값: 없음\n\n" //
        + "같은 위치로 여려 유형의 자료를 송신할 경우 구분하기 위해 사용\n\n" //
        + "기본 규칙: (파일명접두사_)사용자ID_송신일시(yyyyMMddHHmmss)_사업부_송신구분_송신정의번호_송신일자_송신번호_DOC.자료구분(확장자)\n\n" //
        + "송신 파일명 재정의\n파일명접두사를 <> 묶어서 지정 함, 확장자는 자료구분으로 자동지정(자료구분에 따른 확장자)\n" //
        + "재정의시 지정 가능 값\n[USER_ID] 사용자ID, [SYSDATE] 송신일시, [BU_CD] 사업부코드, [EDI_DIV] 송신구분\n" //
        + "[DEFINE_NO] 정의번호, [SEND_DATE] 송신일자, [SEND_NO] 송신번호\n\n" //
        + "EX) <send_[USER_ID][SYSDATE]> --> send_WMS20230101120000.[txt, xls, xlsx, xml]" //
    );
...
function setRemoteParamMapTooltip(remoteActionType) {

    // 2 - TABLE<INSERT>
    // 3 - PROCEDURE<CALL>
    // 4 - SCRIPT<CALL>
    switch (remoteActionType) {
        case "3":
            $NC.setTooltip("#edtRemote_Param_Map", "원격파라메터 매핑 설정, PROCEDURE<CALL>일 경우\n" //
                + "ALL_PARAMS: PROCEDURE의 호출 파라메터 전체를 순서대로 입력\n" //
                + "  형식: 파라메터1,파라메터2,...\n" //
                + "OUT_PARAMS: OUTPUT 파라메터명,데이터타입(컬럼타입과 동일)을 입력\n" //
                + "  형식: 파라메터1,파라메터2,...;데이터타입1,데이터타입2,...\n" //
                + "ERROR_CD: 호출 성공여부 체크정보 입력\n" //
                + "  형식: ERROR_CD=컬럼명;컬럼값\n" //
                + "ERROR_MSG: 호출 오류시 메시지정보 입력\n" //
                + "  형식: ERROR_MSG=컬럼명;메시지포맷\n" //
                + "EX)\n" //
                + "  ALL_PARAMS=P_CENTER_CD,P_BU_CD,...,O_ERROR_CD,O_ERROR_MSG\n" //
                + "  OUT_PARAMS=O_ERROR_CD,O_ERROR_MSG;3,1\n" //
                + "  ERROR_CD=O_ERROR_CD;0\n" //
                + "  ERROR_MSG=O_ERROR_CD,O_ERROR_MSG;[%s]%s");
            break;
        case "4":
            $NC.setTooltip("#edtRemote_Param_Map", "원격파라메터 매핑 설정, SCRIPT<CALL>일 경우\n" //
                + "스크립트 작성 방법\n" //
                + "DECLARE\n" //
                + "  --<변수 선언>--\n" //
                + "BEGIN\n" //
                + "  --<데이터 처리 스크립트 작성>--\n" //
                + "END;\n\n" //
                + "※ 스크립트 내에서 오류처리 하지 않음, 오류가 발생해야 롤백 처리 됨.\n\n" //
                + "송신정의 상세내역에 원격컬럼이 지정된 컬럼만 대상\n" //
                + "변수 선언 - 변수명 데이터타입 := 바인딩변수 형태로 지정 함\n" //
                + "※ 원격컬럼지정 컬럼과 바인딩변수의 갯수가 동일해야 동작 함\n" //
                + "변수에 값 바인딩하는 순서는 컬럼순서로 지정 됨" //
                + "EX)\n" //
                + "  V_CENTER_CD   VARCHAR2(2) := ?;\n" //
                + "  V_CUST_CD     VARCHAR2(4) := ?;\n\n" //
                + "EX)\n" //
                + "DECLARE\n" //
                + "  --<변수 선언>--\n" //
                + "  V_CENTER_CD   VARCHAR2(2)   := ?;\n" //
                ...
                + "BEGIN\n" //
                + "  --<데이터 처리 스크립트 작성>--\n" //
                + "  INSERT INTO SAMPLE (CENTER_CD ,CENTER_NM) VALUES (V_CENTER_CD ,V_CENTER_NM);\n" //
                + "END;");
            break;
        default:
            $NC.setTooltip("#edtRemote_Param_Map");
            break;
    }
}