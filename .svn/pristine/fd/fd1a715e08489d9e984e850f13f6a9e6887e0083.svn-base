...
import nexos.service.lc.LCC02020E0Service;
 ...
 */
@Controller
@RequestMapping("/LCC02020E0")
public class LCC02020E0Controller extends ControllerSupport {

    final String              SP_ID_GET_LC_CONFIRM_YN = "WF.GET_LC_CONFIRM_YN";

    @Autowired
    private LCC02020E0Service service;
     ...
     * @return
     */
    @RequestMapping(value = "/callLCFWStockChangeEntry.do", method = RequestMethod.POST)
    public ResponseEntity<String> callLCFWStateChangeEntry(HttpServletRequest request, @RequestBody Map<String, Object> params) {

        ResponseEntity<String> result = null;

        try {
            result = getResponseEntity(request, service.callLCFWStockChangeEntry(params));
        } catch (Exception e) {
            result = getResponseEntityError(request, e);
        }
