...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        case "BU_CD":
            ...
            return;
        case "ADJUST_MONTH":
            $NC.setValueMonthPicker(view, val, $NC.getDisplayMsg("JS.LFC06020Q0.001", "정산월을 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LFC06020Q0.002", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_NM)) {
        alert($NC.getDisplayMsg("JS.LFC06020Q0.003", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var ADJUST_MONTH = $NC.getValue("#mtpQAdjust_Month");
    if ($NC.isNull(ADJUST_MONTH)) {
        alert($NC.getDisplayMsg("JS.LFC06020Q0.004", "정산월을 입력하십시오."));
    ...
    // 데이터 조회
    $NC.serviceCall("/LFC06020Q0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
