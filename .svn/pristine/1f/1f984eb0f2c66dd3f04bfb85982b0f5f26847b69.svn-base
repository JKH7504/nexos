...
/**
 * Class: LOB02010E0DAOImpl<br>
 * Description: LOB02010E0 DAO (Data Access Object)<br>
 ...
 *          </pre>
 */
@Repository("LOB02010E0DAO")
public class LOB02010E0DAOImpl extends DaoSupport implements LOB02010E0DAO {

    // private final Logger logger = LoggerFactory.getLogger(LOB02010E0DAOImpl.class);

    final String PROGRAM_ID                     = "LOB02010E0";
    final String PROGRAM_ID_LOB01010E0          = "LOB01010E0";

    final String TABLE_NM_LO010NM               = "LO010NM";
    final String UPDATE_ID_LO010NM              = PROGRAM_ID_LOB01010E0 + ".UPDATE_" + TABLE_NM_LO010NM;

    final String TABLE_NM_LO010ND               = "LO010ND";
    final String UPDATE_ID_LO010ND              = PROGRAM_ID_LOB01010E0 + ".UPDATE_" + TABLE_NM_LO010ND;

    final String TABLE_NM_LO020NM               = "LO020NM";
    final String INSERT_ID_LO020NM              = PROGRAM_ID + ".INSERT_" + TABLE_NM_LO020NM;
    final String UPDATE_ID_LO020NM              = PROGRAM_ID + ".UPDATE_" + TABLE_NM_LO020NM;

    final String TABLE_NM_LO020ND               = "LO020ND";
    final String INSERT_ID_LO020ND              = PROGRAM_ID + ".INSERT_" + TABLE_NM_LO020ND;
    final String UPDATE_ID_LO020ND              = PROGRAM_ID + ".UPDATE_" + TABLE_NM_LO020ND;
    final String DELETE_ID_LO020ND              = PROGRAM_ID + ".DELETE_" + TABLE_NM_LO020ND;

    final String TABLE_NM_LO030NM               = "LO030NM";
    final String UPDATE_ID_LO030NM              = PROGRAM_ID + ".UPDATE_" + TABLE_NM_LO030NM;
    ...
    final String SP_ID_LO_OPTION_UPDATE_ADDRESS = "LO_OPTION_UPDATE_ADDRESS";
    final String SP_ID_LD_CLOSING_DAILY_CHECK   = "LD_CLOSING_DAILY_CHECK";
    ...
    private void checkingPStockQty(Map<String, Object> rowData, int rownum) {
        ...
        if (sumEntryQty > pstockQty) {
            String errMsg = NexosMessage.getDisplayMsg("JAVA.LOB02010E0.001",
    ...
    public void save(Map<String, Object> params) throws Exception {
        ...
        if (Consts.PROCESS_ORDER.equals(PROCESS_CD) || Consts.PROCESS_ENTRY_CREATE.equals(PROCESS_CD)) {
            ...
            if (dsCount < 1) {
                throw new RuntimeException(NexosMessage.getDisplayMsg("JAVA.LOB02010E0.002", "출고등록 상세내역이 존재하지 않습니다."));
            }
            ...
            // LO720 - N 배차생성 안함 일경우 운송차수 999
            // 등록버튼 클릭시의 운송차수가 "999"가 아닌 경우 일마감 여부 체크 : LO720 -Y 배차생성
            if (!"999".equals(Util.nullToDefault(masterRowData.get("P_DELIVERY_BATCH"), "999"))) {
                callParams.clear();
                callParams.put("P_CENTER_CD", CENTER_CD);
                callParams.put("P_BU_CD", BU_CD);
                callParams.put("P_OUTBOUND_DATE", OUTBOUND_DATE);
                callParams.put(Consts.PK_USER_ID, USER_ID);
                Map<String, Object> closingDailyCheckMap = getClosingDailyCheck(callParams);

                if (Consts.YES.equals(closingDailyCheckMap.get("O_DAILY_CHECK"))) {
                    oMsg = Util.getOutMessage(closingDailyCheckMap);
                    if (!Consts.OK.equals(oMsg)) {
                        throw new RuntimeException(oMsg);
                    }
                }
    ...
    public void saveConfirm(Map<String, Object> params) throws Exception {
        ...
        // 디테일 수정(LO020ND) 반영
        for (int rIndex = 0, rCount = Util.getCount(dsDetail); rIndex < rCount; rIndex++) {
            Map<String, Object> rowData = dsDetail.get(rIndex);
            rowData.put(Consts.PK_USER_ID, USER_ID);

            if (Consts.DV_CRUD_U.equals(rowData.get(Consts.PK_CRUD))) {
                updateSql(UPDATE_ID_LO020ND, rowData);
            }
        }
        ...
        for (int rIndex = 0, rCount = Util.getCount(dsSub); rIndex < rCount; rIndex++) {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public void saveConfirmData(Map<String, Object> params) throws Exception {
    ...
    public void saveDelivery(Map<String, Object> params) throws Exception {
        ...
        for (int i = 0; i < dsCount; i++) {
            ...
            if (Consts.DV_CRUD_U.equals(rowCrud)) {
                int count = updateSql(UPDATE_ID_LO020ND, rowData);
                if (count == 0) {
                    throw new RuntimeException(NexosMessage.getDisplayMsg("JAVA.LOB02010E0.005", "출고확정된 전표만 미배송 저장 처리가 가능합니다."));
                }
            }
        }
    }
        ...
        for (int i = 0; i < dsCount; i++) {
            ...
            if (Consts.DV_CRUD_U.equals(rowCrud)) {
                if (updateSql(UPDATE_ID_LO010ND, rowData) == 0) {
                    throw new RuntimeException(NexosMessage.getDisplayMsg("JAVA.LOB02010E0.006",
     ...
     */
    @Override
    public void callLoSetAddress(Map<String, Object> params) throws Exception {
        ...
        for (int rIndex = 0, rCount = dsMaster.size(); rIndex < rCount; rIndex++) {
            ...
            resultMap = callProcedure(SP_ID_LO_OPTION_UPDATE_ADDRESS, rowData);
            String oMsg = Util.getOutMessage(resultMap);
            if (!Consts.OK.equals(oMsg)) {
                throw new RuntimeException(oMsg);
            }
     ...
     */
    @Override
    public Map<String, Object> getClosingDailyCheck(Map<String, Object> params) {

        Map<String, Object> resultMap = callProcedure(SP_ID_LD_CLOSING_DAILY_CHECK, new HashMap<String, Object>(params));
        String oMsg = Util.getOutMessage(resultMap);
        if (!Consts.OK.equals(oMsg)) {
            throw new RuntimeException(oMsg);
        }
