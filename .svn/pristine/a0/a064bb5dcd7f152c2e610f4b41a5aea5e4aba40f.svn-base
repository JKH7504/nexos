...
function _InquiryD(args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/ROC02010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTERD), onGetMasterD);
}
...
function _SaveD() {
    ...
    if (G_GRDMASTERD.data.getLength() == 0 || $NC.isNull(G_GRDMASTERD.lastRow) || G_GRDSUBD.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.031", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (refRowData.CRUD == $ND.C_DV_CRUD_R && dsSub.length == 0) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.032", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/ROC02010E0/saveConfirm.do", {
        P_DS_MASTER: {
...
function onProcessPreD() {
    ...
    if (rCount == 0) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.017", "조회 후 처리하십시오."));
        return;
    }
    if ($NC.G_VAR.PROCESS_YN != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.018", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록 후 처리하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.ROC02010E0.033", "반출확정취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.034", "반출확정취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.035", "선택한 데이터 중 반출확정취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtD() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.ROC02010E0.036", "반출확정 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBD)) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.037", "데이터가 수정되었습니다. 저장 후 반출확정 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.038", "반출확정 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.ROC02010E0.039", "선택한 데이터 중 반출확정처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterDInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterD", {
        ...
        queryId: "ROC02010E0.RS_T2_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
...
function grdMasterDOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/ROC02010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAILD), onGetDetailD);
    ...
    // 데이터 조회
    $NC.serviceCall("/ROC02010E0/getDataSet.do", $NC.getGridParams(G_GRDSUBD), onGetSubD);

    // 반출지시 수정 가능 여부 -> 진행상태: 30
...
function grdDetailDInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailD", {
        ...
        queryId: "ROC02010E0.RS_T2_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdSubDOnCellChange(e, args) {
    ...
    switch (G_GRDSUBD.view.getColumnId(args.cell)) {
        case "CONFIRM_QTY":
            ...
            if (CONFIRM_QTY < 0) {
                alert($NC.getDisplayMsg("JS.ROC02010E0.040", "확정수량이 0보다 작을 수 없습니다."));
            ...
            if (Number(rowData.ENTRY_QTY) < CONFIRM_QTY) {
                alert($NC.getDisplayMsg("JS.ROC02010E0.041", "확정수량이 등록수량을 초과할 수 없습니다."));
                rowData.CONFIRM_QTY = args.oldItem.CONFIRM_QTY;
                rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
                rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
                rowData.CONFIRM_WEIGHT = args.oldItem.CONFIRM_WEIGHT;
        ...
        case "CONFIRM_BOX":
            ...
            if (CONFIRM_BOX < 0) {
                alert($NC.getDisplayMsg("JS.ROC02010E0.042", "확정BOX는 0보다 작을 수 없습니다."));
                rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
                $NC.setFocusGrid(G_GRDSUBD, args.row, args.cell, true);
                break;
            }
        ...
        case "CONFIRM_EA":
            ...
            if (CONFIRM_EA < 0) {
                alert($NC.getDisplayMsg("JS.ROC02010E0.043", "확정EA는 0보다 작을 수 없습니다."));
                rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
                $NC.setFocusGrid(G_GRDSUBD, args.row, args.cell, true);
                break;
            }
...
function grdSubDOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONFIRM_QTY)) {
            alert($NC.getDisplayMsg("JS.ROC02010E0.044", "확정수량을 입력하십시오."));
...
function grdSubDOnBeforeEditCell(e, args) {
    ...
    switch (args.column.id) {
        ...
        case "CONFIRM_BOX":
        case "CONFIRM_EA":
            return $NC.G_VAR.policyVal.RO410 == $ND.C_YES; // 확정수량 수정 가능여부
    }
    return true;
}
