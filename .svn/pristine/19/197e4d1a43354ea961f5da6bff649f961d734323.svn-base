 ...
 *  ==================================================================================================================================================
 *  프로그램ID         : LOF01011P0
 *  프로그램명         : 온라인출고예정등록[풀필먼트] 팝업
 *  프로그램설명       : 온라인출고예정등록[풀필먼트] 팝업 화면 Javascript
...
function _Initialize() {
    ...
    $NC.setGlobalVar({
        ...
        policyVal: {
            LO113: "", // [공통]출고예정 삭제 기준 --------- 사업부 정책
            LO151: "", // [B2C/B2CF]발주마감시각 ----------- 사업부 정책
            LO190: "", // 공급금액 계산 정책 --------------- 사업부 정책
            LO250: "", // 유통기한/제조배치번호 지정 정책 -- 사업부 정책
            LS210: "" // 재고 관리 기준 -------------------- 사업부 정책
        },
        // 마스터 데이터
        masterData: null
    });
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.LOF01011P0.030", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.LOF01011P0.031", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.LOF01011P0.032", "저장"));
...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOF01010E0/getDataSet.do", $NC.getGridParams(G_GRDSTOCK), onGetStock);
}
...
function _New() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.002", "먼저 사업부를 입력하십시오."));
        $NC.setFocus("#edtBu_Cd");
        return;
    }
    if ($NC.isNull($NC.G_VAR.masterData.DELIVERY_CD)) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.003", "먼저 온라인몰 코드를 입력하십시오."));
        $NC.setFocus("#edtDelivery_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_NM)) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.004", "먼저 주문자명을 입력하십시오."));
        $NC.setFocus("#edtOrderer_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_HP)) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.005", "먼저 주문자 휴대폰번호를 입력하십시오."));
        $NC.setFocus("#edtOrderer_Hp");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_NM)) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.006", "먼저 수령자명을 입력하십시오."));
        $NC.setFocus("#edtShipper_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_HP)) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.007", "먼저 수령자 휴대폰번호를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_ZIP_CD) || $NC.isNull($NC.G_VAR.masterData.SHIPPER_ADDR_BASIC)) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.008", "먼저 수령자 주소를 입력하십시오."));
        $NC.setFocus("#edtShipper_Addr_Detail");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.CAR_DIV)) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.033", "먼저 차량구분을 선택하십시오."));
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.010", "물류센터를 확인할 수 없습니다. 취소 후 다시 작업하십시오."));
        return;
    }
    ...
    if ($NC.isNull($NC.G_VAR.masterData.ORDER_DATE)) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.011", "먼저 예정일자를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.012", "먼저 출고구분을 선택하십시오."));
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.009", "저장할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOF01011P0.013", "저장하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.014", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LOF01010E0/save.do", {
        P_DS_MASTER: {
            P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
            P_BU_CD: $NC.G_VAR.masterData.BU_CD,
            P_ORDER_DATE: $NC.G_VAR.masterData.ORDER_DATE,
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.015", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        ...
        case "CAR_DIV":
            ...
            break;
        case "ORDER_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LOF01011P0.016", "예정일자를 정확히 입력하십시오."));
        ...
        case "DELIVERY_CD":
            ...
            if ($NC.isNull($NC.getValue("#edtBu_Cd"))) {
                alert($NC.getDisplayMsg("JS.LOF01010P0.017", "온라인몰을 등록하려면 먼저 사업부를 입력하십시오."));
            ...
            $NP.onDeliveryChange(args.val, {
                ...
                title: $NC.getDisplayMsg("JS.LOF01011P0.018", "온라인몰 검색"),
                columnTitle: [
                    "온라인몰코드",
                    "온라인몰명"
                ],
                errorMessage: $NC.getDisplayMsg("JS.LOF01011P0.019", "등록되어 있지 않은 온라인몰입니다.")
            });
            return;
        case "REMARK1":
...
function grdDetailOnCellChange(e, args) {
    ...
    switch (columnId) {
        ...
        case "ORDER_QTY":
            if (Number(rowData.ORDER_QTY) < 1) {
                alert($NC.getDisplayMsg("JS.LOF01011P0.020", "예정수량은 1보다 작을 수 없습니다."));
        ...
        case "ORDER_BOX":
            if (Number(rowData.ORDER_BOX) < 0) {
                alert($NC.getDisplayMsg("JS.LOF01011P0.021", "예정BOX는 0보다 작을 수 없습니다."));
        ...
        case "ORDER_EA":
            if (Number(rowData.ORDER_EA) < 0) {
                alert($NC.getDisplayMsg("JS.LOF01011P0.022", "예정EA는 0보다 작을 수 없습니다."));
                rowData.ORDER_EA = args.oldItem.ORDER_EA;
                $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                break;
            }
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.LOF01011P0.023", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                } else {
...
function onItemPopup(resultInfo) {
    ...
    if ($NC.isNotNull(resultInfo)) {
        ...
        if ($NC.G_VAR.policyVal.LO190 == "2") { // 공급금액 계산정책
...
function grdDetailOnCheckPStock(row, isCheckOnly) {
    ...
    // 데이터 조회
    $NC.serviceCallAndWait("/LOF01010E0/getData.do", {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LOF01011P0.028", "출고가능량 정보를 가져오지 못했습니다."));
            return;
        }
        var oMsg = $NC.getOutMessage(resultData);
        if (oMsg != $ND.C_OK) {
...
function btnStockAddItemOnClick() {
    ...
    if (dsStock.length == 0) {
        alert($NC.getDisplayMsg("JS.LOF01011P0.029", "예정등록할 재고를 선택하십시오."));
        return;
    }
...
function setUserProgramPermission() {
    ...
    // 수기등록일 경우
    if ($NC.G_VAR.masterData.DATA_DIV == "00") {
        if ($NC.G_VAR.G_PARAMETER.P_POLICY_LO119 != $ND.C_YES) {
...
function setPolicyValInfo() {
    ...
    $NC.setPolicyValInfo({
        ...
        if ($NC.G_VAR.policyVal.LO151 != "0000") {
            ...
            $NC.setValue("#lblOrderDeadline", $NC.getDisplayMsg("JS.LOF01011P0.034", "[발주마감시간: " + deadlineTime + "] 해당 시간 전까지만 출고예정 신규 등록 가능합니다.",
                deadlineTime));
