...
function onProcessPreE() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E2.076", "입고적치취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.077", "입고적치취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.078", "선택한 데이터 중 입고적치취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtE() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E2.079", "입고적치 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBE)) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.080", "데이터가 수정되었습니다. 저장 후 입고적치 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.081", "입고적치 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.082", "선택한 데이터 중 입고적치처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onPartialPreE() {

    var rCount = G_GRDDETAILE.data.getLength();
    if (G_GRDMASTERE.data.getLength() == 0 || $NC.isNull(G_GRDMASTERE.lastRow) || rCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.026", "조회 후 처리하십시오."));
        return;
    }
    ...
    var checkedData = $NC.getGridCheckedValues(G_GRDDETAILE, {
    ...
    if (checkedData.values.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.083", "입고 부분 확정취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (rCount == checkedData.values.length) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.084", "부분 확정 취소할 상품내역을 일부만 선택하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LIC02010E2.085", "입고 부분 확정취소 처리하시겠습니까?"))) {
        return;
    }
...
function grdMasterEInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterE", {
        ...
        queryId: "LIC02010E2.RS_T4_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options
    });
...
function grdMasterEOnAfterScroll(e, args) {
    ...
    // 부분취소 버튼 표시
    $NC.setEnable("#btnPartialPreE", rowData.INBOUND_STATE >= $ND.C_STATE_CONFIRM);
...
function grdDetailEInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailE", {
        ...
        queryId: "LIC02010E2.RS_T4_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });

    G_GRDDETAILE.view.onSelectedRowsChanged.subscribe(grdDetailEOnAfterScroll);
    G_GRDDETAILE.view.onHeaderClick.subscribe(grdDetailEOnHeaderClick);

    $NC.setGridColumnHeaderCheckBox(G_GRDDETAILE, "CHECK_YN");
}
...
function grdDetailEOnHeaderClick(e, args) {

    switch (args.column.id) {
        case "CHECK_YN":
            $NC.onGridColumnHeaderCheckBoxChange(G_GRDDETAILE, e, args);
            break;
    }
}
...
function grdSubEOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.PUTAWAY_LOCATION_CD)) {
            alert($NC.getDisplayMsg("JS.LIC02010E2.086", "적치 로케이션을 지정하십시오."));
