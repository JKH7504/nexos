...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            if ($NC.isNull(BU_CD)) {
                alert($NC.getDisplayMsg("JS.LCC02080E0.014", "사업부를 먼저 선택 하십시오."));
            ...
            return;
        case "ETC_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02080E0.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ETC_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02080E0.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02080E0.003", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02080E0.004", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var ETC_DATE1 = $NC.getValue("#dtpQEtc_Date1");
    if ($NC.isNull(ETC_DATE1)) {
        alert($NC.getDisplayMsg("JS.LCC02080E0.005", "검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQEtc_Date1");
        return;
    }
    var ETC_DATE2 = $NC.getValue("#dtpQEtc_Date2");
    if ($NC.isNull(ETC_DATE2)) {
        alert($NC.getDisplayMsg("JS.LCC02080E0.006", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQEtc_Date2");
        return;
    }
    if (ETC_DATE1 > ETC_DATE2) {
        alert($NC.getDisplayMsg("JS.LCC02080E0.007", "입출고일자 검색 범위 오류입니다."));
        $NC.setFocus("#dtpQEtc_Date1");
        return;
    }
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        // 데이터 조회
        $NC.serviceCall("/LCC02080E0/getDataSet.do", $NC.getGridParams(G_GRDT1MASTER), onGetT1Master);
    }
    // 내역 조회
    else {
        ...
        // 데이터 조회
        $NC.serviceCall("/LCC02080E0/getDataSet.do", $NC.getGridParams(G_GRDT2MASTER), onGetT2Master);
    }
}
...
function _New() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        $NC.showProgramSubPopup({
            PROGRAM_ID: "LCC02081P0",
            PROGRAM_NM: $NC.getDisplayMsg("JS.LCC02080E0.008", "라벨링작업 등록"),
            url: "lc/LCC02081P0.html",
            width: 1024,
            height: 600,
            G_PARAMETER: {
...
function _Delete() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
            alert($NC.getDisplayMsg("JS.LCC02080E0.009", "삭제할 데이터가 없습니다."));
            return;
        }

        var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
        if (rowData.CONFIRM_YN == $ND.C_YES) {
            alert($NC.getDisplayMsg("JS.LCC02080E0.010", "이미 확정한 데이터는 삭제할수 없습니다."));
            return;
        }

        if (!confirm($NC.getDisplayMsg("JS.LCC02080E0.011", "삭제 하시겠습니까?"))) {
            return;
        }

        $NC.serviceCall("/LCC02080E0/callLCProcessing.do", {
            P_PROCESS_DIV: "DEL",
...
function _Print(reportInfo) {
    ...
    if ($NC.isNull(rowData)) {
        alert($NC.getDisplayMsg("JS.LCC02080E0.012", "출력할 데이터를 먼저 선택하십시오."));
        return;
    }
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "LCC02080E0.RS_T1_MASTER",
        sortCol: "ETC_DATE",
        gridOptions: options
    });
...
function grdT1MasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LCC02080E0/getDataSet.do", $NC.getGridParams(G_GRDT1DETAIL), onGetT1Detail);
...
function grdT1DetailInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Detail", {
        ...
        queryId: "LCC02080E0.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "LCC02080E0.RS_T2_MASTER",
        sortCol: "ETC_DATE",
        gridOptions: options
    });
...
function btnAutoEntryOnClick() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC02080E0.022", "조회 후 처리하십시오."));
        return;
    }
    ...
    if (refRowData.ETC_STATE != $ND.C_STATE_ORDER) {
        alert($NC.getDisplayMsg("JS.LCC02080E0.023", "예정 상태의 전표만 처리할 수 있습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC02080E0.024", "자동등록 처리 하시겠습니까?"))) {
        return;
    }

    $NC.serviceCall("/LCC02080E0/callLCFWStateChangeEntry.do", {
...
function onProcessPre() {
    ...
    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.CONFIRM_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LCC02080E0.025", "확정된 전표만 취소 할 수 있습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC02080E0.026", "취소 처리 하시겠습니까?"))) {
        return;
    }
...
function onProcessNxt() {
    ...
    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.CONFIRM_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC02080E0.013", "확정 된 전표입니다."));
        return;
    }

    if (rowData.ETC_STATE == $ND.C_STATE_ORDER) {
        alert($NC.getDisplayMsg("JS.LCC02080E0.027", "예정 데이터입니다. 등록 처리를 먼저 하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC02080E0.028", "확정 처리 하시겠습니까?"))) {
        return;
    }
...
function getEtcState(params, onSuccess) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LCC02080E0/getData.do", {
