    ...
    switch (reportInfo.REPORT_CD) {
        ...
        case "PAPER_LIC02":
            ...
            checkedData = $NC.getGridCheckedValues(grdObject, {
                valueColumns: "INBOUND_NO",
                compareFn: function(rowData) {
                    return rowData.VIRTUAL_YN == $ND.C_YES && rowData.INBOUND_STATE >= $ND.C_STATE_CONFIRM;
                }
            });
            // checkedValues 외 쿼리 파라메터 세팅
            queryParams = {
...
function setReportInfo() {
    ...
    $NC.setProgramReportInfo({
        ...
        switch (reportInfo.REPORT_CD) {
            ...
            case "LABEL_CMC04": // [LABEL_CMC04]파렛트ID라벨(멀티상품)
                switch ($NC.G_VAR.policyVal.LI230) {
                    case "1":
                        return false;
                    default:
            ...
            case "LABEL_CMC07": // [LABEL_CMC07]파렛트ID라벨
                switch ($NC.G_VAR.policyVal.LI230) {
                    case "1":
                    case "2":
                        return false;
