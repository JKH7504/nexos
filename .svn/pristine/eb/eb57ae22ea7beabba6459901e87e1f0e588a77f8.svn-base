...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        // 수불일자 입력값 체크
        case "INOUT_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LFC05060E0.001", "수불 시작일자를 정확히 입력하십시오."));
            break;
        case "INOUT_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LFC05060E0.002", "수불 종료일자를 정확히 입력하십시오."));
            break;
        case "ADJUST_MONTH":
            $("#grpInout_Date").hide();
            ...
            // 정산월 입력 시 체크
            if (val != "A") {
                $NC.setValueMonthPicker(view, val, $NC.getDisplayMsg("JS.LFC05060E0.003", "정산월을 정확히 입력하십시오."));
            }
            break;
        case "CONTRACT_NO":
...
function _Inquiry() {
    ...
    if (DATE_DIV == "I") {
        ...
        if ($NC.isNull(INOUT_DATE1)) {
            alert($NC.getDisplayMsg("JS.LFC05060E0.006", "수불 시작일자를 입력하십시오."));
            $NC.setFocus("#dtpQInout_Date1");
            return;
        }
        if ($NC.isNull(INOUT_DATE2)) {
            alert($NC.getDisplayMsg("JS.LFC05060E0.007", "수불 종료일자를 입력하십시오."));
        ...
        if ($NC.isNull(ADJUST_MONTH)) {
            alert($NC.getDisplayMsg("JS.LFC05060E0.007", "정산월을 입력하십시오."));
            $NC.setFocus("#mtpQAdjust_Month");
            return;
        }
        INOUT_DATE1 = ADJUST_MONTH + "-01";
        INOUT_DATE2 = $NC.getLastDate(ADJUST_MONTH + "-01");
    }
