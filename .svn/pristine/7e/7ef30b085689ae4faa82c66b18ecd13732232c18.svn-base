...
function _Initialize() {
    ...
    // 미사용기능 숨김 처리
    $("#btnSetDirectYnD").hide();
    $("#btnSetDirectYnE").hide();
    // 초기 숨김 처리
    // 초기 비활성화 처리
    $NC.setEnable("#btnSetSorterPutYn", false);
    $NC.setEnable("#btnSendWcsWeight", false);
    $NC.setEnable("#btnSetDirectYnD", false);
    ...
    $("#btnSetSorterPutYn").click(window.btnSetSorterPutYnOnClick);
    $("#btnSendWcsWeight").click(window.btnSendWcsWeightOnClick);
    $("#btnSetDirectYnD").click(window.btnSetDirectYnDOnClick);
    $("#btnSetDirectYnE").click(window.btnSetDirectYnEOnClick);
    ...
    // 취소/처리 버튼 툴팁 세팅
    $NC.setTooltip("#btnProcessPreB", $NC.getDisplayMsg("JS.LIC02010E1.001", $NC.getValue("#btnProcessB") + " 취소", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessNxtB", $NC.getDisplayMsg("JS.LIC02010E1.002", $NC.getValue("#btnProcessB") + " 처리", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessPreC", $NC.getDisplayMsg("JS.LIC02010E1.001", $NC.getValue("#btnProcessC") + " 취소", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessNxtC", $NC.getDisplayMsg("JS.LIC02010E1.002", $NC.getValue("#btnProcessC") + " 처리", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessPreD", $NC.getDisplayMsg("JS.LIC02010E1.001", $NC.getValue("#btnProcessD") + " 취소", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessNxtD", $NC.getDisplayMsg("JS.LIC02010E1.002", $NC.getValue("#btnProcessD") + " 처리", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessPreE", $NC.getDisplayMsg("JS.LIC02010E1.001", $NC.getValue("#btnProcessE") + " 취소", $NC.getValue("#btnProcessE")));
    $NC.setTooltip("#btnProcessNxtE", $NC.getDisplayMsg("JS.LIC02010E1.002", $NC.getValue("#btnProcessE") + " 처리", $NC.getValue("#btnProcessE")));

    $NC.setTooltip("#btnNewPalletIdC", $NC.getDisplayMsg("JS.LIC02010E1.003", "신규"));
    $NC.setTooltip("#btnNewDirectionsC", $NC.getDisplayMsg("JS.LIC02010E1.004", "추가"));
    $NC.setTooltip("#btnDeleteDirectionsC", $NC.getDisplayMsg("JS.LIC02010E1.005", "삭제"));
}
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "VENDOR_CD":
            ...
            return;
        case "INBOUND_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LIC02010E1.006", "입고일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LIC02010E1.007", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LIC02010E1.008", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function onChangingCondition() {
    ...
    $NC.setValue("#edtTotal_Directions_Qty" + $NC.G_VAR.activeView.PROCESS_CD);
    $NC.setValue("#edtTotal_Confirm_Qty" + $NC.G_VAR.activeView.PROCESS_CD);
    $NC.setValue("#edtTotal_Vendor_Cnt" + $NC.G_VAR.activeView.PROCESS_CD);
    ...
    switch ($NC.G_VAR.activeView.PROCESS_CD) {
        ...
        default:
            ...
            if ($NC.isSplitter($NC.G_VAR.activeView.container)) {
                ...
                $NC.setInitSplitter($NC.G_VAR.activeView.container, "v", 560, 270, 400);
...
function _Inquiry() {
    ...
    if (STATE_PRE_YN == $ND.C_NO && STATE_CUR_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.019", "검색구분을 선택하십시오."));
...
function _Print(reportInfo) {
    ...
    switch ($NC.G_VAR.activeView.PROCESS_CD) {
        ...
        case $ND.C_PROCESS_PUTAWAY:
            ...
            break;
        default:
            alert($NC.getDisplayMsg("JS.LIC02010E1.020", "해당 프로세스는 출력물이 없습니다."));
            return;
    }
    ...
    // 레포트별 출력 데이터 세팅
    var checkedData = {};
    var queryParams, refRowData;
    switch (reportInfo.REPORT_CD) {
        // LABEL_CMC03 - 파렛트ID라벨(단일상품)
        case "LABEL_CMC03":
            /*
             * // TODO: 일단 보류 if ($NC.G_VAR.policyVal.LI230 == "2" || $NC.G_VAR.policyVal.LI230 == "3") { alert($NC.getDisplayMsg("JS.LIC02010E1.021",
             * "[" + reportInfo.REPORT_NM + "]파렛트ID 사용 기준 정책에 의해 해당 라벨은 사용할 수 없습니다.", reportInfo.REPORT_NM)); return; }
             */
            // 선택 데이터 가져오기
        ...
        case "LABEL_CMC04":
            /*
             * // TODO: 일단 보류 if ($NC.G_VAR.policyVal.LI230 == "1") { alert($NC.getDisplayMsg("JS.LIC02010E1.021", "[" + reportInfo.REPORT_NM +
            ...
            break;
        // LABEL_CMC06 - 어소트라벨(전체)
        // LABEL_CMC07 - 어소트라벨(품목)
        case "LABEL_CMC06":
        case "LABEL_CMC07":
            refRowData = grdObject.data.getItem(grdObject.globalVar.lastRow);
            if (refRowData.ASSORT_GRP != "2") {
                alert($NC.getDisplayMsg("JS.LIC02010E1.022", "[" + reportInfo.REPORT_NM + "]어소트 상품만 어소트라벨 출력 가능합니다.", reportInfo.REPORT_NM));
                return;
            }

            if (refRowData.SORTER_PUT_YN != $ND.C_YES) {
                alert($NC.getDisplayMsg("JS.LIC02010E1.023", "[" + reportInfo.REPORT_NM + "]소터 투입 대상 어소트가 아닙니다. 출력 할수 없습니다.", reportInfo.REPORT_NM));
                return;
            }
            ...
            queryParams = {
                ...
                P_ALL_YN: reportInfo.REPORT_CD == "LABEL_CMC06" ? $ND.C_YES : $ND.C_NO
            };
            break;
        // LABEL_CMC08 - 파렛트라벨
        case "LABEL_CMC08":
            ...
            queryParams = {
                ...
                P_BU_CD: BU_CD,
                P_INBOUND_DATE: INBOUND_DATE,
                P_INOUT_SUB_CD: refRowData.INOUT_SUB_CD
            };
            break;
        // PAPER_LIC01 - 입고지시서
        case "PAPER_LIC01":
...
function onGetProcessInfo(ajaxData) {
    ...
    // alert($NC.getDisplayMsg("JS.LIC02010E1.024", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록하십시오."));
    $NC.G_VAR.activeView.PROCESS_CD = "";
    $NC.showMessage({
        message: $NC.getDisplayMsg("JS.LIC02010E1.024", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록하십시오."),
        width: 400,
...
function setMasterSummaryInfo() {
    ...
    $NC.serviceCall("/LIC02010E0/getDataSet.do", {
        P_QUERY_ID: "LIC02010E1.RS_MASTER",
        P_QUERY_PARAMS: {
            P_CENTER_CD: CENTER_CD,
            P_BU_CD: BU_CD,
            P_INBOUND_DATE: INBOUND_DATE,
...
function setPolicyValInfo() {
    ...
    $NC.setPolicyValInfo({
        ...
        if ($NC.G_VAR.policyVal.LS220 == "1" || $NC.G_VAR.policyVal.LS220 == "2" && $NC.G_VAR.policyVal.LI230 == "1") {
