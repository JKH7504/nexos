...
function _InquiryD(args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIM02010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTERD), onGetMasterD);
}
...
function _SaveD() {
    ...
    if (G_GRDMASTERD.data.getLength() == 0 || $NC.isNull(G_GRDMASTERD.lastRow) || G_GRDSUBD.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.034", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (refRowData.CRUD == $ND.C_DV_CRUD_R && dsSub.length == 0 && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.035", "데이터 수정 후 저장하십시오."));
        return;
    }
...
function onProcessPreD() {
    ...
    if (rCount == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.021", "조회 후 처리하십시오."));
        return;
    }
    if ($NC.G_VAR.PROCESS_YN != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.022", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록 후 처리하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.RIM02010E0.055", "반입확정취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.056", "반입확정취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.057", "선택한 데이터 중 반입확정취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtD() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.RIM02010E0.058", "반입확정 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBD)) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.059", "데이터가 수정되었습니다. 저장 후 반입확정 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.060", "반입확정 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.061", "선택한 데이터 중 반입확정처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterDInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterD", {
        ...
        queryId: "RIM02010E0.RS_T3_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options
    });
...
function grdMasterDOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIM02010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAILD), onGetDetailD);
    ...
    // 데이터 조회
    $NC.serviceCall("/RIM02010E0/getDataSet.do", $NC.getGridParams(G_GRDSUBD), onGetSubD);
...
function grdDetailDInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailD", {
        ...
        queryId: "RIM02010E0.RS_T3_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdSubDOnCellChange(e, args) {
    ...
    switch (G_GRDSUBD.view.getColumnId(args.cell)) {
        case "CONFIRM_QTY":
            ...
            if (CONFIRM_QTY < 0) {
                alert($NC.getDisplayMsg("JS.RIM02010E0.062", "확정수량이 0보다 작을 수 없습니다."));
            ...
            if (Number(rowData.ENTRY_QTY) < CONFIRM_QTY) {
                alert($NC.getDisplayMsg("JS.RIM02010E0.063", "확정수량이 등록수량보다 클 수 없습니다."));
                rowData.CONFIRM_QTY = args.oldItem.CONFIRM_QTY;
                rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
                rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
                rowData.CONFIRM_WEIGHT = args.oldItem.CONFIRM_WEIGHT;
        ...
        case "CONFIRM_BOX":
            ...
            if (CONFIRM_BOX < 0) {
                alert($NC.getDisplayMsg("JS.RIM02010E0.064", "확정BOX는 0보다 작을 수 없습니다."));
                rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
                $NC.setFocusGrid(G_GRDSUBD, args.row, args.cell, true);
                break;
            }
            ...
            if ($NC.G_VAR.policyVal.LI710 == $ND.C_POLICY_VAL_2 && rowData.IN_GRP == $ND.C_INOUT_GRP_E2) { // 수송입고전표 등록수량 이하로 수량 수정 가능
                if ($NC.toNumber(rowData.ENTRY_QTY) < rowData.CONFIRM_QTY) {
                    alert($NC.getDisplayMsg("JS.RIM02010E0.065", "확정수량이 등록수량을 초과할 수 없습니다."));
                    rowData.CONFIRM_QTY = args.oldItem.CONFIRM_QTY;
                    rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
                    rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
                    rowData.CONFIRM_WEIGHT = args.oldItem.CONFIRM_WEIGHT;
        ...
        case "CONFIRM_EA":
            ...
            if (CONFIRM_EA < 0) {
                alert($NC.getDisplayMsg("JS.RIM02010E0.066", "확정EA는 0보다 작을 수 없습니다."));
                rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
                $NC.setFocusGrid(G_GRDSUBD, args.row, args.cell, true);
                break;
            }
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.RIM02010E0.052", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDSUBD, args.row, args.cell, true);
                } else {
...
function grdSubDOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONFIRM_QTY)) {
            alert($NC.getDisplayMsg("JS.RIM02010E0.067", "확정수량을 입력하십시오."));
...
function grdSubDOnBeforeEditCell(e, args) {
    ...
    switch (args.column.id) {
        ...
        case "CONFIRM_BOX":
        case "CONFIRM_EA":
            return $NC.G_VAR.policyVal.RI410 == $ND.C_YES;
