...
function onProcessPreD() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.RIB02010E0.056", "반입확정취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.057", "반입확정취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.058", "선택한 데이터 중 반입확정취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtD() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.RIB02010E0.059", "반입확정 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBD)) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.060", "데이터가 수정되었습니다. 저장 후 반입확정 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.061", "반입확정 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.062", "선택한 데이터 중 반입확정처리 가능한 데이터가 없습니다."));
        return;
    }
    if (noInspectCount > 0) {
        if (!confirm($NC.getDisplayMsg("JS.RIB02010E0.063", "검수 완료 되지 않은 전표가 존재합니다. \n확정 처리하시겠습니까?"))) {
            return;
        }
    }
...
function grdMasterDInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterD", {
        ...
        queryId: "RIB02010E0.RS_T3_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options
    });
...
function grdDetailDInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailD", {
        ...
        queryId: "RIB02010E0.RS_T3_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdSubDOnCellChange(e, args) {
    ...
    switch (G_GRDSUBD.view.getColumnId(args.cell)) {
        case "CONFIRM_QTY":
            ...
            if (CONFIRM_QTY < 0) {
                alert($NC.getDisplayMsg("JS.RIB02010E0.064", "확정수량이 0보다 작을 수 없습니다."));
        ...
        case "CONFIRM_BOX":
            ...
            if (CONFIRM_BOX < 0) {
                alert($NC.getDisplayMsg("JS.RIB02010E0.065", "확정BOX는 0보다 작을 수 없습니다."));
                rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
        ...
        case "CONFIRM_EA":
            ...
            if (CONFIRM_EA < 0) {
                alert($NC.getDisplayMsg("JS.RIB02010E0.066", "확정EA는 0보다 작을 수 없습니다."));
                rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
                $NC.setFocusGrid(G_GRDSUBD, args.row, args.cell, true);
                break;
            }
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.RIB02010E0.067", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDSUBD, args.row, args.cell, true);
                } else {
...
function grdSubDOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONFIRM_QTY)) {
            alert($NC.getDisplayMsg("JS.RIB02010E0.068", "확정수량을 입력하십시오."));
