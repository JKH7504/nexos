 ...
 *  프로그램ID         : LFC05020E0
 *  프로그램명         : 셀구분보관료 조정
 *  프로그램설명       : 셀구분보관료 조정 화면 Javascript
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BU_CD":
            ...
            return;
        case "INOUT_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LFC05020E0.001", "정산 시작일자를 정확히 입력하십시오."));
            break;
        case "INOUT_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LFC05020E0.002", "정산 종료일자를 정확히 입력하십시오."));
            break;
        case "CONTRACT_NO":
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LFC05020E0.003", "물류센터를 선택하십시오."));
    ...
    // 데이터 조회
    $NC.serviceCall("/LFC05020E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LFC05020E0.007", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LFC05020E0.008", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LFC05020E0/save.do", {
    ...
    switch (G_GRDMASTER.view.getColumnId(args.cell)) {
        case "FINAL_AMT":
            if (Number(rowData.FINAL_AMT) < 0) {
                alert($NC.getDisplayMsg("JS.LFC05020E0.011", "최종확정금액은 0보다 작을 수 없습니다."));
...
function onSetBillComboInfo() {
    $NC.setInitCombo("/LFC05020E0/getDataSet.do", {
        P_QUERY_ID: "LFC05020E0.RS_SUB",
        P_QUERY_PARAMS: {
            P_CENTER_CD: $NC.getValueCombo("#cboQCenter_Cd"),
            P_BU_CD: $NC.getValue("#edtQBu_Cd"),
