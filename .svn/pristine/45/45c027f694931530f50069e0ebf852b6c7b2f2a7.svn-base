...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            return;
        case "OUTBOUND_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB06010E0.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "OUTBOUND_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB06010E0.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
        case "DELIVERY_CD":
...
function _OnInputChange(e, view, val) {
    ...
    switch (id) {
        case "OUTBOUND_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB06010E0.011", "변경일자를 정확히 입력하십시오."));
            break;
    }
}
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOB06010E0.003", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LOB06010E0.004", "사업부를 입력하십시오."));
    ...
    if ($NC.isNull(OUTBOUND_DATE1)) {
        alert($NC.getDisplayMsg("JS.LOB06010E0.005", "검색 시작일자를 입력하십시오."));
    ...
    if ($NC.isNull(OUTBOUND_DATE2)) {
        alert($NC.getDisplayMsg("JS.LOB06010E0.006", "검색 종료일자를 입력하십시오."));
    ...
    // 데이터 조회
    $NC.serviceCall("/LOB06010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function grdMasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOB06010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
...
function onProc(ajaxData) {
    ...
    alert($NC.getDisplayMsg("JS.LOB06010E0.007", "변경이 완료되었습니다."));
}
...
function btnChangeDateOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOB06010E0.008", "조회 후 처리하십시오."));
        return;
    }

    var OUTBOUND_DATE_NEW = $NC.getValue("#dtpOutbound_Date");
    if ($NC.isNull(OUTBOUND_DATE_NEW)) {
        alert($NC.getDisplayMsg("JS.LOB06010E0.009", "변경일자를 선택하십시오."));
        $NC.setFocus("#dtpOutbound_Date");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOB06010E0.010", "선택한 일자로 출고일자를 변경 하시겠습니까?"))) {
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    // LI_CHANGE_OUTBOUND_DATE 호출
    $NC.serviceCall("/LOB06010E0/callLOChangeOutboundDate.do", {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_OUTBOUND_DATE: rowData.OUTBOUND_DATE,
        P_OUTBOUND_NO: rowData.OUTBOUND_NO,
