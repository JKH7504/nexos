...
function _New() {
    ...
    if (checkedRows.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC02042P0.007", "추가할 재고내역 데이터를 선택하십시오."));
        return;
    }

    var MLOCATION_CD = $NC.getValue("#edtLocation_Cd");
    if ($NC.isNull(MLOCATION_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02042P0.008", "재고 추가시 먼저 이동할 로케이션을 지정하십시오."));
    ...
    if (remainQty < 1) {
        alert($NC.getDisplayMsg("JS.LCC02042P0.009", "이미 지시내역이 모두 추가되어 있습니다.\n\n기존 지시내역을 삭제하거나 이동수량 조정 후 추가하십시오."));
        return;
    }
    ...
    if (addedCount == 0) {
        if (checkedRows.length == sameLocCount) {
            alert($NC.getDisplayMsg("JS.LCC02042P0.010", "선택한 상품의 재고로케이션과 이동로케이션이 모두 같습니다.\n이동로케이션 변경 후 추가하십시오."));
        } else {
...
function grdDetailOnBeforeEditCell(e, args) {
     ...
     * (args.column.id) { default: return false; } }
     */
    return true;
}
