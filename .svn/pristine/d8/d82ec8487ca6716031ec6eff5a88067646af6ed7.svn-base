 ...
 *  프로그램ID         : LCC01011P0
 *  프로그램명         : 기타입출고등록 팝업
 *  프로그램설명       : 기타입출고등록 팝업 화면 Javascript
    ...
    switch (id) {
        ...
        case "ZONE_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM121)) {
                alert($NC.getDisplayMsg("JS.LCC01011P0.001", "로케이션 존코드 길이(" + $NC.G_VAR.policyVal.CM121 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BANK_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM122)) {
                alert($NC.getDisplayMsg("JS.LCC01011P0.002", "로케이션 행코드 길이(" + $NC.G_VAR.policyVal.CM122 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BAY_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM123)) {
                alert($NC.getDisplayMsg("JS.LCC01011P0.003", "로케이션 열코드 길이(" + $NC.G_VAR.policyVal.CM123 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "LEV_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM124)) {
                alert($NC.getDisplayMsg("JS.LCC01011P0.004", "로케이션 단코드 길이(" + $NC.G_VAR.policyVal.CM124 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "STOCK_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC01011P0.005", "입고일자를 정확히 입력하십시오."));
            }
            break;
        case "VALID_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC01011P0.006", "유통기한를 정확히 입력하십시오."));
            }
            break;
    }
}
...
function _New() {
    ...
    // 기타출고지시 화면의 상품추가 버튼 클릭 시
        ...
        if (checkedRows.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC01011P0.008", "추가할 재고내역 데이터를 선택하십시오."));
            return;
        }
        ...
        if (addedCount == 0) {
            alert($NC.getDisplayMsg("JS.LCC01011P0.009", "선택한 상품은 이미 추가된 재고내역입니다."));
            return;
        }
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC01011P0.010", "물류센터를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC01011P0.011", "사업부를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LCC01011P0.012", "먼저 입출고구분을 선택하십시오."));
        $NC.setFocus("#cboInout_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ETC_DATE)) {
        alert($NC.getDisplayMsg("JS.LCC01011P0.013", "먼저 입출고일자를 입력하십시오."));
    ...
    if (grdObject.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LCC01011P0.007", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    for (var rIndex = 0, rCount = dsTarget.length; rIndex < rCount; rIndex++) {
        ...
        if ($NC.isNull(rowData.ETC_DIV)) {
            alert($NC.getDisplayMsg("JS.LCC01011P0.014", "사유구분을 입력하십시오."));
            return;
        }
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_C) {
        if (dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC01011P0.015", "저장할 지시내역 정보가 없습니다."));
            return;
        }
    } else {
        if ($NC.G_VAR.masterData.CRUD != $ND.C_DV_CRUD_U && dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC01011P0.016", "데이터 수정 후 저장하십시오."));
            return;
        }
    }
...
function _Delete() {
    ...
    if (grdObject.data.getLength() == 0 || $NC.isNull(grdObject.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC01011P0.017", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        case "LOCATION_CD":
            ...
            return;
        case "ETC_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LCC01011P0.018", "입출고일자를 정확히 입력하십시오."));
...
function grdSub1OnCellChange(e, args) {
    ...
    var rowData = args.item;
    var columnId = G_GRDSUB1.view.getColumnId(args.cell);
    switch (G_GRDSUB1.view.getColumnId(args.cell)) {
        ...
        case "CONFIRM_EA":
            ...
            if (Number(rowData.PSTOCK_QTY) < Number(rowData.CONFIRM_QTY)) {
                alert($NC.getDisplayMsg("JS.LCC01011P0.019", "재고수량 보다 많은 수량을 입력하실 수 없습니다."));
                ...
                $NC.setFocusGrid(G_GRDSUB1, args.row, args.cell, true);
            }
            break;
        case "ETC_DIV_F":
            if ($NC.isNull(rowData.ETC_DIV_F)) {
                alert($NC.getDisplayMsg("JS.LCC01011P0.020", "사유구분을 선택하십시오"));
                rowData.ETC_DIV = args.oldItem.ETC_DIV;
                rowData.ETC_DIV_F = args.oldItem.ETC_DIV_F;
                $NC.setFocusGrid(G_GRDSUB1, args.row, "ETC_DIV_F", true);
            }
            break;
    }
...
/**
 * grdSub1 데이터(기타입출고 지시 데이터) 필터링 이벤트
 */
...
function grdSub1OnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONFIRM_QTY)) {
            alert($NC.getDisplayMsg("JS.LCC01011P0.021", "등록수량을 입력하십시오."));
    ...
    switch (G_GRDSUB2.view.getColumnId(args.cell)) {
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.LCC01011P0.006", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
        ...
        case "ETC_DIV_F":
            if ($NC.isNull(rowData.ETC_DIV_F)) {
                ...
                rowData.ETC_DIV = args.oldItem.ETC_DIV;
                rowData.ETC_DIV_F = args.oldItem.ETC_DIV_F;
                $NC.setFocusGrid(G_GRDSUB2, args.row, "ETC_DIV_F", true);
            }
            break;
    }
...
function grdSub2OnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.ITEM_CD)) {
            alert($NC.getDisplayMsg("JS.LCC01011P0.022", "상품코드를 선택하십시오."));
            $NC.setGridSelectRow(G_GRDSUB2, {
                selectRow: row,
        ...
        if ($NC.isNull(rowData.ITEM_STATE)) {
            alert($NC.getDisplayMsg("JS.LCC01011P0.023", "상태를 선택하십시오."));
            $NC.setGridSelectRow(G_GRDSUB2, {
                selectRow: row,
        ...
        if ($NC.isNull(rowData.ITEM_LOT)) {
            alert($NC.getDisplayMsg("JS.LCC01011P0.024", "LOT번호를 입력하십시오."));
            $NC.setGridSelectRow(G_GRDSUB2, {
                selectRow: row,
        ...
        if ($NC.isNull(rowData.LOCATION_CD)) {
            alert($NC.getDisplayMsg("JS.LCC01011P0.025", "로케이션을 입력하십시오."));
            $NC.setGridSelectRow(G_GRDSUB2, {
                selectRow: row,
        ...
        if ($NC.isNull(rowData.CONFIRM_QTY) || Number(rowData.CONFIRM_QTY) == 0) {
            alert($NC.getDisplayMsg("JS.LCC01011P0.026", "등록수량에 0보다 큰 수량을 입력하십시오."));
            $NC.setGridSelectRow(G_GRDSUB2, {
                selectRow: row,
