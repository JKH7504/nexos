...
function _Initialize() {
    ...
    // 초기 숨김 처리
    $NC.setVisible("#btnChangeQCState", false);
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            return;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LIC01010E0.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LIC01010E0.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
        case "VENDOR_CD":
            ...
            return;
        case "ARRIVAL_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LIC01010E0.XXX", "입항일자를 정확히 입력하십시오."), "N");
            break;
        case "DEPT_CD":
            $NP.onDeptChange(val, {
                P_CUST_CD: $NC.getValue("#edtQCust_Cd"),
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LIC01010E0.003", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LIC01010E0.004", "사업부를 입력하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE1)) {
        alert($NC.getDisplayMsg("JS.LIC01010E0.005", "검색 시작일자를 입력하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE2)) {
        alert($NC.getDisplayMsg("JS.LIC01010E0.006", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQOrder_Date2");
        return;
    }

    if (ORDER_DATE1 > ORDER_DATE2) {
        alert($NC.getDisplayMsg("JS.LIC01010E0.007", "입고예정일자 범위 입력오류입니다."));
        $NC.setFocus("#dtpQOrder_Date1");
        return;
    }
...
function _New() {
    ...
    if ($NC.G_VAR.policyVal.LI110 != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LIC01010E0.008", "입고예정정보 신규등록/수정이 불가능한 사업부입니다."));
        return;
    }
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "LIC01011P0",
        PROGRAM_NM: $NC.getDisplayMsg("JS.LIC01010E0.009", "입고예정등록/수정"),
        url: "li/LIC01011P0.html",
        width: 1024,
        height: 600,
        G_PARAMETER: {
            ...
            P_POLICY_LI113: $NC.G_VAR.policyVal.LI113,
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LIC01010E0.010", "삭제할 데이터가 없습니다."));
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    if ($NC.G_VAR.stateFWBW.CANCEL != rowData.INBOUND_STATE) {
        alert($NC.getDisplayMsg("JS.LIC01010E0.011", "삭제가 불가능한 전표입니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LIC01010E0.012", "예정전표를 삭제하시겠습니까?"))) {
        return;
    }
...
function grdMasterOnDblClick(e, args) {
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LIC01010E0.013", "입고진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        ...
        if (refRowData.INBOUND_STATE != resultData.O_INBOUND_STATE) {
            alert($NC.getDisplayMsg("JS.LIC01010E0.014", "[진행상태 : " + resultData.O_INBOUND_STATE + "] 데이터가 변경되었습니다.\n다시 조회 후 데이터를 확인하십시오.",
        ...
        if (dsDetail.length == 0) {
            alert($NC.getDisplayMsg("JS.LIC01010E0.015", "등록/수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function setPolicyValInfo() {
    ...
    $NC.setPolicyValInfo({
        ...
        // 품질검사기준에 따라 표시/미표시
        $NC.setVisible("#btnChangeQCState", $NC.getProgramPermission().canSave && $NC.G_VAR.policyVal.LI120 == "2");
    });
}
...
function btnOrderClosingOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LIC01010E0.016", "종결처리할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LIC01010E0.017", "종결처리 하시겠습니까?"))) {
        return;
    }
...
function btnChangeQCStateOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LIC01010E0.018", "QC상태전환할 데이터가 없습니다."));
        return;
    }
    ...
    if ($NC.G_VAR.stateFWBW.CANCEL != refRowData.INBOUND_STATE) {
        alert($NC.getDisplayMsg("JS.LIC01010E0.019", "삭제가 불가능한 전표입니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LIC01010E0.020", "QC상태전환 처리 하시겠습니까?"))) {
        return;
    }
    ...
    if (dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC01010E0.021", "QC상태전환 처리할 데이터를 선택하십시오."));
        return;
    }
