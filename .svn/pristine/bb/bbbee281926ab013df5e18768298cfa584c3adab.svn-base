...
function _Initialize() {
    ...
    $("#btnEntryNewFree").click(btnEntryNewFreeOnClick);
    $("#btnEntryDeleteFree").click(btnEntryDeleteFreeOnClick);

    // 그리드 초기화
    grdDetailInitialize();

    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.LIC02011P0.001", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.LIC02011P0.002", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.LIC02011P0.003", "저장"));
}
        ...
        try {
            for (var rIndex = 0, rCount = dsDetail.length; rIndex < rCount; rIndex++) {
                ...
                if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ORDER) {
                    ...
                    rowData.ENTRY_QTY = REMAIN_QTY;
                    rowData.CONFIRM_QTY = REMAIN_QTY;
                    rowData.PUTAWAY_QTY = REMAIN_QTY;
                }
                // 등록
                else {
                ...
                // 덤입고의 경우, BU_DATE, BU_NO에 값 설정 하고, BU_LINE_NO에는 값 설정 하지 않음
                // 덤추가 가능 정책 + 등록(지시)상태인 경우는 detail의 BU_DATE/BU_NO를, 그 외는 master의 BU_DATE/BU_NO를 설정
                // if (!($NC.G_VAR.G_PARAMETER.P_POLICY_LI250 == $ND.C_YES && $NC.G_VAR.G_PARAMETER.P_PROCESS_CD != $ND.C_PROCESS_ORDER)) {
                if ($NC.G_VAR.G_PARAMETER.P_POLICY_LI250 == $ND.C_YES || $NC.G_VAR.G_PARAMETER.P_PROCESS_CD != $ND.C_PROCESS_ENTRY_CREATE) {
...
function _New() {
    ...
    if ($NC.isNull(VENDOR_CD)) {
        alert($NC.getDisplayMsg("JS.LIC02011P0.004", "먼저 공급처 코드를 입력하십시오."));
        $NC.setFocus("#edtVendor_Cd");
        return;
    }
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.INBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.LIC02011P0.006", "먼저 입고일자를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LIC02011P0.007", "먼저 입고구분을 선택하십시오."));
    ...
    if ($NC.G_VAR.G_PARAMETER.P_POLICY_LS220 == "2" && $NC.G_VAR.G_PARAMETER.P_POLICY_LI230 == "3") {
        if ($NC.toNumber($NC.G_VAR.masterData.PALLET_ID_CNT) < 1) {
            alert($NC.getDisplayMsg("JS.LIC02011P0.008", "파렛트ID수를 정확히 입력하십시오[1이상]"));
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LIC02011P0.005", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02011P0.009", "수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LIC02010E0/save.do", {
        P_DS_MASTER: {
            P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
            P_BU_CD: $NC.G_VAR.masterData.BU_CD,
            ...
            P_SHIPMENT_NO: $NC.G_VAR.masterData.SHIPMENT_NO,
            P_PALLET_ID_CNT: $NC.G_VAR.masterData.PALLET_ID_CNT,
            P_OLD_PALLET_ID_CNT: $NC.G_VAR.masterData.OLD_PALLET_ID_CNT,
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.LIC02011P0.010", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        ...
        case "INOUT_CD":
            ...
            if (args.val == "E20") {
                if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ENTRY_CREATE) {
                    alert($NC.getDisplayMsg("JS.LIC02011P0.011", "수송입고는 신규로 등록할 수 없습니다."));
                    args.val = "E10";
            ...
            break;
        case "INBOUND_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LIC02011P0.012", "입고일자를 정확히 입력하십시오."));
...
function grdDetailOnBeforeEditCell(e, args) {
    ...
    switch (args.column.id) {
        case "ITEM_CD":
            // 덤추가 한 상품인 경우는 편집 불가
            if ($NC.G_VAR.G_PARAMETER.P_POLICY_LI250 == $ND.C_YES && $NC.isNull(rowData.BU_LINE_NO) && !$NC.isNull(rowData.BU_NO)) {
                return false;
            } else {
                return isNew;
            }
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        ...
        case "ITEM_LOT":
            if (rowData.LOT_NO_REQ_YN == $ND.C_YES && ($NC.isNull(rowData.ITEM_LOT) || rowData.ITEM_LOT == $ND.C_BASE_ITEM_LOT)) {
                alert($NC.getDisplayMsg("JS.LIC02011P0.027", "LOT번호필수대상 상품입니다. LOT번호를 입력하십시오."));
        ...
        case "ENTRY_QTY":
            ...
            if (ENTRY_QTY < CHECK_QTY) {
                alert($NC.getDisplayMsg("JS.LIC02011P0.013", "등록수량이 " + CHECK_QTY + "보다 작을 수 없습니다.", CHECK_QTY));
            ...
            if ($NC.G_VAR.ORDER_YN == $ND.C_YES && $NC.G_VAR.G_PARAMETER.P_POLICY_LI220 != "1") {
                if (Number(rowData.ORDER_QTY) < ENTRY_QTY) {
                    alert($NC.getDisplayMsg("JS.LIC02011P0.014", "등록수량이 예정수량을 초과할 수 없습니다."));
                    rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                    break;
                }
        ...
        case "ENTRY_BOX":
            ...
            if (ENTRY_BOX < 0) {
                alert($NC.getDisplayMsg("JS.LIC02011P0.015", "등록BOX는 0보다 작을 수 없습니다."));
        ...
        case "ENTRY_EA":
            ...
            if (ENTRY_EA < 0) {
                alert($NC.getDisplayMsg("JS.LIC02011P0.016", "등록EA는 0보다 작을 수 없습니다."));
                rowData.ENTRY_EA = args.oldItem.ENTRY_EA;
                $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                break;
            }
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.LIC02011P0.017", "사용기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                } else {
        ...
        case "ARRIVAL_DATE":
            if ($NC.isNotNull(rowData.ARRIVAL_DATE)) {
                if (!$NC.isDate(rowData.ARRIVAL_DATE)) {
                    alert($NC.getDisplayMsg("JS.LIC02011P0.XXX", "입항일자을 정확히 입력하십시오."));
...
function grdDetailOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.ITEM_CD) || $NC.isNull(rowData.ITEM_NM)) {
            alert($NC.getDisplayMsg("JS.LIC02011P0.018", "상품코드를 입력하십시오."));
            $NC.setFocusGrid(G_GRDDETAIL, row, "ITEM_CD", true);
            return false;
        }
        if ($NC.isNull(rowData.ITEM_STATE)) {
            alert($NC.getDisplayMsg("JS.LIC02011P0.019", "상태를 선택하십시오."));
        ...
        if ($NC.isNull(rowData.ITEM_LOT)) {
            alert($NC.getDisplayMsg("JS.LIC02011P0.020", "LOT번호를 입력하십시오."));
            $NC.setFocusGrid(G_GRDDETAIL, row, "ITEM_LOT", true);
            return false;
        }
        if ($NC.isNull(rowData.ENTRY_QTY)) {
            alert($NC.getDisplayMsg("JS.LIC02011P0.021", "등록수량을 입력하십시오."));
...
function grdDetailOnAfterScroll(e, args) {
    ...
    // 덤상품인 경우 덤삭제버튼 활성화 처리
    if ($NC.G_VAR.G_PARAMETER.P_POLICY_LI250 == $ND.C_YES) {
        var rowData = G_GRDDETAIL.data.getItem(row);
        $NC.setEnable("#btnEntryDeleteFree", $NC.isNull(rowData.BU_LINE_NO));
    }
...
function setUserProgramPermission() {
    ...
    if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ORDER || $NC.G_VAR.ORDER_YN == $ND.C_YES) {
        ...
        // 입고등록 덤추가 정책에 따라 버튼 표시(일반입고인 경우만 표시)
        if ($NC.G_VAR.G_PARAMETER.P_POLICY_LI250 == $ND.C_YES && permission.canSave
            && $NC.G_VAR.G_PARAMETER.P_MASTER_DS.INOUT_SUB_CD == $ND.C_INOUT_GRP_E1) {
            $("#btnEntryNewFree").show();
            $("#btnEntryDeleteFree").show();
            $NC.setEnable("#btnEntryDeleteFree", false);
        }
    } else {
 ...
 * 덤추가 버튼 클릭 이벤트 처리
 */
function btnEntryNewFreeOnClick() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.LIC02011P0.022", "덤 추가 대상 데이터가 없습니다."));
        return;
    }

    var refRowData = G_GRDDETAIL.data.getItem(G_GRDDETAIL.lastRow);
    // 전표가 인터페이스 받은 전표인 경우만 가능
    if ($NC.isNull($NC.getValue("#edtBu_No"))) {
        alert($NC.getDisplayMsg("JS.LIC02011P0.023", "덤 추가 불가능한 전표입니다.\n전표번호가 존재하는 전표의 경우만 덤추가 가능합니다."));
        return;
    }
    // 선택한 상품이 인터페이스 받은 상품인지 체크
    if ($NC.isNull(refRowData.BU_NO)) {
        alert($NC.getDisplayMsg("JS.LIC02011P0.024", "덤 추가 불가능한 상품입니다.\n인터페이스가 아닌 수기 등록한 상품은 덤추가 불가능 합니다."));
        return;
    }

    // 다른 행에 동일 덤추가 상품이 있는지 체크
    if ($NC.getGridSearchRow(G_GRDDETAIL, {
        compareFn: function(rowData) {
            return rowData.ITEM_CD == refRowData.ITEM_CD && $NC.isNull(rowData.BU_LINE_NO);
        }
    }) != -1) {
        alert($NC.getDisplayMsg("JS.LIC02011P0.025", "덤 추가 하려는 상품이 이미 덤추가 상품으로 존재합니다.\n확인 후 다시 작업하세요."));
        return;
    ...
    var newRowData = {
        ...
        BRAND_NM: refRowData.BRAND_NM,
        ITEM_CD: refRowData.ITEM_CD,
        ITEM_BAR_CD: refRowData.ITEM_BAR_CD,
        ITEM_NM: refRowData.ITEM_NM,
        ITEM_STATE: refRowData.ITEM_STATE,
        ITEM_SPEC: refRowData.ITEM_SPEC,
        ITEM_STATE_F: refRowData.ITEM_STATE_F,
        ITEM_LOT: refRowData.ITEM_LOT,
        QTY_IN_BOX: refRowData.QTY_IN_BOX,
        IN_UNIT_DIV_F: refRowData.IN_UNIT_DIV_F,
        LABELLING_DIV_D: refRowData.LABELLING_DIV_D,
        VALID_DATE: refRowData.VALID_DATE,
        BATCH_NO: refRowData.BATCH_NO,
        ORDER_QTY: 0,
        ENTRY_QTY: "", // 덤추가의 경우 등록수량 필수입력 체크 위해
        ENTRY_BOX: "", // 덤추가의 경우 등록수량 필수입력 체크 위해
        ENTRY_EA: "", // 덤추가의 경우 등록수량 필수입력 체크 위해
        CONFIRM_QTY: 0,
        PUTAWAY_QTY: 0,
        BOX_WEIGHT: 0,
        ENTRY_WEIGHT: 0,
        ...
        ORDER_NO: $NC.G_VAR.masterData.ORDER_NO,
        ORDER_LINE_NO: "",
        BU_DATE: refRowData.BU_DATE,
        BU_NO: refRowData.BU_NO,
        BU_LINE_NO: "",
        BU_KEY: "",
        REMARK1: "",
        SPARE1_NOTE: "",
 ...
 * 덤삭제 버튼 이벤트 처리
 */
function btnEntryDeleteFreeOnClick() {

    var rowData = G_GRDDETAIL.data.getItem(G_GRDDETAIL.lastRow);
    // 선택한 상품이 인터페이스 받은 상품인지 체크
    if ($NC.isNotNull(rowData.BU_LINE_NO)) {
        alert($NC.getDisplayMsg("JS.LIC02011P0.026", "덤추가 상품만 삭제 가능합니다.\n선택하신 상품은 덤추가 상품이 아닙니다."));
        return;
    }
