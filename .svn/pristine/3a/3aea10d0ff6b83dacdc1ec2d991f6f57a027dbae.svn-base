...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BU_CD":
            ...
            return;
        case "OUTBOUND_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB07020E0.001", "검색 출고 시작 일자를 정확히 입력하십시오."));
            setOutboundBatchCombo();
            break;
        case "OUTBOUND_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB07020E0.002", "검색 출고 종료 일자를 정확히 입력하십시오."));
            setOutboundBatchCombo();
            break;
        case "DELIVERY_CD":
            var CUST_CD = $NC.getValue("#edtQCust_Cd");
            if ($NC.isNull(CUST_CD)) {
                alert($NC.getDisplayMsg("JS.LOB07020E0.003", "사업부를 먼저 선택 하십시오."));
...
function _Inquiry(scanVal) {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.006", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd", true);
    // if ($NC.isNull(BU_CD)) {
    // alert($NC.getDisplayMsg("JS.LOB07020E0.007", "사업부를 입력하십시오."));
    // $NC.setFocus("#edtQBu_Cd");
    // return;
    // }
    var OUTBOUND_DATE1 = $NC.getValue("#dtpQOutbound_Date1");
    if ($NC.isNull(OUTBOUND_DATE1)) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.008", "출고 시작 일자를 입력하십시오."));
        $NC.setFocus("#dtpQOutbound_Date1");
        return;
    }
    var OUTBOUND_DATE2 = $NC.getValue("#dtpQOutbound_Date2");
    if ($NC.isNull(OUTBOUND_DATE2)) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.009", "출고 종료 일자를 입력하십시오."));
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        // 데이터 조회
        $NC.serviceCall("/LOB07020E0/getDataSet.do", $NC.getGridParams(G_GRDT1MASTER), onGetT1Master);
    } else {
        // 조회시 전역 변수 값 초기화
        ...
        // 데이터 조회
        $NC.serviceCall("/LOB07020E0/getDataSet.do", $NC.getGridParams(G_GRDT2MASTER), onGetT2Master);
    }
}
...
function _Delete() {
    ...
    if (rCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.010", "조회 후 처리하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOB07020E0.011", "선택한 운송장 번호를 삭제 하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.012", "운송장 번호 삭제 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.013", "선택한 데이터 중 삭제 처리 가능한 운송장 번호가 없습니다."));
        return;
    }

    $NC.serviceCall("/LOB07020E0/delete.do", {
...
function _Print(reportInfo) {
        ...
        case "LABEL_LOB04":
            ...
            checkedData = $NC.getGridCheckedValues(G_GRDT2MASTER, {
                ...
                compareFn: function(rowData) {
                    ...
                    return rowData.HDC_DIV == $ND.C_HDC_DIV_EPOST_B2B || rowData.HDC_DIV == $ND.C_HDC_DIV_EPOST_B2C;
                }
            });
            queryParams = {
                P_CENTER_CD: CENTER_CD,
        ...
        case "LABEL_LOB07":
            ...
            checkedData = $NC.getGridCheckedValues(G_GRDT2MASTER, {
                ...
                compareFn: function(rowData) {
                    ...
                    return rowData.HDC_DIV == $ND.C_HDC_DIV_HANJIN_B2B || rowData.HDC_DIV == $ND.C_HDC_DIV_HANJIN_B2C;
                }
            });
            queryParams = {
                P_CENTER_CD: CENTER_CD,
        ...
        case "LABEL_LOB06":
            ...
            checkedData = $NC.getGridCheckedValues(G_GRDT2MASTER, {
                ...
                compareFn: function(rowData) {
                    ...
                    return rowData.HDC_DIV == $ND.C_HDC_DIV_CJ_B2B || rowData.HDC_DIV == $ND.C_HDC_DIV_CJ_B2C;
                }
            });
            queryParams = {
                P_CENTER_CD: CENTER_CD,
    ...
    if (noInspectCount > 0) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.004", "미검수 송장이 존재합니다."));
        return;
    }
    if (cancelCount > 0) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.005", "주문이 취소된 송장이 존재합니다."));
        return;
    }
    ...
    if (reportInfo.REPORT_CD == "LABEL_LOB04" || reportInfo.REPORT_CD == "LABEL_LOB05" || reportInfo.REPORT_CD == "LABEL_LOB06") {
        printOptions.onAfterPrint = function() {
            for (var i = 0; i < checkedData.values.length; i++) {
                ...
                $NC.serviceCall("/LOB07020E0/callSetWbNoPrintCntUpdate.do", {
                    P_CENTER_CD: CENTER_CD,
                    P_BU_CD: BU_CD,
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "LOB07020E0.RS_T1_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options,
        canDblClick: true
    });
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "LOB07020E0.RS_T2_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options,
        canDblClick: true
    });
...
function btnWBDeliveryInfoOnClick() {
    ...
    if (G_GRDT2MASTER.lastRow == null) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.015", "배송조회할 운송장을 선택하십시오."));
        return;
    }

    var rowData = G_GRDT2MASTER.data.getItem(G_GRDT2MASTER.lastRow);
    if (rowData.SEND_STATE != "50") {
        alert($NC.getDisplayMsg("JS.LOB07020E0.016", "아직 택배사로 송신되지 않은 운송장입니다. 배송조회할 수 없습니다."));
        return;
    }

    if ($NC.isNull(rowData.WB_NO)) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.017", "운송장번호가 없는 데이터입니다. 배송조회할 수 없습니다."));
        return;
    }

    $NC.showProgramSubPopup({
        PROGRAM_ID: "LOB07022P0",
        PROGRAM_NM: $NC.getDisplayMsg("JS.LOB07022P0.001", "택배사 배송조회"),
        url: "lo/LOB07022P0.html",
        width: 1100,
        height: 600,
        G_PARAMETER: {
            P_CENTER_CD: rowData.CENTER_CD,
...
function grdOnDblClick(e, args) {
    ...
    if (!permission.canSave) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.014", "해당 프로그램의 저장권한이 없습니다."));
        return;
    }
    ...
    if (G_GRDT1MASTER.view == args.grid) {
        ...
        callParams = {
            ...
            P_HDC_DIV: rowData.HDC_DIV,
            P_PERMISSION: permission,
            P_QUERY_ID: "LOB07020E0.RS_SUB2"
        };
    }

    $NC.showProgramSubPopup({
        PROGRAM_ID: "LOB07021P0",
        PROGRAM_NM: $NC.getDisplayMsg("JS.LOB07021P0.001", "운송장 상품내역"),
        url: "lo/LOB07021P0.html",
        width: 870,
        height: 450,
...
function btnT1XlsDownloadOnClick() {
    ...
    if (!$NC.getProgramPermission().canApproval) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.018", "마스킹 해제된 엑셀 다운로드 권한이 없습니다."));
        return;
    }

    $NC.excelFileDownload({
        P_QUERY_ID: "LOB07020E0.RS_T1_MASTER",
...
function btnT2XlsDownloadOnClick() {
    ...
    $NC.excelFileDownload({
        P_QUERY_ID: "LOB07020E0.RS_T2_MASTER",
...
function btnWbConfirmOnClick() {
    ...
    if (!$NC.getProgramPermission().canConfirm) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.019", "검수확정 권한이 없습니다."));
        return;
    }
    ...
    if (rowData.OUTBOUND_STATE == $ND.C_STATE_ENTRY) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.020", "출고지시 상태의 전표만 처리 가능합니다.\n출고지시 후 처리하십시오."));
        return;
    }

    if (rowData.CANCEL_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LOB07020E0.021", "취소 처리된 전표는 검수 처리할 수 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOB07020E0.022", "선택한 운송장 번호를 확정 하시겠습니까?"))) {
        return;
    }

    $NC.serviceCall("/LOB07020E0/callLOWbConfirm.do", {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_OUTBOUND_DATE: rowData.OUTBOUND_DATE,
        P_OUTBOUND_NO: rowData.OUTBOUND_NO,
