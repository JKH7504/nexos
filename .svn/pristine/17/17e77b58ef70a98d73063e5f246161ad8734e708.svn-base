...
import nexos.service.ed.EDR04010E0Service;
 ...
 */
@Controller
@RequestMapping("/EDR04010E0")
public class EDR04010E0Controller extends ControllerSupport {

    @Autowired
    private EDR04010E0Service service;
     ...
     *        조회조건
     */
    @RequestMapping(value = "/callERLOOrderDelete.do", method = RequestMethod.POST)
    public ResponseEntity<String> callERLOOrderDelete(HttpServletRequest request, @RequestBody Map<String, Object> params) {

        ResponseEntity<String> result = null;

        try {
            result = getResponseEntity(request, service.callERLOOrderDelete(params));
        } catch (Exception e) {
            result = getResponseEntityError(request, e);
        }
