...
function _InquiryD(args) {
    ...
    G_GRDMASTERD.queryParams = {
        ...
        P_DELIVERY_DIV: args.DELIVERY_DIV,
        P_ERP_BATCH: args.ERP_BATCH,
        P_REGION_DIV: args.REGION_DIV,
        P_AREA_CD: args.AREA_CD,
        P_STATE_PRE_YN: args.STATE_PRE_YN,
        P_STATE_CUR_YN: args.STATE_CUR_YN,
        P_POLICY_CM510: $NC.G_VAR.policyVal.CM510
    };
    // 데이터 조회
    $NC.serviceCall("/LOB02010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTERD), onGetMasterD);
}
...
function _SaveD() {
    ...
    if (G_GRDMASTERD.data.getLength() == 0 || $NC.isNull(G_GRDMASTERD.lastRow) || G_GRDSUBD.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.020", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    // 마지막 선택 Row Validation 체크
    if (!$NC.isGridValidLastRow(G_GRDDETAILD)) {
        return;
    }
    ...
    var dsDetail = [ ];
    dsTarget = G_GRDDETAILD.data.getItems();
    for (rIndex = 0, rCount = dsTarget.length; rIndex < rCount; rIndex++) {
        rowData = dsTarget[rIndex];
        if (rowData.CRUD == $ND.C_DV_CRUD_R) {
            continue;
    ...
    if (refRowData.CRUD == $ND.C_DV_CRUD_R && dsSub.length == 0 && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.021", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LOB02010E0/saveConfirm.do", {
        P_DS_MASTER: {
...
function onProcessPreD() {
    ...
    if (rCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.022", "조회 후 처리하십시오."));
        return;
    }
    if ($NC.G_VAR.PROCESS_YN != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.023", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록 후 처리하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOB02010E0.075", "출고확정 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.076", "출고확정취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.077", "선택한 데이터 중 출고확정취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtD() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOB02010E0.078", "출고확정 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBD)) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.079", "데이터가 수정되었습니다. 저장 후 출고확정 처리하십시오."));
        return;
    }
    ...
    for (var rIndex = 0; rIndex < rCount; rIndex++) {
        ...
        checkedCount++;
        if (rowData.INSPECT_DONE == $ND.C_NO) {
            noInspectCount++;
        }
        // 출고지시 상태인 전표만 대상
        if (rowData.OUTBOUND_STATE != PROCESS_STATE) {
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.080", "출고확정 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.081", "선택한 데이터 중 확정처리 가능한 데이터가 없습니다."));
        return;
    }
    if ($NC.G_VAR.policyVal.LO490 == $ND.C_YES && noInspectCount > 0) {
        if (!confirm($NC.getDisplayMsg("JS.LOB02010E0.082", "검수 완료 되지 않은 전표가 존재합니다. \n확정 처리하시겠습니까?"))) {
            return;
        }
    }
...
function grdMasterDInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterD", {
        ...
        queryId: "LOB02010E0.RS_T3_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
...
function grdMasterDOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOB02010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAILD), onGetDetailD);
    ...
    // 데이터 조회
    $NC.serviceCall("/LOB02010E0/getDataSet.do", $NC.getGridParams(G_GRDSUBD), onGetSubD);

    // 출고지시 수정 가능 여부 -> 진행상태: 30
...
function grdDetailDOnSetColumns() {

    $NC.setGridColumns(G_GRDDETAILD, [ // 숨김컬럼 세팅
        $NC.G_VAR.policyVal.LO492 != $ND.C_YES ? "SHORTAGE_QTY,SHORTAGE_DIV_F,SHORTAGE_COMMENT" : ""
    ]);
}
...
function grdDetailDInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailD", {
        ...
        queryId: "LOB02010E0.RS_T3_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });

    G_GRDDETAILD.view.onSelectedRowsChanged.subscribe(grdDetailDOnAfterScroll);
    G_GRDDETAILD.view.onBeforeEditCell.subscribe(grdDetailDOnBeforeEditCell);
    G_GRDDETAILD.view.onCellChange.subscribe(grdDetailDOnCellChange);
}
...
function grdDetailDOnBeforeEditCell(e, args) {
    ...
    switch (args.column.id) {
        ...
        case "SHORTAGE_COMMENT":
            // 미출고 수량이 0 보다 크고 진행상태가 30이면 수정
            return rowData.SHORTAGE_QTY > "0" && rowData.OUTBOUND_STATE == PROCESS_STATE;
    }
    return true;
}
 ...
 * 그리드의 입력항목 값 변경
 */
function grdDetailDOnCellChange(e, args) {

    var rowData = args.item;

    // 마지막 선택 Row 수정 데이터 반영 및 상태 변경
    $NC.setGridApplyChange(G_GRDDETAILD, rowData);
}
    ...
    var options = {
        ...
        specialRow: {
            compareFn: function(specialRow, rowData, column, row, cell, colspan) {
                ...
                if (rowData.VIRTUAL_YN == $ND.C_YES && rowData.STOCK_IN_GRP == $ND.C_BASE_BATCH_NO) {
                    return "stySpecial";
                }
            }
        }
...
function grdSubDOnCellChange(e, args) {
    ...
    var rowData = args.item;
    var columnId = G_GRDSUBD.view.getColumnId(args.cell);
    switch (G_GRDSUBD.view.getColumnId(args.cell)) {
        case "CONFIRM_QTY":
            var CONFIRM_QTY = Number(rowData.CONFIRM_QTY);
            if (CONFIRM_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LOB02010E0.083", "확정수량이 0보다 작을 수 없습니다."));
            ...
            if (Number(rowData.ENTRY_QTY) < CONFIRM_QTY) {
                alert($NC.getDisplayMsg("JS.LOB02010E0.084", "확정수량이 등록수량을 초과할 수 없습니다."));
                rowData.CONFIRM_QTY = args.oldItem.CONFIRM_QTY;
                rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
                rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
                rowData.CONFIRM_WEIGHT = args.oldItem.CONFIRM_WEIGHT;
            ...
            break;
        case "CONFIRM_BOX":
            var CONFIRM_BOX = Number(rowData.CONFIRM_BOX);
            if (CONFIRM_BOX < 0) {
                alert($NC.getDisplayMsg("JS.LOB02010E0.085", "확정BOX는 0보다 작을 수 없습니다."));
                rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
                $NC.setFocusGrid(G_GRDSUBD, args.row, args.cell, true);
                break;
            }
            rowData = grdSubOnCalc(rowData, columnId);
            if (Number(rowData.ENTRY_QTY) < rowData.CONFIRM_QTY) {
                alert($NC.getDisplayMsg("JS.LOB02010E0.084", "확정수량이 등록수량을 초과할 수 없습니다."));
                rowData.CONFIRM_QTY = args.oldItem.CONFIRM_QTY;
                rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
                rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
            ...
            break;
        case "CONFIRM_EA":
            var CONFIRM_EA = Number(rowData.CONFIRM_EA);
            if (CONFIRM_EA < 0) {
                alert($NC.getDisplayMsg("JS.LOB02010E0.086", "확정EA는 0보다 작을 수 없습니다."));
                rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
                $NC.setFocusGrid(G_GRDSUBD, args.row, args.cell, true);
                break;
            }
...
function grdSubDOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONFIRM_QTY)) {
            alert($NC.getDisplayMsg("JS.LOB02010E0.087", "확정수량을 입력하십시오."));
...
function grdSubDOnBeforeEditCell(e, args) {
    ...
    switch (args.column.id) {
        ...
        case "CONFIRM_BOX":
        case "CONFIRM_EA":
            return $NC.G_VAR.policyVal.LO410 == "1"; // [B2B]출고 확정 수량 수정 허용 기준
    }
    return true;
}
...
function setUserProgramPermissionD() {

    var enable = G_GRDMASTERD.data.getLength() > 0;
    // var canSave = $NC.getProgramPermission().canSave;

    $NC.setEnable("#btnAttachFileDownloadD", enable);
}

/**
 * 거래명세서 수정 버튼 클릭 처리
 */
function btnInoutDateDOnClick() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOB02010E0.088", "거래명세서일자를 수정하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBD)) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.089", "데이터가 수정되었습니다. 저장 후 거래명세서일자 수정 처리하십시오."));
        return;
    }

    var dsMaster = [ ];
    var PROCESS_STATE = $NC.G_VAR.stateFWBW[$NC.G_VAR.activeView.PROCESS_CD].CONFIRM;
    var INOUT_DATE = $NC.getValue("#dtpInout_DateD");
    var checkedCount = 0;
    var rowData;
    for (var rIndex = 0; rIndex < rCount; rIndex++) {
        rowData = G_GRDMASTERD.data.getItem(rIndex);
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.090", "거래명세서일자를 수정할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.091", "선택한 데이터 중 거래명세서일자 수정 가능한 데이터가 없습니다."));
        return;
    }

    $NC.serviceCall("/LOB02010E0/callInoutDateUpdate.do", {
        P_DS_MASTER: dsMaster
    }, onSaveD, onSaveErrorD);
}
...
function btnAttachFileDownloadDOnClick(e) {
    ...
    if ($NC.isNull(G_GRDMASTERD.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.114", "파일첨부할 전표를 선택하십시오."));
        return;
    }
    ...
    if ($NC.isNull(rowData.ATTACH_FILE_NM)) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.118", "첨부 파일이 등록되어 있지 않은 전표입니다."));
        return;
    }

    $NC.fileDownload("/LOB02010E0/attachmentFileDownload.do", {
