 ...
 *  프로그램ID         : LCC03020E1
 *  프로그램명         : 파렛트ID병합관리 (의류)
 *  프로그램설명       : 파렛트ID병합관리 (의류) 화면 Javascript
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            if ($NC.isNull(BU_CD)) {
                alert($NC.getDisplayMsg("JS.LCC03020E1.020", "사업부를 먼저 선택 하십시오."));
            ...
            return;
        case "MOVE_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC03020E1.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "MOVE_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC03020E1.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC03020E1.003", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC03020E1.004", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var MOVE_DATE1 = $NC.getValue("#dtpQMove_Date1");
    if ($NC.isNull(MOVE_DATE1)) {
        alert($NC.getDisplayMsg("JS.LCC03020E1.005", "검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQMove_Date1");
        return;
    }
    var MOVE_DATE2 = $NC.getValue("#dtpQMove_Date2");
    if ($NC.isNull(MOVE_DATE2)) {
        alert($NC.getDisplayMsg("JS.LCC03020E1.006", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQMove_Date2");
        return;
    }
    if (MOVE_DATE1 > MOVE_DATE2) {
        alert($NC.getDisplayMsg("JS.LCC03020E1.007", "병합일자 검색 범위 오류입니다."));
        $NC.setFocus("#dtpQMove_Date1");
        return;
    }
...
function _New() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        $NC.showProgramSubPopup({
            PROGRAM_ID: "LCC03021P1",
            PROGRAM_NM: $NC.getDisplayMsg("JS.LCC03020E1.008", "파렛트ID병합등록/수정"),
            url: "lc/LCC03021P1.html",
            width: 1024,
            height: 610,
            G_PARAMETER: {
...
function _Delete() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
            alert($NC.getDisplayMsg("JS.LCC03020E1.009", "삭제할 데이터가 없습니다."));
            return;
        }

        var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
        if (rowData.CONFIRM_YN == $ND.C_YES) {
            alert($NC.getDisplayMsg("JS.LCC03020E1.010", "확정된 데이터는 삭제할수 없습니다."));
            return;
        }

        if (!confirm($NC.getDisplayMsg("JS.LCC03020E1.011", "삭제 하시겠습니까?"))) {
            return;
        }
...
function _Print(reportInfo) {
    ...
    if ($NC.isNull(rowData)) {
        alert($NC.getDisplayMsg("JS.LCC03020E1.012", "출력할 데이터를 먼저 선택하십시오."));
        return;
    }
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "LCC03020E1.RS_T1_MASTER",
        sortCol: "MOVE_DATE",
        gridOptions: options,
        canDblClick: true
    });
...
function grdT1MasterOnDblClick(e, args) {
    ...
    if (refRowData.CONFIRM_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC03020E1.013", "이미 확정처리된 전표입니다."));
        return;
    }
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LCC03020E1.014", "확정여부를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        var oMsg = $NC.getOutMessage(resultData);
        if (oMsg != $ND.C_OK) {
        ...
        if (resultData.O_CONFIRM_YN != $ND.C_NO) {
            alert($NC.getDisplayMsg("JS.LCC03020E1.015", "이미 확정처리된 데이터입니다.\n다시 조회 후 데이터를 확인하십시오."));
            return;
        }
        ...
        if (dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC03020E1.016", "수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "LCC03020E1.RS_T2_MASTER",
        sortCol: "MOVE_DATE",
        gridOptions: options
    });
...
function onProcessNxt() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC03020E1.017", "조회 후 처리하십시오."));
        return;
    }

    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.CONFIRM_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC03020E1.018", "이미 확정 처리한 데이터입니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC03020E1.019", "확정 처리하시겠습니까?"))) {
        return;
    }
