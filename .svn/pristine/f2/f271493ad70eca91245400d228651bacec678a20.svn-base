 ...
 *  프로그램ID         : RIM02011P1
 *  프로그램명         : 온라인반입등록 (의류) 팝업
 *  프로그램설명       : 온라인반입등록 (의류) 팝업 화면 Javascript
...
function _Initialize() {
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.RIM02011P1.001", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.RIM02011P1.002", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.RIM02011P1.003", "저장"));
}
...
function _OnLoaded() {
    ...
    // 예정 -> 등록, 등록 수정
        ...
        $NC.setEnable("#btnShipper_Zip_Cd", false);
        // 예정으로 등록
        if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ORDER) {
        }
        // 등록수정
...
function _New() {
    ...
    if ($NC.isNull(DELIVERY_CD)) {
        alert($NC.getDisplayMsg("JS.RIM02011P1.004", "먼저 온라인몰 코드를 입력하십시오."));
        $NC.setFocus("#edtDelivery_Cd");
        return;
    }
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.RIM02011P1.006", "물류센터가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.RIM02011P1.007", "사업부가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.INBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.RIM02011P1.008", "먼저 반입일자를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.RIM02011P1.009", "먼저 반입구분을 선택하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_NM)) {
        alert($NC.getDisplayMsg("JS.RIM02011P1.010", "먼저 주문자명을 입력하십시오."));
        $NC.setFocus("#edtOrderer_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_HP)) {
        alert($NC.getDisplayMsg("JS.RIM02011P1.011", "먼저 주문자 휴대폰번호를 입력하십시오."));
        $NC.setFocus("#edtOrderer_Hp");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_NM)) {
        alert($NC.getDisplayMsg("JS.RIM02011P1.012", "먼저 수령자명을 입력하십시오."));
        $NC.setFocus("#edtShipper_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_HP)) {
        alert($NC.getDisplayMsg("JS.RIM02011P1.013", "먼저 수령자 휴대폰번호를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_ZIP_CD) || $NC.isNull($NC.G_VAR.masterData.SHIPPER_ADDR_BASIC)) {
        alert($NC.getDisplayMsg("JS.RIM02011P1.014", "먼저 수령자 주소를 입력하십시오."));
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.RIM02011P1.005", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.RIM02011P1.015", "수정 후 저장하십시오."));
        return;
    }
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.RIM02011P1.016", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        ...
        case "INOUT_CD":
            ...
            break;
        case "INBOUND_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.RIM02011P1.017", "반입일자를 정확히 입력하십시오."));
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        ...
        case "ENTRY_QTY":
            ...
            if (ENTRY_QTY < CHECK_QTY) {
                alert($NC.getDisplayMsg("JS.RIM02011P1.018", "등록수량이 " + CHECK_QTY + "보다 작을 수 없습니다.", CHECK_QTY));
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.RIM02011P1.019", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                } else {
