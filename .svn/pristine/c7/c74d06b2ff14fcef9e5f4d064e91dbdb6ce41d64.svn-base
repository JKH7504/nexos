 ...
 *  프로그램ID         : CMC05040E1
 *  프로그램명         : 고정로케이션관리
 *  프로그램설명       : 고정로케이션관리 화면(의류) Javascript
...
function _Initialize() {
    ...
    // 운영구분 초기값 상품별로 세팅
    $NC.setValueRadioGroup("rgbOperation_Div", "1");

    // 이벤트 연결
    $("#btnInquiryItem").click(btnInquiryItemOnClick); // 상품 조회 클릭시 처리
    $("#btnAddItem").click(btnAddItemOnClick); // 고정로케이션 등록 버튼 클릭시 처리
    $("#btnLocationFixReset").click(btnLocationFixResetOnClick); // 고정로케이션 재설정 버튼 클릭시 처리
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.CMC05040E1.001", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var ITEM_STATE = $NC.getValue("#cboQItem_State");
    if ($NC.isNull(ITEM_STATE)) {
        alert($NC.getDisplayMsg("JS.CMC05040E1.002", "상품상태를 선택하십시오."));
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.CMC05040E1.003", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    var OPERATION_DIV = $NC.getValueRadioGroup("rgbOperation_Div");

    var dsMaster = [ ];
    // 운영구분이 스타일별이거나 스타일+컬러별일 경우의 삭제 처리 - 동일 스타일 / 스타일+컬러의 상품 삭제
    if (OPERATION_DIV != "1" && rowData.CRUD == $ND.C_DV_CRUD_D) {
        var COLOR_CD;
        if (OPERATION_DIV == "2") {
            if ($NC.isNull(rowData.STYLE_CD)) {
                alert($NC.getDisplayMsg("JS.CMC05040E1.005", "스타일별 삭제가 불가능한 상품입니다."));
                return;
            }
            COLOR_CD = $ND.C_ALL;
        } else {
            if ($NC.isNull(rowData.STYLE_CD) || $NC.isNull(rowData.COLOR_CD)) {
                alert($NC.getDisplayMsg("JS.CMC05040E1.006", "스타일+컬러별 삭제가 불가능한 상품입니다."));
                return;
            }
            COLOR_CD = rowData.COLOR_CD;
        }
        dsMaster.push({
    ...
    // 그 외 저장처리
        ...
        if (dsMaster.length == 0) {
            alert($NC.getDisplayMsg("JS.CMC05040E1.004", "데이터 수정 후 저장하십시오."));
            return;
        }
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.CMC05040E1.007", "삭제할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.CMC05040E1.008", "삭제 하시겠습니까?"))) {
        return;
    }
...
function setUserProgramPermission() {
    ...
    $NC.setEnable("#btnLocationFixReset", permission.canSave && enable);
}
    ...
    switch (G_GRDMASTER.view.getColumnId(args.cell)) {
        case "MIN_UNIT_QTY":
            if (Number(rowData.MIN_UNIT_QTY) < 0) {
                alert($NC.getDisplayMsg("JS.CMC05040E1.009", "최소보관수량에 0이상의 정수를 입력하십시오."));
                rowData.MIN_UNIT_QTY = args.oldItem.MIN_UNIT_QTY;
                $NC.setFocusGrid(G_GRDMASTER, args.row, args.cell, true);
            }
            break;
        case "MAX_UNIT_QTY":
            if (Number(rowData.MAX_UNIT_QTY) < 0) {
                alert($NC.getDisplayMsg("JS.CMC05040E1.010", "최대보관수량에 0이상의 정수를 입력하십시오."));
                rowData.MAX_UNIT_QTY = args.oldItem.MAX_UNIT_QTY;
                $NC.setFocusGrid(G_GRDMASTER, args.row, args.cell, true);
            }
            break;
        case "FILL_SAFETY_QTY":
            if (Number(rowData.FILL_SAFETY_QTY) < 0) {
                alert($NC.getDisplayMsg("JS.CMC05040E1.011", "보충적정수량에 0이상의 정수를 입력하십시오."));
        ...
        case "FILL_UNIT_QTY":
            if (Number(rowData.FILL_UNIT_QTY) < 0) {
                alert($NC.getDisplayMsg("JS.CMC05040E1.012", "보충단위수량에 0이상의 정수를 입력하십시오."));
...
function grdMasterOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.MIN_UNIT_QTY)) {
            alert($NC.getDisplayMsg("JS.CMC05040E1.013", "최소보관수량을 입력하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "MIN_UNIT_QTY", true);
            return false;
        }
        if ($NC.isNull(rowData.MAX_UNIT_QTY)) {
            alert($NC.getDisplayMsg("JS.CMC05040E1.014", "최대보관수량을 입력하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "MAX_UNIT_QTY", true);
            return false;
        }
        if ($NC.isNull(rowData.FILL_SAFETY_QTY)) {
            alert($NC.getDisplayMsg("JS.CMC05040E1.015", "보충적정수량을 입력하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "FILL_SAFETY_QTY", true);
            return false;
        }
        if ($NC.isNull(rowData.FILL_UNIT_QTY)) {
            alert($NC.getDisplayMsg("JS.CMC05040E1.016", "보충단위수량을 입력하십시오."));
...
function onLocationFixReset(ajaxData) {

    _Inquiry();
    alert($NC.getDisplayMsg("JS.CMC05040E1.017", "고정로케이션 재설정이 완료되었습니다."));
}
...
function btnInquiryItemOnClick() {
    ...
    if ($NC.isNull(BU_NM)) {
        alert($NC.getDisplayMsg("JS.CMC05040E1.018", "사업부를 입력하십시오."));
...
function btnAddItemOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.CMC05040E1.019", "데이터 조회 후 등록 처리하십시오."));
        return;
    }
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.CMC05040E1.020", "상품 조회 후 등록 처리하십시오."));
        return;
    }
    ...
    if (OPERATION_DIV == "1") {
        ...
        if (rowData.ENTRY_YN == $ND.C_YES) {
            ...
            if (rowData.LOCATION_CD == refRowData.LOCATION_CD) {
                alert($NC.getDisplayMsg("JS.CMC05040E1.021", "로케이션: " + rowData.LOCATION_CD + "\n해당 로케이션에 이미 등록된 상품입니다. " //
                    + "등록할 수 없습니다.", rowData.LOCATION_CD));
                return;
            }
            // 다른 로케이션
            else if (rowData.LOCATION_CD != refRowData.LOCATION_CD) {
                if (!confirm($NC.getDisplayMsg("JS.CMC05040E1.022", "기존로케이션: " + rowData.LOCATION_CD //
        ...
        if (OPERATION_DIV == "2") {
            if ($NC.isNull(rowData.STYLE_CD)) {
                alert($NC.getDisplayMsg("JS.CMC05040E1.023", "스타일별 등록이 불가능한 상품입니다."));
                return;
            }
            COLOR_CD = $ND.C_ALL;
        } else {
            if ($NC.isNull(rowData.STYLE_CD) || $NC.isNull(rowData.COLOR_CD)) {
                alert($NC.getDisplayMsg("JS.CMC05040E1.024", "스타일+컬러별 등록이 불가능한 상품입니다."));
                return;
            }
            COLOR_CD = rowData.COLOR_CD;
        }
            ...
            P_DS_MASTER: [
                {
                    ...
                    P_FILL_UNIT_QTY: rowData.QTY_IN_BOX,
                    P_FILL_SAFETY_QTY: rowData.QTY_IN_BOX,
                    P_USER_ID: $NC.G_USERINFO.USER_ID,
                    P_CRUD: CRUD
                }
            ],
    ...
    // 운영구분이 스타일별이거나 스타일+컬러별일 경우 고정로케이션 등록 처리
    else {
        // CM_LOCATIONFIX_ENTRY 호출
        $NC.serviceCall("/CMC05040E0/callCMLocationFixEntry.do", {
            P_CENTER_CD: CENTER_CD,
            P_BU_CD: BU_CD,
            P_ITEM_STATE: ITEM_STATE,
            P_BRAND_CD: BRAND_CD,
...
function btnLocationFixResetOnClick() {

    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.CMC05040E1.025", "데이터조회 후 고정로케이션을 재설정하십시오."));
        return;
    }
    ...
    var message;
    if (OPERATION_DIV == "1") {
        message = $NC.getDisplayMsg("JS.CMC05040E1.026", "상품별로 고정로케이션을 재설정 하시겠습니까?");
    } else if (OPERATION_DIV == "2") {
        message = $NC.getDisplayMsg("JS.CMC05040E1.027", "스타일별로 고정로케이션을 재설정 하시겠습니까?");
    } else {
        message = $NC.getDisplayMsg("JS.CMC05040E1.028", "스타일+컬러별로 고정로케이션을 재설정 하시겠습니까?");
    }
    if (!confirm(message)) {
        return;
    }
    ...
    if ($NC.isNull(ZONE_CD)) {
        alert($NC.getDisplayMsg("JS.CMC05040E1.029", "존코드를 선택하십시오."));
        return;
    }

    // CM_LOCATIONFIX_ENTRY 호출
    $NC.serviceCall("/CMC05040E0/callCMLocationFixReset.do", {
        P_CENTER_CD: CENTER_CD,
        P_BU_CD: BU_CD,
        P_ITEM_STATE: ITEM_STATE,
        P_ZONE_CD: ZONE_CD,
        P_OPERATION_DIV: OPERATION_DIV,
        P_USER_ID: $NC.G_USERINFO.USER_ID
    }, onLocationFixReset, onSaveError);
}
