...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            return;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOM01040E0.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOM01040E0.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
        case "DELIVERY_CD":
            $NP.onDeliveryChange(val, {
                ...
                title: $NC.getDisplayMsg("JS.LOM01040E0.003", "온라인몰 검색"),
                columnTitle: [
                    "온라인몰코드",
                    "온라인몰명"
                ],
                errorMessage: $NC.getDisplayMsg("JS.LOM01040E0.004", "등록되어 있지 않은 온라인몰입니다.")
            });
            return;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOM01040E0.005", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LOM01040E0.006", "사업부를 입력하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE1)) {
        alert($NC.getDisplayMsg("JS.LOM01040E0.007", "검색 시작일자를 입력하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE2)) {
        alert($NC.getDisplayMsg("JS.LOM01040E0.008", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQOrder_Date2");
        return;
    }

    if (ORDER_DATE1 > ORDER_DATE2) {
        alert($NC.getDisplayMsg("JS.LOM01040E0.009", "출고예정일자 범위 입력오류입니다."));
        $NC.setFocus("#dtpQOrder_Date1");
        return;
    }
    ...
    // 데이터 조회
    $NC.serviceCall("/LOM01040E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function grdMasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOM01040E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
...
function getOutboundState(params, onSuccess) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOM01040E0/getData.do", {
...
function btnOrderMergeOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LOM01040E0.010", "병합처리할 데이터가 없습니다."));
        return;
    }
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOM01040E0.011", "병합처리 하시겠습니까?"))) {
        return;
    }

    // 병합처리 호출
    $NC.serviceCall("/LOM01040E0/callLOOrderMerge.do", {
        P_CENTER_CD: CENTER_CD,
        P_BU_CD: BU_CD,
        P_ORDER_DATE1: ORDER_DATE1,
        P_ORDER_DATE2: ORDER_DATE2,
...
function btnOrderMergeCancelOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOM01040E0.012", "병합취소 처리할 데이터를 선택하세요."));
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    if (rowData.MERGE_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LOM01040E0.013", "병합취소 대상 전표가 아닙니다.\n병합처리된 전표를 선택하세요."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOM01040E0.014", "병합취소 처리 하시겠습니까?"))) {
        return;
    }
    // 병합처리 호출
    $NC.serviceCall("/LOM01040E0/callLOOrderMergeCancel.do", {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_ORDER_DATE: rowData.ORDER_DATE,
        P_ORDER_NO: rowData.ORDER_NO,
