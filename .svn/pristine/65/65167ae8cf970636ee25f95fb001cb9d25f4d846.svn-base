 ...
 *  프로그램ID         : LOM02010E1
 *  프로그램명         : 출고작업[B2C] (의류)
 *  프로그램설명       : 출고작업[B2C] (의류) 화면 Javascript
...
function _Initialize() {
    ...
    $("#btnEquipSendCancel").click(window.btnEquipSendCancelOnClick); // 출하장비전송취소 버튼 클릭
    ...
    $("#btnAcsGetAddress").click(window.btnAcsGetAddressOnClick); // 주소정제처리 버튼 클릭
    ...
    // 취소/처리 버튼 툴팁 세팅
    $NC.setTooltip("#btnProcessPreB", $NC.getDisplayMsg("JS.LOM02010E1.001", $NC.getValue("#btnProcessB") + " 취소", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessNxtB", $NC.getDisplayMsg("JS.LOM02010E1.002", $NC.getValue("#btnProcessB") + " 처리", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessPreC", $NC.getDisplayMsg("JS.LOM02010E1.001", $NC.getValue("#btnProcessC") + " 취소", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessNxtC", $NC.getDisplayMsg("JS.LOM02010E1.002", $NC.getValue("#btnProcessC") + " 처리", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessPreD", $NC.getDisplayMsg("JS.LOM02010E1.001", $NC.getValue("#btnProcessD") + " 취소", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessNxtD", $NC.getDisplayMsg("JS.LOM02010E1.002", $NC.getValue("#btnProcessD") + " 처리", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessPreE", $NC.getDisplayMsg("JS.LOM02010E1.001", $NC.getValue("#btnProcessE") + " 취소", $NC.getValue("#btnProcessE")));
    $NC.setTooltip("#btnProcessNxtE", $NC.getDisplayMsg("JS.LOM02010E1.002", $NC.getValue("#btnProcessE") + " 처리", $NC.getValue("#btnProcessE")));
}
    ...
    switch (id) {
        ...
        case "DELIVERY_CD":
            $NP.onDeliveryChange(val, {
                ...
                title: $NC.getDisplayMsg("JS.LOM02010E1.003", "온라인몰 검색"),
                columnTitle: [
                    "온라인몰코드",
                    "온라인몰명"
                ],
                errorMessage: $NC.getDisplayMsg("JS.LOM02010E1.004", "등록되어 있지 않은 온라인몰입니다.")
            });
            return;
        case "OUTBOUND_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOM02010E1.005", "검색 출고일자를 정확히 입력하십시오."));
            setOutboundBatchCombo("#cboQOutbound_BatchC", false);
            setOutboundBatchCombo("#cboOutbound_BatchC", false);
            setOutboundBatchCombo("#cboQOutbound_BatchD", true);
            break;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOM02010E1.006", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOM02010E1.007", "검색 종료일자를 정확히 입력하십시오."));
            break;
...
function _OnInputChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "OUTBOUND_DATEBT":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOM02010E1.008", "일괄처리생성정보의 출고일자를 정확히 입력하십시오."));
            setDeliveryBatchCombo($ND.C_PROCESS_ENTRY_BATCH);
            break;
        case "OUTBOUND_DATEB":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOM02010E1.009", "출고일자를 정확히 입력하십시오."));
            setDeliveryBatchCombo($ND.C_PROCESS_ENTRY);
            break;
        ...
        case "QVIEW_DIV1":
            ...
            break;
        case "INOUT_DATED":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOM02010E1.010", "거래명세서일자를 정확히 입력하십시오."));
            break;
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.011", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.012", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var OUTBOUND_DATE = $NC.getValue("#dtpQOutbound_Date");
    if ($NC.isNull(OUTBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.013", "출고일자를 입력하십시오."));
        $NC.setFocus("#dtpQOutbound_Date");
        return;
    }
    var ORDER_DATE1 = $NC.getValue("#dtpQOrder_Date1");
    if ($NC.isNull(ORDER_DATE1)) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.014", "출고예정 검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQOrder_Date1");
        return;
    }
    var ORDER_DATE2 = $NC.getValue("#dtpQOrder_Date2");
    if ($NC.isNull(ORDER_DATE2)) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.015", "출고예정 검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQOrder_Date2");
        return;
    }
    var INOUT_CD = $NC.getValue("#cboQInout_Cd");
    if ($NC.isNull(INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.016", "출고구분을 선택하십시오."));
    ...
    if (STATE_PRE_YN == $ND.C_NO && STATE_CUR_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.017", "검색구분을 선택하십시오."));
...
function _Print(reportInfo) {
    ...
    switch ($NC.G_VAR.activeView.PROCESS_CD) {
        ...
        case $ND.C_PROCESS_DIRECTIONS:
            ...
            if ($NC.isNull(OUTBOUND_BATCH)) {
                alert($NC.getDisplayMsg("JS.LOM02010E1.018", "출고차수를 선택하십시오."));
        ...
        case $ND.C_PROCESS_DELIVERY:
            ...
            // PAPER_LOM03 - 토탈피킹지시서
            // PAPER_LOM04 - 분배지시서
            if (reportInfo.REPORT_CD == "PAPER_LOM02" || reportInfo.REPORT_CD == "PAPER_LOM03" || reportInfo.REPORT_CD == "PAPER_LOM04"
            ...
            break;
        default:
            alert($NC.getDisplayMsg("JS.LOM02010E1.019", "해당 프로세스는 출력물이 없습니다."));
            return;
    }
    ...
    switch (reportInfo.REPORT_CD) {
        ...
        case "LABEL_LOM02":
            ...
            if (checkedData.checkedCount == 0 && $NC.G_VAR.activeView.PROCESS_CD == $ND.C_PROCESS_DIRECTIONS) {
                checkedData = {};
            }
            // checkedValues 외 쿼리 파라메터 세팅
            queryParams = {
                P_CENTER_CD: CENTER_CD,
                P_BU_CD: BU_CD,
                P_OUTBOUND_BATCH: $NC.isEmpty(checkedData) ? OUTBOUND_BATCH : null,
                P_OUTBOUND_DATE: OUTBOUND_DATE,
                P_POLICY_LO450: $NC.G_VAR.policyVal.LO450
            };

            $NC.serviceCallAndWait("/LOM02010E0/getDataSet.do", {
                P_QUERY_ID: "LOM02010E1.RS_SUB3",
                P_QUERY_PARAMS: {
                    P_CENTER_CD: CENTER_CD,
                    P_BU_CD: BU_CD,
                    P_OUTBOUND_DATE: OUTBOUND_DATE,
                ...
                if ($NC.isNull(dsResult)) {
                    alert($NC.getDisplayMsg("JS.LOM02010E1_C.020", "주소정제 성공 여부 체크 중 오류가 발생 했습니다."));
                    return;
                }

                var rowData = dsResult[0];
                if (rowData.ALL_ADDR_REFINE_YN != $ND.C_YES) {
                    alert($NC.getDisplayMsg("JS.LOM02010E1_C.021", "출고차수(" + OUTBOUND_BATCH_F
                        + ")의 전표 중 주소정제 오류건이 있습니다.\n주소정제 오류 건이 있을 경우 택배송장 출력 할 수 없습니다.", OUTBOUND_BATCH_F));
                    checkOk = false;
                }
            });
            break;
    ...
    if (reportInfo.REPORT_CD == "LABEL_LOM02") {
        printOptions.onAfterPrint = function() {
            ...
            if (dsMaster.length > 0) {
                $NC.serviceCall("/LOM02010E0/callSetWbnoPrintCntUpdate.do", {
...
function onGetProcessInfo(ajaxData) {
    ...
    // alert($NC.getDisplayMsg("JS.LOM02010E1.022", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록하십시오."));
    $NC.G_VAR.activeView.PROCESS_CD = "";
    $NC.showMessage({
        message: $NC.getDisplayMsg("JS.LOM02010E1.022", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록하십시오."),
        width: 400,
...
function setMasterSummaryInfo() {
    ...
    $NC.serviceCall("/LOM02010E0/getDataSet.do", {
        P_QUERY_ID: "LOM02010E1.RS_MASTER",
        P_QUERY_PARAMS: {
            P_CENTER_CD: CENTER_CD,
            P_BU_CD: BU_CD,
            P_OUTBOUND_DATE: OUTBOUND_DATE,
