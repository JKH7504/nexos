...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.lastPaperNo)) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "지시서를 먼저 스캔하십시오."));
        onShelfError();
        return;
    }

    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "처리할 상품 내역이 없습니다."));
        onShelfError();
        return;
    }

    if ($NC.isNull($NC.G_VAR.distributeData)) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "분배할 상품이 없습니다."));
        onShelfError();
        return;
    }
    ...
    $NC.serviceCall("/PDA_LOC02350E0/callLOProcDistribute.do", {
...
function grdMasterInitialize() {
    ...
    $NC.setInitGridObject("#grdMaster", {
        ...
        queryId: "PDA_LOC02350E0.RS_MASTER",
        sortCol: "ITEM_CD",
        gridOptions: options
    });
...
function onGetMaster(ajaxData) {
    ...
    if (dsResult.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "출고지시 정보가 없습니다."));
        _Cancel();
        return;
    }
    ...
    if (resultData.ALL_DISTRIBUTE_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "분배가 완료된 지시서입니다."));
    }
...
function onGetDetail(ajaxData) {
    ...
    if ($NC.isEmpty(resultData)) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "상품바코드 존재여부를 확인하지 못했습니다. 다시 스캔하십시오."));
    ...
    if (searchRow == -1) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "해당 출고차수에 존재하지 않는 상품입니다."));
        _Cancel($ND.C_CLEAR_TYPE_DETAIL);
        return;
    }
...
function onGetDistribute(ajaxData) {
    ...
    var dsResult = $NC.toArray(ajaxData);
    if (dsResult.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "분배가 완료된 상품입니다."));
        if ($NC.isVisible("#ctrDistributeOverlay")) {
    ...
    if (!$NC.isVisible("#ctrDistributeOverlay")) {
        $NC.showOverlay("#ctrDistributeOverlay", {
            title: $NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "분배작업"),
            fullscreen: true,
            onComplete: function() {
                $NC.onGlobalResize();
...
function onPaperScan($scan, scanVal) {
    ...
    // 데이터 조회
    $NC.serviceCall("/PDA_LOC02350E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster, onPaperError);
}
...
function onShelfScan($scan, scanVal) {
    ...
    if ($NC.G_VAR.distributeData.SHELF_NO != scanVal) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "현재 작업해야할 선반과 다른 선반을 스캔했습니다. 다시 스캔하십시오."));
        $NC.setValue($scan);
        setFocusScan($scan);
        return;
    }
...
function onSave(ajaxData) {
    ...
    var title = $NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "확인"), //
    message = $NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "정상 처리되었습니다."), //
        ...
        switch (onSaveAction) {
            ...
            case "N":
                ...
                // 다음 선반 조회
                    ...
                    $NC.serviceCall("/PDA_LOC02350E0/getDataSet.do", {
                        P_QUERY_ID: "PDA_LOC02350E0.RS_DETAIL",
                        P_QUERY_PARAMS: {
                            P_CENTER_CD: refRowData.CENTER_CD,
                            P_BU_CD: refRowData.BU_CD,
                            P_OUTBOUND_DATE: refRowData.OUTBOUND_DATE,
...
function btnDistributeCancelOnClick() {
    ...
    if (refRowData.DISTRIBUTE_QTY == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "분배되지 않은 상품입니다."));
        setFocusScan("#edtItem_Bar_Cd");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "분배취소 처리하시겠습니까?"))) {
        setFocusScan("#edtItem_Bar_Cd");
        return;
    }

    $NC.G_VAR.onSaveAction = "C";
    $NC.serviceCall("/PDA_LOC02350E0/callLOProcDistributeCancel.do", {
        P_CENTER_CD: refRowData.CENTER_CD,
        P_BU_CD: refRowData.BU_CD,
        P_OUTBOUND_DATE: refRowData.OUTBOUND_DATE,
        P_OUTBOUND_BATCH: refRowData.OUTBOUND_BATCH,
...
function btnDistributeOnClick() {
    ...
    if ($NC.isNull($NC.G_VAR.lastItemBarCd)) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02350E0.XXX", "분배할 상품을 스캔하십시오."));
        setFocusScan("#edtItem_Bar_Cd");
        return;
    }
