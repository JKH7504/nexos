...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.lastPaperNo)) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "지시서를 먼저 스캔하십시오."));
        setFocusScan();
        return;
    }

    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "처리할 상품 내역이 없습니다."));
        setFocusScan();
        return;
    }
    ...
    if (checkedData.checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "피킹작업 후 처리하십시오.\n(변경사항 없음)"));
        setFocusScan();
        return;
    }
    ...
    if (sumVals.ENTRY_QTY == sumVals.PICK_QTY) {
        if (!confirm($NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "피킹완료 처리하시겠습니까?"))) {
            setFocusScan();
            return;
        }
    } else {
        if (!confirm($NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "지시수량과 피킹수량이 동일하지 않습니다.\n피킹완료 처리하시겠습니까?"))) {
            setFocusScan();
            return;
        }
    }
    ...
    $NC.serviceCall("/PDA_LOC02310E0/callLOProcPick.do", {
...
function onContainerFocus(e) {
    ...
    if (!$view.is(":focus")) {
        // 지시서 스캔하여 정상 조회되어 있으면 로케이션에 포커스
        if ($NC.isNull($NC.G_VAR.lastPaperNo)) {
            setFocusScan();
        } else {
            setFocusScan("#edtLocation_Cd");
...
function setFocusScan(selector) {
    ...
    if ($NC.isNull(selector)) {
        if ($NC.isNull($NC.G_VAR.lastPaperNo)) {
            $view = $NC.getView("#edtPaper_No");
        } else {
            $view = $NC.getView("#edtLocation_Cd");
        }
    } else {
...
function grdMasterInitialize() {
    ...
    $NC.setInitGridObject("#grdMaster", {
        ...
        queryId: "PDA_LOC02310E0.RS_MASTER",
        sortCol: "ITEM_CD",
        gridOptions: options
    });
...
function grdMasterOnCellChange(e, args) {
    ...
    switch (args.col) {
        case "PICK_BOX":
            ...
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "피킹BOX 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
            pickQty = $NC.getBQty(changeQty, rowData.PICK_EA, rowData.QTY_IN_BOX);
            if (rowData.ENTRY_QTY < pickQty) {
                alert($NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "피킹수량이 지시수량을 초과할 수 없습니다."));
                $NC.setFocus(args.view);
                return;
            }
        ...
        case "PICK_EA":
            changeQty = $NC.toNumber(args.val, -1);
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "피킹EA 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
        ...
        case "PICK_QTY":
            changeQty = $NC.toNumber(args.val, -1);
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "피킹 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
...
function grdMasterOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.PICK_QTY)) {
            alert($NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "피킹 수량을 입력하십시오."));
...
function setDetailInputValue(rowData) {
    ...
    // 입력 활성/비활성
    $NC.setEnableGroup("#ctrDetailView", $NC.isNotNull($NC.G_VAR.lastLocationCd));
...
function onGetMaster(ajaxData) {
    ...
    if (dsResult.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "출고지시 정보가 없습니다."));
        _Cancel();
        return;
    }
    ...
    if (resultData.OUTBOUND_STATE != $ND.C_STATE_DIRECTIONS) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "출고지시 상태가 아닙니다. 확인 후 작업하십시오."));
        _Cancel();
        return;
    }
...
function onGetDetail() {
    ...
    if (searchRows.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02310E0.XXX", "피킹지시서에 존재하지 않는 로케이션입니다."));
    ...
    // 해당 로케이션 피킹 완료면, 메시지 표시 후 첫번째 상품 선택
    if (targetRow == -1) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02310E0.XXX", "해당 로케이션은 피킹이 완료되었습니다."));
...
function onGetItem(ajaxData) {
    ...
    if ($NC.isEmpty(resultData)) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02310E0.XXX", "상품바코드 존재여부를 확인하지 못했습니다. 다시 스캔하십시오."));
        _Cancel($ND.C_CLEAR_TYPE_DETAIL);
        return;
    }
    ...
    if (searchRows.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02310E0.XXX", "해당 로케이션에 존재하지 않는 상품입니다."));
        _Cancel($ND.C_CLEAR_TYPE_DETAIL);
        return;
    }

    // 피킹 안한 상품 검색
    var rIndex, rCount, targetRow = -1;
    for (rIndex = 0, rCount = searchRows.length; rIndex < rCount; rIndex++) {
        rowData = G_GRDMASTER.data.getItem(searchRows[rIndex]);
        if (rowData.PICK_YN == $ND.C_NO) {
            targetRow = searchRows[rIndex];
    ...
    // 해당 로케이션 피킹 완료면, 메시지 표시 후 첫번째 상품 선택
    if (targetRow == -1) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02310E0.XXX", "해당 상품은 피킹이 완료되었습니다."));
        targetRow = searchRows[0];
    }
...
function onPaperScan($scan, scanVal) {
    ...
    // 데이터 조회
    $NC.serviceCall("/PDA_LOC02310E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster, onPaperError);
}
...
function onSave(ajaxData) {
    ...
    var title = $NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "확인"), //
    message = $NC.getDisplayMsg("JS.PDA_LOC02310E0.XXX", "정상 처리되었습니다."), //
    buttons = {};
...
function onQtyApply() {
    ...
    // 피킹수량 초기값 지시수량과 동일, 수량이 동일하고 피킹했을 경우는 처리 안함
