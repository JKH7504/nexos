...
function _OnInputChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BILL_EA_QTY":
            ...
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LOC02380E0.001", "정산수량을 정확히 입력하십시오."));
                $NC.setFocus(view);
                return;
            }
...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/PDA_LOC02380E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster, onError);
}
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02380E0.002", "처리할 내역이 없습니다."));
        setFocusScan();
        return;
    }
    ...
    if (searchRow == -1 && checkedData.checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02380E0.017", "수정한 작업비 내역이 없습니다."));
        $NC.setFocus("#edtBill_Plt_Qty");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.PDA_LOC02380E0.005", "처리하시겠습니까?"))) {
        setFocusScan();
        return;
    }

    $NC.serviceCall("/PDA_LOC02380E0/callLOProcExtraWorkDiv.do", {
...
function grdMasterInitialize() {
    ...
    $NC.setInitGridObject("#grdMaster", {
        ...
        queryId: "PDA_LOC02380E0.RS_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
...
function grdMasterOnAfterScroll(e, args) {
    ...
    // lastRow 존재 시, CRUD 값 체크 및 DETAIL 그리드 수정여부 체크
    if (lastRowData.CRUD == $ND.C_DV_CRUD_U || $NC.isGridModified(G_GRDDETAIL)) {
        // 수정 ROW는 저장 후 처리되도록 return처리
        alert($NC.getDisplayMsg("JS.PDA_LOC02380E0.017", "입력한 작업수량을 저장 후 진행하십시오."));
        $NC.setFocusGrid(G_GRDMASTER, G_GRDMASTER.lastRow, null, false, true);
        return;
    }
    ...
    // 데이터 조회
    $NC.serviceCall("/PDA_LOC02380E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
...
function grdDetailOnCellChange(e, args) {
    ...
    switch (args.col) {
        case "EXTRA_WORK_QTY":
            ...
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LOC02380E0.001", "추가작업비 수량을 정확히 입력하십시오."));
...
function grdDetailInitialize() {
    ...
    $NC.setInitGridObject("#grdDetail", {
        ...
        queryId: "PDA_LOC02380E0.RS_DETAIL",
...
function grdDetailOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.EXTRA_WORK_QTY)) {
            alert($NC.getDisplayMsg("JS.PDA_LOC02380E0.006", "작업 수량을 입력하십시오."));
            $NC.setGridSelectRow(G_GRDDETAIL, row);
            setFocusScan("#edtExtra_Work_Qty");
            return false;
        }
        if (Number(rowData.EXTRA_WORK_QTY) < 0) {
            alert($NC.getDisplayMsg("JS.PDA_LOC02380E0.007", "작업 수량을 정확히 입력하십시오."));
            $NC.setGridSelectRow(G_GRDDETAIL, row);
            setFocusScan("#edtExtra_Work_Qty");
            return false;
        }
...
function onGetMaster(ajaxData) {
    ...
    if (dsResult.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LOC02380E0.008", "출고내역이 존재하지 않습니다."));
        _Cancel();
        return;
    }
    ...
    $NC.setInitGridAfterOpen(G_GRDMASTER, "OUTBOUND_NO", true);
