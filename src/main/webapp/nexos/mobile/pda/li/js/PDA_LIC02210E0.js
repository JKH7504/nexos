 ...
 * <pre>
 *  ==================================================================================================================================================
 *  프로그램ID         : PDA_LIC02210E0
 *  프로그램명         : PDA 파렛트매핑
 *  프로그램설명       : PDA 파렛트매핑 화면 Javascript
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.lastPalletId)) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "파렛트ID를 먼저 스캔하십시오."));
        setFocusScan();
        return;
    }

    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "처리할 상품 내역이 없습니다."));
        setFocusScan();
        return;
    }
    ...
    if (searchRows.length > 0) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "유통기한필수대상 상품입니다. 유통기한을 입력하십시오."));
    ...
    if (searchRows.length > 0) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "제조번호필수대상 상품입니다. 제조번호를 입력하십시오."));
        $NC.setGridSelectRow(G_GRDMASTER, searchRows[0]);
        // Scroll에서 데이터 표시 후 포커스 이동하도록 딜레이 처리
        setTimeout(function() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "매핑 처리하시겠습니까?"))) {
        setFocusScan();
        return;
    }

    var refRowData = G_GRDMASTER.data.getItem(0);
    $NC.serviceCall("/PDA_LIC02210E0/callLIProcMapping.do", {
...
function grdMasterInitialize() {
    ...
    $NC.setInitGridObject("#grdMaster", {
        ...
        queryId: "PDA_LIC02210E0.RS_MASTER",
        sortCol: "ITEM_CD",
        gridOptions: options
    });
    ...
    switch (args.col) {
        case "VALID_DATE":
            if ($NC.isNotNull(args.val)) {
                $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.PDA_LIC02210E0.009", "유통기한을 정확히 입력하십시오."), "N");
            }
        ...
        case "CONFIRM_BOX":
            ...
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "검수BOX 수량을 정확히 입력하십시오."));
        ...
        case "CONFIRM_EA":
            changeQty = $NC.toNumber(args.val, -1);
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "검수EA 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
        ...
        case "CONFIRM_QTY":
            changeQty = $NC.toNumber(args.val, -1);
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "검수 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
...
function grdMasterOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONFIRM_QTY)) {
            alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "검수 수량을 입력하십시오."));
...
function setMasterInputValue(rowData) {
    ...
    // Row 데이터로 에디터 세팅
    $NC.G_VAR.lastInboundNo = rowData["INBOUND_NO"];
...
function onGetMaster(ajaxData) {
    ...
    if (dsResult.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "파렛트ID 정보가 없습니다."));
        _Cancel();
        return;
    }
    // 기본 체크, 첫번째 ROW 데이터로
    var resultData = dsResult[0];
    if (resultData.INBOUND_STATE != $ND.C_STATE_ENTRY) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "입고진행상태: [" + resultData.INBOUND_STATE + "] 파렛트매핑 가능한 상태가 아닙니다.", resultData.INBOUND_STATE));
        _Cancel();
        return;
    }
    if (resultData.CENTER_CD != $NC.G_USERINFO.CENTER_CD) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "물류센터: [" + resultData.CENTER_CD + "] 설정된 물류센터와 다른 물류센터의 파렛트ID입니다.", resultData.BU_CD));
        _Cancel();
        return;
    }
    if (resultData.BU_CD != $NC.G_USERINFO.BU_CD) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "사업부: [" + resultData.BU_CD + "] 설정된 사업부와 다른 사업부의 파렛트ID입니다.", resultData.BU_CD));
        _Cancel();
        return;
    }
    if (resultData.INBOUND_DATE != $NC.G_USERINFO.WORK_DATE) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "입고일자: [" + resultData.INBOUND_DATE + "] 설정된 작업일자와 다른 입고일자의 파렛트ID입니다.",
    ...
    // 상품이 있을 경우, 매핑한 파렛트ID일 경우 데이터 표시
    if ($NC.isNotNull(resultData.ITEM_CD)) {
        // 상품정보 활성
...
function onGetDetail(ajaxData) {
    ...
    var dsResult = $NC.toArray(ajaxData);
    if (dsResult.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "상품바코드 존재여부를 확인하지 못했습니다. 다시 스캔하십시오."));
    ...
    if (dsResult.length == 1) {
        ...
        if (searchRow == -1) {
            ...
            rowData.PALLET_ID = $NC.G_VAR.lastPalletId;
        ...
        for (rIndex = 0, rCount = dsResult.length; rIndex < rCount; rIndex++) {
            ...
            if (searchRow == -1) {
                ...
                rowData.CONFIRM_BOX = 0;
                rowData.CONFIRM_EA = 0;
                rowData.CRUD = $ND.C_DV_CRUD_C;
                rowData.id = $NC.getGridNewRowId();

                // 스크롤 무시, 아래에서 첫번째 ROW 선택
                G_GRDMASTER.lastRow = G_GRDMASTER.data.getLength();
                $NC.newGridRowData(G_GRDMASTER, rowData);
...
function onPalletScan($scan, scanVal) {
    ...
    // 데이터 조회
    $NC.serviceCall("/PDA_LIC02210E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster, onPalletError);
}
...
function onItemScan($scan, scanVal) {
    ...
    $NC.serviceCall("/PDA_LIC02210E0/getDataSet.do", {
        P_QUERY_ID: "PDA_LIC02210E0.RS_DETAIL",
        P_QUERY_PARAMS: {
            P_CENTER_CD: $NC.G_USERINFO.CENTER_CD,
            P_BU_CD: $NC.G_USERINFO.BU_CD,
            P_INBOUND_DATE: $NC.G_USERINFO.WORK_DATE,
            P_INBOUND_NO: $NC.G_VAR.lastInboundNo,
...
function onSave(ajaxData) {
    ...
    var title = $NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "확인"), //
    message = $NC.getDisplayMsg("JS.PDA_LIC02210E0.XXX", "정상 처리되었습니다."), //
    buttons = {};
...
function setProcessInfo() {
    ...
    $NC.serviceCall("/PDA_LIC02210E0/getDataSet.do", {
        ...
        // 프로세스 입고지시 수행여부가 N이면 프로그램 종료
        var dsResult = $NC.toArray(ajaxData);
        if (dsResult[2].EXEC_PROCESS_YN == $ND.C_NO) {
