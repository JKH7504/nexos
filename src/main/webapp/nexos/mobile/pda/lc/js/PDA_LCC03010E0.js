...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.lastLocationCd)) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "로케이션을 먼저 스캔하십시오."));
        setFocusScan();
        return;
    }

    if ($NC.isNull($NC.G_VAR.lastMoveLocationCd)) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "이동로케이션을 먼저 스캔하십시오."));
        setFocusScan("#edtMove_Location_Cd");
        return;
    }

    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "처리할 상품 내역이 없습니다."));
        return;
    }
    ...
    if (checkedRows.length > 0) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "라벨링상태의 상품은 부분이동이 불가능합니다."));
        return;
    }
    ...
    if (checkedData.checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "처리할 상품 내역이 없습니다. 재고이동할 상품을 선택하고 이동수량을 입력하십시오."));
        setFocusScan();
        setFocusScan();
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "재고이동 처리하시겠습니까?"))) {
        setFocusScan();
        setFocusScan();
        return;
    }
    ...
    $NC.serviceCall("/PDA_LCC03010E0/callLCProcMove.do", {
...
function grdMasterInitialize() {
    ...
    $NC.setInitGridObject("#grdMaster", {
        ...
        queryId: "PDA_LCC03010E0.RS_MASTER",
        sortCol: "ITEM_CD",
        gridOptions: options
    });
...
function grdMasterOnCellChange(e, args) {
    ...
    switch (args.col) {
        case "CONFIRM_BOX":
            ...
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "이동BOX 수량을 정확히 입력하십시오."));
            ...
            if (rowData.STOCK_QTY < confirmQty) {
                alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "이동수량이 재고수량을 초과할 수 없습니다."));
                $NC.setFocus(args.view);
                return;
            }
        ...
        case "CONFIRM_EA":
            changeQty = $NC.toNumber(args.val, -1);
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "이동EA 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
        ...
        case "CONFIRM_QTY":
            changeQty = $NC.toNumber(args.val, -1);
            if (changeQty < 0) {
                alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "이동 수량을 정확히 입력하십시오."));
                $NC.setFocus(args.view);
                return;
            }
...
function grdMasterOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONFIRM_QTY)) {
            alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "검수 수량을 입력하십시오."));
            $NC.setGridSelectRow(G_GRDMASTER, row);
            setFocusScan("#edtConfirm_Qty");
            return false;
        }
        if (Number(rowData.CONFIRM_QTY) < 0) {
            alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "검수 수량을 정확히 입력하십시오."));
            $NC.setGridSelectRow(G_GRDMASTER, row);
            setFocusScan("#edtConfirm_Qty");
            return false;
        }
...
function onGetMaster(ajaxData) {
    ...
    if (dsResult.length == 0) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "해당 로케이션에 재고가 존재하지 않습니다."));
        _Cancel();
        return;
    }
...
function onGetDetail(ajaxData) {
    ...
    if ($NC.isEmpty(resultData)) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "상품바코드 존재여부를 확인하지 못했습니다. 다시 스캔하십시오."));
    ...
    if (searchRow == -1) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "로케이션에 존재하지 않는 상품입니다."));
        _Cancel($ND.C_CLEAR_TYPE_DETAIL);
        return;
    }
...
function onGetSub(ajaxData) {
    ...
    // 리턴 값이 없으면 다시
    if ($NC.isEmpty(resultData)) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "로케이션 존재여부를 확인하지 못했습니다. 다시 스캔하십시오."));
...
function onLocationScan($scan, scanVal) {
    ...
    // 데이터 조회
    $NC.serviceCall("/PDA_LCC03010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster, onLocationError);
}
...
function onMoveLocationScan($scan, scanVal) {
    ...
    if (scanVal == $NC.getValue("#edtLocation_Cd")) {
        alert($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "이동로케이션이 기존 로케이션과 동일합니다."));
        $NC.setValue("#edtMove_Location_Cd");
        setFocusScan($scan);
        return;
    }
...
function onSave(ajaxData) {
    ...
    var title = $NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "확인"), //
    message = $NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "정상 처리되었습니다."), //
    buttons = {};
...
function btnAllOnClick() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.PDA_LCC03010E0.XXX", "전체 상품의 이동수량을 입력 처리하시겠습니까? "))) {
        return;
    }
