...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            return;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.ROC06020E0.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.ROC06020E0.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
        case "VENDOR_CD":
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.ROC06020E0.003", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.ROC06020E0.004", "사업부를 입력하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE1)) {
        alert($NC.getDisplayMsg("JS.ROC06020E0.005", "검색 시작일자를 입력하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE2)) {
        alert($NC.getDisplayMsg("JS.ROC06020E0.006", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQOrder_Date2");
        return;
    }

    if (ORDER_DATE1 > ORDER_DATE2) {
        alert($NC.getDisplayMsg("JS.ROC06020E0.007", "출고예정일자 범위 입력오류입니다."));
        $NC.setFocus("#dtpQOrder_Date1");
        return;
    }
...
function onProc(ajaxData) {
    ...
    _Inquiry();
    alert($NC.getDisplayMsg("JS.ROC06020E0.008", "변경이 완료되었습니다."));
}

function btnChangeCenterCdOnClick() {

    if (!$NC.getProgramPermission().canSave) {
        alert($NC.getDisplayMsg("JS.ROC06020E0.009", "해당 프로그램의 저장권한이 없습니다."));
        return;
    }

    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.ROC06020E0.010", "조회 후 처리하십시오."));
        return;
    }

    var CENTER_CD_NEW = $NC.getValue("#cboCenter_Cd");
    if ($NC.isNull(CENTER_CD_NEW)) {
        alert($NC.getDisplayMsg("JS.ROC06020E0.011", "변경물류센터를 선택하십시오."));
    ...
    if (CENTER_CD == CENTER_CD_NEW) {
        alert($NC.getDisplayMsg("JS.ROC06020E0.012", "동일한 물류센터로 변경할 수 없습니다."));
        $NC.setFocus("#cboCenter_Cd");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.ROC06020E0.013", "선택한 물류센터로 변경 하시겠습니까?"))) {
        return;
    }
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.ROC06020E0.014", "물류센터변경 처리할 데이터를 선택하십시오."));
        return;
    }
