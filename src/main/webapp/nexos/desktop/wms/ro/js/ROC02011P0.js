...
function _Initialize() {
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.ROC02011P0.001", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.ROC02011P0.002", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.ROC02011P0.003", "저장"));
}
        ...
        try {
            for (rIndex = 0, rCount = dsDetail.length; rIndex < rCount; rIndex++) {
                ...
                newRowData = {
                    ...
                    BU_DATE: dsMaster.BU_DATE, // 예정에서는 마스터 항목
                    BU_NO: dsMaster.BU_NO, // 예정에서는 마스터 항목
...
function _New() {
    ...
    if ($NC.isNull(VENDOR_CD)) {
        alert($NC.getDisplayMsg("JS.ROC02011P0.004", "먼저 공급처 코드를 입력하십시오."));
        $NC.setFocus("#edtVendor_Cd");
        return;
    }
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.ROC02011P0.005", "물류센터가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.ROC02011P0.006", "사업부가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.OUTBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.ROC02011P0.007", "먼저 반출일자를 입력하십시오."));
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.ROC02011P0.008", "저장할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.ROC02011P0.009", "저장하시겠습니까?"))) {
        return;
    }
    ...
    for (rIndex = 0, rCount = G_GRDDETAIL.data.getLength(); rIndex < rCount; rIndex++) {
        ...
        if (rowData.SHORTAGE_YN == $ND.C_YES) {
            alert($NC.getDisplayMsg("JS.ROC02011P0.010", "출고가능량을 초과한 상품이 존재합니다\n조정 후 저장하십시오."));
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.ROC02011P0.011", "데이터 수정 후 저장하십시오."));
        return;
    }
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.ROC02011P0.012", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        ...
        case "CAR_NO":
            ...
            break;
        case "OUTBOUND_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.ROC02011P0.013", "반출일자를 정확히 입력하십시오."));
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        ...
        case "ENTRY_QTY":
            ...
            if (ENTRY_QTY < CHECK_QTY) {
                alert($NC.getDisplayMsg("JS.ROC02011P0.014", "등록수량이 " + CHECK_QTY + "보다 작을 수 없습니다.", CHECK_QTY));
            ...
            if ($NC.G_VAR.ORDER_YN == $ND.C_YES && $NC.G_VAR.G_PARAMETER.P_POLICY_RO220 != "1") {
                if (Number(rowData.ORDER_QTY) < ENTRY_QTY) {
                    alert($NC.getDisplayMsg("JS.ROC02011P0.015", "등록수량이 예정수량을 초과할 수 없습니다."));
                    rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                    break;
                }
        ...
        case "ENTRY_BOX":
            ...
            if (ENTRY_BOX < 0) {
                alert($NC.getDisplayMsg("JS.ROC02011P0.016", "등록BOX는 0보다 작을 수 없습니다.", CHECK_QTY));
        ...
        case "ENTRY_EA":
            ...
            if (ENTRY_EA < 0) {
                alert($NC.getDisplayMsg("JS.ROC02011P0.017", "등록EA는 0보다 작을 수 없습니다.", CHECK_QTY));
                rowData.ENTRY_EA = args.oldItem.ENTRY_EA;
                $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                break;
            }
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.ROC02011P0.018", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                } else {
...
function grdDetailOnCheckPStock(row, isCheckOnly) {
    ...
    $NC.serviceCallAndWait("/ROC02010E0/getData.do", {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.ROC02011P0.024", "출고가능량 정보를 가져오지 못했습니다."));
            return;
        }
        var oMsg = $NC.getOutMessage(resultData);
        if (oMsg != $ND.C_OK) {
...
function showOutwaitOverlay(e) {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.ROC02011P0.025", "출고대기 내역을 확인할 상품을 선택하십시오."));
        return;
    }
    ...
    if (G_GRDOUTWAIT.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.ROC02011P0.026", "출고대기 내역이 존재하지 않습니다."));
        return;
    }

    $NC.showOverlay("#divOutwaitOverlay", {
        title: $NC.getDisplayMsg("JS.ROC02011P0.029", "출고대기 내역"),
        fullscreen: false,
        width: 700,
        height: 300,
        onComplete: function() {
...
function btnStockAddItemOnClick() {
    ...
    for (rIndex = 0, rCount = G_GRDSTOCK.data.getLength(); rIndex < rCount; rIndex++) {
        ...
        if (Number(rowData.INPUT_QTY) <= 0) {
            alert($NC.getDisplayMsg("JS.ROC02011P0.027", "등록수량이 0보다 작을 수 없습니다."));
    ...
    if (dsStock.length == 0) {
        alert($NC.getDisplayMsg("JS.ROC02011P0.028", "출고등록할 재고를 선택하십시오."));
        return;
    }
