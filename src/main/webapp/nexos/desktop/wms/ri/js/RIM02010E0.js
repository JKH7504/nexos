...
function _Initialize() {
    ...
    // 취소/처리 버튼 툴팁 세팅
    $NC.setTooltip("#btnProcessPreB", $NC.getDisplayMsg("JS.RIM02010E0.001", $NC.getValue("#btnProcessB") + " 취소", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessNxtB", $NC.getDisplayMsg("JS.RIM02010E0.002", $NC.getValue("#btnProcessB") + " 처리", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessPreC", $NC.getDisplayMsg("JS.RIM02010E0.001", $NC.getValue("#btnProcessC") + " 취소", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessNxtC", $NC.getDisplayMsg("JS.RIM02010E0.002", $NC.getValue("#btnProcessC") + " 처리", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessPreD", $NC.getDisplayMsg("JS.RIM02010E0.001", $NC.getValue("#btnProcessD") + " 취소", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessNxtD", $NC.getDisplayMsg("JS.RIM02010E0.002", $NC.getValue("#btnProcessD") + " 처리", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessPreE", $NC.getDisplayMsg("JS.RIM02010E0.001", $NC.getValue("#btnProcessE") + " 취소", $NC.getValue("#btnProcessE")));
    $NC.setTooltip("#btnProcessNxtE", $NC.getDisplayMsg("JS.RIM02010E0.002", $NC.getValue("#btnProcessE") + " 처리", $NC.getValue("#btnProcessE")));

    $NC.setTooltip("#btnNewPalletIdC", $NC.getDisplayMsg("JS.RIM02010E0.003", "신규"));
    $NC.setTooltip("#btnNewDirectionsC", $NC.getDisplayMsg("JS.RIM02010E0.004", "추가"));
    $NC.setTooltip("#btnDeleteDirectionsC", $NC.getDisplayMsg("JS.RIM02010E0.005", "삭제"));
}
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "DELIVERY_CD":
            ...
            return;
        case "INBOUND_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIM02010E0.006", "반입일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIM02010E0.007", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIM02010E0.008", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.009", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.010", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var INBOUND_DATE = $NC.getValue("#dtpQInbound_Date");
    if ($NC.isNull(INBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.011", "반입일자를 입력하십시오."));
        $NC.setFocus("#dtpQInbound_Date");
        return;
    }
    var ORDER_DATE1 = $NC.getValue("#dtpQOrder_Date1");
    if ($NC.isNull(ORDER_DATE1)) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.012", "반입예정 검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQOrder_Date1");
        return;
    }
    var ORDER_DATE2 = $NC.getValue("#dtpQOrder_Date2");
    if ($NC.isNull(ORDER_DATE2)) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.013", "반입예정 검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQOrder_Date2");
        return;
    }
    if (ORDER_DATE1 > ORDER_DATE2) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.014", "반입예정일자 범위 입력오류입니다."));
        $NC.setFocus("#dtpQOrder_Date1");
        return;
    }
    var INOUT_CD = $NC.getValue("#cboQInout_Cd");
    if ($NC.isNull(INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.015", "반입구분을 선택하십시오."));
    ...
    if (STATE_PRE_YN == $ND.C_NO && STATE_CUR_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.016", "검색구분을 선택하십시오."));
...
function _Print(reportInfo) {
    ...
    switch ($NC.G_VAR.activeView.PROCESS_CD) {
        ...
        case $ND.C_PROCESS_PUTAWAY:
            ...
            break;
        default:
            alert($NC.getDisplayMsg("JS.RIM02010E0.017", "해당 프로세스는 출력물이 없습니다."));
            return;
    }
    ...
    switch (reportInfo.REPORT_CD) {
        ...
        case "LABEL_CMC03":
            ...
            if ($NC.G_VAR.policyVal.RI230 == "2" || $NC.G_VAR.policyVal.RI230 == "3" || $NC.G_VAR.policyVal.RI230 == "4") {
                alert($NC.getDisplayMsg("JS.RIM02010E0.018", "[" + reportInfo.REPORT_NM + "]파렛트ID 사용 기준 정책에 의해 해당 라벨은 사용할 수 없습니다.",
...
function onGetProcessInfo(ajaxData) {
    ...
    // alert($NC.getDisplayMsg("JS.RIM02010E0.019", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록하십시오."));
    $NC.G_VAR.activeView.PROCESS_CD = "";
    $NC.showMessage({
        message: $NC.getDisplayMsg("JS.RIM02010E0.019", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록하십시오."),
        width: 400,
...
function setMasterSummaryInfo() {
    ...
    $NC.serviceCall("/RIM02010E0/getDataSet.do", {
        P_QUERY_ID: "RIM02010E0.RS_MASTER",
        P_QUERY_PARAMS: {
            P_CENTER_CD: CENTER_CD,
            P_BU_CD: BU_CD,
            P_INBOUND_DATE: INBOUND_DATE,
