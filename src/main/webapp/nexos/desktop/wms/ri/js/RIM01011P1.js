 ...
 *  ==================================================================================================================================================
 *  프로그램ID         : RIM01011P1
 *  프로그램명         : 온라인반입예정등록 (의류) 팝업
 *  프로그램설명       : 온라인반입예정등록 (의류) 팝업 화면 Javascript
...
function _Initialize() {
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.RIM01011P1.001", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.RIM01011P1.002", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.RIM01011P1.003", "저장"));
}
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.RIM01011P1.005", "물류센터가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.RIM01011P1.006", "사업부가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDER_DATE)) {
        alert($NC.getDisplayMsg("JS.RIM01011P1.007", "먼저 예정일자를 입력하십시오."));
        $NC.setFocus("#dtpOrder_Date");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.RIM01011P1.008", "먼저 반입구분을 선택하십시오."));
        $NC.setFocus("#cboInout_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.DELIVERY_CD)) {
        alert($NC.getDisplayMsg("JS.RIM01011P1.009", "먼저 온라인몰 코드를 입력하십시오."));
        $NC.setFocus("#edtDelivery_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_NM)) {
        alert($NC.getDisplayMsg("JS.RIM01011P1.010", "먼저 주문자명을 입력하십시오."));
        $NC.setFocus("#edtOrderer_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_HP)) {
        alert($NC.getDisplayMsg("JS.RIM01011P1.011", "먼저 주문자 휴대폰번호를 입력하십시오."));
        $NC.setFocus("#edtOrderer_Hp");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_NM)) {
        alert($NC.getDisplayMsg("JS.RIM01011P1.012", "먼저 수령자명을 입력하십시오."));
        $NC.setFocus("#edtShipper_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_HP)) {
        alert($NC.getDisplayMsg("JS.RIM01011P1.013", "먼저 수령자 휴대폰번호를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_ZIP_CD) || $NC.isNull($NC.G_VAR.masterData.SHIPPER_ADDR_BASIC)) {
        alert($NC.getDisplayMsg("JS.RIM01011P1.014", "먼저 수령자 주소를 입력하십시오."));
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.RIM01011P1.004", "저장할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.RIM01011P1.015", "저장하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.RIM01011P1.016", "데이터 수정 후 저장하십시오."));
        return;
    }
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.RIM01011P1.017", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        ...
        case "INOUT_CD":
            ...
            break;
        case "ORDER_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.RIM01011P1.018", "예정일자를 정확히 입력하십시오."));
        ...
        case "PLANED_DATETIME":
            if ($NC.isNotNull(args.val)) {
                $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.RIM01011P1.019", "도착예정일을 정확히 입력하십시오."));
            }
            setPlanedDatetime();
            break;
        case "AMPM":
...
function grdDetailOnCellChange(e, args) {
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        ...
        case "ORDER_QTY":
            if (Number(rowData.ORDER_QTY) < 1) {
                alert($NC.getDisplayMsg("JS.RIM01011P1.020", "예정수량은 1보다 작을 수 없습니다."));
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.RIM01011P1.021", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                } else {
