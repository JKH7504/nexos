...
function _NewB() {
    ...
    if ($NC.G_VAR.policyVal.RI210 != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.RIB02010E1.024", "반입등록 신규처리가 불가능한 사업부입니다."));
        return;
    }
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "RIB02011P1",
        PROGRAM_NM: "반입등록/수정",
        url: "ri/RIB02011P1.html",
        width: 1024,
        height: 600,
        G_PARAMETER: {
...
function onProcessPreB() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.RIB02010E1.027", "반입등록 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    for (var rIndex = 0; rIndex < rCount; rIndex++) {
        ...
        if (rowData.INOUT_SUB_CD == $ND.C_INOUT_GRP_E8) {
            alert($NC.getDisplayMsg("JS.RIB02010E1.028", "반입작업화면에서 미배송입고 전표에 대하여 취소 처리할 수 없습니다.\n미배송입고전표를 취소하시려면 배송완료화면에서 해당 전표에 대하여 취소하십시오."));
            return;
        }
        checkedCount++;
        // 반입등록 상태인 전표만 대상
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E1.029", "반입등록 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E1.030", "선택한 데이터 중 반입등록 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtB() {
    ...
    if ($NC.isNull(INBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.RIB02010E1.031", "입고일자를 먼저 입력하십시오."));
        $NC.setFocus("#dtpInbound_DateB");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.RIB02010E1.032", "반입등록 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E1.033", "반입등록 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E1.034", "선택한 데이터 중 반입등록 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterBInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterB", {
        ...
        queryId: "RIB02010E1.RS_T1_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options,
        canDblClick: true
    });
...
function grdMasterBOnDblClick(e, args) {
    ...
    if (refRowData.INOUT_SUB_CD == $ND.C_INOUT_GRP_E8) {
        alert($NC.getDisplayMsg("JS.RIB02010E1.035", "반입작업화면에서 미배송입고 전표에 대하여 수정 처리할 수 없습니다.\n미배송입고전표를 취소하시려면 배송완료화면에서 해당 전표에 대하여 취소하십시오."));
        return;
    }
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.RIB02010E1.036", "반입진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        ...
        if (refRowData.INBOUND_STATE != resultData.O_INBOUND_STATE) {
            alert($NC.getDisplayMsg("JS.RIB02010E1.037", "[진행상태 : " + resultData.O_INBOUND_STATE + "] 데이터가 변경되었습니다.\n다시 조회 후 데이터를 확인하십시오.",
        ...
        if (dsDetail.length == 0) {
            alert($NC.getDisplayMsg("JS.RIB02010E1.038", "등록/수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
        ...
        $NC.showProgramSubPopup({
            PROGRAM_ID: "RIB02011P1",
            PROGRAM_NM: $NC.getDisplayMsg("JS.RIB02010E1.039", "반입등록/수정"),
            url: "ri/RIB02011P1.html",
            width: 1024,
            height: 600,
            G_PARAMETER: {
...
function grdDetailBInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailB", {
        ...
        queryId: "RIB02010E1.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
