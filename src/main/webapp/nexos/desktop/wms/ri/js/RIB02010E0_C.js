...
function _InquiryC(args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIB02010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTERC), onGetMasterC);
}
...
function _SaveC() {
    ...
    if (G_GRDMASTERC.data.getLength() == 0 || $NC.isNull(G_GRDMASTERC.lastRow) || G_GRDSUBC.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.037", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if ($NC.G_VAR.policyVal.LS220 == "1") {
        ...
        if (dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.RIB02010E0.038", "데이터 수정 후 저장하십시오."));
            return;
        }
        ...
        $NC.serviceCall("/RIB02010E0/saveDirectionsPltId.do", {
            P_DS_MASTER: {
...
function onProcessPreC() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.RIB02010E0.039", "반입지시 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.040", "반입지시 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.041", "선택한 데이터 중 반입지시 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtC() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.RIB02010E0.042", "반입지시 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBC)) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.043", "데이터가 수정되었습니다. 저장 후 반입지시 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.044", "반입지시 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.045", "선택한 데이터 중 반입지시 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function btnNewPalletIdCOnClick() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.RIB02010E0.046", "새 파렛트ID를 생성하시겠습니까?"))) {
        return;
    }
    ...
    $NC.serviceCall("/RIB02010E0/callCMPalletIdGetNo.do", {
        P_CENTER_CD: refRowData.CENTER_CD,
        P_BU_CD: refRowData.BU_CD,
        P_INBOUND_DATE: refRowData.INBOUND_DATE,
        P_INBOUND_NO: refRowData.INBOUND_NO,
...
function onGetNewPalletIdC(ajaxData) {
    ...
    $NC.serviceCall("/RIB02010E0/getDataSet.do", {
        P_QUERY_ID: "RIB02010E0.RS_T2_SUB1",
        P_QUERY_PARAMS: {
            P_CENTER_CD: rowData.CENTER_CD,
            P_BU_CD: rowData.BU_CD,
            P_INBOUND_DATE: rowData.INBOUND_DATE,
...
function btnNewDirectionsCOnClick() {
    ...
    if (G_GRDPALLETIDC.data.getLength() == 0 || $NC.isNull(G_GRDPALLETIDC.lastRow)) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.047", "신규 파렛트ID를 먼저 생성 후 처리하십시오."));
        return;
    }
    ...
    if (rowData.ENTRY_QTY == rowData.DIRECTIONS_QTY) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.048", "추가하려는 상품은 이미 등록수량만큼 지시 처리되었습니다."));
        return;
    }
    ...
    if (searchIndex != -1) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.049", "추가하려는 상품은 해당 파렛트ID에 이미 추가되어 있습니다."));
    ...
    // [RI230]파렛트ID 매핑 기준 : 3 - 공급처별 자동 매핑(멀티상품)
    // [RI230]파렛트ID 매핑 기준 : 4 - 파렛트ID 하나로 자동 매핑(멀티상품)
    if ($NC.G_VAR.policyVal.RI230 == "1" && G_GRDSUBC.data.getLength() > 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.077", "[파렛트ID 매핑 기준]파렛트ID 하나에 여러 상품을 추가할 수 없습니다."));
        return;
    }
...
function btnDeleteDirectionsCOnClick() {
    ...
    if (G_GRDSUBC.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.RIB02010E0.050", "삭제하려는 데이터를 선택하십시오."));
        return;
    }
...
function grdMasterCInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterC", {
        ...
        queryId: "RIB02010E0.RS_T2_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options
    });
...
function grdMasterCOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIB02010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAILC), onGetDetailC);
    ...
    // 데이터 조회
    $NC.serviceCall("/RIB02010E0/getDataSet.do", $NC.getGridParams(G_GRDPALLETIDC), onGetPalletIdC);
    ...
    // 데이터 조회
    $NC.serviceCall("/RIB02010E0/getDataSet.do", $NC.getGridParams(G_GRDSUBC), onGetSubC);
...
function grdDetailCInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailC", {
        ...
        queryId: "RIB02010E0.RS_T2_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdSubCOnSetColumns() {
    ...
    $NC.setGridColumnOptions(G_GRDSUBC, {
        ENTRY_QTY: {
            ...
            editorOptions: $NC.G_VAR.policyVal.LS220 == "2" ? $NC.getGridNumberColumnOptions({
    ...
    $NC.setGridColumns(G_GRDSUBC, [ // 숨김컬럼 세팅
        $NC.G_VAR.policyVal.LS220 == "1" ? "PALLET_ID,LINE_NO,ITEM_CD,ITEM_BAR_CD,ITEM_NM,ITEM_SPEC,BRAND_NM,ITEM_STATE_F,ITEM_LOT" : "",
        $NC.G_VAR.policyVal.LS210 != "2" ? "VALID_DATE,BATCH_NO" : ""
    ]);
}
...
function grdSubCOnCellChange(e, args) {
    ...
    switch (G_GRDSUBC.view.getColumnId(args.cell)) {
        case "ENTRY_QTY":
            ...
            if (ENTRY_QTY < 0) {
                alert($NC.getDisplayMsg("JS.RIB02010E0.051", "지시수량이 0보다 작을 수 없습니다."));
            ...
            if (searchIndex == -1) {
                alert($NC.getDisplayMsg("JS.RIB02010E0.052", "상세정보에서 해당 상품을 검색하지 못해 지시수량을 반영하지 못했습니다."));
                rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                rowData.ENTRY_BOX = args.oldItem.ENTRY_BOX;
                rowData.ENTRY_EA = args.oldItem.ENTRY_EA;
                rowData.ENTRY_WEIGHT = args.oldItem.ENTRY_WEIGHT;
            ...
            if (D_ENTRY_QTY < S0_ENTRY_QTY) {
                alert($NC.getDisplayMsg("JS.RIB02010E0.053", "지시수량이 등록수량보다 클 수 없습니다."));
                rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                rowData.ENTRY_BOX = args.oldItem.ENTRY_BOX;
                rowData.ENTRY_EA = args.oldItem.ENTRY_EA;
                rowData.ENTRY_WEIGHT = args.oldItem.ENTRY_WEIGHT;
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.RIB02010E0.054", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDSUBC, args.row, args.cell, true);
                } else {
...
function grdSubCOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.ENTRY_QTY)) {
            alert($NC.getDisplayMsg("JS.RIB02010E0.055", "지시수량을 입력하십시오."));
