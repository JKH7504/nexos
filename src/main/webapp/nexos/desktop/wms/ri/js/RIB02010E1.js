...
function _Initialize() {
    ...
    // 취소/처리 버튼 툴팁 세팅
    $NC.setTooltip("#btnProcessPreB", $NC.getDisplayMsg("JS.RIB02010E1.001", $NC.getValue("#btnProcessB") + " 취소", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessNxtB", $NC.getDisplayMsg("JS.RIB02010E1.002", $NC.getValue("#btnProcessB") + " 처리", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessPreC", $NC.getDisplayMsg("JS.RIB02010E1.001", $NC.getValue("#btnProcessC") + " 취소", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessNxtC", $NC.getDisplayMsg("JS.RIB02010E1.002", $NC.getValue("#btnProcessC") + " 처리", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessPreD", $NC.getDisplayMsg("JS.RIB02010E1.001", $NC.getValue("#btnProcessD") + " 취소", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessNxtD", $NC.getDisplayMsg("JS.RIB02010E1.002", $NC.getValue("#btnProcessD") + " 처리", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessPreE", $NC.getDisplayMsg("JS.RIB02010E1.001", $NC.getValue("#btnProcessE") + " 취소", $NC.getValue("#btnProcessE")));
    $NC.setTooltip("#btnProcessNxtE", $NC.getDisplayMsg("JS.RIB02010E1.002", $NC.getValue("#btnProcessE") + " 처리", $NC.getValue("#btnProcessE")));

    $NC.setTooltip("#btnNewPalletIdC", $NC.getDisplayMsg("JS.RIB02010E1.003", "신규"));
    $NC.setTooltip("#btnNewDirectionsC", $NC.getDisplayMsg("JS.RIB02010E1.004", "추가"));
    $NC.setTooltip("#btnDeleteDirectionsC", $NC.getDisplayMsg("JS.RIB02010E1.005", "삭제"));
}
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "RDELIVERY_CD":
            ...
            return;
            // case "INBOUND_DATE":
            // $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIB02010E1.010", "반입일자를 정확히 입력하십시오."));
            // break;
        case "INBOUND_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIB02010E1.006", "반입 시작일자를 정확히 입력하십시오."));
            break;
        case "INBOUND_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIB02010E1.007", "반입 종료일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIB02010E1.008", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIB02010E1.009", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if (STATE_PRE_YN == $ND.C_NO && STATE_CUR_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.RIB02010E1.019", "검색구분을 선택하십시오."));
    ...
    var ENTRY_USER_ID = $NC.getValue("#edtQEntry_User_Id");
    ...
    if ($NC.isNotNull(callFn)) {
        callFn({
            CENTER_CD: CENTER_CD,
            BU_CD: BU_CD,
            INBOUND_DATE1: INBOUND_DATE1,
            INBOUND_DATE2: INBOUND_DATE2,
            ...
            BOX_SEQ: BOX_SEQ,
            RETURN_BATCH_DIV: RETURN_BATCH_DIV,
...
function _Print(reportInfo) {
    ...
    if ($NC.isNull(INBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.RIB02010E1.020", "반입일자를 입력하십시오."));
        $NC.setFocus("#dtpQInbound_Date2");
        return;
    }
    ...
    switch ($NC.G_VAR.activeView.PROCESS_CD) {
        ...
        case $ND.C_PROCESS_PUTAWAY:
            ...
            break;
        default:
            alert($NC.getDisplayMsg("JS.RIB02010E1.021", "해당 프로세스는 출력물이 없습니다."));
            return;
    }
    ...
    switch (reportInfo.REPORT_CD) {
        ...
        case "LABEL_CMC03":
             ...
             * alert($NC.getDisplayMsg("JS.RIB02010E1.022", "[" + reportInfo.REPORT_NM + "]파렛트ID 사용 기준 정책에 의해 해당 라벨은 사용할 수 없습니다.",
        ...
        case "LABEL_CMC04":
            /*
             * // TODO: 일단 보류 if ($NC.G_VAR.policyVal.RI230 == "1") { alert($NC.getDisplayMsg("JS.RIB02010E1.022", "[" + reportInfo.REPORT_NM +
...
function _OnGridCheckBoxFormatterClick(e, view, args) {
    ...
    window.setSummaryInfo(grdObject, rowData.CHECK_YN, args.row);
}
...
function onGetProcessInfo(ajaxData) {
    ...
    $NC.showMessage({
        message: $NC.getDisplayMsg("JS.RIB02010E1.023", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록하십시오."),
        width: 400,
...
function setMasterSummaryInfo() {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIB02010E0/getDataSet.do", {
        P_QUERY_ID: "RIB02010E1.RS_MASTER",
        P_QUERY_PARAMS: {
            P_CENTER_CD: CENTER_CD,
            P_BU_CD: BU_CD,
