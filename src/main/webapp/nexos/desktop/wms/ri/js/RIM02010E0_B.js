...
function _InquiryB(args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIM02010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTERB), onGetMasterB);
}
...
function _NewB() {
    ...
    if ($NC.G_VAR.policyVal.RI211 != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.020", "반입등록 신규처리가 불가능한 사업부입니다."));
        return;
    }
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "RIM02011P0",
        PROGRAM_NM: "반입등록/수정",
        url: "ri/RIM02011P0.html",
        width: 1024,
        height: 600,
        G_PARAMETER: {
            ...
            P_POLICY_RI211: $NC.G_VAR.policyVal.RI211,
            P_POLICY_RI222: $NC.G_VAR.policyVal.RI222,
            P_POLICY_RI230: $NC.G_VAR.policyVal.RI230,
            P_POLICY_RI240: $NC.G_VAR.policyVal.RI240,
            P_POLICY_RI251: $NC.G_VAR.policyVal.RI251,
...
function onProcessPreB() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.RIM02010E0.023", "반입등록 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.024", "반입등록 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.025", "선택한 데이터 중 반입등록 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtB() {
    ...
    if ($NC.isNull(INBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.026", "입고일자를 먼저 입력하십시오."));
        $NC.setFocus("#dtpInbound_DateB");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.RIM02010E0.027", "반입등록 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.028", "반입등록 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.RIM02010E0.029", "선택한 데이터 중 반입등록 처리 가능한 데이터가 없습니다."));
        return;
    }

    $NC.serviceCall("/RIM02010E0/callRIEntryProcessing.do", {
        P_PROCESS_CD: $ND.C_PROCESS_ENTRY,
...
function grdMasterBInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterB", {
        ...
        queryId: "RIM02010E0.RS_T1_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options,
        canDblClick: true
    });
...
function grdMasterBOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/RIM02010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAILB), onGetDetailB);
...
function grdMasterBOnDblClick(e, args) {
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.RIM02010E0.030", "반입진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        ...
        if (refRowData.INBOUND_STATE != resultData.O_INBOUND_STATE) {
            alert($NC.getDisplayMsg("JS.RIM02010E0.031", "[진행상태 : " + resultData.O_INBOUND_STATE + "] 데이터가 변경되었습니다.\n다시 조회 후 데이터를 확인하십시오.",
        ...
        $NC.serviceCallAndWait("/RIM02010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAILB), onGetDetailB, function(subAjaxData) {
        ...
        if (dsDetail.length == 0) {
            alert($NC.getDisplayMsg("JS.RIM02010E0.032", "등록/수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
        ...
        $NC.showProgramSubPopup({
            PROGRAM_ID: "RIM02011P0",
            PROGRAM_NM: $NC.getDisplayMsg("JS.RIM02010E0.033", "반입등록/수정"),
            url: "ri/RIM02011P0.html",
            width: 1024,
            height: 600,
            G_PARAMETER: {
...
function grdDetailBInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailB", {
        ...
        queryId: "RIM02010E0.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
