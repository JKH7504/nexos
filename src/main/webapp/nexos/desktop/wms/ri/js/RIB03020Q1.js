...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            return;
        case "INBOUND_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIB03020Q1.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "INBOUND_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIB03020Q1.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.RIB03020Q1.003", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.RIB03020Q1.004", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var INBOUND_DATE1 = $NC.getValue("#dtpQInbound_Date1");
    if ($NC.isNull(INBOUND_DATE1)) {
        alert($NC.getDisplayMsg("JS.RIB03020Q1.005", "검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQInbound_Date1");
        return;
    }
    var INBOUND_DATE2 = $NC.getValue("#dtpQInbound_Date2");
    if ($NC.isNull(INBOUND_DATE2)) {
        alert($NC.getDisplayMsg("JS.RIB03020Q1.006", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQInbound_Date2");
        return;
    }
    if (INBOUND_DATE1 > INBOUND_DATE2) {
        alert($NC.getDisplayMsg("JS.RIB03020Q1.007", "반입일자 검색 범위 오류입니다."));
        $NC.setFocus("#dtpQInbound_Date1");
        return;
    }
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "RIB03020Q1.RS_T1_MASTER",
        sortCol: "ITEM_CD",
        gridOptions: options
    });
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "RIB03020Q1.RS_T2_MASTER",
        sortCol: "DELIVERY_CD",
        gridOptions: options
    });
