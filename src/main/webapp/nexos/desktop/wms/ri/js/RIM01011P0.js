 ...
 *  프로그램ID         : RIM01011P0
 *  프로그램명         : 온라인반입예정등록 팝업
 *  프로그램설명       : 온라인반입예정등록 팝업 화면 Javascript
...
function _Initialize() {
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.RIM01011P0.001", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.RIM01011P0.002", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.RIM01011P0.003", "저장"));
}
...
function _OnLoaded() {
    ...
    if ($NC.G_VAR.G_PARAMETER.P_POLICY_RI111 != $ND.C_YES) {
    ...
    // 예정 -> 등록, 등록 수정
        ...
        // 반입예정 삭제 기준, 수기등록만 삭제 가능일 경우 신규, 수정 불가능
        if ($NC.G_VAR.masterData.DATA_DIV != "00" && $NC.G_VAR.G_PARAMETER.P_POLICY_RI113 == "1") {
            $NC.setEnable("#btnEntryNew", false);
            $NC.setEnable("#btnEntryDelete", false);
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.RIM01011P0.005", "물류센터가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.RIM01011P0.006", "사업부가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDER_DATE)) {
        alert($NC.getDisplayMsg("JS.RIM01011P0.007", "먼저 예정일자를 입력하십시오."));
        $NC.setFocus("#dtpOrder_Date");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.RIM01011P0.008", "먼저 반입구분을 선택하십시오."));
        $NC.setFocus("#cboInout_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.DELIVERY_CD)) {
        alert($NC.getDisplayMsg("JS.RIM01011P0.009", "먼저 온라인몰 코드를 입력하십시오."));
        $NC.setFocus("#edtDelivery_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_NM)) {
        alert($NC.getDisplayMsg("JS.RIM01011P0.010", "먼저 주문자명을 입력하십시오."));
        $NC.setFocus("#edtOrderer_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_HP)) {
        alert($NC.getDisplayMsg("JS.RIM01011P0.011", "먼저 주문자 휴대폰번호를 입력하십시오."));
        $NC.setFocus("#edtOrderer_Hp");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_NM)) {
        alert($NC.getDisplayMsg("JS.RIM01011P0.012", "먼저 수령자명을 입력하십시오."));
        $NC.setFocus("#edtShipper_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_HP)) {
        alert($NC.getDisplayMsg("JS.RIM01011P0.013", "먼저 수령자 휴대폰번호를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_ZIP_CD) || $NC.isNull($NC.G_VAR.masterData.SHIPPER_ADDR_BASIC)) {
        alert($NC.getDisplayMsg("JS.RIM01011P0.014", "먼저 수령자 주소를 입력하십시오."));
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.RIM01011P0.004", "저장할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.RIM01011P0.015", "저장하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.RIM01011P0.016", "데이터 수정 후 저장하십시오."));
        return;
    }
    ...
    $NC.serviceCall("/RIM01010E0/save.do", {
        P_DS_MASTER: {
            P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
            P_BU_CD: $NC.G_VAR.masterData.BU_CD,
            P_ORDER_DATE: $NC.G_VAR.masterData.ORDER_DATE,
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.RIM01011P0.017", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        ...
        case "INOUT_CD":
            ...
            break;
        case "ORDER_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.RIM01011P0.018", "예정일자를 정확히 입력하십시오."));
        ...
        case "PLANED_DATETIME":
            if ($NC.isNotNull(args.val)) {
                $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.RIM01011P0.019", "도착예정일을 정확히 입력하십시오."));
            }
            setPlanedDatetime();
            break;
        case "AMPM":
...
function grdDetailOnCellChange(e, args) {
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        ...
        case "ORDER_QTY":
            if (Number(rowData.ORDER_QTY) < 1) {
                alert($NC.getDisplayMsg("JS.RIM01011P0.020", "예정수량은 1보다 작을 수 없습니다."));
        ...
        case "ORDER_BOX":
            if (Number(rowData.ORDER_BOX) < 0) {
                alert($NC.getDisplayMsg("JS.RIM01011P0.021", "예정BOX는 0보다 작을 수 없습니다."));
        ...
        case "ORDER_EA":
            if (Number(rowData.ORDER_EA) < 0) {
                alert($NC.getDisplayMsg("JS.RIM01011P0.022", "예정EA는 0보다 작을 수 없습니다."));
                rowData.ORDER_EA = args.oldItem.ORDER_EA;
                $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                break;
            }
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.RIM01011P0.023", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                } else {
