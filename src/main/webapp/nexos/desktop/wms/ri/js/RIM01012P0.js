 ...
 *  프로그램ID         : RIM01012P0
 *  프로그램명         : 반입예정생성 팝업
 *  프로그램설명       : 반입예정생성 팝업 화면 Javascript
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        case "OUTBOUND_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIM01012P0.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "OUTBOUND_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.RIM01012P0.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull($NC.G_VAR.G_PARAMETER.P_CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.RIM01012P0.003", "물류센터를 선택하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.G_PARAMETER.P_BU_CD)) {
        alert($NC.getDisplayMsg("JS.RIM01012P0.004", "사업부를 입력하십시오."));
        return;
    }

    var OUTBOUND_DATE1 = $NC.getValue("#dtpQOutbound_Date1");
    if ($NC.isNull(OUTBOUND_DATE1)) {
        alert($NC.getDisplayMsg("JS.RIM01012P0.005", "검색 시작일자를 입력하십시오."));
    ...
    if ($NC.isNull(OUTBOUND_DATE2)) {
        alert($NC.getDisplayMsg("JS.RIM01012P0.006", "검색 종료일자를 입력하십시오."));
...
function _Save() {
    ...
    if (rCount == 0) {
        alert($NC.getDisplayMsg("JS.RIM01012P0.007", "조회 후 처리하십시오."));
        return;
    }

    var ORDER_DATE = $NC.getValue("#dtpOrder_Date");
    if ($NC.isNull(ORDER_DATE)) {
        alert($NC.getDisplayMsg("JS.RIM01012P0.008", "반입 예정일자를 입력하십시오."));
    ...
    if ($NC.isNull(INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.RIM01012P0.009", "반입구분을 선택하십시오."));
    ...
    if ($NC.isNull(ITEM_STATE)) {
        alert($NC.getDisplayMsg("JS.RIM01012P0.010", "상품상태를 선택하십시오."));
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.RIM01012P0.011", "반입예정 생성 할 출고전표를 선택하십시오."));
        return;
    }

    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.RIM01012P0.012", "선택한 데이터 중 반입예정 생성 가능한 데이터가 없습니다.\n(이미 반입예정 생성 된 출고전표인지 확인 하세요.)"));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.RIM01012P0.013", "반입예정 생성 하시겠습니까?"))) {
        return;
    }
