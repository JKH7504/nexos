...
function _OnLoaded() {
    ...
    $NC.setInitSplitter("#divT1TabSheetView", "h", 300, 150, 300);
    $NC.setInitSplitter("#divRightBottom", "h", 200, 150, 150);
    $NC.setInitSplitter("#divBottomView", "vr", 600, 600, 300);
}
...
function _OnResize(parent, viewWidth, viewHeight) {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        var containerWidth;
        if ($NC.isVisible("#divRightBottom")) {
            containerWidth = parseInt($("#divBottomView").children(".splitter-bar-vertical").css("left"));
        }
        $NC.resizeGridView("#divLeftBottom", "#grdT1Detail", containerWidth);
    }
}
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            if ($NC.isNull(BU_CD)) {
                alert($NC.getDisplayMsg("JS.LCC02010E0.003", "사업부를 먼저 선택 하십시오."));
            ...
            return;
        case "ETC_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02010E0.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ETC_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02010E0.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.004", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.005", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var ETC_DATE1 = $NC.getValue("#dtpQEtc_Date1");
    if ($NC.isNull(ETC_DATE1)) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.006", "검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQEtc_Date1");
        return;
    }
    var ETC_DATE2 = $NC.getValue("#dtpQEtc_Date2");
    if ($NC.isNull(ETC_DATE2)) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.007", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQEtc_Date2");
        return;
    }
    if (ETC_DATE1 > ETC_DATE2) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.008", "입출고일자 검색 범위 오류입니다."));
        $NC.setFocus("#dtpQEtc_Date1");
        return;
    }
...
function _New() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        $NC.showProgramSubPopup({
            PROGRAM_ID: "LCC02011P0",
            PROGRAM_NM: $NC.getDisplayMsg("JS.LCC02010E0.009", "상태변환 등록"),
            url: "lc/LCC02011P0.html",
            width: 1024,
            height: 600,
            G_PARAMETER: {
...
function _Save() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.010", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.011", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LCC02010E0/updateRemark.do", {
...
function _Delete() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
            alert($NC.getDisplayMsg("JS.LCC02010E0.012", "삭제할 데이터가 없습니다."));
            return;
        }

        var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
        if (rowData.CONFIRM_YN == $ND.C_YES) {
            alert($NC.getDisplayMsg("JS.LCC02010E0.013", "이미 확정한 데이터는 삭제할수 없습니다."));
            return;
        }

        if (rowData.ETC_STATE == $ND.C_STATE_ORDER) {
            alert($NC.getDisplayMsg("JS.LCC02010E0.014", "예정 데이터는 삭제할 수 없습니다."));
            return;
        }

        if (!confirm($NC.getDisplayMsg("JS.LCC02010E0.015", "삭제 하시겠습니까?"))) {
            return;
        }
...
function _Print(reportInfo) {
    ...
    if ($NC.isNull(rowData)) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.016", "출력할 데이터를 먼저 선택하십시오."));
        return;
    }
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "LCC02010E0.RS_T1_MASTER",
        sortCol: "ETC_DATE",
        gridOptions: options,
        canDblClick: true
    });
...
function grdT1MasterOnDblClick(e, args) {
    ...
    if (refRowData.CONFIRM_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.017", "확정 된 전표입니다."));
        return;
    }

    // 예정으로 등록한 전표는 수정불가
    if (refRowData.ETC_STATE == $ND.C_STATE_ENTRY && $NC.isNotNull(refRowData.ORDER_NO)) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.018", "예정으로 등록한 데이터는 수정할 수 없습니다. 삭제 후 다시 등록하십시오."));
        return;
    }

    // 예정, 자동할당만 사용일 경우
    if ($NC.G_VAR.policyVal.LC320 == "2" && refRowData.ETC_STATE == $ND.C_STATE_ORDER) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.019", "[자동등록] 버튼으로 등록 처리하십시오."));
        return;
    }
    ...
    var showSubPopupFn = function(dsResult, isOrder) {
        ...
        $NC.showProgramSubPopup({
            PROGRAM_ID: "LCC02011P0",
            PROGRAM_NM: $NC.getDisplayMsg("JS.LCC02010E0.098", "상태변환 등록"),
            url: "lc/LCC02011P0.html",
            width: 1024,
            height: 600,
            G_PARAMETER: {
    ...
    if ($NC.isNotNull(refRowData.ORDER_NO)) {
        ...
        dataServiceCallFn = function() {
            ...
            if (dsDetail.length == 0) {
                alert($NC.getDisplayMsg("JS.LCC02010E0.020", "등록할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
                return;
            }
        ...
        checkServiceCallFn = function() {
            ...
            function(ajaxData) {
                ...
                if ($NC.isEmpty(resultData)) {
                    alert($NC.getDisplayMsg("JS.LCC02010E0.021", "진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
                    return;
                }
                var oMsg = $NC.getOutMessage(resultData);
                if (oMsg != $ND.C_OK) {
                ...
                if (refRowData.ETC_STATE != resultData.O_ETC_STATE) {
                    alert($NC.getDisplayMsg("JS.LCC02010E0.022", "[진행상태 : " //
    ...
    // 신규 등록 데이터 수정, 예정 등록 건은 수정 불가
        ...
        dataServiceCallFn = function() {
            ...
            if (dsSub.length == 0) {
                alert($NC.getDisplayMsg("JS.LCC02010E0.023", "수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
                return;
            }
        ...
        checkServiceCallFn = function() {
            ...
            function(ajaxData) {
                var resultData = $NC.toObject(ajaxData);
                if ($NC.isEmpty(resultData)) {
                    alert($NC.getDisplayMsg("JS.LCC02010E0.024", "확정여부를 확인하지 못했습니다.\n다시 처리하십시오."));
                    return;
                }
                var oMsg = $NC.getOutMessage(resultData);
                if (oMsg != $ND.C_OK) {
                ...
                if (resultData.O_CONFIRM_YN != $ND.C_NO) {
                    alert($NC.getDisplayMsg("JS.LCC02010E0.025", "이미 확정처리된 데이터입니다.\n다시 조회 후 데이터를 확인하십시오."));
                    return;
                }
...
function grdT1MasterOnAfterScroll(e, args) {
    ...
    $NC.clearGridData(G_GRDT1SUB1);
    $NC.clearGridData(G_GRDT1SUB2);
    ...
    // detail 그리드 resize하기 전 세팅
    if (rowData.ETC_STATE == $ND.C_STATE_ORDER) {
        // 지시 그리드 보일 경우 안보이게 세팅
        if ($NC.isVisible("#divRightBottom")) {
            // 지시 그리드 숨기기 전 bottom 그리드 크기 저장
            $NC.G_OFFSET.leftViewWidth = $("#divLeftBottom").width();
            $("#divRightBottom").hide();
            $NC.onGlobalResize();

            $("#divBottomView").children(".splitter-bar").css("left", $("#divBottomView").width()).hide();
        }
    } else {
        // 지시 그리드 숨겨져 있을 경우 보이게 세팅
        if (!$NC.isVisible("#divRightBottom")) {
            $("#divRightBottom").show();
            $("#divBottomView").children(".splitter-bar").css("left", $NC.G_OFFSET.leftViewWidth).show();

            $("#divBottomView").trigger("resize", [
                $NC.G_OFFSET.leftViewWidth
            ]);
        }
    }
...
/**
 * 상태변환 등록 탭 : 상태가 등록인 경우만 그리드 편집 가능
 * 
 * @param e
 * @param args
 * @returns {Boolean}
...
function grdT1MasterOnBeforeEditCell(e, args) {
    ...
    switch (args.column.id) {
        case "REMARK1":
            return rowData.ETC_STATE == $ND.C_STATE_ENTRY;
        default:
            return false;
    }
}
...
function grdT1DetailInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Detail", {
        ...
        queryId: "LCC02010E0.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdT1DetailOnAfterScroll(e, args) {
    ...
    // 출고지시 조회시 전역 변수 값 초기화
    $NC.clearGridData(G_GRDT1SUB1);
    G_GRDT1SUB1.queryParams = {
        P_CENTER_CD: refRowData.CENTER_CD,
        P_BU_CD: refRowData.BU_CD,
        P_ETC_DATE: refRowData.ETC_DATE,
        P_ETC_NO: refRowData.ETC_NO,
        P_LINE_NO: rowData.LINE_NO,
        P_INOUT_GRP: $ND.C_INOUT_GRP_D5
    };
    // 데이터 조회
    $NC.serviceCall("/LCC02010E0/getDataSet.do", $NC.getGridParams(G_GRDT1SUB1), onGetT1Sub1);

    // 입고지시 조회시 전역 변수 값 초기화
    $NC.clearGridData(G_GRDT1SUB2);
    G_GRDT1SUB2.queryParams = {
        P_CENTER_CD: refRowData.CENTER_CD,
        P_BU_CD: refRowData.BU_CD,
        P_ETC_DATE: refRowData.ETC_DATE,
        P_ETC_NO: refRowData.LINK_ETC_NO,
        P_LINE_NO: rowData.LINE_NO,
        P_INOUT_GRP: $ND.C_INOUT_GRP_E5
    };
    // 데이터 조회
    $NC.serviceCall("/LCC02010E0/getDataSet.do", $NC.getGridParams(G_GRDT1SUB2), onGetT1Sub2);

    // 상단 현재로우/총건수 업데이트
    $NC.setGridDisplayRows(G_GRDT1DETAIL, row + 1);
}

function grdT1Sub1OnGetColumns() {
...
function grdT1Sub1OnSetColumns() {

    // 재고 관리 기준 : 2 - 입고일자, 유효일자, 배치번호별 관리
    $NC.setGridColumns(G_GRDT1SUB1, [ // 숨김컬럼 세팅
        $NC.G_VAR.policyVal.LS210 != "2" ? "VALID_DATE,BATCH_NO" : ""
    ]);
}

function grdT1Sub1Initialize() {
    ...
    // Grid Object, DataView 생성 및 초기화
    $NC.setInitGridObject("#grdT1Sub1", {
        columns: grdT1Sub1OnGetColumns(),
        queryId: "LCC02010E0.RS_T1_SUB",
        sortCol: "LINE_NO",
        gridOptions: options
    });

    G_GRDT1SUB1.view.onSelectedRowsChanged.subscribe(grdT1Sub1OnAfterScroll);
}
 ...
 * @param args
 */
function grdT1Sub1OnAfterScroll(e, args) {

    if (!$NC.isGridValidLastRow(G_GRDT1SUB1, args.rows, e)) {
        return;
    }
    var row = args.rows[0];

    // 상단 현재로우/총건수 업데이트
    $NC.setGridDisplayRows(G_GRDT1SUB1, row + 1);
}

function grdT1Sub2OnGetColumns() {
...
function grdT1Sub2OnSetColumns() {

    // 재고 관리 기준 : 2 - 입고일자, 유효일자, 배치번호별 관리
    $NC.setGridColumns(G_GRDT1SUB2, [ // 숨김컬럼 세팅
        $NC.G_VAR.policyVal.LS210 != "2" ? "VALID_DATE,BATCH_NO" : ""
    ]);
}

function grdT1Sub2Initialize() {
    ...
    // Grid Object, DataView 생성 및 초기화
    $NC.setInitGridObject("#grdT1Sub2", {
        columns: grdT1Sub2OnGetColumns(),
        queryId: "LCC02010E0.RS_T1_SUB",
        sortCol: "LINE_NO",
        gridOptions: options
    });

    G_GRDT1SUB2.view.onSelectedRowsChanged.subscribe(grdT1Sub2OnAfterScroll);
}
 ...
 * @param args
 */
function grdT1Sub2OnAfterScroll(e, args) {

    if (!$NC.isGridValidLastRow(G_GRDT1SUB2, args.rows, e)) {
        return;
    }
    var row = args.rows[0];

    // 상단 현재로우/총건수 업데이트
    $NC.setGridDisplayRows(G_GRDT1SUB2, row + 1);
}
...
function grdT2MasterOnSetColums() {

    // 재고 관리 기준 : 2 - 입고일자, 유효일자, 배치번호 관리
    $NC.setGridColumns(G_GRDT2MASTER, [
        $NC.G_VAR.policyVal.LS210 != "2" ? "VALID_DATE,BATCH_NO,LINK_VALID_DATE,LINK_BATCH_NO" : ""
    ]);
}
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "LCC02010E0.RS_T2_MASTER",
        sortCol: "ETC_DATE",
        gridOptions: options
    });
...
/**
 * 상태변환상세 행 클릭후 우측 그리드에 데이터 표시처리(출고지시)
 * 
 * @param ajaxData
 */
function onGetT1Sub1(ajaxData) {

    $NC.setInitGridData(G_GRDT1SUB1, ajaxData);
    $NC.setInitGridAfterOpen(G_GRDT1SUB1);
}

/**
 * 상태변환상세 행 클릭후 우측 그리드에 데이터 표시처리(입고지시)
 * 
 * @param ajaxData
 */
function onGetT1Sub2(ajaxData) {

    $NC.setInitGridData(G_GRDT1SUB2, ajaxData);
    $NC.setInitGridAfterOpen(G_GRDT1SUB2);
}
...
function onChangingCondition() {
    ...
    if ($NC.isVisible("#divRightBottom")) {
        $("#divRightBottom").show();
        $("#divBottomView").children(".splitter-bar").show();

        // 지시 그리드 숨긴 후 보여줄 때 element 위치 및 사이즈 세팅
        $("#divBottomView").children(".splitter-bar").css("left", $NC.G_OFFSET.leftViewWidth);
        $("#divBottomView").trigger("resize", [
            $NC.G_OFFSET.leftViewWidth
        ]);
    }
...
function btnAutoEntryOnClick() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.026", "조회 후 처리하십시오."));
        return;
    }
    ...
    if (refRowData.ETC_STATE != $ND.C_STATE_ORDER) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.027", "예정 상태의 전표만 처리할 수 있습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC02010E0.028", "자동등록 처리 하시겠습니까?"))) {
        return;
    }
...
function onProcessPre() {
    ...
    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.CONFIRM_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.029", "확정된 전표만 취소 할 수 있습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC02010E0.030", "취소 처리 하시겠습니까?"))) {
        return;
    }
...
function onProcessNxt() {
    ...
    if (rowData.ETC_STATE == $ND.C_STATE_ORDER) {
        alert($NC.getDisplayMsg("JS.LCC02010E0.031", "예정 데이터입니다. 등록 처리를 먼저 하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC02010E0.032", "확정 처리 하시겠습니까?"))) {
        return;
    }
...
function setPolicyValInfo() {
    ...
    $NC.setPolicyValInfo({
        ...
        // 컬럼 표시 조정
        grdT1Sub1OnSetColumns();
        grdT1Sub2OnSetColumns();
    });
}
