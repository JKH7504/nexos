 ...
 *  프로그램ID         : LCC02011P1
 *  프로그램명         : 상태변환등록 (의류) 팝업
 *  프로그램설명       : 상태변환등록 (의류) 팝업 화면 Javascript
            ...
            onComplete: function() {
                if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ENTRY_CREATE) {
                    ...
                    if ($NC.G_VAR.ORDER_YN == $ND.C_YES && G_GRDDETAIL.data.getLength() > 0) {
                        ...
                        $NC.setValue("#cboTag_Div", refRowData.TAG_DIV);
                    } else {
    ...
    switch (id) {
        ...
        case "ZONE_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM121)) {
                alert($NC.getDisplayMsg("JS.LCC02011P1.001", "로케이션 존코드 길이(" + $NC.G_VAR.policyVal.CM121 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BANK_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM122)) {
                alert($NC.getDisplayMsg("JS.LCC02011P1.002", "로케이션 행코드 길이(" + $NC.G_VAR.policyVal.CM122 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BAY_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM123)) {
                alert($NC.getDisplayMsg("JS.LCC02011P1.003", "로케이션 열코드 길이(" + $NC.G_VAR.policyVal.CM123 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "LEV_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM124)) {
                alert($NC.getDisplayMsg("JS.LCC02011P1.004", "로케이션 단코드 길이(" + $NC.G_VAR.policyVal.CM124 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "STOCK_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02011P1.005", "입고일자를 정확히 입력하십시오."));
            }
            break;
        case "VALID_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02011P1.006", "유통기한를 정확히 입력하십시오."));
            }
            break;
    }
}
...
function _New() {
    ...
    if (checkedRows.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC02011P1.008", "추가할 재고내역 데이터를 선택하십시오."));
        return;
    }
    ...
    if (G_GRDSTOCK.data.getItem(checkedRows[0]).ITEM_STATE == LINK_ITEM_STATE) {
        alert($NC.getDisplayMsg("JS.LCC02011P1.009", "같은 상태로는 변환 처리 할 수 없습니다."));
        return;
    }
    ...
    if ($NC.G_VAR.ORDER_YN == $ND.C_YES) {
        ...
        if (remainQty < 1) {
            alert($NC.getDisplayMsg("JS.LCC02011P1.010", "이미 지시내역이 모두 추가되어 있습니다.\n\n기존 지시내역을 삭제하거나 확정수량 조정 후 추가하십시오."));
            return;
        }
    }
    ...
    var addedCount = 0;
    var rowData, newRowData, confirmQty;
    G_GRDSUB.data.beginUpdate();
    try {
    ...
    if (addedCount == 0) {
        alert($NC.getDisplayMsg("JS.LCC02011P1.011", "선택한 상품은 이미 추가된 재고내역입니다."));
        return;
    }
...
function _Save() {
    ...
    if (G_GRDSUB.data.getItems().length == 0) {
        alert($NC.getDisplayMsg("JS.LCC02011P1.012", "저장할 데이터가 없습니다."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02011P1.013", "물류센터를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02011P1.014", "사업부를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02011P1.015", "먼저 입출고구분을 선택하십시오."));
        $NC.setFocus("#cboInout_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ETC_DATE)) {
        alert($NC.getDisplayMsg("JS.LCC02011P1.016", "먼저 입출고일자를 입력하십시오."));
    ...
    if ($NC.G_VAR.ORDER_YN == $ND.C_YES) {
        ...
        if (checkedData.checkedCount == 0) {
            alert($NC.getDisplayMsg("JS.LCC02011P1.017", "저장할 지시내역 정보가 없습니다."));
            return;
        }
    ...
    // 신규 등록/수정
        ...
        if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_C) {
            ...
            if ($NC.G_VAR.masterData.CRUD != $ND.C_DV_CRUD_U && dsSub.length == 0) {
                alert($NC.getDisplayMsg("JS.LCC02011P1.018", "수정 후 저장하십시오."));
                return;
            }
        }
...
function _Delete() {
    ...
    if (G_GRDSUB.data.getLength() == 0 || $NC.isNull(G_GRDSUB.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC02011P1.019", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        case "LOCATION_CD":
            ...
            return;
        case "ETC_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LCC02011P1.020", "입출고일자를 정확히 입력하십시오."));
...
function grdDetailOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if (rowData.ORDER_QTY < rowData.ENTRY_QTY) {
            alert($NC.getDisplayMsg("JS.LCC02011P1.021", "등록수량이 예정수량보다 클 수 없습니다."));
    ...
    if ($NC.isNull(rowData.ETC_DIV)) {
        alert($NC.getDisplayMsg("JS.LCC02011P1.022", "사유구분을 선택하십시오."));
...
function grdStockOnSetColumns() {
    ...
    $NC.setGridColumns(G_GRDSTOCK, [ // 숨김컬럼 세팅
        ...
        $NC.G_VAR.ORDER_YN == $ND.C_YES ? "ITEM_CD,ITEM_NM,ITEM_SPEC,BRAND_NM,ITEM_STATE_F,FLOCATION_CD,ITEM_LOT,QTY_IN_BOX" : ""
    ]);
}
...
function grdSubOnSetColumns() {
    ...
    $NC.setGridColumns(G_GRDSUB, [ // 숨김컬럼 세팅
        $NC.G_VAR.policyVal.LS210 != "2" ? "VALID_DATE,BATCH_NO" : "",
        $NC.G_VAR.ORDER_YN == $ND.C_YES ? "ITEM_CD,ITEM_NM,ITEM_SPEC,BRAND_NM,ITEM_STATE_F," //
...
function validateSubData(row, rowData) {
    ...
    if ($NC.isNull(rowData.CONFIRM_QTY) || Number(rowData.CONFIRM_QTY) <= 0) {
        alert($NC.getDisplayMsg("JS.LCC02011P1.023", "변환수량에 0보다 큰 수량을 입력하십시오."));
    ...
    if (rowData.CRUD == $ND.C_DV_CRUD_C) {
        ...
        if ($NC.G_VAR.ORDER_YN == $ND.C_YES) {
            ...
            if (rowData.PSTOCK_QTY < totConfirmQty) {
                alert($NC.getDisplayMsg("JS.LCC02011P1.024", "재고수량 보다 많은 수량을 입력하실 수 없습니다."));
                $NC.setFocusGrid(G_GRDSUB, row, "CONFIRM_QTY", true);
                return false;
            }
        }
        ...
        if ($NC.isNull(rowData.LINK_LOCATION_CD)) {
            alert($NC.getDisplayMsg("JS.LCC02011P1.025", "이동로케이션을 입력하십시오."));
    ...
    if ($NC.isNull(rowData.ETC_DIV)) {
        alert($NC.getDisplayMsg("JS.LCC02011P1.026", "사유구분을 선택하십시오."));
