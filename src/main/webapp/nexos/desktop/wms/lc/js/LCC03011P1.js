 ...
 *  프로그램ID         : LCC03011P1
 *  프로그램명         : 재고이동등록 팝업 (의류)
 *  프로그램설명       : 재고이동등록 팝업 (의류) 화면 Javascript
...
function _Initialize() {
    ...
    $NC.setInitDateRangePicker("#dtpQStock_Date1", "#dtpQStock_Date2", null, "N");
...
function _OnLoaded() {
    ...
    // 예정 -> 등록, 등록 수정
        ...
        NEW_ENTER_YN = dsMaster.NEW_ENTER_YN;
    ...
    $NC.setInitCombo("/WC/getDataSet.do", {
        ...
        onComplete: function() {
            if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ENTRY_CREATE) {
                ...
                $NC.setValue("#cboMove_Div", $NC.G_VAR.masterData.MOVE_DIV);
            }
        }
    });
}
    ...
    switch (id) {
        ...
        case "ZONE_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM121)) {
                alert($NC.getDisplayMsg("JS.LCC03011P1.001", "로케이션 존코드 길이(" + $NC.G_VAR.policyVal.CM121 + "자리) 를 정확히 입력하여 주십시오.",
            ...
            $NP.onZoneChange(val, {
                ...
                errorMessage: $NC.getDisplayMsg("JS.LCC03011P1.002", "선택된 상품 상태와 매칭되는 존이 검색되지 않았습니다.")
            });
            return;
        case "BANK_CD":
            if ($NC.isNull(val)) {
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM122)) {
                alert($NC.getDisplayMsg("JS.LCC03011P1.003", "로케이션 행코드 길이(" + $NC.G_VAR.policyVal.CM122 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BAY_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM123)) {
                alert($NC.getDisplayMsg("JS.LCC03011P1.004", "로케이션 열코드 길이(" + $NC.G_VAR.policyVal.CM123 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "LEV_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM124)) {
                alert($NC.getDisplayMsg("JS.LCC03011P1.005", "로케이션 단코드 길이(" + $NC.G_VAR.policyVal.CM124 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "STOCK_DATE1":
            if ($NC.isNotNull(val)) {
                if (!$NC.isDate(val)) {
                    alert($NC.getDisplayMsg("JS.LCC03011P1.006", "입고 시작일자를 정확히 입력하십시오."));
                    $NC.setValue("#dtpQStock_Date1");
                    $NC.setFocus("#dtpQStock_Date1");
                    return;
                } else {
                    if ($NC.isNotNull($NC.getValue("#dtpQStock_Date2"))) {
                        if (val > $NC.getValue("#dtpQStock_Date2")) {
                            $NC.setValue("#dtpQStock_Date2", $NC.getValue("#dtpQStock_Date1"));
        ...
        case "STOCK_DATE2":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC03011P1.007", "입고 종료일자를 정확히 입력하십시오."));
            }
            break;
        case "VALID_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC03011P1.008", "유통기한을 정확히 입력하십시오."));
            }
            break;
        case "ITEM_STATE":
            if ($NC.isNotNull(val)) {
...
function _Inquiry() {
    ...
    var RETURN_BATCH_DIV = $NC.getValue("#cboQReturn_Batch_Div");
    ...
    if ($NC.isNotNull(DELIVERY_CD)) {
        if ($NC.isNull(STOCK_DATE1) || $NC.isNull(STOCK_DATE2)) {
            alert($NC.getDisplayMsg("JS.LCC03011P1.009", "배송처를 입력하였을 경우 입고 시작일자와 종료일자를 반드시 입력 하셔야 합니다."));
            $NC.setFocus("#dtpQStock_Date1");
            return;
        }
    } else {
        if ($NC.isNotNull(STOCK_DATE2)) {
            if ($NC.isNull(STOCK_DATE1)) {
                alert($NC.getDisplayMsg("JS.LCC03011P1.010", "입고 시작일자를 입력하거나 입고 종료일자를 지워주십시오."));
                $NC.setFocus("#dtpQStock_Date1");
                return;
            }
        }
        if ($NC.isNotNull(STOCK_DATE1)) {
            if ($NC.isNull(STOCK_DATE2)) {
                alert($NC.getDisplayMsg("JS.LCC03011P1.011", "입고 종료일자를 입력하거나 입고 시작일자를 지워주십시오."));
                $NC.setFocus("#dtpQStock_Date2");
                return;
            }
        }
    ...
    G_GRDDETAIL.queryParams = {
        ...
        P_TAG_DIV: TAG_DIV,
        P_RETURN_BATCH_DIV: RETURN_BATCH_DIV,
...
function _New() {
    ...
    if (checkedRows.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC03011P1.012", "추가할 재고내역 데이터를 선택하십시오."));
        return;
    }
    var MLOCATION_CD = $NC.getValue("#edtLocation_Cd");
    if ($NC.isNull(MLOCATION_CD)) {
        alert($NC.getDisplayMsg("JS.LCC03011P1.013", "재고 추가시 먼저 이동할 로케이션을 지정하십시오."));
    ...
    var addedCount = 0, sameLocCount = 0, pstockQtyCount = 0;
    ...
    if (addedCount == 0) {
        if (checkedRows.length == sameLocCount) {
            alert($NC.getDisplayMsg("JS.LCC03011P1.014", "선택한 상품의 재고로케이션과 이동로케이션이 모두 같습니다.\n이동로케이션 변경 후 추가하십시오."));
        } else if (checkedRows.length == pstockQtyCount) {
            alert($NC.getDisplayMsg("JS.LCC03011P1.015", "선택한 상품의 이동가능 수량이 존재하지않습니다."));
        } else {
            alert($NC.getDisplayMsg("JS.LCC03011P1.016", "선택한 상품은 이미 추가된 재고내역입니다."));
        }
        return;
    }
    ...
    // 이동구분이 13-ASS이동일 경우 로케이션 비활성화
    if ($NC.getValue("#cboMove_Div") == "13") {
...
function _Save() {
    ...
    if (G_GRDSUB.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LCC03011P1.017", "저장할 데이터가 없습니다."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC03011P1.018", "물류센터를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC03011P1.019", "사업부를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.MOVE_DATE)) {
        alert($NC.getDisplayMsg("JS.LCC03011P1.020", "이동일자를 입력하십시오."));
        $NC.setFocus("#dtpMove_Date");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.MOVE_DIV)) {
        alert($NC.getDisplayMsg("JS.LCC03011P1.032", "이동구분을 선택하십시오."));
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_C) {
        if (dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC03011P1.021", "저장할 지시내역 정보가 없습니다."));
            return;
        }
    } else {
        if ($NC.G_VAR.masterData.CRUD != $ND.C_DV_CRUD_U && dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC03011P1.022", "데이터 수정 후 저장하십시오."));
            return;
        }
    }
...
function _Delete() {
    ...
    if (G_GRDSUB.data.getLength() == 0 || $NC.isNull(G_GRDSUB.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC03011P1.023", "삭제할 데이터가 없습니다."));
        return;
    }
    ...
    switch (args.col) {
        case "LOCATION_CD":
            $NP.onLocationChange(args.val, {
                ...
                // 1 - 일반, 2 - 유통가공, 3 - 보세
                P_ZONE_DIV_ATTR01_CD: getZoneDivAttr01Cd($NC.getValue("#cboQItem_State")),
                P_LOC_DIV_ATTR03_CD: $NC.G_VAR.masterData.MOVE_DIV == "13" ? "20" : "10"
            }, onLocationPopup, {
                errorMessage: $NC.getDisplayMsg("JS.LCC03011P1.024", "이동 가능한 로케이션이 검색되지 않았습니다.")
            });
            return;
        case "MOVE_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LCC03011P1.025", "이동일자를 정확히 입력하십시오."));
...
function validateSubData(row, rowData) {
    ...
    if ($NC.isNull(rowData.MSTOCK_QTY) || Number(rowData.MSTOCK_QTY) <= 0) {
        alert($NC.getDisplayMsg("JS.LCC03011P1.026", "이동수량에 0보다 큰 수량을 입력하십시오."));
        $NC.setFocusGrid(G_GRDSUB, row, "MSTOCK_QTY", true);
        return false;
    }

    if (rowData.CRUD == $ND.C_DV_CRUD_C) {
        // 재고 중복 등록 가능하므로 합계 체크
        ...
        if (Number(rowData.PSTOCK_QTY) < sumMStockQty) {
            alert($NC.getDisplayMsg("JS.LCC03011P1.027", "재고수량 보다 많은 수량을 입력하실 수 없습니다."));
            $NC.setFocusGrid(G_GRDSUB, row, "MSTOCK_QTY", true);
            return false;
        }

        if ($NC.isNull(rowData.MLOCATION_CD)) {
            alert($NC.getDisplayMsg("JS.LCC03011P1.028", "이동할 로케이션을 선택하십시오."));
...
function showQLocationPopup() {
    ...
    $NP.showLocationPopup({
        ...
        // 1 - 일반, 2 - 유통가공, 3 - 보세
        P_ZONE_DIV_ATTR01_CD: getZoneDivAttr01Cd($NC.getValue("#cboQItem_State")),
        P_LOC_DIV_ATTR03_CD: $NC.G_VAR.masterData.MOVE_DIV == "13" ? "10" : [
            "10",
            "20"
        ].join($ND.C_SEP_DATA)
...
function btnUploadXLSOnClick() {
    ...
    if ($NC.isNotNull($NC.G_VAR.masterData.MOVE_NO)) {
        alert($NC.getDisplayMsg("JS.LCC03011P1.029", "엑셀 업로드는 기존 데이터에 추가할 수 없습니다. 저장 후 신규로 처리하십시오."));
        return;
    }

    if (G_GRDSUB.data.getLength() > 0) {
        if (!confirm($NC.getDisplayMsg("JS.LCC03011P1.030", "엑셀 업로드시 기존 내역 삭제 후 추가됩니다. 엑셀 업로드하시겠습니까?"))) {
            return;
        }
    }
    ...
    $NC.excelFileUpload({
        P_QUERY_ID: "LCC03010E1.RS_SUB3",
        P_QUERY_PARAMS: {
            P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
            P_BU_CD: $NC.G_VAR.masterData.BU_CD,
            P_MOVE_DIV: $NC.getValue("#cboMove_Div"),
            P_POLICY_LS210: $NC.G_VAR.policyVal.LS210
        },
        P_SERVICE_PARAMS: {
        ...
        if ($NC.isNull(dsResultData) || dsResultData.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC03011P1.031", "업로드할 수 있는 대상 데이터가 없습니다. 엑셀 파일을 확인하십시오."));
            return;
        }
