...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            if ($NC.isNull(BU_CD)) {
                alert($NC.getDisplayMsg("JS.LCC02020E0.001", "사업부를 먼저 선택 하십시오."));
            ...
            return;
        case "ETC_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02020E0.002", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ETC_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02020E0.003", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02020E0.004", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02020E0.005", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var ETC_DATE1 = $NC.getValue("#dtpQEtc_Date1");
    if ($NC.isNull(ETC_DATE1)) {
        alert($NC.getDisplayMsg("JS.LCC02020E0.006", "검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQEtc_Date1");
        return;
    }
    var ETC_DATE2 = $NC.getValue("#dtpQEtc_Date2");
    if ($NC.isNull(ETC_DATE2)) {
        alert($NC.getDisplayMsg("JS.LCC02020E0.007", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQEtc_Date2");
        return;
    }
    if (ETC_DATE1 > ETC_DATE2) {
        alert($NC.getDisplayMsg("JS.LCC02020E0.008", "입출고일자 검색 범위 오류입니다."));
        $NC.setFocus("#dtpQEtc_Date1");
        return;
    }
...
function _New() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        $NC.showProgramSubPopup({
            PROGRAM_ID: "LCC02021P0",
            PROGRAM_NM: $NC.getDisplayMsg("JS.LCC02020E0.009", "상품변환 등록"),
            url: "lc/LCC02021P0.html",
            width: 1024,
            height: 600,
            G_PARAMETER: {
...
function _Save() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LCC02020E0.010", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC02020E0.011", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LCC02020E0/updateRemark.do", {
...
function _Delete() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
            alert($NC.getDisplayMsg("JS.LCC02020E0.012", "삭제할 데이터가 없습니다."));
            return;
        }

        var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
        if (rowData.CONFIRM_YN == $ND.C_YES) {
            alert($NC.getDisplayMsg("JS.LCC02020E0.013", "이미 확정한 데이터는 삭제할수 없습니다."));
            return;
        }

        if (rowData.ETC_STATE == $ND.C_STATE_ORDER) {
            alert($NC.getDisplayMsg("JS.LCC02040E0.014", "예정 데이터는 삭제할 수 없습니다."));
            return;
        }

        if (!confirm($NC.getDisplayMsg("JS.LCC02020E0.015", "삭제 하시겠습니까?"))) {
            return;
        }
...
function _Print(reportInfo) {
    ...
    if ($NC.isNull(rowData)) {
        alert($NC.getDisplayMsg("JS.LCC02020E0.016", "출력할 데이터를 먼저 선택하십시오."));
        return;
    }
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "LCC02020E0.RS_T1_MASTER",
        sortCol: "ETC_DATE",
        gridOptions: options,
        canDblClick: true
    });
...
function grdT1MasterOnDblClick(e, args) {
    ...
    if (refRowData.CONFIRM_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC02020E0.017", "확정 된 전표입니다."));
        return;
    }

    // 예정으로 등록한 전표는 수정불가
    if (refRowData.ETC_STATE == $ND.C_STATE_ENTRY && $NC.isNotNull(refRowData.ORDER_NO)) {
        alert($NC.getDisplayMsg("JS.LCC02020E0.018", "예정으로 등록한 데이터는 수정할 수 없습니다. 삭제 후 다시 등록하십시오."));
        return;
    }
    ...
    if ($NC.isNotNull(refRowData.ORDER_NO)) {
        ...
        dataServiceCallFn = function() {
            ...
            if (dsDetail.length == 0) {
                alert($NC.getDisplayMsg("JS.LCC02020E0.019", "등록할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
                return;
            }
        ...
        checkServiceCallFn = function() {
            ...
            function(ajaxData) {
                ...
                if ($NC.isEmpty(resultData)) {
                    alert($NC.getDisplayMsg("JS.LCC02020E0.020", "진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
                    return;
                }
                var oMsg = $NC.getOutMessage(resultData);
                if (oMsg != $ND.C_OK) {
                ...
                if (refRowData.ETC_STATE != resultData.O_ETC_STATE) {
                    alert($NC.getDisplayMsg("JS.LCC02020E0.021", "[진행상태 : " //
    ...
    // 신규 등록 데이터 수정, 예정 등록 건은 수정 불가
        ...
        dataServiceCallFn = function() {
            ...
            if (dsSub.length == 0) {
                alert($NC.getDisplayMsg("JS.LCC02020E0.022", "수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
                return;
            }
        ...
        checkServiceCallFn = function() {
            ...
            function(ajaxData) {
                var resultData = $NC.toObject(ajaxData);
                if ($NC.isEmpty(resultData)) {
                    alert($NC.getDisplayMsg("JS.LCC02020E0.023", "확정여부를 확인하지 못했습니다.\n다시 처리하십시오."));
                    return;
                }
                var oMsg = $NC.getOutMessage(resultData);
                if (oMsg != $ND.C_OK) {
                ...
                if (resultData.O_CONFIRM_YN != $ND.C_NO) {
                    alert($NC.getDisplayMsg("JS.LCC02020E0.024", "이미 확정처리된 데이터입니다.\n다시 조회 후 데이터를 확인하십시오."));
                    return;
                }
...
/**
 * 상품변환 등록 탭 : 상태가 등록인 경우만 그리드 편집 가능
 * 
 * @param e
 * @param args
 * @returns {Boolean}
...
function grdT1DetailOnAfterScroll(e, args) {
    ...
    G_GRDT1SUB1.queryParams = {
        ...
        P_INOUT_GRP: $ND.C_INOUT_GRP_D6
    };
    // 데이터 조회
    $NC.serviceCall("/LCC02020E0/getDataSet.do", $NC.getGridParams(G_GRDT1SUB1), onGetT1Sub1);
    ...
    G_GRDT1SUB2.queryParams = {
        ...
        P_LINE_NO: rowData.LINE_NO,
        P_INOUT_GRP: $ND.C_INOUT_GRP_E6
    };
    // 데이터 조회
    $NC.serviceCall("/LCC02020E0/getDataSet.do", $NC.getGridParams(G_GRDT1SUB2), onGetT1Sub2);
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "LCC02020E0.RS_T2_MASTER",
        sortCol: "ETC_DATE",
        gridOptions: options
    });
...
/**
 * 상품변환상세 행 클릭후 우측 그리드에 데이터 표시처리(출고지시)
 * 
 * @param ajaxData
 */
...
/**
 * 상품변환상세 행 클릭후 우측 그리드에 데이터 표시처리(입고지시)
 * 
 * @param ajaxData
 */
...
function onProcessPre() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC02020E0.025", "조회 후 처리하십시오."));
        return;
    }

    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.CONFIRM_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LCC02020E0.026", "확정된 전표만 취소 할 수 있습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LCC02020E0.027", "취소 처리 하시겠습니까?"))) {
        return;
    }
...
function onProcessNxt() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LCC02020E0.028", "확정 처리 하시겠습니까?"))) {
        return;
    }
...
function getEtcState(params, onSuccess) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LCC02020E0/getData.do", {
