 ...
 *  프로그램ID         : LCC03011P0
 *  프로그램명         : 재고이동등록 팝업
 *  프로그램설명       : 재고이동등록 팝업 화면 Javascript
...
function _Initialize() {
    ...
    $("#btnQItem_Cd").click(showQItemPopup); // 현재고 검색의 상품검색
    ...
    $NC.setInitCombo("/WC/getDataSet.do", {
        ...
        addAll: false,
        onComplete: function() {
            // 신규가 아닐 경우, 기등록된 상품상태와 동일하게 지정
            if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD != $ND.C_PROCESS_ENTRY_CREATE && G_GRDSUB.data.getLength() > 0) {
                $NC.setValue("#cboQItem_State", G_GRDSUB.data.getItem(G_GRDSUB.lastRow).ITEM_STATE);
            }
        }
    });
...
function _OnLoaded() {
    ...
    // 예정 -> 등록, 등록 수정
        ...
        NEW_ENTRY_YN = dsMaster.NEW_ENTRY_YN;
        ...
        $NC.setEnable("#cboMove_Div", false);
    ...
    $NC.setInitCombo("/WC/getDataSet.do", {
        ...
        P_QUERY_PARAMS: {
            ...
            P_COMMON_CD: $ND.C_ALL,
            P_ATTR01_CD: "1",
            P_ATTR04_CD: NEW_ENTRY_YN,
            P_VIEW_DIV: "1" // 1:등록팝업, 2:조회팝업
        }
    }, {
        selector: "#cboMove_Div",
        codeField: "COMMON_CD",
        nameField: "COMMON_NM",
        fullNameField: "COMMON_CD_F",
        addAll: false,
        onComplete: function() {
            // 신규가 아닐 경우, 기등록된 상품상태와 동일하게 지정
            if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD != $ND.C_PROCESS_ENTRY_CREATE) {
                $NC.setValue("#cboMove_Div", $NC.G_VAR.G_PARAMETER.P_MASTER_DS.MOVE_DIV);
            } else {
                $NC.G_VAR.masterData.MOVE_DIV = $NC.getValue("#cboMove_Div");
            }
        }
    });
}
    ...
    switch (id) {
        ...
        case "ZONE_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM121)) {
                alert($NC.getDisplayMsg("JS.LCC03011P0.001", "로케이션 존코드 길이(" + $NC.G_VAR.policyVal.CM121 + "자리) 를 정확히 입력하여 주십시오.",
            ...
            $NP.onZoneChange(val, {
                ...
                errorMessage: $NC.getDisplayMsg("JS.LCC03011P0.002", "선택된 상품 상태와 매칭되는 존이 검색되지 않았습니다.")
            });
            return;
        case "BANK_CD":
            if ($NC.isNull(val)) {
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM122)) {
                alert($NC.getDisplayMsg("JS.LCC03011P0.003", "로케이션 행코드 길이(" + $NC.G_VAR.policyVal.CM122 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BAY_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM123)) {
                alert($NC.getDisplayMsg("JS.LCC03011P0.004", "로케이션 열코드 길이(" + $NC.G_VAR.policyVal.CM123 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "LEV_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM124)) {
                alert($NC.getDisplayMsg("JS.LCC03011P0.005", "로케이션 단코드 길이(" + $NC.G_VAR.policyVal.CM124 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "STOCK_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC03011P0.006", "입고일자를 정확히 입력하십시오."));
            }
            break;
        case "VALID_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC03011P0.007", "유통기한를 정확히 입력하십시오."));
            }
            break;
        case "ITEM_STATE":
            if ($NC.isNotNull(val)) {
...
function _Inquiry() {
    ...
    var PALLET_ID_YN = $NC.getValue("#chkPallet_Id_Yn");
    ...
    var LOC_DIV_ATTR03_CD;
    if ($NC.G_VAR.masterData.MOVE_DIV == "13") {
        // ASS이동인 경우 일반셀만 이동 가능
        LOC_DIV_ATTR03_CD = "10";
    } else if ($NC.G_VAR.masterData.MOVE_DIV == "14") {
        // ASS역이동인 경우 ASS셀만 이동 가능
        LOC_DIV_ATTR03_CD = "21";
    } else {
        LOC_DIV_ATTR03_CD = [
            "10",
            "20"
    ...
    G_GRDDETAIL.queryParams = {
        ...
        P_PALLET_ID_YN: PALLET_ID_YN,
        P_POLICY_LS210: $NC.G_VAR.policyVal.LS210,
        P_LOC_DIV_ATTR03_CD: LOC_DIV_ATTR03_CD
    };
    // 데이터 조회
...
function _New() {
    ...
    if (checkedRows.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC03011P0.009", "추가할 재고내역 데이터를 선택하십시오."));
        return;
    }
    var MLOCATION_CD = $NC.getValue("#edtLocation_Cd");
    if ($NC.isNull(MLOCATION_CD)) {
        alert($NC.getDisplayMsg("JS.LCC03011P0.010", "재고 추가시 먼저 이동할 로케이션을 지정하십시오."));
        $NC.setFocus("#edtLocation_Cd");
        return;
    }

    // ASS이동, ASS역이동인 경우, ITEM_LOT == $ND.C_BASE_ITEM_LOT 체크
    var MOVE_DIV = $NC.getValue("#cboMove_Div");
    if (MOVE_DIV == "13" || MOVE_DIV == "14") {
        var checkedASSRows = $NC.getGridSearchRows(G_GRDDETAIL, {
            searchKey: [
                "CHECK_YN",
                "ITEM_LOT"
            ],
        ...
        if (checkedASSRows.length != checkedRows.length) {
            alert($NC.getDisplayMsg("JS.LCC03011P0.028", "LOT번호는 [*]만 가능합니다."));
            $NC.setFocus("#edtItem_Lot");
            return;
        }
    }
    ...
    var rowData, newRowData, preMStockQty, newMStockQty;
    G_GRDSUB.data.beginUpdate();
    try {
        for (var rIndex = 0, rCount = checkedRows.length; rIndex < rCount; rIndex++) {
            ...
            // 기등록 수량 체크
            preMStockQty = $NC.getGridSumVal(G_GRDSUB, {
                searchKey: searchKey,
                searchVal: [
                    $ND.C_DV_CRUD_C,
                    rowData.BRAND_CD,
            ...
            // 가용재고만큼 이동처리되어 있으면 추가하지 않음
            if (rowData.PSTOCK_QTY <= preMStockQty) {
                continue;
            }

            // 추가할 가용재고 수량 계산
            newMStockQty = rowData.PSTOCK_QTY - preMStockQty;
            ...
            newRowData = {
                ...
                MSTOCK_QTY: newMStockQty,
                MSTOCK_BOX: $NC.getBBox(newMStockQty, rowData.QTY_IN_BOX),
                MSTOCK_EA: $NC.getBEa(newMStockQty, rowData.QTY_IN_BOX),
                ITEM_NM: rowData.ITEM_NM,
                ITEM_SPEC: rowData.ITEM_SPEC,
                QTY_IN_BOX: rowData.QTY_IN_BOX,
                IN_UNIT_DIV_F: rowData.IN_UNIT_DIV_F,
    ...
    if (addedCount == 0) {
        if (checkedRows.length == sameLocCount) {
            alert($NC.getDisplayMsg("JS.LCC03011P0.011", "선택한 상품의 재고로케이션과 이동로케이션이 모두 같습니다.\n이동로케이션 변경 후 추가하십시오."));
        } else {
            alert($NC.getDisplayMsg("JS.LCC03011P0.012", "선택한 상품은 이미 추가된 재고내역입니다."));
        }
        return;
    }
    ...
    // 이동구분, 상태 콤보, 파렛트ID 기준 disable
    $NC.setEnable("#cboMove_Div", false);
    $NC.setEnable("#cboQItem_State", false);
    $NC.setEnable("#chkPallet_Id_Yn", false);
    if ($NC.G_VAR.masterData.MOVE_DIV == "13" || $NC.G_VAR.masterData.MOVE_DIV == "14") {
        $NC.setEnable("#edtLocation_Cd", false);
        $NC.setEnable("#btnLocation_Cd", false);
    }
}
...
function _Save() {
    ...
    if (G_GRDSUB.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LCC03011P0.013", "저장할 데이터가 없습니다."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC03011P0.014", "물류센터를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC03011P0.015", "사업부를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.MOVE_DATE)) {
        alert($NC.getDisplayMsg("JS.LCC03011P0.016", "이동일자를 입력하십시오."));
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_C) {
        if (dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC03011P0.017", "저장할 지시내역 정보가 없습니다."));
            return;
        }
    } else {
        if ($NC.G_VAR.masterData.CRUD != $ND.C_DV_CRUD_U && dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC03011P0.018", "수정 후 저장하십시오."));
            return;
        }
    }

    $NC.serviceCall("/LCC03010E0/save.do", {
        P_DS_MASTER: {
            P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
            P_BU_CD: $NC.G_VAR.masterData.BU_CD,
            P_MOVE_DATE: $NC.G_VAR.masterData.MOVE_DATE,
...
function _Delete() {
    ...
    if (G_GRDSUB.data.getLength() == 0 || $NC.isNull(G_GRDSUB.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC03011P0.019", "삭제할 데이터가 없습니다."));
        return;
    }
    ...
    // 이동구분, 상태 콤보, 파렛트ID 기준 enable
    if (G_GRDSUB.data.getLength() == 0 && $NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ENTRY_CREATE) {
        $NC.setEnable("#edtLocation_Cd");
        $NC.setEnable("#btnLocation_Cd");
        $NC.setEnable("#cboMove_Div");
        $NC.setEnableGroup("#divStockCondition");
        // ASS이동, ASS역이동인 경우
        if ($NC.getValue("#cboMove_Div") == "13" || $NC.getValue("#cboMove_Div") == "14") {
            $NC.setEnable("#cboQItem_State", false);
            $NC.setEnable("#edtQItem_Lot", false);
        }
    } else if (G_GRDSUB.data.getLength() == 0) {
        $NC.setEnable("#cboQItem_State");
        $NC.setEnable("#chkPallet_Id_Yn");
        $NC.setEnable("#edtLocation_Cd");
        $NC.setEnable("#btnLocation_Cd");
    }
}
    ...
    switch (args.col) {
        case "LOCATION_CD":
            $NP.onLocationChange(args.val, {
                ...
                // 1 - 일반, 2 - 유통가공, 3 - 보세
                P_ZONE_DIV_ATTR01_CD: getZoneDivAttr01Cd($NC.getValue("#cboQItem_State")),
                P_LOC_DIV_ATTR03_CD: $NC.G_VAR.masterData.MOVE_DIV == "11" ? "10" : "20"
            }, onLocationPopup, {
                errorMessage: $NC.getDisplayMsg("JS.LCC03011P0.020", "이동 가능한 로케이션이 검색되지 않았습니다.")
            });
            return;
        case "MOVE_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LCC03011P0.021", "이동일자를 정확히 입력하십시오."));
        ...
        case "REMARK1":
            ...
            break;
        case "MOVE_DIV":
            $NC.G_VAR.masterData.MOVE_DIV = args.val;
            $NC.setValue("#edtLocation_Cd");
            $NC.setValue("#edtQZone_Cd");
            $NC.setValue("#edtQZone_Nm");

            if (args.val == "13" || args.val == "14") {
                // ASS이동, ASS역이동인 경우, 상태 A, LOT [*]로 고정
                $NC.setValue("#cboQItem_State", $ND.C_BASE_ITEM_STATE);
                $NC.setValue("#edtQItem_Lot", $ND.C_BASE_ITEM_LOT);
                $NC.setEnable("#cboQItem_State", false);
                $NC.setEnable("#edtQItem_Lot", false);
            } else {
                $NC.setEnable("#cboQItem_State");
                $NC.setValue("#edtQItem_Lot");
                $NC.setEnable("#edtQItem_Lot");
            }
    ...
    var options = {
        ...
        specialRow: {
            compareFn: function(specialRow, rowData, column, row, cell, colspan) {
                // 엑셀업로드일 경우 원이동수량보다 이동수량이 작으면 색상 다르게 표시
                if (rowData.XLS_YN == $ND.C_YES && rowData.MOVE_QTY != rowData.MSTOCK_QTY) {
                    return "styDiff";
                }
            }
        }
...
function grdSubOnBeforeEditCell(e, args) {
    ...
    // 파렛트ID 기준으로 조회 시 수량 변경 제한
    if (rowData.ITEM_STATE == "L") {
        switch (args.column.id) {
            case "MSTOCK_QTY":
            case "MSTOCK_BOX":
            case "MSTOCK_EA":
...
function grdSubOnCellChange(e, args) {
    ...
    switch (columnId) {
        ...
        case "MSTOCK_BOX":
        case "MSTOCK_EA":
            rowData = grdSubOnCalc(rowData, columnId);
            break;
        case "MLOCATION_CD":
...
function validateSubData(row, rowData) {
    ...
    if ($NC.isNull(rowData.MSTOCK_QTY) || Number(rowData.MSTOCK_QTY) <= 0) {
        alert($NC.getDisplayMsg("JS.LCC03011P0.029", "이동수량에 0보다 큰 수량을 입력하십시오."));
        $NC.setFocusGrid(G_GRDSUB, row, "MSTOCK_QTY", true);
        return false;
    }

    if (rowData.CRUD == $ND.C_DV_CRUD_C) {
        // 재고 중복 등록 가능으로 합계로 체크
        var sumMStockQty = $NC.getGridSumVal(G_GRDSUB, {
            searchKey: [
                "CRUD",
                "BRAND_CD",
                "ITEM_CD",
        ...
        if (Number(rowData.PSTOCK_QTY) < sumMStockQty) {
            alert($NC.getDisplayMsg("JS.LCC03011P0.022", "재고수량 보다 많은 수량을 입력하실 수 없습니다."));
            $NC.setFocusGrid(G_GRDSUB, row, "MSTOCK_QTY", true);
            return false;
        }

        if ($NC.isNull(rowData.MLOCATION_CD)) {
            alert($NC.getDisplayMsg("JS.LCC03011P0.023", "이동할 로케이션을 선택하십시오."));
            $NC.setFocusGrid(G_GRDSUB, row, "MLOCATION_CD", true);
            return false;
        }

        if (rowData.LOCATION_CD == rowData.MLOCATION_CD) {
            alert($NC.getDisplayMsg("JS.LCC03011P0.024", "현재 로케이션과 이동할 로케이션이 같습니다."));
            $NC.setFocusGrid(G_GRDSUB, row, "MLOCATION_CD", true);
            return false;
        }
    }
...
function btnDownloadXLSFormatOnClick() {
    ...
    COLUMN_INFO.push({
        P_COLUMN_NM: "MLOCATION_CD",
        P_COLUMN_TITLE: "이동로케이션",
        P_COLUMN_WIDTH: 15
    });
    COLUMN_INFO.push({
        P_COLUMN_NM: "MOVE_QTY",
        P_COLUMN_TITLE: "이동수량",
        P_COLUMN_WIDTH: 10
    });
    COLUMN_INFO.push({
    ...
    $NC.excelFileDownload({
        ...
        P_SERVICE_PARAMS: {
            P_XLS_SHEET_NAME: "재고이동업로드", // Excel Sheet Title
...
function btnUploadXLSOnClick() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.MOVE_DIV)) {
        alert($NC.getDisplayMsg("JS.LCC03011P0.030", "이동구분을 입력하십시오."));
        $NC.setFocus("#cboMove_Div");
        return;
    }

    if ($NC.isNotNull($NC.G_VAR.masterData.MOVE_NO)) {
        alert($NC.getDisplayMsg("JS.LCC03011P0.025", "엑셀 업로드는 기존 데이터에 추가할 수 없습니다. 저장 후 신규로 처리하십시오."));
        return;
    }

    if (G_GRDSUB.data.getLength() > 0) {
        if (!confirm($NC.getDisplayMsg("JS.LCC03011P0.026", "엑셀 업로드시 기존 내역 삭제 후 추가됩니다. 엑셀 업로드하시겠습니까?"))) {
            return;
        }
    }
    ...
    $NC.excelFileUpload({
        ...
        if ($NC.isNull(dsResultData) || dsResultData.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC03011P0.027", "업로드할 수 있는 대상 데이터가 없습니다. 엑셀 파일을 확인하십시오."));
            return;
        }
        // 엑셀 정상 업로드가 될 경우 재고검색 및 그리드 초기화
        $NC.clearGridData(G_GRDDETAIL);
        $NC.setEnableGroup("#divStockCondition", false);
        $NC.setEnable("#cboMove_Div", false);
        $NC.setEnable("#edtLocation_Cd", false);
        $NC.setEnable("#btnLocation_Cd", false);
        ...
        $NC.setInitGridData(G_GRDSUB, dsResultData);
        $NC.setGridSelectRow(G_GRDSUB, 0);
    });
}
...
function grdSubOnCalc(rowData, changedColumnId) {
    ...
    switch (changedColumnId) {
        case "MSTOCK_BOX":
        case "MSTOCK_EA":
            rowData.MSTOCK_QTY = $NC.getBQty(rowData.MSTOCK_BOX, rowData.MSTOCK_EA, rowData.QTY_IN_BOX);
            break;
    }
