...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            if ($NC.isNull(BU_CD)) {
                alert($NC.getDisplayMsg("JS.LCC02090E0.024", "사업부를 먼저 선택 하십시오."));
...
function _Save() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        var rowData, rIndex, rCount, refRowData;
        // 가공작업(마스터) 데이터
        var dsMaster = [];
        ...
        if (dsMaster.length > 0 || dsDetail.length > 0) {
            for (rIndex = 0, rCount = G_GRDT1MASTER.data.getLength(); rIndex < rCount; rIndex++) {
                refRowData = G_GRDT1MASTER.data.getItem(rIndex);
                // 디테일은 포커스 되어 있는 ROW만 수정 가능
                if (refRowData.PROCESSING_DATE == rowData.PROCESSING_DATE && refRowData.PROCESSING_NO == rowData.PROCESSING_NO) {
                    // 마스터 데이터와 현재 DETAIL 데이터가 변경되었는지 체크
                    if (refRowData.CRUD == $ND.C_DV_CRUD_R && !$NC.isGridModified(G_GRDT1DETAIL)) {
                        continue;
                    }
                } else {
                    // 포커스 그리드 제외 ROW, 수정여부 체크
        ...
        if ($NC.G_VAR.updateQty) {
            ...
            $NC.serviceCall("/LCC02090E0/updateRemark.do", {
                P_DS_MASTER: dsMaster,
                P_DS_DETAIL: dsDetail,
                P_USER_ID: $NC.G_USERINFO.USER_ID
            }, onSave);
...
function _Delete() {
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        if (rowData.CONFIRM_YN == $ND.C_YES) {
            alert($NC.getDisplayMsg("JS.LCC02090E0.010", "이미 확정한 데이터는 삭제할 수 없습니다."));
            return;
        }
        ...
        $NC.serviceCall("/LCC02090E0/callLCBwProcessingEntry.do", {
            P_CENTER_CD: rowData.CENTER_CD,
            P_BU_CD: rowData.BU_CD,
            P_PROCESSING_DATE: rowData.PROCESSING_DATE,
            P_PROCESSING_NO: rowData.PROCESSING_NO,
...
function _Print(reportInfo) {
    ...
    if (activeTab == 0) {
        ...
        if ($NC.isNull(rowData)) {
            alert($NC.getDisplayMsg("JS.LCC02090E0.025", "출력할 데이터를 먼저 선택하십시오."));
            return;
        }
    }
...
/**
 * 등록 탭에서 저장에 성공했을 경우의 처리
 * 
 * @param ajaxData
 */
function onResultSave(ajaxData) {
...
function grdT1MasterOnFocus(e, args) {
    ...
    // 셀구분별 수수료 Post 처리
    if (!$NC.isGridValidLastRow(G_GRDT1DETAIL)) {
        $NC.setFocusGrid(G_GRDT1DETAIL, G_GRDT1DETAIL.lastRow, null, true);
    }
}
...
function grdT1MasterOnDblClick(e, args) {
    ...
    if (refRowData.CONFIRM_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LCC02090E0.022", "확정 된 전표입니다."));
        return;
    }

    // 조회후 상태가 바뀌었는지 한번더 상태 체크
    $NC.serviceCall("/LCC02090E0/getConfirmYn.do", {
        P_CENTER_CD: refRowData.CENTER_CD,
        P_BU_CD: refRowData.BU_CD,
        P_ETC_DATE: refRowData.PROCESSING_DATE,
        P_ETC_NO: refRowData.PROCESSING_NO,
        P_TABLE_DIV: "E" // 테이블구분([A]기타입출고, [B]재고이동, [C]재고실사, [E]가공작업)
    }, //
    // ServiceCall SuccessCallback
    function(ajaxDataState) {
        var resultData = $NC.toObject(ajaxDataState);
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LCC02090E0.026", "확정여부를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        var oMsg = $NC.getOutMessage(resultData);
        if (oMsg != $ND.C_OK) {
        ...
        if (resultData.O_CONFIRM_YN != $ND.C_NO) {
            alert($NC.getDisplayMsg("JS.LCC02090E0.027", "이미 확정처리된 데이터입니다.\n다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function onGetT1Detail(ajaxData) {
    ...
    $NC.setInitGridAfterOpen(G_GRDT1DETAIL, "LINE_NO", G_GRDT1DETAIL.focused);
}
...
function onProcessPre() {
    ...
    $NC.serviceCall("/LCC02090E0/callLCProcessing.do", {
        P_DS_MASTER: {
            P_CENTER_CD: rowData.CENTER_CD,
            P_BU_CD: rowData.BU_CD,
            P_PROCESSING_DATE: rowData.PROCESSING_DATE,
