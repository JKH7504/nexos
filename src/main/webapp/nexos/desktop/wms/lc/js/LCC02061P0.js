 ...
 *  프로그램ID         : LCC02061P0
 *  프로그램명         : 품질검사등록 팝업
 *  프로그램설명       : 품질검사등록 팝업 화면 Javascript
    ...
    switch (id) {
        ...
        case "ZONE_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM121)) {
                alert($NC.getDisplayMsg("JS.LCC02061P0.001", "로케이션 존코드 길이(" + $NC.G_VAR.policyVal.CM121 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BANK_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM122)) {
                alert($NC.getDisplayMsg("JS.LCC02061P0.002", "로케이션 행코드 길이(" + $NC.G_VAR.policyVal.CM122 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BAY_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM123)) {
                alert($NC.getDisplayMsg("JS.LCC02061P0.003", "로케이션 열코드 길이(" + $NC.G_VAR.policyVal.CM123 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "LEV_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM124)) {
                alert($NC.getDisplayMsg("JS.LCC02061P0.004", "로케이션 단코드 길이(" + $NC.G_VAR.policyVal.CM124 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "STOCK_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02061P0.005", "입고일자를 정확히 입력하십시오."));
            }
            break;
        case "VALID_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02061P0.006", "유통기한를 정확히 입력하십시오."));
            }
            break;
    }
}
...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/LCC02060E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
}
...
function _New() {
    ...
    if (checkedRows.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC02061P0.007", "추가할 재고내역 데이터를 선택하십시오."));
        return;
    }
    ...
    if (G_GRDDETAIL.data.getItem(checkedRows[0]).ITEM_STATE == LINK_ITEM_STATE) {
        alert($NC.getDisplayMsg("JS.LCC02061P0.008", "같은 상태로는 변환 처리 할 수 없습니다."));
        return;
    }
    ...
    if (addedCount == 0) {
        alert($NC.getDisplayMsg("JS.LCC02061P0.009", "선택한 상품은 이미 추가된 재고내역입니다."));
        return;
    }
...
function _Save() {
    ...
    if (G_GRDSUB.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LCC02061P0.010", "저장할 데이터가 없습니다."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02061P0.011", "물류센터를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02061P0.012", "사업부를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02061P0.013", "먼저 입출고구분을 선택하십시오."));
        $NC.setFocus("#cboInout_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ETC_DATE)) {
        alert($NC.getDisplayMsg("JS.LCC02061P0.014", "먼저 입출고일자를 입력하십시오."));
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_C) {
        if (dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC02061P0.015", "저장할 지시내역 정보가 없습니다."));
            return;
        }
    } else {
        if ($NC.G_VAR.masterData.CRUD != $ND.C_DV_CRUD_U && dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC02061P0.016", "수정 후 저장하십시오."));
            return;
        }
    }

    $NC.serviceCall("/LCC02060E0/save.do", {
        P_DS_MASTER: {
            P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
            P_BU_CD: $NC.G_VAR.masterData.BU_CD,
            P_ETC_DATE: $NC.G_VAR.masterData.ETC_DATE,
...
function _Delete() {
    ...
    if (G_GRDSUB.data.getLength() == 0 || $NC.isNull(G_GRDSUB.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC02061P0.017", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        case "LOCATION_CD":
            ...
            return;
        case "ETC_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LCC02061P0.018", "입출고일자를 정확히 입력하십시오."));
    ...
    $NC.setGridColumn(columns, {
        ...
        editorOptions: $NC.getGridComboEditorOptions("/WC/getDataSet.do", {
            ...
            P_QUERY_PARAMS: {
                ...
                P_COMMON_CD: $ND.C_ALL,
                P_ATTR07_CD: $ND.C_YES, // Y - 품질검사사유여부
                P_VIEW_DIV: "1" // 1:등록팝업, 2:조회팝업
            }
        }, {
            codeField: "ETC_DIV",
...
function grdSubInitialize() {
    ...
    $NC.setInitGridObject("#grdSub", {
        ...
        queryId: "LCC02060E0.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options,
    ...
    switch (G_GRDSUB.view.getColumnId(args.cell)) {
        ...
        case "CONFIRM_QTY":
            if ($NC.isNull(rowData.CONFIRM_QTY) || Number(rowData.CONFIRM_QTY) < 0) {
                alert($NC.getDisplayMsg("JS.LCC02061P0.019", "변환수량을 정확히 입력하십시오."));
                rowData.CONFIRM_QTY = 0;
                $NC.setFocusGrid(G_GRDSUB, args.row, "CONFIRM_QTY", true);
            }
            break;
    }
...
function validateSubData(row, rowData) {
    ...
    if ($NC.isNull(rowData.CONFIRM_QTY) || Number(rowData.CONFIRM_QTY) < 0) {
        alert($NC.getDisplayMsg("JS.LCC02061P0.020", "변환수량에 0보다 같거나 큰 수량을 입력하십시오."));
    ...
    if (rowData.CRUD == $ND.C_DV_CRUD_C && Number(rowData.PSTOCK_QTY) < Number(rowData.CONFIRM_QTY)) {
        alert($NC.getDisplayMsg("JS.LCC02061P0.021", "재고수량 보다 많은 수량을 입력하실 수 없습니다."));
        $NC.setFocusGrid(G_GRDSUB, row, "CONFIRM_QTY", true);
        return false;
    }
    if (rowData.CRUD == $ND.C_DV_CRUD_C && $NC.isNull(rowData.LINK_LOCATION_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02061P0.022", "이동로케이션을 입력하십시오."));
        $NC.setFocusGrid(G_GRDSUB, row, "LINK_LOCATION_CD", true);
        return false;
    }
    if ($NC.isNull(rowData.ETC_DIV)) {
        alert($NC.getDisplayMsg("JS.LCC02061P0.023", "사유구분을 선택하십시오."));
