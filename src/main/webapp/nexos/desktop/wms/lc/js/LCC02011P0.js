 ...
 *  프로그램ID         : LCC02011P0
 *  프로그램명         : 상태변환등록 팝업
 *  프로그램설명       : 상태변환등록 팝업 화면 Javascript
...
function _Initialize() {
    ...
    $("#btnQBrand_Cd").click(showBuBrandPopup); // 현재고 검색의 브랜드검색 버튼 클릭
    ...
    $NC.serviceCall("/WC/getMultiDataSet.do", {
        ...
        $NC.setInitComboData({
            ...
            onComplete: function() {
                ...
                if ($NC.G_VAR.ORDER_YN == $ND.C_YES && G_GRDDETAIL.data.getLength() > 0) {
        ...
        $NC.setInitComboData({
            ...
            onComplete: function() {
                ...
                if ($NC.G_VAR.ORDER_YN == $ND.C_YES && G_GRDDETAIL.data.getLength() > 0) {
                    var rowData = G_GRDDETAIL.data.getItem(G_GRDDETAIL.lastRow);
                    $NC.setValue("#cboConvert_Item_State", rowData.LINK_ITEM_STATE);
                }
            }
        });
    });
...
function _OnLoaded() {
    ...
    // 예정으로 등록
        ...
        $NC.setEnable("#cboConvert_Item_State", false);
    ...
    switch (id) {
        ...
        case "ZONE_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM121)) {
                alert($NC.getDisplayMsg("JS.LCC02011P0.001", "로케이션 존코드 길이(" + $NC.G_VAR.policyVal.CM121 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BANK_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM122)) {
                alert($NC.getDisplayMsg("JS.LCC02011P0.002", "로케이션 행코드 길이(" + $NC.G_VAR.policyVal.CM122 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "BAY_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM123)) {
                alert($NC.getDisplayMsg("JS.LCC02011P0.003", "로케이션 열코드 길이(" + $NC.G_VAR.policyVal.CM123 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "LEV_CD":
            ...
            if (val.length != Number($NC.G_VAR.policyVal.CM124)) {
                alert($NC.getDisplayMsg("JS.LCC02011P0.004", "로케이션 단코드 길이(" + $NC.G_VAR.policyVal.CM124 + "자리) 를 정확히 입력하여 주십시오.",
        ...
        case "STOCK_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02011P0.005", "입고일자를 정확히 입력하십시오."));
            }
            break;
        case "VALID_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02011P0.006", "유통기한를 정확히 입력하십시오."));
            }
            break;
    }
}
...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/LCC02010E0/getDataSet.do", $NC.getGridParams(G_GRDSTOCK), onGetDetail);
}
...
function _New() {
    ...
    if (checkedRows.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.008", "추가할 재고내역 데이터를 선택하십시오."));
        return;
    }

    // 상품상태 == 변환상태 일 경우 에러
    var LINK_ITEM_STATE = $NC.getValue("#cboConvert_Item_State");
    var LINK_ITEM_STATE_F = $NC.getValueCombo("#cboConvert_Item_State", "F");
    if (G_GRDSTOCK.data.getItem(checkedRows[0]).ITEM_STATE == LINK_ITEM_STATE) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.009", "같은 상태로는 변환 처리 할 수 없습니다."));
        return;
    }
    ...
    if ($NC.G_VAR.ORDER_YN == $ND.C_YES) {
        ...
        if (remainQty < 1) {
            alert($NC.getDisplayMsg("JS.LCC02011P0.010", "이미 지시내역이 모두 추가되어 있습니다.\n\n기존 지시내역을 삭제하거나 확정수량 조정 후 추가하십시오."));
            return;
        }
    }
    ...
    try {
        for (var rIndex = 0, rCount = checkedRows.length; rIndex < rCount; rIndex++) {
            ...
            newRowData = {
                ...
                LINK_ITEM_STATE: LINK_ITEM_STATE,
                LINK_ITEM_STATE_F: LINK_ITEM_STATE_F,
                DRUG_DIV_D: rowData.DRUG_DIV_D,
                MEDICATION_DIV_D: rowData.MEDICATION_DIV_D,
                KEEP_DIV_D: rowData.KEEP_DIV_D,
                DRUG_CD: rowData.DRUG_CD,
    ...
    if (addedCount == 0) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.011", "선택한 상품은 이미 추가된 재고내역입니다."));
        return;
    }
...
function _Save() {
    ...
    if (G_GRDSUB.data.getItems().length == 0) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.012", "저장할 데이터가 없습니다."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.013", "물류센터를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.014", "사업부를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.015", "먼저 입출고구분을 선택하십시오."));
        $NC.setFocus("#cboInout_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ETC_DATE)) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.016", "먼저 입출고일자를 입력하십시오."));
    ...
    if ($NC.G_VAR.ORDER_YN == $ND.C_YES) {
        ...
        if (checkedData.checkedCount == 0) {
            alert($NC.getDisplayMsg("JS.LCC02011P0.017", "저장할 지시내역 정보가 없습니다."));
            return;
        }
    ...
    // 신규 등록/수정
        ...
        if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_C) {
            ...
            if ($NC.G_VAR.masterData.CRUD != $ND.C_DV_CRUD_U && dsSub.length == 0) {
                alert($NC.getDisplayMsg("JS.LCC02011P0.018", "수정 후 저장하십시오."));
                return;
            }
        }

        $NC.serviceCall("/LCC02010E0/save.do", {
            P_DS_MASTER: {
                P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
                P_BU_CD: $NC.G_VAR.masterData.BU_CD,
                P_ETC_DATE: $NC.G_VAR.masterData.ETC_DATE,
...
function _Delete() {
    ...
    if (G_GRDSUB.data.getLength() == 0 || $NC.isNull(G_GRDSUB.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.019", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        case "LOCATION_CD":
            ...
            return;
        case "ETC_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LCC02011P0.020", "입출고일자를 정확히 입력하십시오."));
...
function grdDetailOnBeforeEditCell(e, args) {
    ...
    /*
     * var rowData = args.item; // 신규일 경우 모두 수정 가능, 그외 아래 컬럼만 수정 가능 if (rowData.CRUD == $ND.C_DV_CRUD_R || rowData.CRUD == $ND.C_DV_CRUD_U) { switch
     * (args.column.id) { case "ETC_DIV_F": case "ETC_COMMENT": return true; default: return false; } }
     */
    return true;
}
...
function grdDetailOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if (rowData.ORDER_QTY < rowData.ENTRY_QTY) {
            alert($NC.getDisplayMsg("JS.LCC02011P0.021", "등록수량이 예정수량보다 클 수 없습니다."));
    ...
    if ($NC.isNull(rowData.ETC_DIV)) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.022", "사유구분을 선택하십시오."));
...
function grdSubOnSetColumns() {
    ...
    $NC.setGridColumns(G_GRDSUB, [ // 숨김컬럼 세팅
        $NC.G_VAR.policyVal.LS210 != "2" ? "VALID_DATE,BATCH_NO" : "",
        $NC.G_VAR.ORDER_YN == $ND.C_YES ? "ITEM_CD,ITEM_BAR_CD,ITEM_NM,ITEM_SPEC,BRAND_NM,ITEM_STATE_F," //
            + "LINK_ITEM_STATE_F,ITEM_LOT,QTY_IN_BOX,ETC_DIV_F,ETC_COMMENT" : ""
    ]);
}
    ...
    var options = {
        ...
        specialRow: {
            compareFn: function(specialRow, rowData, column, row, cell, colspan) {
                // 엑셀업로드일 경우 원변환수량보다 확정수량과 다르면 색상 다르게 표시
                if (rowData.XLS_YN == $ND.C_YES && rowData.PRE_CONFIRM_QTY != rowData.CONFIRM_QTY) {
                    return "styDiff";
                }
            }
        }
...
function validateSubData(row, rowData) {
    ...
    if ($NC.isNull(rowData.CONFIRM_QTY) || Number(rowData.CONFIRM_QTY) <= 0) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.023", "변환수량에 0보다 큰 수량을 입력하십시오."));
    ...
    if (rowData.CRUD == $ND.C_DV_CRUD_C) {
        ...
        if ($NC.G_VAR.ORDER_YN == $ND.C_YES) {
            ...
            if (rowData.PSTOCK_QTY < totConfirmQty) {
                alert($NC.getDisplayMsg("JS.LCC02011P0.024", "재고수량 보다 많은 수량을 입력하실 수 없습니다."));
                $NC.setFocusGrid(G_GRDSUB, row, "CONFIRM_QTY", true);
                return false;
            }
        }
        ...
        if ($NC.isNull(rowData.LINK_LOCATION_CD)) {
            alert($NC.getDisplayMsg("JS.LCC02011P0.025", "이동로케이션을 입력하십시오."));
...
function btnDownloadXLSFormatOnClick() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.026", "물류센터를 입력하십시오."));
        return;
    }
    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.027", "사업부를 입력하십시오."));
        return;
    }
    ...
    var COLUMN_INFO = [
        ...
        {
            P_COLUMN_NM: "LINK_ITEM_STATE",
            P_COLUMN_TITLE: "변환상품상태",
            P_COLUMN_WIDTH: 10
        },
        {
    ...
    COLUMN_INFO.push({
        P_COLUMN_NM: "LINK_LOCATION_CD",
        P_COLUMN_TITLE: "이동로케이션",
        P_COLUMN_WIDTH: 15
    });
    COLUMN_INFO.push({
        P_COLUMN_NM: "CONFIRM_QTY",
        P_COLUMN_TITLE: "변경수량",
        P_COLUMN_WIDTH: 10
    });
    COLUMN_INFO.push({
    ...
    $NC.excelFileDownload({
        ...
        P_SERVICE_PARAMS: {
            P_XLS_SHEET_NAME: "상태변환업로드", // Excel Sheet Title
...
function btnUploadXLSOnClick() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.028", "물류센터를 입력하십시오."));
        return;
    }
    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.029", "사업부를 입력하십시오."));
        return;
    }
    if ($NC.isNull($NC.G_VAR.masterData.ETC_DATE)) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.030", "입출고일자를 입력하십시오."));
        $NC.setFocus("#dtpEtc_Date");
        return;
    }
    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.031", "입출고구분을 입력하십시오."));
        $NC.setFocus("#cboInout_Cd");
        return;
    }

    if ($NC.isNotNull($NC.G_VAR.masterData.ETC_NO)) {
        alert($NC.getDisplayMsg("JS.LCC02011P0.032", "엑셀 업로드는 기존 데이터에 추가할 수 없습니다. 저장 후 신규로 처리하십시오."));
        return;
    }

    if (G_GRDSUB.data.getLength() > 0) {
        if (!confirm($NC.getDisplayMsg("JS.LCC02011P0.033", "엑셀 업로드시 기존 내역 삭제 후 추가됩니다. 엑셀 업로드하시겠습니까?"))) {
            return;
        }
    }
    ...
    $NC.excelFileUpload({
        ...
        P_SERVICE_PARAMS: {
            ...
            P_XLS_FIRST_ROW: 3,
            P_XLS_FILE_DIV: "ITEM_STATE"
        },
        P_COLUMN_INFO: COLUMN_INFO
    }, function(ajaxData, dsResultData) {

        if ($NC.isNull(dsResultData) || dsResultData.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC02011P0.034", "업로드할 수 있는 대상 데이터가 없습니다. 엑셀 파일을 확인하십시오."));
            return;
        }
        ...
        $NC.setEnable("#btnLocation", false);
        $NC.setEnable("#cboConvert_Item_State", false);
