 ...
 *  프로그램ID         : LCC02051P0
 *  프로그램명         : 소분변환등록 팝업
 *  프로그램설명       : 소분변환등록 팝업 화면 Javascript
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "STOCK_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02051P0.001", "입고일자를 정확히 입력하십시오."));
            }
            break;
        case "VALID_DATE":
            if ($NC.isNotNull(val)) {
                $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LCC02051P0.002", "유통기한를 정확히 입력하십시오."));
            }
            break;
    }
}
...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/LCC02050E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
}
...
function _New() {
    ...
    if (checkedRows.length == 0) {
        alert($NC.getDisplayMsg("JS.LCC02051P0.004", "추가할 재고내역 데이터를 선택하십시오."));
        return;
    }
    ...
    try {
        for (var rIndex = 0, rCount = checkedRows.length; rIndex < rCount; rIndex++) {
            ...
            newRowData = {
                ...
                LINK_ITEM_SPEC: "",
                LINK_ITEM_LOT: rowData.ITEM_LOT,
                LINK_QTY_IN_BOX: 1,
                id: $NC.getGridNewRowId(),
                CRUD: $ND.C_DV_CRUD_C
            };
    ...
    if (addedCount == 0) {
        alert($NC.getDisplayMsg("JS.LCC02051P0.005", "선택한 상품은 이미 추가된 재고내역입니다."));
        return;
    }
...
function _Save() {
    ...
    if (G_GRDSUB.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LCC02051P0.006", "저장할 데이터가 없습니다."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02051P0.007", "물류센터를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02051P0.008", "사업부를 입력하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LCC02051P0.009", "먼저 입출고구분을 선택하십시오."));
        $NC.setFocus("#cboInout_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ETC_DATE)) {
        alert($NC.getDisplayMsg("JS.LCC02051P0.010", "먼저 입출고일자를 입력하십시오."));
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_C) {
        if (dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC02051P0.011", "저장할 지시내역 정보가 없습니다."));
            return;
        }
    } else {
        if ($NC.G_VAR.masterData.CRUD != $ND.C_DV_CRUD_U && dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LCC02051P0.012", "수정 후 저장하십시오."));
            return;
        }
    }

    $NC.serviceCall("/LCC02050E0/save.do", {
        P_DS_MASTER: {
            P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
            P_BU_CD: $NC.G_VAR.masterData.BU_CD,
            P_ETC_DATE: $NC.G_VAR.masterData.ETC_DATE,
...
function _Delete() {
    ...
    if (G_GRDSUB.data.getLength() == 0 || $NC.isNull(G_GRDSUB.lastRow)) {
        alert($NC.getDisplayMsg("JS.LCC02051P0.013", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        case "LOCATION_CD":
            ...
            return;
        case "ETC_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LCC02051P0.014", "입출고일자를 정확히 입력하십시오."));
    ...
    $NC.setGridColumn(columns, {
        ...
        editorOptions: $NC.getGridComboEditorOptions("/WC/getDataSet.do", {
            ...
            P_QUERY_PARAMS: {
                ...
                P_COMMON_CD: $ND.C_ALL,
                P_ATTR06_CD: $ND.C_YES, // Y - 소분변환사유여부
                P_VIEW_DIV: "1" // 1:등록팝업, 2:조회팝업
            }
        }, {
            codeField: "ETC_DIV",
...
function validateSubData(row, rowData) {
    ...
    if ($NC.isNull(rowData.CONFIRM_QTY) || Number(rowData.CONFIRM_QTY) <= 0) {
        alert($NC.getDisplayMsg("JS.LCC02051P0.015", "변환수량에 0보다 큰 수량을 입력하십시오."));
