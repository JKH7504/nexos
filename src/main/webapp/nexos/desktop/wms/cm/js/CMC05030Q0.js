 ...
 *  프로그램ID         : CMC05030Q0
 *  프로그램명         : 고정로케이션라벨출력
 *  프로그램설명       : 고정로케이션라벨출력 화면 Javascript
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.CMC05030Q0.001", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_NM)) {
        alert($NC.getDisplayMsg("JS.CMC05030Q0.002", "사업부를 입력하십시오."));
    ...
    if ($NC.isNull(BANK_CD1)) {
        ...
        if (!$NC.equals(BANK_CD1.length, Number($NC.G_VAR.policyVal.CM122))) {
            alert($NC.getDisplayMsg("JS.CMC05030Q0.003", "시작 행코드 길이(" + $NC.G_VAR.policyVal.CM122 + "자리)를 정확히 입력하여 주십시오.", $NC.G_VAR.policyVal.CM122));
    ...
    if ($NC.isNull(BANK_CD2)) {
        ...
        if (!$NC.equals(BANK_CD2.length, Number($NC.G_VAR.policyVal.CM122))) {
            alert($NC.getDisplayMsg("JS.CMC05030Q0.004", "종료 행코드 길이(" + $NC.G_VAR.policyVal.CM122 + "자리)를 정확히 입력하여 주십시오.", $NC.G_VAR.policyVal.CM122));
    ...
    if (BANK_CD1 > BANK_CD2) {
        alert($NC.getDisplayMsg("JS.CMC05030Q0.005", "행 첫번째 값이 두번째 값보다 클 수 없습니다."));
        $NC.setFocus("#edtQBank_Cd1");
        return;
    }
    ...
    // 데이터 조회
    $NC.serviceCall("/CMC05030Q0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
