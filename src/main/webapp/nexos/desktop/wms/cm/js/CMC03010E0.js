...
function _Initialize() {
    ...
    $NC.serviceCall("/WC/getMultiDataSet.do", {
        P_SERVICE_PARAMS: [
            {
                P_RESULT_ID: "O_WC_POP_CMCODE_CUST_DIV",
                P_QUERY_ID: "WC.POP_CMCODE",
                P_QUERY_PARAMS: {
                    P_COMMON_GRP: "CUST_DIV",
        ...
        $NC.setInitComboData({
            ...
            addEmpty: true,
            data: $NC.toArray(multipleData.O_WC_POP_CMCODE_CUST_DIV),
            onComplete: function() {
                $NC.setValue("#cboCust_Div");
            }
        });
    });
}
...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/CMC03010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _New() {
    ...
    var newRowData = {
        ...
        CUST_FULL_NM: null,
        CUST_DIV: "1",
        CUST_DIV_F: $NC.getValueCombo("#cboCust_Div", {
            searchVal: "1",
            returnVal: "F"
        }),
        BUSINESS_NO: null,
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.CMC03010E0.001", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.CMC03010E0.002", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/CMC03010E0/save.do", {
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.CMC03010E0.003", "삭제할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.CMC03010E0.004", "삭제 하시겠습니까?"))) {
        return;
    }
...
function grdMasterOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CUST_CD)) {
            alert($NC.getDisplayMsg("JS.CMC03010E0.005", "고객사코드를 입력하십시오."));
            $NC.setFocus("#edtCust_Cd");
            $NC.setGridSelectRow(G_GRDMASTER, row);
            return false;
        }
        if ($NC.isNull(rowData.CUST_NM)) {
            alert($NC.getDisplayMsg("JS.CMC03010E0.006", "고객사명을 입력하십시오."));
            $NC.setFocus("#edtCust_Nm");
            $NC.setGridSelectRow(G_GRDMASTER, row);
            return false;
        }
        if ($NC.isNull(rowData.DEAL_DIV)) {
            alert($NC.getDisplayMsg("JS.CMC03010E0.008", "거래구분을 선택하십시오."));
        ...
        if ($NC.isNotNull(rowData.OPEN_DATE) && $NC.isNotNull(rowData.CLOSE_DATE)) {
            if (rowData.CLOSE_DATE < rowData.OPEN_DATE) {
                alert($NC.getDisplayMsg("JS.CMC03010E0.009", "거래일자와 종료일자의 범위 입력오류입니다."));
                $NC.setGridSelectRow(G_GRDMASTER, row);
...
function grdMasterOnCellChange(e, args) {
    ...
    switch (args.col) {
        ...
        case "CUST_DIV":
            ...
            rowData.CUST_DIV_F = $NC.getValueCombo(args.view, "F");
            break;
        case "BUSINESS_NO":
        ...
        case "OPEN_DATE":
            if ($NC.isNotNull(args.val)) {
                $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.CMC03010E0.010", "거래일자를 정확히 입력하십시오."), "N");
            }
            rowData.OPEN_DATE = $NC.getValue(args.view);
            break;
        case "CLOSE_DATE":
            if ($NC.isNotNull(args.val)) {
                $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.CMC03010E0.011", "종료일자를 정확히 입력하십시오."), "N");
            }
...
function btnBasicMasterOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.CMC03010E0.012", "생성할 데이터가 없습니다."));
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    $NC.serviceCall("/CMC03010E0/callBasicMasterCreate.do", {
        P_CUST_CD: rowData.CUST_CD,
        P_USER_ID: $NC.G_USERINFO.USER_ID
    }, function() {
        alert($NC.getDisplayMsg("JS.CMC03010E0.013", "기본 마스터가 생성되었습니다."));
    });
}
