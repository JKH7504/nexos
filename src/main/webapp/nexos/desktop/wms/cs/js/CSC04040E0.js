 ...
 *  프로그램ID         : CSC04040E0
 *  프로그램명         : 물류센터사업부정책관리
 *  프로그램설명       : 물류센터사업부정책관리 화면 Javascript
...
function _Initialize() {
    ...
    $NC.setEnable("#btnCopyCenterBuPolicy", false);

    // 이벤트 연결
    $("#btnQBu_Cd").click(showUserBuPopup);
    $("#btnCopyCenterBuPolicy").click(btnCopyCenterBuPolicyOnClick);
        ...
        case "SELECT_YN":
            ...
            if (refRowData.SELECT_YN == $ND.C_YES) {
                ...
                $NC.G_VAR.recommendParams = {
                    P_POLICY_DIV: "4", // 물류센터사업부정책관리 화면은 4로 입력
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.CSC04040E0.001", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.CSC04040E0.002", "사업부를 입력하십시오."));
    ...
    // 데이터 조회
    $NC.serviceCall("/CSC04040E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
    ...
    // 데이터 조회
    $NC.serviceCall("/CSC04040E0/getDataSet.do", $NC.getGridParams(G_GRDSUB), onGetSub);
}
...
function _Save() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.CSC04040E0.003", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (selectedCount > 1) {
        alert($NC.getDisplayMsg("JS.CSC04040E0.005", "정책 적용값은 하나만 선택하여야 합니다."));
        return;
    }

    var dsMaster = dsD.concat(dsC);
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.CSC04040E0.004", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/CSC04040E0/save.do", {
...
function grdMasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/CSC04040E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
...
function onGetMaster(ajaxData) {
    ...
    if (G_GRDMASTER.data.getLength() > 0) {
        $NC.setEnable("#btnCopyCenterBuPolicy");
    }
    // 공통 버튼 활성화 처리
 ...
 * 물류센터사업부정책복사 버튼 호출.
 */
function btnCopyCenterBuPolicyOnClick() {
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "CSC04041P0",
        PROGRAM_NM: $NC.getDisplayMsg("JS.CSC04041P0.001", "물류센터사업부정책 복사"),
        url: "cs/CSC04041P0.html",
        width: 610,
        height: 210,
        resizeable: false,
        G_PARAMETER: {
