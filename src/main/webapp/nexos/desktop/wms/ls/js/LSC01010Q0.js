...
function _Initialize() {
    ...
    $NC.G_VAR.printOptions = [
        {
            PRINT_INDEX: 1,
            PRINT_COMMENT: $NC.getDisplayMsg("JS.LSC01010Q0.001", "파렛트 ID")
        }
    ];
        ...
        case "BANK_CD":
            if (!$NC.isNull(val)) {
                if (val.length != Number($NC.G_VAR.policyVal.CM122)) {
                    alert($NC.getDisplayMsg("JS.LSC01010Q0.006", "행코드 길이(" + $NC.G_VAR.policyVal.CM122 + "자리)를 정확히 입력하여 주십시오.",
                        $NC.G_VAR.policyVal.CM122));
                    $NC.setFocus("#edtQBank_Cd");
                } else {
                    $NC.setValue("#edtQBank_Cd", val.toUpperCase());
                }
            }
            break;
        case "DEPT_CD":
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LSC01010Q0.002", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LSC01010Q0.003", "사업부를 입력하십시오."));
    ...
    if (!$NC.isNull(BANK_CD)) {
        if (!$NC.equals(BANK_CD.length, Number($NC.G_VAR.policyVal.CM122))) {
            alert($NC.getDisplayMsg("JS.LSC01010Q0.006", "행코드 길이(" + $NC.G_VAR.policyVal.CM122 + "자리)를 정확히 입력하여 주십시오.", $NC.G_VAR.policyVal.CM122));
            $NC.setFocus("#edtQBank_Cd");
            return;
        } else {
            BANK_CD = BANK_CD.toUpperCase();
            $NC.setValue("#edtQBank_Cd", BANK_CD);
        }
    } else {
        BANK_CD = $ND.C_ALL;
    }
    ...
    if ($NC.getTabActiveIndex("#divMasterView") == 0) {
        ...
        G_GRDT2MASTER.queryParams = {
            ...
            P_CLASS_CD: CLASS_CD,
            P_ITEM_BAR_CD: ITEM_BAR_CD,
            P_EMPTY_CELL_YN: $NC.getValue("#chkEmpty_Cell_Yn")
        };
        // 데이터 조회
...
function _Print(reportInfo) {
    ...
    if (G_GRDT2DETAIL.data.getLength() == 0 || $NC.isNull(G_GRDT2DETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.LSC01010Q0.004", "출력할 데이터가 없습니다."));
        return;
    }
    ...
    switch (reportInfo.REPORT_CD) {
        ...
        case "LABEL_CMC04":
            ...
            if ($NC.isNull(rowData.PALLET_ID)) {
                alert($NC.getDisplayMsg("JS.LSC01010Q0.005", "파렛트ID가 없는 재고 데이터입니다. 파렛트ID가 있는 재고를 선택하십시오."));
                return;
            }
...
function tabOnActivate(event, ui) {
    ...
    if ($NC.isSplitter(container)) {
        ...
        if (tabActiveIndex == 0) {
            ...
            $NC.setInitSplitter(container, "vl", $NC.G_OFFSET.leftViewWidth);
        } else {
            $NC.onGlobalResize();
        }
    }
    ...
    if (tabActiveIndex == 1) {
        $NC.G_VAR.buttons._print = $NC.iif(G_GRDT2MASTER.data.getLength() > 0, "1", "0");
        $("#tdQEmpty_Cell_Yn").show();
    } else {
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "LSC01010Q0.RS_T1_MASTER",
        sortCol: "ITEM_CD",
        gridOptions: options,
        canDblClick: true
    });
...
function grdT1DetailInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Detail", {
        ...
        queryId: "LSC01010Q0.RS_T1_DETAIL",
        sortCol: "LOCATION_CD",
        gridOptions: options,
        canDblClick: true
    });
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "LSC01010Q0.RS_T2_MASTER",
        sortCol: "LOCATION_CD",
        gridOptions: options
    });
...
function grdT2DetailInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Detail", {
        ...
        queryId: "LSC01010Q0.RS_T2_DETAIL",
        sortCol: "ITEM_CD",
        gridOptions: options,
        canDblClick: true
    });
...
function grdT3MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT3Master", {
        ...
        queryId: "LSC01010Q0.RS_T3_MASTER",
        sortCol: "ITEM_CD",
        gridOptions: options,
        canDblClick: true
    });
...
function grdT4MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT4Master", {
        ...
        queryId: "LSC01010Q0.RS_T4_MASTER",
        sortCol: "ITEM_CD",
        gridOptions: options
    });
...
function onOutwaitPopup(e, args) {
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "LSC01011P0",
        PROGRAM_NM: $NC.getDisplayMsg("JS.LSC01010Q0.007", "출고대기상세내역"),
        url: "ls/LSC01011P0.html",
        width: 900,
        height: 500,
