...
function _Initialize() {
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.LIC02011P1.001", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.LIC02011P1.002", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.LIC02011P1.003", "저장"));
}
...
function _OnLoaded() {
    ...
    // 예정 -> 등록, 등록 수정
        ...
        if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ORDER) {
            ...
            INBOUND_NO = "";
            INBOUND_STATE = $ND.C_STATE_ENTRY;
            ORDER_BOX_QTY = dsMaster.ENTRY_BOX_QTY;
            CRUD = $ND.C_DV_CRUD_C;
        } else {
        ...
        $NC.setValue("#edtEntry_Box_Qty", dsMaster.ENTRY_BOX_QTY);
        ...
        $NC.G_VAR.masterData = {
            ...
            ENTRY_BOX_QTY: dsMaster.ENTRY_BOX_QTY,
        ...
        $NC.setEnable("#edtEntry_Box_Qty", false);
        ...
        onComplete: function() {
            if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ENTRY_CREATE) {
                ...
                if ($NC.G_VAR.masterData.ASSORT_GRP == "1") {
                    ...
                    $NC.setEnable("#btnEntryNew", $NC.G_VAR.G_PARAMETER.P_POLICY_LI210 == $ND.C_YES);
                    $NC.setEnable("#btnEntryDelete", $NC.G_VAR.G_PARAMETER.P_POLICY_LI210 == $ND.C_YES);
                } else {
...
function _New() {
    ...
    if ($NC.isNull(VENDOR_CD)) {
        alert($NC.getDisplayMsg("JS.LIC02011P1.004", "먼저 공급처 코드를 입력하십시오."));
        $NC.setFocus("#edtVendor_Cd");
        return;
    }
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.INBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.LIC02011P1.006", "먼저 입고일자를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LIC02011P1.007", "먼저 입고구분을 선택하십시오."));
    ...
    if ($NC.G_VAR.G_PARAMETER.P_POLICY_LS220 == "2" && $NC.G_VAR.G_PARAMETER.P_POLICY_LI230 == "3") {
        if ($NC.toNumber($NC.G_VAR.masterData.PALLET_ID_CNT) < 1) {
            alert($NC.getDisplayMsg("JS.LIC02011P1.008", "파렛트ID수를 정확히 입력하십시오[1이상]"));
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LIC02011P1.005", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02011P1.009", "수정 후 저장하십시오."));
        return;
    }
    ...
    $NC.serviceCall("/LIC02010E0/save.do", {
        P_DS_MASTER: {
            ...
            P_ENTRY_BOX_QTY: $NC.G_VAR.masterData.ENTRY_BOX_QTY,
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.LIC02011P1.010", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        ...
        case "ASSORT_QTY":
            ...
            // 어소트일 경우는 디테일의 등록수량을 변경
            if ($NC.G_VAR.masterData.ASSORT_GRP == "2") {
                setAssortEntryQty();
            }
            break;
        case "ENTRY_BOX_QTY":
            $NC.G_VAR.masterData.ENTRY_BOX_QTY = args.val;
            break;
        case "INOUT_CD":
            // 수송입고는 신규 등록 불가
            if (args.val == "E20") {
                if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ENTRY_CREATE) {
                    alert($NC.getDisplayMsg("JS.LIC02011P1.011", "수송입고는 신규로 등록할 수 없습니다."));
                    args.val = "E10";
            ...
            break;
        case "INBOUND_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LIC02011P1.012", "입고일자를 정확히 입력하십시오."));
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        ...
        case "ENTRY_QTY":
            ...
            if (ENTRY_QTY < CHECK_QTY) {
                alert($NC.getDisplayMsg("JS.LIC02011P1.013", "등록수량이 " + CHECK_QTY + "보다 작을 수 없습니다.", CHECK_QTY));
            ...
            if ($NC.G_VAR.ORDER_YN == $ND.C_YES && $NC.G_VAR.G_PARAMETER.P_POLICY_LI220 != "1") {
                if (Number(rowData.ORDER_QTY) < ENTRY_QTY) {
                    alert($NC.getDisplayMsg("JS.LIC02011P1.014", "등록수량이 예정수량을 초과할 수 없습니다."));
                    rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                    break;
                }
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.LIC02011P1.015", "사용기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                } else {
...
function setUserProgramPermission() {
    ...
    if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ORDER || $NC.G_VAR.ORDER_YN == $ND.C_YES) {
        ...
        // Y -> 등록시 신규, 수정 가능, 프로그램 권한에 따라 가능/불가능
        if ($NC.G_VAR.masterData.ASSORT_GRP == "1") {
...
function setAssortEntryQty() {
    ...
    try {
        for (var rIndex = 0, rCount = G_GRDDETAIL.data.getLength(); rIndex < rCount; rIndex++) {
            ...
            rowData.ENTRY_QTY = rowData.CONS_QTY * ASSORT_QTY;
