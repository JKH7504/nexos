...
function _NewB() {
    ...
    if ($NC.G_VAR.policyVal.LI210 != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.025", "입고등록 신규처리가 불가능한 사업부입니다."));
        return;
    }
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "LIC02011P1",
        PROGRAM_NM: $NC.getDisplayMsg("JS.LIC02010E1.026", "입고등록/수정"),
        url: "li/LIC02011P1.html",
        width: 1024,
        height: 600,
        G_PARAMETER: {
...
function _SaveB() {
    ...
    if (!$NC.isGridValidLastRow(G_GRDMASTERB)) {
        return;
    }

    // 마지막 선택 Row Validation 체크
    if (!$NC.isGridValidLastRow(G_GRDDETAILB)) {
        return;
    }
    ...
    for (rIndex = 0, rCount = G_GRDMASTERB.data.getLength(); rIndex < rCount; rIndex++) {
        ...
        } else if (rowData.INBOUND_STATE == $ND.C_STATE_ENTRY) {
            dsMasterEntry.push({
                P_CENTER_CD: rowData.CENTER_CD,
                P_BU_CD: rowData.BU_CD,
    ...
    // 입고등록디테일 수정 데이터
    var dsDetailEntry = [];
    for (rIndex = 0, rCount = G_GRDDETAILB.data.getLength(); rIndex < rCount; rIndex++) {
        rowData = G_GRDDETAILB.data.getItem(rIndex);
        if (rowData.CRUD == $ND.C_DV_CRUD_R) {
            continue;
        }
        if (rowData.INBOUND_STATE == $ND.C_STATE_ORDER) {
    ...
    if (dsMasterOrder.length == 0 && dsDetailOrder.length == 0 && dsMasterEntry.length == 0 && dsDetailEntry.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.028", "데이터 수정 후 저장하십시오."));
        return;
    }

    // 예정데이터 저장 처리
    if (dsMasterOrder.length > 0 || dsDetailOrder.length > 0) {
        $NC.serviceCall("/LIC01010E0/saveOrder.do", {
            P_DS_MASTER: dsMasterOrder,
            P_DS_DETAIL: dsDetailOrder,
            P_USER_ID: $NC.G_USERINFO.USER_ID
        }, onSaveB, onSaveErrorB);
    }

    // 등록데이터 저장 처리
    if (dsMasterEntry.length > 0 || dsDetailEntry.length > 0) {
        $NC.serviceCall("/LIC02010E0/saveEntry.do", {
            P_DS_MASTER: dsMasterEntry,
            P_DS_DETAIL: dsDetailEntry,
            P_USER_ID: $NC.G_USERINFO.USER_ID
        }, onSaveB, onSaveErrorB);
    }
}
...
function onProcessPreB() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E1.031", "입고등록 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.032", "입고등록 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.033", "선택한 데이터 중 입고등록 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtB() {
    ...
    if ($NC.isNull(INBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.034", "입고일자를 먼저 입력하십시오."));
        $NC.setFocus("#dtpInbound_DateB");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LIC02010E1.035", "입고등록 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.036", "입고등록 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.037", "선택한 데이터 중 입고등록 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterBInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterB", {
        ...
        queryId: "LIC02010E1.RS_T1_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options,
        canDblClick: true
    });
    ...
    G_GRDMASTERB.view.onCellChange.subscribe(grdMasterBOnCellChange);
...
function grdMasterBOnAfterScroll(e, args) {
    ...
    // 소터투입여부 값에 따라 button value toggle
    if (rowData.SORTER_PUT_YN == $ND.C_YES) {
        $NC.setValue("#btnSetSorterPutYn", "소터투입해제");
    } else {
        $NC.setValue("#btnSetSorterPutYn", "소터투입설정");
    }
 ...
 * @param args
 */
function grdMasterBOnCellChange(e, args) {

    var rowData = args.item;

    // 마지막 선택 Row 수정 데이터 반영 및 상태 변경
    $NC.setGridApplyChange(G_GRDMASTERB, rowData);
}
...
function grdMasterBOnDblClick(e, args) {
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LIC02010E1.038", "입고진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        ...
        if (refRowData.INBOUND_STATE != resultData.O_INBOUND_STATE) {
            alert($NC.getDisplayMsg("JS.LIC02010E1.039", "[진행상태 : " + resultData.O_INBOUND_STATE + "] 데이터가 변경되었습니다.\n다시 조회 후 데이터를 확인하십시오.",
        ...
        if (dsDetail.length == 0) {
            alert($NC.getDisplayMsg("JS.LIC02010E1.040", "등록/수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function grdDetailBInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailB", {
        ...
        queryId: "LIC02010E1.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdDetailBOnCellChange(e, args) {
    ...
    // 마지막 선택 Row 수정 데이터 반영 및 상태 변경
    $NC.setGridApplyChange(G_GRDDETAILB, rowData);
}
...
function onGetMasterB(ajaxData) {
    ...
    if ($NC.setInitGridAfterOpen(G_GRDMASTERB, "INBOUND_NO", true)) {
        // 소터투입설정/해제 버튼 활성화
        $NC.setEnable("#btnSetSorterPutYn");
        $NC.setEnable("#btnSendWcsWeight");
        // 마스터 총수량 세팅
        ...
        // 디테일 초기화
        $NC.clearGridData(G_GRDDETAILB);
        // 소터투입설정/해제 버튼 초기화, 비활성화
        $("#btnSetSorterPutYn").val("소터투입설정");
...
function btnSetSorterPutYnOnClick() {
    ...
    if (G_GRDMASTERB.data.getLength() == 0 || $NC.isNull(G_GRDMASTERB.lastRow)) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.029", "데이터조회 후 처리하십시오."));
        return;
    }

    var rowData = G_GRDMASTERB.data.getItem(G_GRDMASTERB.lastRow);
    if (rowData.SORTER_PUT_YN == $ND.C_YES) {
        if (!confirm($NC.getDisplayMsg("JS.LIC02010E1.041", "소터투입해제로 변경 하시겠습니까?"))) {
            return;
        }
    } else {
        if (!confirm($NC.getDisplayMsg("JS.LIC02010E1.042", "소터투입설정으로 변경 하시겠습니까?"))) {
            return;
        }
    }

    // LI_SET_SORTER_PUT_YN 호출
    $NC.serviceCall("/LIC02010E0/callLISetSorterPutYn.do", {
        P_BRAND_CD: rowData.BRAND_CD,
        P_ASSORT_CD: rowData.ITEM_LOT,
        P_STYLE_CD: rowData.STYLE_CD,
        P_SORTER_PUT_YN: rowData.SORTER_PUT_YN == $ND.C_YES ? $ND.C_NO : $ND.C_YES,
        P_ASSORT_GRP: rowData.ASSORT_GRP,
        P_USER_ID: $NC.G_USERINFO.USER_ID
    }, onProcB, onSaveErrorB);
}

function btnSendWcsWeightOnClick() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E1.043", "WCS중량계전송 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.044", "WCS중량계전송 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.045", "선택한 데이터 중 WCS중량계전송 처리 가능한 데이터가 없습니다."));
        return;
    }

    $NC.serviceCall("/LIC02010E0/callSendFwLiOrder.do", {
        P_DS_MASTER: dsMaster,
        P_USER_ID: $NC.G_USERINFO.USER_ID
    }, onProcB, onSaveErrorB);
}

function onProcB(ajaxData) {
    ...
    window._Inquiry();
    G_GRDMASTERB.lastKeyVal = lastKeyVal;
    alert($NC.getDisplayMsg("JS.LIC02010E1.046", "변경이 완료되었습니다."));
}