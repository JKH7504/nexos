...
function _Initialize() {
    ...
    // 초기 숨김 처리
    // 초기 비활성화 처리
    $NC.setEnable("#btnPartialPreD", false);
    $NC.setEnable("#btnPartialPreE", false);
    $NC.setEnable("#btnVConfirmD", false);
    ...
    $("#btnPrintPallet").click(window.btnPrintPalletOnClick);
    ...
    // 취소/처리 버튼 툴팁 세팅
    $NC.setTooltip("#btnProcessPreB", $NC.getDisplayMsg("JS.LIC02010E2.001", $NC.getValue("#btnProcessB") + " 취소", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessNxtB", $NC.getDisplayMsg("JS.LIC02010E2.002", $NC.getValue("#btnProcessB") + " 처리", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessPreC", $NC.getDisplayMsg("JS.LIC02010E2.001", $NC.getValue("#btnProcessC") + " 취소", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessNxtC", $NC.getDisplayMsg("JS.LIC02010E2.002", $NC.getValue("#btnProcessC") + " 처리", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessPreD", $NC.getDisplayMsg("JS.LIC02010E2.001", $NC.getValue("#btnProcessD") + " 취소", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessNxtD", $NC.getDisplayMsg("JS.LIC02010E2.002", $NC.getValue("#btnProcessD") + " 처리", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessPreE", $NC.getDisplayMsg("JS.LIC02010E2.001", $NC.getValue("#btnProcessE") + " 취소", $NC.getValue("#btnProcessE")));
    $NC.setTooltip("#btnProcessNxtE", $NC.getDisplayMsg("JS.LIC02010E2.002", $NC.getValue("#btnProcessE") + " 처리", $NC.getValue("#btnProcessE")));

    $NC.setTooltip("#btnNewPalletIdC", $NC.getDisplayMsg("JS.LIC02010E2.003", "신규"));
    $NC.setTooltip("#btnNewDirectionsC", $NC.getDisplayMsg("JS.LIC02010E2.004", "추가"));
    $NC.setTooltip("#btnDeleteDirectionsC", $NC.getDisplayMsg("JS.LIC02010E2.005", "삭제"));
}
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "VENDOR_CD":
            ...
            return;
        case "INBOUND_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LIC02010E2.006", "입고일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LIC02010E2.007", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LIC02010E2.008", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _OnInputChange(e, view, val) {
    ...
    switch (id) {
        case "INBOUND_DATEB":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LIC02010E2.009", "입고일자를 정확히 입력하십시오."));
            break;
    }
}
...
function _Inquiry() {
    ...
    if (STATE_PRE_YN == $ND.C_NO && STATE_CUR_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LIC02010E2.020", "검색구분을 선택하십시오."));
...
function _Print(reportInfo) {
    ...
    switch ($NC.G_VAR.activeView.PROCESS_CD) {
        ...
        case $ND.C_PROCESS_PUTAWAY:
            ...
            break;
        default:
            alert($NC.getDisplayMsg("JS.LIC02010E2.021", "해당 프로세스는 출력물이 없습니다."));
            return;
    }
    ...
    switch (reportInfo.REPORT_CD) {
        ...
        case "LABEL_CMC03":
            /*
             * // TODO: 일단 보류 if ($NC.G_VAR.policyVal.LI230 == "2" || $NC.G_VAR.policyVal.LI230 == "3") { alert($NC.getDisplayMsg("JS.LIC02010E2.022",
        ...
        case "LABEL_CMC04":
            /*
             * // TODO: 일단 보류 if ($NC.G_VAR.policyVal.LI230 == "1") { alert($NC.getDisplayMsg("JS.LIC02010E2.XXX", "[" + reportInfo.REPORT_NM +
...
function setUserProgramPermission() {
    ...
    if (permission.canSave) {
        ...
        $("#btnVConfirmD").click(window.btnVConfirmDOnClick);
    }
    ...
    if (permission.canConfirmCancel) {
        ...
        $("#btnPartialPreD").click(window.onPartialPreD); // 부분취소
        $("#btnPartialPreE").click(window.onPartialPreE); // 부분취소
    }
...
function onGetProcessInfo(ajaxData) {
    ...
    // alert($NC.getDisplayMsg("JS.LIC02010E2.023", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록하십시오."));
    $NC.G_VAR.activeView.PROCESS_CD = "";
    $NC.showMessage({
        message: $NC.getDisplayMsg("JS.LIC02010E2.023", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록하십시오."),
        width: 400,
...
function setMasterSummaryInfo() {
    ...
    $NC.serviceCall("/LIC02010E0/getDataSet.do", {
        P_QUERY_ID: "LIC02010E2.RS_MASTER",
        P_QUERY_PARAMS: {
            P_CENTER_CD: CENTER_CD,
            P_BU_CD: BU_CD,
            P_INBOUND_DATE: INBOUND_DATE,
