...
function _SaveD() {
    ...
    if (dsDetail.length > 0) {
        $NC.serviceCall("/LIC02010E0/saveConfirmDetail.do", {
...
function onProcessPreD() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E1.064", "입고확정취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.065", "입고확정취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.066", "선택한 데이터 중 입고확정취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtD() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E1.067", "입고확정 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBD)) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.068", "데이터가 수정되었습니다. 저장 후 입고확정 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.069", "입고확정 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.070", "선택한 데이터 중 입고확정처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterDInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterD", {
        ...
        queryId: "LIC02010E1.RS_T3_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options
    });
...
function grdMasterDOnAfterScroll(e, args) {
    ...
    // 직송처리/취소 버튼 활성화
    $NC.setEnable("#btnSetDirectYnD");
    // 직송여부 값에 따라 button value toggle
    $NC.setValue("#btnSetDirectYnD", rowData.DIRECT_YN == $ND.C_YES ? "직송취소" : "직송처리");
...
function grdDetailDInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailD", {
        ...
        queryId: "LIC02010E1.RS_T3_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdDetailDOnBeforeEditCell(e, args) {
    ...
    switch (args.column.id) {
        case "UNPAID_DIV_F":
        case "UNPAID_COMMENT":
            // 미입고 수량이 0이거나 진행상태가 30이면 수정 불가
            return rowData.UNPAID_QTY > "0" && rowData.INBOUND_STATE == PROCESS_STATE;
    }
    return true;
}
...
function grdSubDOnCellChange(e, args) {
    ...
    switch (G_GRDSUBD.view.getColumnId(args.cell)) {
        case "CONFIRM_QTY":
            ...
            if (CONFIRM_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LIC02010E1.071", "확정수량이 0보다 작을 수 없습니다."));
            ...
            if ($NC.G_VAR.policyVal.LI710 == $ND.C_POLICY_VAL_2 && rowData.IN_GRP == $ND.C_INOUT_GRP_E2) { // 수송입고전표 등록수량 이하로 수량 수정 가능
                if ($NC.toNumber(rowData.ENTRY_QTY) < CONFIRM_QTY) {
                    alert($NC.getDisplayMsg("JS.LIC02010E1.072", "확정수량이 등록수량을 초과할 수 없습니다."));
                    rowData.CONFIRM_QTY = args.oldItem.CONFIRM_QTY;
                    rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
                    rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
                    rowData.CONFIRM_WEIGHT = args.oldItem.CONFIRM_WEIGHT;
...
function grdSubDOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONFIRM_QTY)) {
            alert($NC.getDisplayMsg("JS.LIC02010E1.073", "확정수량을 입력하십시오."));
...
function onGetMasterD(ajaxData) {
    ...
    if ($NC.setInitGridAfterOpen(G_GRDMASTERD, "INBOUND_NO", true)) {
        ...
        $NC.setValue("#edtTotal_Vendor_CntD", $NC.getDisplayNumber(rowData.TOTAL_VENDOR_CNT));
    } else {
        // 디테일 초기화
        $NC.clearGridData(G_GRDDETAILD);
        // 입고지시 초기화
...
function btnSetDirectYnDOnClick() {
    ...
    var PROCESS_STATE = $NC.G_VAR.stateFWBW[$NC.G_VAR.activeView.PROCESS_CD].CANCEL;
    if (rowData.INBOUND_STATE != PROCESS_STATE) {
        alert($NC.getDisplayMsg("JS.LIC02010E1.075", "입고확정 후 처리하십시오."));
        return;
    }
