...
function onProcessPreD() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E0.057", "입고확정취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.058", "입고확정취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.059", "선택한 데이터 중 입고확정취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtD() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E0.060", "입고확정 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBD)) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.061", "데이터가 수정되었습니다. 저장 후 입고확정 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.062", "입고확정 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.063", "선택한 데이터 중 입고확정처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterDInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterD", {
        ...
        queryId: "LIC02010E0.RS_T3_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options
    });
...
function grdDetailDInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailD", {
        ...
        queryId: "LIC02010E0.RS_T3_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdSubDOnCellChange(e, args) {
    ...
    switch (G_GRDSUBD.view.getColumnId(args.cell)) {
        case "CONFIRM_QTY":
            ...
            if (CONFIRM_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LIC02010E0.064", "확정수량이 0보다 작을 수 없습니다."));
            ...
            if ($NC.G_VAR.policyVal.LI710 == $ND.C_POLICY_VAL_2 && rowData.IN_GRP == $ND.C_INOUT_GRP_E2) { // 수송입고전표 등록수량 이하로 수량 수정 가능
                if ($NC.toNumber(rowData.ENTRY_QTY) < CONFIRM_QTY) {
                    alert($NC.getDisplayMsg("JS.LIC02010E0.065", "확정수량이 등록수량을 초과할 수 없습니다."));
                    rowData.CONFIRM_QTY = args.oldItem.CONFIRM_QTY;
                    rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
                    rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
                    rowData.CONFIRM_WEIGHT = args.oldItem.CONFIRM_WEIGHT;
        ...
        case "CONFIRM_BOX":
            ...
            if (CONFIRM_BOX < 0) {
                alert($NC.getDisplayMsg("JS.LIC02010E0.066", "확정BOX는 0보다 작을 수 없습니다."));
                rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
                $NC.setFocusGrid(G_GRDSUBD, args.row, args.cell, true);
                break;
            }
        ...
        case "CONFIRM_EA":
            ...
            if (CONFIRM_EA < 0) {
                alert($NC.getDisplayMsg("JS.LIC02010E0.067", "확정EA는 0보다 작을 수 없습니다."));
                rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
                $NC.setFocusGrid(G_GRDSUBD, args.row, args.cell, true);
                break;
            }
