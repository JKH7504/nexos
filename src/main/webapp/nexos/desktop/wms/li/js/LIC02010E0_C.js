...
function _InquiryC(args) {
    ...
    G_GRDMASTERC.queryParams = {
        ...
        P_SHIPMENT_NO: args.SHIPMENT_NO,
        P_BRAND_CD: args.BRAND_CD,
        P_ITEM_CD: args.ITEM_CD,
        P_ITEM_NM: args.ITEM_NM,
        P_BU_NO: args.BU_NO,
        P_BU_KEY: args.BU_KEY,
        P_ITEM_LOT: args.ITEM_LOT,
        P_ARRIVAL_DATE: args.ARRIVAL_DATE,
        P_FLIGHT_NO: args.FLIGHT_NO,
        P_DEPT_CD: args.DEPT_CD,
        P_STATE_PRE_YN: args.STATE_PRE_YN,
        P_STATE_CUR_YN: args.STATE_CUR_YN
    };
    // 데이터 조회
    $NC.serviceCall("/LIC02010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTERC), onGetMasterC);
}
...
function _SaveC() {
    ...
    if (G_GRDMASTERC.data.getLength() == 0 || $NC.isNull(G_GRDMASTERC.lastRow) || G_GRDSUBC.data.getItems().length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.038", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if ($NC.G_VAR.policyVal.LS220 == "1") {
        ...
        if (dsSub.length == 0) {
            alert($NC.getDisplayMsg("JS.LIC02010E0.039", "데이터 수정 후 저장하십시오."));
            return;
        }
        ...
        $NC.serviceCall("/LIC02010E0/saveDirectionsPltId.do", {
            P_DS_MASTER: {
...
function onProcessPreC() {
    ...
    if (rCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.026", "조회 후 처리하십시오."));
        return;
    }
    if ($NC.G_VAR.PROCESS_YN != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.027", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록 후 처리하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LIC02010E0.040", "입고지시 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.041", "입고지시 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.042", "선택한 데이터 중 입고지시 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtC() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E0.043", "입고지시 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBC)) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.044", "데이터가 수정되었습니다. 저장 후 입고지시 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.045", "입고지시 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.046", "선택한 데이터 중 입고지시 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function btnNewPalletIdCOnClick() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC02010E0.047", "새 파렛트ID를 생성하시겠습니까?"))) {
        return;
    }
    ...
    $NC.serviceCall("/LIC02010E0/callCMPalletIdGetNo.do", {
        P_CENTER_CD: refRowData.CENTER_CD,
        P_BU_CD: refRowData.BU_CD,
        P_INBOUND_DATE: refRowData.INBOUND_DATE,
        P_INBOUND_NO: refRowData.INBOUND_NO,
...
function onGetNewPalletIdC(ajaxData) {
    ...
    $NC.serviceCall("/LIC02010E0/getDataSet.do", {
        P_QUERY_ID: "LIC02010E0.RS_T2_SUB1",
        P_QUERY_PARAMS: {
            P_CENTER_CD: rowData.CENTER_CD,
            P_BU_CD: rowData.BU_CD,
            P_INBOUND_DATE: rowData.INBOUND_DATE,
...
function btnNewDirectionsCOnClick() {
    ...
    if (G_GRDPALLETIDC.data.getLength() == 0 || $NC.isNull(G_GRDPALLETIDC.lastRow)) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.048", "신규 파렛트ID를 먼저 생성 후 처리하십시오."));
        return;
    }
    ...
    if (rowData.ENTRY_QTY == rowData.DIRECTIONS_QTY) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.049", "추가하려는 상품은 이미 등록수량만큼 지시 처리되었습니다."));
        return;
    }
    ...
    if (searchIndex != -1) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.050", "추가하려는 상품은 해당 파렛트ID에 이미 추가되어 있습니다."));
    ...
    // [LI230]파렛트ID 매핑 기준 : 2 - 파렛트 분할구분별 자동매핑(멀티상품)
    // [LI230]파렛트ID 매핑 기준 : 3 - 사용자 수작업 매핑(멀티상품)
    if ($NC.G_VAR.policyVal.LI230 == "1" && G_GRDSUBC.data.getLength() > 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.XXX", "[파렛트ID 매핑 기준]파렛트ID 하나에 여러 상품을 추가할 수 없습니다."));
        return;
    }
    ...
    var newRowData = {
        ...
        VENDOR_CD: refRowData.VENDOR_CD,
        // BU_CD: rowData.BU_CD,
    ...
    // 남은 수량 모두 지시 생성하므로 지시수량을 동일하게 조정
    rowData.DIRECTIONS_QTY = rowData.ENTRY_QTY;
...
function btnDeleteDirectionsCOnClick() {
    ...
    if (G_GRDSUBC.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LIC02010E0.051", "삭제하려는 데이터를 선택하십시오."));
        return;
    }
...
function grdMasterCInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterC", {
        ...
        queryId: "LIC02010E0.RS_T2_MASTER",
        sortCol: "INBOUND_NO",
        gridOptions: options
    });
...
function grdMasterCOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LIC02010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAILC), onGetDetailC);
    ...
    // 데이터 조회
    $NC.serviceCall("/LIC02010E0/getDataSet.do", $NC.getGridParams(G_GRDPALLETIDC), onGetPalletIdC);
    ...
    // 데이터 조회
    $NC.serviceCall("/LIC02010E0/getDataSet.do", $NC.getGridParams(G_GRDSUBC), onGetSubC);
...
function grdDetailCInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailC", {
        ...
        queryId: "LIC02010E0.RS_T2_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdSubCOnSetColumns() {
    ...
    $NC.setGridColumnOptions(G_GRDSUBC, {
        ENTRY_QTY: {
            editor: $NC.G_VAR.policyVal.LS220 == "2" ? Slick.Editors.Number : null
        }
    });
    ...
    $NC
        .setGridColumns(
            G_GRDSUBC,
            [ // 숨김컬럼 세팅
                $NC.G_VAR.policyVal.LS220 == "1" ? "PALLET_ID,LINE_NO,ITEM_CD,ITEM_BAR_CD,ITEM_NM,ITEM_SPEC,BRAND_NM,ITEM_STATE_F,FLOCATION_CD,ITEM_LOT" : "",
                $NC.G_VAR.policyVal.LS210 != "2" ? "VALID_DATE,BATCH_NO" : ""
            ]);
}
...
function grdSubCOnCellChange(e, args) {
    ...
    switch (G_GRDSUBC.view.getColumnId(args.cell)) {
        case "ENTRY_QTY":
            ...
            if (ENTRY_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LIC02010E0.052", "지시수량이 0보다 작을 수 없습니다."));
            ...
            if (searchIndex == -1) {
                alert($NC.getDisplayMsg("JS.LIC02010E0.053", "상세정보에서 해당 상품을 검색하지 못해 지시수량을 반영하지 못했습니다."));
                rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                rowData.ENTRY_BOX = args.oldItem.ENTRY_BOX;
                rowData.ENTRY_EA = args.oldItem.ENTRY_EA;
                rowData.ENTRY_WEIGHT = args.oldItem.ENTRY_WEIGHT;
            ...
            if (D_ENTRY_QTY < S0_ENTRY_QTY) {
                alert($NC.getDisplayMsg("JS.LIC02010E0.054", "지시수량이 등록수량보다 클 수 없습니다."));
                rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                rowData.ENTRY_BOX = args.oldItem.ENTRY_BOX;
                rowData.ENTRY_EA = args.oldItem.ENTRY_EA;
                rowData.ENTRY_WEIGHT = args.oldItem.ENTRY_WEIGHT;
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.LIC02010E0.055", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDSUBC, args.row, args.cell, true);
                } else {
...
function grdSubCOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.ENTRY_QTY)) {
            alert($NC.getDisplayMsg("JS.LIC02010E0.056", "지시수량을 입력하십시오."));
            $NC.setFocusGrid(G_GRDDETAILC, row, "ENTRY_QTY", true);
            return false;
        }
    }
...
function grdSubCOnBeforeEditCell(e, args) {
    ...
    switch (args.column.id) {
        case "ENTRY_QTY":
            return $NC.G_VAR.policyVal.LS220 == "2";
        case "VALID_DATE":
        case "BATCH_NO":
