 ...
 *  프로그램ID         : LIC01010E1
 *  프로그램명         : 입고예정작업 (의류)
 *  프로그램설명       : 입고예정작업 (의류) 화면 Javascript
...
function _Initialize() {
    ...
    $("#btnChangeQCState").click(btnChangeQCStateOnClick);
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            return;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LIC01010E1.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LIC01010E1.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
        case "VENDOR_CD":
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.003", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.004", "사업부를 입력하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE1)) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.005", "검색 시작일자를 입력하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE2)) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.006", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQOrder_Date2");
        return;
    }

    if (ORDER_DATE1 > ORDER_DATE2) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.007", "입고예정일자 범위 입력오류입니다."));
        $NC.setFocus("#dtpQOrder_Date1");
        return;
    }
    ...
    // 데이터 조회
    $NC.serviceCall("/LIC01010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _New() {
    ...
    if ($NC.G_VAR.policyVal.LI110 != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.008", "입고예정정보 신규등록/수정이 불가능한 사업부입니다."));
        return;
    }
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "LIC01011P1",
        PROGRAM_NM: $NC.getDisplayMsg("JS.LIC01010E1.009", "입고예정등록/수정"),
        url: "li/LIC01011P1.html",
        width: 1024,
        height: 600,
        G_PARAMETER: {
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.010", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (!confirm($NC.getDisplayMsg("JS.LIC01010E1.011", "저장하시겠습니까?"))) {
        return;
    }
    ...
    if (dsMaster.length == 0 && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.012", "데이터 수정 후 저장하십시오."));
        return;
    }
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.013", "삭제할 데이터가 없습니다."));
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    if ($NC.G_VAR.stateFWBW.CANCEL != rowData.INBOUND_STATE) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.014", "삭제가 불가능한 전표입니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LIC01010E1.015", "예정전표를 삭제하시겠습니까?"))) {
        return;
    }

    // 삭제처리 호출
    $NC.serviceCall("/LIC01010E0/callLIOrderDelete.do", {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_ORDER_DATE: rowData.ORDER_DATE,
        P_ORDER_NO: rowData.ORDER_NO,
...
function grdMasterInitialize() {
    ...
    // G_GRDMASTER.view.onDblClick.subscribe(grdMasterOnDblClick); // 수정기능 없음
}
...
function grdMasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LIC01010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
...
function grdMasterOnDblClick(e, args) {
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LIC01010E1.016", "입고진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        if (resultData.O_MSG != $ND.C_OK) {
            alert(resultData.O_MSG);
            return;
        }
        if (refRowData.INBOUND_STATE != resultData.O_INBOUND_STATE) {
            alert($NC.getDisplayMsg("JS.LIC01010E1.017", "[진행상태 : " + resultData.O_INBOUND_STATE + "] 데이터가 변경되었습니다.\n다시 조회 후 데이터를 확인하십시오.",
        ...
        $NC.serviceCallAndWait("/LIC01010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail, function(subAjaxData) {
        ...
        if (dsDetail.length == 0) {
            alert($NC.getDisplayMsg("JS.LIC01010E1.018", "등록/수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function setProcessStateInfo() {
    ...
    // 데이터 조회
    $NC.serviceCall("/LIC01010E0/getData.do", {
...
function btnOrderClosingOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.019", "종결처리할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LIC01010E1.020", "종결처리 하시겠습니까?"))) {
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    // 종결처리 호출
    $NC.serviceCall("/LIC01010E0/callLIOrderClosing.do", {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_ORDER_DATE: rowData.ORDER_DATE,
        P_ORDER_NO: rowData.ORDER_NO,
...
/**
 * QC상태전환 Button Event - QC상태전환 버튼 클릭
 */
function btnChangeQCStateOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.021", "QC상태전환할 데이터가 없습니다."));
        return;
    }
    ...
    if ($NC.G_VAR.stateFWBW.CANCEL != refRowData.INBOUND_STATE) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.022", "삭제가 불가능한 전표입니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LIC01010E1.023", "QC상태전환 처리 하시겠습니까?"))) {
        return;
    }

    var dsDetail = [];
    var rowData, ITEM_STATE;
    for (var rIndex = 0, rowCount = G_GRDDETAIL.data.getLength(); rIndex < rowCount; rIndex++) {
        rowData = G_GRDDETAIL.data.getItem(rIndex);
        if (rowData.CHECK_YN != $ND.C_YES || rowData.ITEM_STATE == "B") {
            continue;
        }
        ITEM_STATE = $NC.iif(rowData.ITEM_STATE == "Q", "A", "Q");
        dsDetail.push({
            P_CENTER_CD: rowData.CENTER_CD,
            P_BU_CD: rowData.BU_CD,
            P_ORDER_DATE: rowData.ORDER_DATE,
    ...
    if (dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.LIC01010E1.024", "QC상태전환 처리할 데이터를 선택하십시오."));
        return;
    }

    $NC.serviceCall("/LIC01010E0/changeItemStateToQCState.do", {
        P_DS_MASTER: {
            P_CENTER_CD: refRowData.CENTER_CD,
            P_BU_CD: refRowData.BU_CD,
            P_ORDER_DATE: refRowData.ORDER_DATE,
