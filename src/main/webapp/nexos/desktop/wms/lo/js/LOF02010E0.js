 ...
 *  프로그램ID         : LOF02010E0
 *  프로그램명         : 출고작업[풀필먼트]
 *  프로그램설명       : 출고작업[풀필먼트] 화면 Javascript
...
function _Initialize() {
    ...
    $NC.setGlobalVar({
        ...
        // 체크할 정책 값
        policyVal: {
            CM530: "", // [택배일반]운송장번호관리정책------------------------공통정책
            LO340: "", // 출고차수 채번 정책----------------------------------공통정책
            LO190: "", // 공급금액 계산 정책----------------------------------사업부정책
            LO219: "", // [B2CF]출고 신규등록 정책----------------------------물류센터정책
            LO231: "NN", // [B2C/B2CF]예정으로 등록 시 추가/삭제 허용 기준----사업부정책
            LO250: "", // 유통기한/제조배치번호 지정 정책---------------------사업부정책
            LO341: "", // 출고지시 선택 기본전표수
            LO419: "", // [B2CF]출고 확정 수량 수정 허용 기준-----------------물류센터정책
            LO450: "", // 송장 공급자 표시 기준
            LO499: "", // [B2CF]출고확정시 검수완료 체크여부------------------물류센터정책
            LO519: "", // [B2CF]출고배송 수량 수정 가능여부-------------------물류센터정책
            LO729: "", // [B2CF]기본배차 생성 기준----------------------------물류센터정책
    ...
    $("#btnOrderGroup").click(window.btnOrderGroupOnClick); // 주문구성선택 버튼 이벤트
    ...
    $NC.setEnable("#btnProcessA", false);
    ...
    $NC.setInitCombo("/WC/getDataSet.do", {
        ...
        P_QUERY_PARAMS: {
            ...
            P_CUST_CD: $ND.C_BASE_CUST_CD, // 풀핀먼트는 운송사별관리로 사용 (고객사별 사용 불가)
            P_HDC_DIV_ATTR03_CD: $ND.C_ALL, // Y:정제대상, N:정제미대상, %: 전체
            P_HDC_DIV_ATTR05_CD: "2", // 1:B2B, 2:B2C
            P_VIEW_DIV: "1" // 1:등록팝업, 2:조회팝업
        }
    }, {
    ...
    // 취소/처리 버튼 툴팁 세팅
    $NC.setTooltip("#btnProcessPreB", $NC.getDisplayMsg("JS.LOF02010E0.001", $NC.getValue("#btnProcessB") + " 취소", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessNxtB", $NC.getDisplayMsg("JS.LOF02010E0.002", $NC.getValue("#btnProcessB") + " 처리", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessPreC", $NC.getDisplayMsg("JS.LOF02010E0.001", $NC.getValue("#btnProcessC") + " 취소", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessNxtC", $NC.getDisplayMsg("JS.LOF02010E0.002", $NC.getValue("#btnProcessC") + " 처리", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessPreD", $NC.getDisplayMsg("JS.LOF02010E0.001", $NC.getValue("#btnProcessD") + " 취소", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessNxtD", $NC.getDisplayMsg("JS.LOF02010E0.002", $NC.getValue("#btnProcessD") + " 처리", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessPreE", $NC.getDisplayMsg("JS.LOF02010E0.001", $NC.getValue("#btnProcessE") + " 취소", $NC.getValue("#btnProcessE")));
    $NC.setTooltip("#btnProcessNxtE", $NC.getDisplayMsg("JS.LOF02010E0.002", $NC.getValue("#btnProcessE") + " 처리", $NC.getValue("#btnProcessE")));
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            if ($NC.isNull($NC.getValue("#edtQBu_Cd"))) {
                alert($NC.getDisplayMsg("JS.LOF02010E0.003", "브랜드를 지정하려면 사업부를 선택하십시오."));
                $NC.setValue(view);
        ...
        case "DELIVERY_CD":
            // 사업부 값을 지정하지 않았을 경우 오류 처리
            if ($NC.isNull($NC.getValue("#edtQBu_Cd"))) {
                alert($NC.getDisplayMsg("JS.LOF02010E0.004", "온라인몰을 지정하려면 사업부를 선택하십시오."));
                $NC.setValue(view);
                $NC.setFocus("#edtQBu_Cd");
                return;
            }
            $NP.onDeliveryChange(val, {
                ...
                title: $NC.getDisplayMsg("JS.LOF02010E0.005", "온라인몰 검색"),
                columnTitle: [
                    "온라인몰코드",
                    "온라인몰명"
                ],
                errorMessage: $NC.getDisplayMsg("JS.LOF02010E0.006", "등록되어 있지 않은 온라인몰입니다.")
            });
            return;
        case "OUTBOUND_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOF02010E0.007", "검색 출고일자를 정확히 입력하십시오."));
            setOutboundBatchCombo("#cboQOutbound_BatchC", false);
            setOutboundBatchCombo("#cboOutbound_BatchC", false);
            setOutboundBatchCombo("#cboQOutbound_BatchD", true);
            break;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOF02010E0.008", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOF02010E0.009", "검색 종료일자를 정확히 입력하십시오."));
            break;
...
function _OnInputChange(e, view, val) {
    ...
    switch (id) {
        case "OUTBOUND_BATCHC":
            ...
            break;
        case "OUTBOUND_DATEB":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOF02010E0.010", "출고일자를 정확히 입력하십시오."));
            setDeliveryBatchCombo($ND.C_PROCESS_ENTRY);
            break;
        case "DELIVERY_BATCHB":
            ...
            break;
        case "INOUT_DATED":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOF02010E0.011", "거래명세서일자를 정확히 입력하십시오."));
            break;
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.012", "물류센터를 선택하십시오."));
    ...
    if (STATE_PRE_YN == $ND.C_NO && STATE_CUR_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.017", "검색구분을 선택하십시오."));
    ...
    if ($NC.isNull(ITEM_CNT1)) {
        ...
        if (!Number(ITEM_CNT1) || ITEM_CNT1.length > 10) {
            alert($NC.getDisplayMsg("JS.LOF02010E0.018", "상품수 첫번째 값을 정확히 입력하십시오."));
    ...
    if (!$NC.isNull(ITEM_CNT2)) {
        if (!Number(ITEM_CNT2) || ITEM_CNT2.length > 10) {
            alert($NC.getDisplayMsg("JS.LOF02010E0.019", "상품수 두번째 값을 정확히 입력하십시오."));
    ...
    if ($NC.isNull(TOT_ITEM_QTY1)) {
        ...
        if (!Number(TOT_ITEM_QTY1) || TOT_ITEM_QTY1.length > 10) {
            alert($NC.getDisplayMsg("JS.LOF02010E0.020", "내품수량 첫번째 값을 정확히 입력하십시오."));
    ...
    if (!$NC.isNull(TOT_ITEM_QTY2)) {
        if (!Number(TOT_ITEM_QTY2) || TOT_ITEM_QTY2.length > 10) {
            alert($NC.getDisplayMsg("JS.LOF02010E0.021", "내품수량 두번째 값을 정확히 입력하십시오."));
    ...
    switch ($NC.G_VAR.activeView.PROCESS_CD) {
        ...
        case $ND.C_PROCESS_DIRECTIONS:
            ...
            if ($NC.isNull(OUTBOUND_BATCH)) {
                alert($NC.getDisplayMsg("JS.LOF02010E0.022", "출고차수를 선택하십시오."));
        ...
        case $ND.C_PROCESS_DELIVERY:
            // 아래 레포트는 차수 필수 선택
            // PAPER_LOF02 - 토탈피킹지시서
            // PAPER_LOF03 - 결품토탈피킹지시서
            // PAPER_LOF04 - 분배지시서
            // PAPER_LOF05 - 토탈(원패킹)지시서
            // PAPER_LOF07 - 토탈(원패킹제외)지시서
            if (reportInfo.REPORT_CD == "PAPER_LOF02" //
                || reportInfo.REPORT_CD == "PAPER_LOF03" //
                || reportInfo.REPORT_CD == "PAPER_LOF04" //
                || reportInfo.REPORT_CD == "PAPER_LOF05" //
                || reportInfo.REPORT_CD == "PAPER_LOF07") {
            ...
            break;
        default:
            alert($NC.getDisplayMsg("JS.LOF02010E0.023", "해당 프로세스는 출력물이 없습니다."));
            return;
    }
    ...
    switch (reportInfo.REPORT_CD) {
        ...
        case "PAPER_LOF03":
            ...
            if (OUTBOUND_BATCH == $ND.C_BASE_BATCH_NO || OUTBOUND_BATCH == $ND.C_ALL) {
                alert($NC.getDisplayMsg("JS.LOF02010E0.024", "결품차수를 선택하십시오."));
        ...
        case "LABEL_LOF04":
            ...
            checkedData = $NC.getGridCheckedValues(grdObject, {
                ...
                dataType: "S", // 문자열 결합
                compareFn: function(rowData) {
                    return rowData.OUTBOUND_STATE >= $ND.C_STATE_DIRECTIONS && rowData.HDC_DIV == $ND.C_HDC_DIV_EPOST_B2C;
                }
            });
            queryParams = {
                P_CENTER_CD: CENTER_CD,
        ...
        case "LABEL_LOF05":
            ...
            checkedData = $NC.getGridCheckedValues(grdObject, {
                ...
                dataType: "S", // 문자열 결합
                compareFn: function(rowData) {
                    return rowData.OUTBOUND_STATE >= $ND.C_STATE_DIRECTIONS && rowData.HDC_DIV == $ND.C_HDC_DIV_HANJIN_B2C;
                }
            });
            queryParams = {
                P_CENTER_CD: CENTER_CD,
        ...
        case "LABEL_LOF06":
            ...
            checkedData = $NC.getGridCheckedValues(grdObject, {
                ...
                dataType: "S", // 문자열 결합
                compareFn: function(rowData) {
                    return rowData.OUTBOUND_STATE >= $ND.C_STATE_DIRECTIONS && rowData.HDC_DIV == $ND.C_HDC_DIV_CJ_B2C;
                }
            });
            queryParams = {
                P_CENTER_CD: CENTER_CD,
...
function setReportInfo() {
    ...
    $NC.setProgramReportInfo({
        ...
        if ($NC.G_VAR.activeView.PROCESS_CD == $ND.C_PROCESS_ENTRY) {
            ...
            // PAPER_LOF04 - 분배지시서
            // PAPER_LOF05 - 토탈피킹지시서(원패킹)
            // PAPER_LOF07 - 토탈피킹지시서(원패킹제외)
            return reportInfo.REPORT_CD != "PAPER_LOF02" //
                && reportInfo.REPORT_CD != "PAPER_LOF03" //
                && reportInfo.REPORT_CD != "PAPER_LOF04" //
                && reportInfo.REPORT_CD != "PAPER_LOF05" //
                && reportInfo.REPORT_CD != "PAPER_LOF07";
...
function setMasterSummaryInfo() {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOF02010E0/getDataSet.do", {
        P_QUERY_ID: "LOF02010E0.RS_MASTER",
        P_QUERY_PARAMS: {
            P_CENTER_CD: CENTER_CD,
            P_BU_CD: BU_CD,
            P_OUTBOUND_DATE: OUTBOUND_DATE,
...
function setPolicyValInfo() {
    ...
    $NC.setPolicyValInfo({
        ...
        // 선택주문자수 설정
        $NC.setValue("#edtBill_Cnt", $NC.G_VAR.policyVal.LO341);
        $NC.setVisible("#divDelivery_BatchB", $NC.G_VAR.policyVal.LO729 == $ND.C_YES);
    });
}
...
function getOutboundState(params, onSuccess) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOF02010E0/getData.do", {
