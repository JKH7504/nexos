...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            return;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOM01020Q0.001", "검색 시작일자를 정확히 입력하십시오."));
            ...
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOM01020Q0.002", "검색 종료일자를 정확히 입력하십시오."));
            $NC.setInitDatePicker("#dtpOrder_Date", val);
            break;
        case "DELIVERY_CD":
            $NP.onDeliveryChange(val, {
                ...
                title: $NC.getDisplayMsg("JS.LOM01020Q0.003", "온라인몰 검색"),
                columnTitle: [
                    "온라인몰코드",
                    "온라인몰명"
                ],
                errorMessage: $NC.getDisplayMsg("JS.LOM01020Q0.004", "등록되어 있지 않은 온라인몰입니다.")
            });
            return;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOM01020Q0.005", "물류센터를 선택하십시오."));
        $NC.setFocus("#cboQCenter_Cd");
        return;
    }
    var BU_CD = $NC.getValue("#edtQBu_Cd");
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LOM01020Q0.006", "사업부를 입력하십시오."));
        $NC.setFocus("#edtQBu_Cd");
        return;
    }
    var ORDER_DATE1 = $NC.getValue("#dtpQOrder_Date1");
    if ($NC.isNull(ORDER_DATE1)) {
        alert($NC.getDisplayMsg("JS.LOM01020Q0.007", "출고예정 검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQOrder_Date1");
        return;
    }
    var ORDER_DATE2 = $NC.getValue("#dtpQOrder_Date2");
    if ($NC.isNull(ORDER_DATE2)) {
        alert($NC.getDisplayMsg("JS.LOM01020Q0.008", "출고예정 검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQOrder_Date2");
        return;
    }
    if (ORDER_DATE1 > ORDER_DATE2) {
        alert($NC.getDisplayMsg("JS.LOM01020Q0.009", "출고예정 검색 범위 오류입니다."));
        $NC.setFocus("#dtpQOrder_Date1");
        return;
    }
    var INOUT_CD = $NC.getValue("#cboQInout_Cd");
    if ($NC.isNull(INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LOM01020Q0.010", "출고구분을 선택하십시오."));
    ...
    // 데이터 조회
    $NC.serviceCall("/LOM01020Q0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function btnSetShortageEntryOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LOB01020Q0.011", "처리할 데이터가 없습니다."));
        return;
    }
    ...
    if ($NC.isNull(ORDER_DATE)) {
        alert($NC.getDisplayMsg("JS.LOM01020Q0.012", "출고일자를 입력하십시오."));
        $NC.setFocus("#dtpOrder_Date");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOM01020Q0.013", "세트결품예정내역을 생성 하시겠습니까?"))) {
        return;
    }

    $NC.serviceCall("/LOM01020Q0/callLOShortageSetItem.do", {
        P_CENTER_CD: CENTER_CD,
        P_BU_CD: BU_CD,
