...
function grdMasterBOnDblClick(e, args) {
    ...
    if (refRowData.CAR_SUB_CD == "2" && refRowData.ACS_ERROR_CD != $ND.C_BASE_CD_L1
        ...
        alert($NC.getDisplayMsg("JS.LOB02010E1.069", "주소 정제 후 팝업화면에서 작업하실 수 있습니다."));
        return;
    }
...
function onSaveAcsB(ajaxData) {
    ...
    if (oMsg != $ND.C_OK) {
        ...
        if (resultData.O_ACS_ERROR_MSG == $ND.C_OK) {
            if (acsError > 0) {
                alert($NC.getDisplayMsg("JS.LOB02010E1.099", "주소정제에 실패한 데이터가 존재합니다. 데이터를 확인하십시오.\n" //
                    + "(전체건수: " + acsTotal + ", 정제호출: " + acsProcess + ", 정제오류: " + acsError + ")", acsTotal, acsProcess, acsError));
            }
        } else {
            alert($NC.getDisplayMsg("JS.LOB02010E1.100", "주소정제 처리 중 오류가 발생했습니다.\n" //
                + "(전체건수: " + acsTotal + ", 정제호출: " + acsProcess + ", 정제오류: " + acsError + ")", acsTotal, acsProcess, acsError) + "\n"
...
function btnAcsGetAddressOnClick() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.101", "주소정제 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.102", "주소정제 처리할 데이터를 선택하십시오."));
        return;
    }

    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.103", "선택한 데이터 중 주소정제 가능한 데이터가 없습니다."));
        return;
    }
    ...
    $NC.serviceCall("/LOB02010E0/callAcsGetAddress.do", {
        ...
        type: 2,
        message: $NC.getDisplayMsg("JS.LOB02010E1.104", "주소정제 처리 중 입니다. 잠시만 기다려 주십시오...")
    });
}
...
function btnSetCarrierCdBOnClick() {
    ...
    if ($NC.isNull(carrierData.CARRIER_CD)) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.105", "변경할 운송사를 선택하십시오."));
        $NC.setFocus("#cboCarrier_CdB");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.106", "운송사를 변경하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.107", "운송사 변경할 데이터를 선택하십시오."));
        return;
    }

    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.108", "선택한 데이터 중 운송사 변경 가능한 데이터가 없습니다."));
        return;
    }
