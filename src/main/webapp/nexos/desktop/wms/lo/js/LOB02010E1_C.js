...
function _InquiryC(args) {
    ...
    G_GRDMASTERC.queryParams = {
        ...
        P_EQUIP_DIV: args.EQUIP_DIV,
        P_STATE_PRE_YN: args.STATE_PRE_YN,
        P_STATE_CUR_YN: args.STATE_CUR_YN
    };
    // 데이터 조회
    $NC.serviceCall("/LOB02010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTERC), onGetMasterC);
}
...
function onProcessPreC() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.053", "출고지시 취소 처리하시겠습니까?\n출고차수(" + OUTBOUND_BATCH_F + ")", OUTBOUND_BATCH_F))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.054", "출고지시 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.055", "선택한 데이터 중 출고지시 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtC() {
    ...
    if (DIRECTIONS_DIV == "2" && !batchInfo.isTotal) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.056", "[신규] 또는 [토탈출고] 차수를 선택하고 출고지시 처리하십시오."));
    ...
    if (DIRECTIONS_DIV == "1") {
        if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.057", "개별 출고지시 처리하시겠습니까?"))) {
            return;
        }
    } else {
        if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.058", "출고지시 처리하시겠습니까?\n출고차수(" + OUTBOUND_BATCH_F + ")", OUTBOUND_BATCH_F))) {
            return;
        }
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.059", "출고지시 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.060", "선택한 데이터 중 출고지시 처리 가능한 데이터가 없습니다."));
        return;
    }
    ...
    if (DIRECTIONS_DIV == "2") {
        ...
        // 기존 차수에 추가
            ...
            // 기존 출고차수에 추가 가능한지 체크
            $NC.serviceCallAndWait("/LOB02010E0/getDataSet.do", {
                P_QUERY_ID: "LOB02010E1.RS_SUB6",
                P_QUERY_PARAMS: {
                ...
                if ($NC.isNull(dsResult) || dsResult.length == 0) {
                    alert($NC.getDisplayMsg("JS.LOB02010E1.061", "기존 출고차수에 추가 가능여부를 확인하지 못했습니다. 다시 처리하십시오."));
                    return;
                }

                if (dsResult[0].EQUIP_STATE > "01") {
                    alert($NC.getDisplayMsg("JS.LOB02010E1.062", "출고차수(" + OUTBOUND_BATCH_F + ")는 이미 출하장비로 전송되어 해당 차수로 추가 출고지시할 수 없습니다.",
                        OUTBOUND_BATCH_F));
                    return;
                }

                if (EQUIP_DIV != dsResult[0].EQUIP_DIV) {
                    alert($NC.getDisplayMsg("JS.LOB02010E1.063", "해당 차수는 다른 출하장비(" + dsResult[0].EQUIP_DIV_D + ")로 할당되어 추가 출고지시할 수 없습니다.",
    ...
    if (OUTBOUND_BATCH == $ND.C_BASE_BATCH_NO) {
        $NC.serviceCallAndWait("/LOB02010E0/getOutboundBatch.do", {
            P_CENTER_CD: CENTER_CD,
            P_BU_CD: BU_CD,
            P_OUTBOUND_DATE: OUTBOUND_DATE,
...
function grdMasterCInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterC", {
        ...
        queryId: "LOB02010E1.RS_T2_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
...
function grdMasterCOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOB02010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAILC), onGetDetailC);
...
function grdDetailCInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailC", {
        ...
        queryId: "LOB02010E1.RS_T2_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function onGetMasterC(ajaxData) {
    ...
    if ($NC.setInitGridAfterOpen(G_GRDMASTERC, "OUTBOUND_NO", true)) {
        // 마스터 총수량 세팅
        rowData = G_GRDMASTERC.data.getItem(0);
        ...
        $NC.setValue("#edtTotal_Rdelivery_CntC", $NC.getDisplayNumber(rowData.TOTAL_RDELIVERY_CNT));
    } else {
        // 디테일 초기화
        $NC.clearGridData(G_GRDDETAILC);
        ...
        $NC.setValue("#edtTotal_Rdelivery_CntC");
    }
...
function btnEquipSendOnClick() {
    ...
    // 신규차수
    if (batchInfo.isNew) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.064", "[출하장비전송] 처리할 출고차수를 선택하십시오."));
        return;
    }
    // 개별차수
    else if (!batchInfo.isTotal) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.065", "[토탈출고] 차수를 선택하고 [출하장비전송] 처리하십시오."));
        return;
    }

    var EQUIP_DIV = $NC.getValue("#cboEquip_Div");
    if ($NC.isNull(EQUIP_DIV)) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.066", "출하장비구분을 선택하십시오."));
    ...
    if ($NC.isNull(EQUIP_ALLOC_DIV)) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.067", "할당구분을 선택하십시오."));
    ...
    $NC.serviceCall("/LOB02010E0/getDataSet.do", {
        ...
        if ($NC.isNull(dsResult)) {
            alert($NC.getDisplayMsg("JS.LOB02010E1.066", "소터투입 불가 상품 체크에 오류가 발생 했습니다."));
            return;
        }
        ...
        if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.067", message + "출하장비전송 처리 하시겠습니까?", message))) {
            return;
        }
        ...
        $NC.serviceCall("/LOB02010E0/callSendFwLoBatch.do", {
            ...
            alert($NC.getDisplayMsg("JS.LOB02010E1.068", "[출하장비전송] 처리가 완료되었습니다."));
            window._Inquiry();
        });
    });
}
...
function btnEquipSendCancelOnClick() {
    ...
    // 신규차수
    if (batchInfo.isNew) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.070", "[출하장비전송취소] 처리할 출고차수를 선택 하십시오."));
        return;
    }
    // 개별차수
    else if (!batchInfo.isTotal) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.071", "[토탈출고] 차수를 선택하고 [출하장비전송취소] 처리하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.072", "출하장비전송취소 처리 하시겠습니까?"))) {
        return;
    }
    ...
    $NC.serviceCall("/LOB02010E0/callSendBwLoBatch.do", {
        ...
        alert($NC.getDisplayMsg("JS.LOB02010E1.073", "[출하장비전송취소] 처리가 완료되었습니다."));
        window._Inquiry();
    });
}
