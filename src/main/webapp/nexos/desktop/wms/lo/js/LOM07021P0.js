...
function onAddWbNo() {
    ...
    if (!isNaN(WB_NO_CNT)) {
        if (WB_NO_CNT <= 0 || WB_NO_CNT > 999) {
            alert($NC.getDisplayMsg("JS.LOM07021P0.002", "발행매수를 확인하십시오."));
            return;
        }
    } else {
        alert($NC.getDisplayMsg("JS.LOM07021P0.003", "발행매수는 숫자만 입력가능합니다."));
        return;
    }

    $NC.serviceCall("/LOM07020E0/callLOWbNoAdd.do", {
        P_CENTER_CD: CENTER_CD,
        P_BU_CD: BU_CD,
        P_OUTBOUND_DATE: OUTBOUND_DATE,
        P_OUTBOUND_NO: OUTBOUND_NO,
...
function doPrint1(dsBoxNo) {
    ...
    if (reportIndex == -1) {
        alert($NC.getDisplayMsg("JS.LOM07021P0.004", "출력할 레포트 정보를 검색할 수 없습니다. 관리자에게 문의하십시오."));
        return;
    }
    ...
    for (rIndex = 0, rCount = dsBoxNo.length; rIndex < rCount; rIndex++) {
        ...
        $NC.serviceCallAndWait("/LOM07020E0/callSetWbNoPrintCntUpdate.do", {
            P_CENTER_CD: rowData.CENTER_CD,
            P_BU_CD: rowData.BU_CD,
