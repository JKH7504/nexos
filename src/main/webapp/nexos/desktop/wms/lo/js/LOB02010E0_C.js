...
function onProcessPreC() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOB02010E0.052", "출고지시 취소 처리하시겠습니까?\n출고차수(" + OUTBOUND_BATCH_F + ")", OUTBOUND_BATCH_F))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.053", "출고지시 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.054", "선택한 데이터 중 출고지시 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtC() {
    ...
    if (DIRECTIONS_DIV == "2" && !batchInfo.isTotal) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.055", "[신규] 또는 [토탈출고] 차수를 선택하고 출고지시 처리하십시오."));
    ...
    if (DIRECTIONS_DIV == "1") {
        if (!confirm($NC.getDisplayMsg("JS.LOB02010E0.056", "개별 출고지시 처리하시겠습니까?"))) {
            return;
        }
    } else {
        if (!confirm($NC.getDisplayMsg("JS.LOB02010E0.057", "출고지시 처리하시겠습니까?\n출고차수(" + OUTBOUND_BATCH_F + ")", OUTBOUND_BATCH_F))) {
            return;
        }
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.058", "출고지시 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.059", "선택한 데이터 중 출고지시 처리 가능한 데이터가 없습니다."));
        return;
    }
        ...
        // 기존 차수에 추가
            ...
            $NC.serviceCallAndWait("/LOB02010E0/getDataSet.do", {
                ...
                if ($NC.isNull(dsResult) || dsResult.length == 0) {
                    alert($NC.getDisplayMsg("JS.LOB02010E0.060", "기존 출고차수에 추가 가능여부를 확인하지 못했습니다. 다시 처리하십시오."));
                    return;
                }

                if (dsResult[0].EQUIP_STATE > "01") {
                    alert($NC.getDisplayMsg("JS.LOB02010E0.061", "출고차수(" + OUTBOUND_BATCH_F + ")는 이미 출하장비로 전송되어 해당 차수로 추가 출고지시할 수 없습니다.",
                        OUTBOUND_BATCH_F));
                    return;
                }

                if (EQUIP_DIV != dsResult[0].EQUIP_DIV) {
                    alert($NC.getDisplayMsg("JS.LOB02010E0.062", "해당 차수는 다른 출하장비(" + dsResult[0].EQUIP_DIV_D + ")로 할당되어 추가 출고지시할 수 없습니다.",
...
function grdMasterCInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterC", {
        ...
        queryId: "LOB02010E0.RS_T2_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
...
function grdDetailCInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailC", {
        ...
        queryId: "LOB02010E0.RS_T2_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function btnBillCntOnClick() {
    ...
    if (billCount <= 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.063", "1이상의 전표수를 입력하십시오."));
...
function btnEquipSendOnClick() {
    ...
    // 신규차수
    if (batchInfo.isNew) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.064", "[출하장비전송] 처리할 출고차수를 선택하십시오."));
        return;
    }
    // 개별차수
    else if (!batchInfo.isTotal) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.065", "[토탈출고] 차수를 선택하고 [출하장비전송] 처리하십시오."));
        return;
    }

    var EQUIP_DIV = $NC.getValue("#cboEquip_Div");
    if ($NC.isNull(EQUIP_DIV)) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.066", "출하장비구분을 선택하십시오."));
    ...
    if ($NC.isNull(EQUIP_ALLOC_DIV)) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.067", "할당구분을 선택하십시오."));
    ...
    $NC.serviceCall("/LOB02010E0/getDataSet.do", {
        ...
        if ($NC.isNull(dsResult)) {
            alert($NC.getDisplayMsg("JS.LOB02010E0.068", "소터투입 불가 상품 체크에 오류가 발생 했습니다."));
            return;
        }
        ...
        if (!confirm($NC.getDisplayMsg("JS.LOB02010E0.069", message + "출하장비전송 처리 하시겠습니까?", message))) {
            return;
        }
        ...
        $NC.serviceCall("/LOB02010E0/callSendFwLoBatch.do", {
            ...
            alert($NC.getDisplayMsg("JS.LOB02010E0.070", "[출하장비전송] 처리가 완료되었습니다."));
            window._Inquiry();
        });
    });
}
...
function btnEquipSendCancelOnClick() {
    ...
    // 신규차수
    if (batchInfo.isNew) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.071", "[출하장비전송취소] 처리할 출고차수를 선택하십시오."));
        return;
    }
    // 개별차수
    else if (!batchInfo.isTotal) {
        alert($NC.getDisplayMsg("JS.LOB02010E0.072", "[토탈출고] 차수를 선택하고 [출하장비전송취소] 처리하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOB02010E0.073", "출하장비전송취소 처리 하시겠습니까?"))) {
        return;
    }
    ...
    $NC.serviceCall("/LOB02010E0/callSendBwLoBatch.do", {
        ...
        alert($NC.getDisplayMsg("JS.LOB02010E0.074", "[출하장비전송취소] 처리가 완료되었습니다."));
        window._Inquiry();
    });
}
