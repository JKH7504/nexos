...
function onProcessPreE() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.088", "배송완료 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.089", "배송완료 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.090", "선택한 데이터 중 배송완료 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtE() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.091", "배송완료 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDDETAILE)) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.092", "데이터가 수정되었습니다. 저장 후 배송완료 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.093", "배송완료 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.094", "선택한 데이터 중 배송완료처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdDetailEOnCellChange(e, args) {
    ...
    switch (G_GRDDETAILE.view.getColumnId(args.cell)) {
        case "DELIVERY_QTY":
            ...
            if (DELIVERY_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LOB02010E1.095", "배송수량에 0보다 작은값을 입력할 수 없습니다."));
                ...
                alert($NC.getDisplayMsg("JS.LOB02010E1.096", "배송수량이 확정수량을 초과할 수 없습니다."));
                rowData.DELIVERY_QTY = args.oldItem.DELIVERY_QTY;
                rowData.MISSED_QTY = args.oldItem.MISSED_QTY;
                $NC.setFocusGrid(G_GRDDETAILE, args.row, args.cell, true);
                break;
