...
function _Initialize() {
    ...
    $("#btnCancelOrderFw").click(btnCancelOrderFwOnClick);
    $("#btnCancelOrderBw").click(btnCancelOrderBwOnClick);
    $("#btnCancelOrderRestore").click(btnCancelOrderRestoreOnClick);
...
function _OnLoaded() {
    ...
    $NC.setInitSplitter("#ctrT1TabSheetView", "hb", $NC.G_OFFSET.topViewWidth);
}
    ...
    switch (id) {
        ...
        case "DELIVERY_CD":
            if ($NC.isNull($NC.getValue("#edtQBu_Cd"))) {
                alert($NC.getDisplayMsg("JS.LOC02020E0.004", "온라인몰을 지정하려면 먼저 사업부를 입력하십시오."));
            ...
            $NP.onDeliveryChange(val, {
                ...
                title: $NC.getDisplayMsg("JS.LOC02020E0.005", "온라인몰 검색"),
                columnTitle: [
                    "온라인몰코드",
                    "온라인몰명"
                ],
                errorMessage: $NC.getDisplayMsg("JS.LOC02020E0.006", "등록되어 있지 않은 온라인몰입니다.")
            });
            return;
        case "OUTBOUND_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOC02020E0.001", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "OUTBOUND_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOC02020E0.002", "검색 종료일자를 정확히 입력하십시오."));
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOC02020E0.003", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(OUTBOUND_DATE1)) {
        alert($NC.getDisplayMsg("JS.LOC02020E0.007", "검색 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQOutbound_Date1");
        return;
    }
    var OUTBOUND_DATE2 = $NC.getValue("#dtpQOutbound_Date2");
    if ($NC.isNull(OUTBOUND_DATE2)) {
        alert($NC.getDisplayMsg("JS.LOC02020E0.008", "검색 종료일자를 입력하십시오."));
    ...
    var SO_NO = $NC.getValue("#edtQSo_No");
    ...
    if ($NC.getTabActiveIndex("#ctrMasterView") == 0) {
        ...
        // 데이터 조회
        $NC.serviceCall("/LOC02020E0/getDataSet.do", $NC.getGridParams(G_GRDT1MASTER), onGetT1Master);
    } else {
        // 조회시 전역 변수 값 초기화
        ...
        // 데이터 조회
        $NC.serviceCall("/LOC02020E0/getDataSet.do", $NC.getGridParams(G_GRDT2MASTER), onGetT2Master);
    }
}
...
function grdT1MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Master", {
        ...
        queryId: "LOC02020E0.RS_T1_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
...
function grdT1MasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOC02020E0/getDataSet.do", $NC.getGridParams(G_GRDT1DETAIL), onGetT1Detail);
...
function grdT1DetailInitialize() {
    ...
    $NC.setInitGridObject("#grdT1Detail", {
        ...
        queryId: "LOC02020E0.RS_T1_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdT2MasterInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Master", {
        ...
        queryId: "LOC02020E0.RS_T2_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
...
function grdT2MasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOC02020E0/getDataSet.do", $NC.getGridParams(G_GRDT2DETAIL), onGetT2Detail);
...
function grdT2DetailInitialize() {
    ...
    $NC.setInitGridObject("#grdT2Detail", {
        ...
        queryId: "LOC02020E0.RS_T2_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function tabOnActivate(event, ui) {
    ...
    var tabActiveIndex = $NC.getTabActiveIndex("#ctrMasterView");
    var container = "#ctrT" + (tabActiveIndex + 1) + "TabSheetView";
...
function btnCancelOrderFwOnClick() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOC02020E0.009", "조회 후 처리하십시오."));
        return;
    }

    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.CANCEL_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LOC02020E0.010", "이미 주문취소 처리된 전표입니다. 주문취소되지 않은 전표를 선택하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOC02020E0.011", "주문취소 처리하시겠습니까?"))) {
        return;
    }

    $NC.serviceCall("/LOC02020E0/callLOCancelOrderProcessing.do", {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_OUTBOUND_DATE: rowData.OUTBOUND_DATE,
        P_OUTBOUND_NO: rowData.OUTBOUND_NO,
...
function btnCancelOrderBwOnClick() {
    ...
    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.CANCEL_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LOC02020E0.012", "주문취소 처리되지 않은 전표입니다. 주문취소 전표 선택 후 취소하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOC02020E0.013", "[주문취소] 취소 처리하시겠습니까?"))) {
        return;
    }
...
function btnCancelOrderRestoreOnClick() {

    if (!$NC.getProgramPermission().canConfirmCancel) {
        alert($NC.getDisplayMsg("JS.LOC02020E0.014", "해당 프로그램의 취소권한이 없습니다."));
        return;
    }
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOC02020E0.015", "취소주문 복원 처리하시겠습니까?\n\n※ 취소주문 복원은 출고예정 상태로 복원됩니다."))) {
        return;
    }
...
function setUserProgramPermission() {
    ...
    var canConfirmCancel = $NC.getProgramPermission().canConfirmCancel;

    $NC.setEnable("#btnCancelOrderFw", enableT1 && canSave);
    $NC.setEnable("#btnCancelOrderBw", enableT1 && canSave);
    $NC.setEnable("#btnCancelOrderRestore", enableT2 && canConfirmCancel);
}