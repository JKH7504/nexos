...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            ...
            return;
        case "OUTBOUND_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB04030E0.001", "검색 출고일자를 정확히 입력하십시오."));
            setOutboundBatchCombo();
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOB04030E0.002", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LOB04030E0.003", "사업부를 입력하십시오."));
    ...
    if ($NC.isNull(OUTBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.LOB04030E0.004", "출고일자를 입력하십시오."));
    ...
    // 데이터 조회
    $NC.serviceCall("/LOB04030E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LOB04030E0.005", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (totPickQty != Number(refRowData.PICK_QTY)) {
        alert($NC.getDisplayMsg("JS.LOB04030E0.006", "배송처별 출고정보의 피킹수량 합(" + totPickQty + ")과\n상품의 피킹수량(" + refRowData.PICK_QTY + ")과  일치하지 않습니다.",
            totPickQty, refRowData.PICK_QTY));
        return;
    }

    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB04030E0.007", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LOB04030E0/callPolicyAssignAdjustment.do", {
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        case "PICK_QTY":
            if (Number(rowData.ENTRY_QTY) < Number(rowData.PICK_QTY)) {
                alert($NC.getDisplayMsg("JS.LOB04030E0.008", "변경할 수량이 등록된 수량보다 많습니다."));
                rowData.PICK_QTY = args.oldItem.PICK_QTY;
...
function grdDetailOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.PICK_QTY)) {
            alert($NC.getDisplayMsg("JS.LOB04030E0.009", "피킹수량을 입력하십시오."));
            $NC.setFocusGrid(G_GRDDETAIL, row, "PICK_QTY", true);
            return false;
...
function grdMasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOB04030E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
...
function onGetDetail(ajaxData) {
    ...
    $NC.setInitGridAfterOpen(G_GRDDETAIL, "DELIVERY_CD");
}
...
function onSave(ajaxData) {
    ...
    // 데이터 조회 - 하단그리드
    $NC.serviceCall("/LOB04030E0/getDataSet.do", {
        P_QUERY_ID: "LOB04030E0.RS_DETAIL",
        P_QUERY_PARAMS: {
            P_CENTER_CD: rowData.CENTER_CD,
            P_BU_CD: rowData.BU_CD,
            P_OUTBOUND_DATE: rowData.OUTBOUND_DATE,
