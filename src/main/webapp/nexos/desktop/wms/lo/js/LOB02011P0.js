...
function _Initialize() {
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.LOB02011P0.029", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.LOB02011P0.030", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.LOB02011P0.031", "저장"));
}
...
function _OnLoaded() {
    ...
    $NC.setVisible("#divDelivery_Batch", $NC.G_VAR.G_PARAMETER.P_POLICY_LO720 == $ND.C_YES);
    ...
    // 예정 -> 등록, 등록 수정
        ...
        $NC.G_VAR.masterData = {
            ...
            CARRIED_FORWARD_AMT: dsMaster.CARRIED_FORWARD_AMT,
            MONTH_PAYMENT_AMT: dsMaster.MONTH_PAYMENT_AMT,
            MONTH_SALE_AMT: dsMaster.MONTH_SALE_AMT,
            DAY_OUTSTANDING_AMT: dsMaster.DAY_OUTSTANDING_AMT,
        ...
        try {
            for (rIndex = 0, rCount = dsDetail.length; rIndex < rCount; rIndex++) {
                ...
                if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ORDER) { // 예정
                    ...
                    rowData.DISTRIBUTE_QTY = ORDER_QTY;
    ...
    // 조회조건 - 출고구분 세팅
    var isNew = $NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ENTRY_CREATE;
    $NC.setInitCombo("/WC/getDataSet.do", {
        P_QUERY_ID: "WC.POP_CMCODE",
        P_QUERY_PARAMS: {
            ...
            P_COMMON_CD: $ND.C_ALL,
            P_ATTR01_CD: [
                !isNew || $NC.G_VAR.G_PARAMETER.P_POLICY_LO210 == $ND.C_YES ? $ND.C_INOUT_GRP_D1 : "",
                !isNew || $NC.G_VAR.G_PARAMETER.P_POLICY_LO212 == $ND.C_YES ? $ND.C_INOUT_GRP_D2 : ""
            ].join($ND.C_SEP_DATA),
            P_VIEW_DIV: "1" // 1:등록팝업, 2:조회팝업
        }
    }, {
...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOB02010E0/getDataSet.do", $NC.getGridParams(G_GRDSTOCK), onGetStock);
}
...
function _New() {
    ...
    if ($NC.isNull(DELIVERY_CD)) {
        alert($NC.getDisplayMsg("JS.LOB02011P0.002", "먼저 배송처 코드를 입력하십시오."));
        $NC.setFocus("#edtDelivery_Cd");
        return;
    }
    ...
    if ($NC.isNull(RDELIVERY_CD)) {
        alert($NC.getDisplayMsg("JS.LOB02011P0.003", "먼저 실배송처 코드를 입력하십시오."));
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOB02011P0.005", "물류센터를 확인할 수 없습니다. 취소 후 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LOB02011P0.006", "사업부를 확인할 수 없습니다. 취소 후 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.OUTBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.LOB02011P0.007", "먼저 출고일자를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LOB02011P0.008", "먼저 출고구분을 선택하십시오."));
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LOB02011P0.004", "저장할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOB02011P0.009", "저장하시겠습니까?"))) {
        return;
    }
    ...
    for (rIndex = 0, rCount = G_GRDDETAIL.data.getLength(); rIndex < rCount; rIndex++) {
        ...
        if (rowData.SHORTAGE_YN == $ND.C_YES) {
            alert($NC.getDisplayMsg("JS.LOB02011P0.010", "출고가능량을 초과한 상품이 존재합니다\n조정 후 저장하십시오."));
    ...
    if ($NC.G_VAR.G_PARAMETER.P_POLICY_LO720 == $ND.C_YES) {
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02011P0.011", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LOB02010E0/save.do", {
        ...
        P_DS_MASTER: {
            ...
            P_CARRIED_FORWARD_AMT: $NC.G_VAR.masterData.CARRIED_FORWARD_AMT,
            P_MONTH_PAYMENT_AMT: $NC.G_VAR.masterData.MONTH_PAYMENT_AMT,
            P_MONTH_SALE_AMT: $NC.G_VAR.masterData.MONTH_SALE_AMT,
            P_DAY_OUTSTANDING_AMT: $NC.G_VAR.masterData.DAY_OUTSTANDING_AMT,
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOB02011P0.012", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        case "INOUT_CD":
            // 입출고그룹 속성 취득
            var ATTR01_CD = $NC.getComboData("#cboInout_Cd", -1).ATTR01_CD;
            // 입출고그룹이 다를 경우 배송처/실배송처 클리어
            if ($NC.G_VAR.masterData.INOUT_CD_SUB != ATTR01_CD) {
                $NC.setValue("#edtDelivery_Cd");
                $NC.setValue("#edtRDelivery_Cd");
                $NC.setValue("#edtDelivery_Nm");
                $NC.setValue("#edtRDelivery_Nm");
                $NC.G_VAR.masterData.INOUT_CD_SUB = ATTR01_CD;
            }
        ...
        case "ORDER_DIV":
            ...
            break;
        case "OUTBOUND_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LOB02011P0.013", "출고일자를 정확히 입력하십시오."));
        ...
        case "DELIVERY_CD":
            $NP.onDeliveryChange(args.val, {
                ...
                P_DELIVERY_DIV: $NC.G_VAR.masterData.INOUT_CD_SUB === $ND.C_INOUT_GRP_D2 ? "91" : $ND.C_ALL, // 수송출고일 경우 91-물류센터
                P_VIEW_DIV: "1"
            }, onDeliveryPopup);
            return;
        case "RDELIVERY_CD":
...
function buttonsFormatter(row, cell, value, columnDef, dataContext) {
    return "<input type='button' class='btnIcoPopup4Stock' style='width: 100%; height: 100%; ' />";
}
...
function grdDetailOnCellChange(e, args) {
    ...
    switch (G_GRDDETAIL.view.getColumnId(args.cell)) {
        ...
        case "ENTRY_QTY":
            ...
            if (ENTRY_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LOB02011P0.014", "등록수량이 0보다 작을 수 없습니다."));
            ...
            if (Number(rowData.ORDER_LINE_NO) > 0 && $NC.getValue("#cboInout_Cd") != "D20") {
                if (Number(rowData.ORDER_QTY) < ENTRY_QTY) {
                    alert($NC.getDisplayMsg("JS.LOB02011P0.015", "등록수량이 예정수량을 초과할 수 없습니다."));
                    rowData.ENTRY_QTY = args.oldItem.ENTRY_QTY;
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                    break;
                }
            ...
            if (Number(rowData.ORDER_QTY) == ENTRY_QTY || $NC.G_VAR.ORDER_YN == $ND.C_NO) {
        ...
        case "ENTRY_BOX":
            ...
            if (ENTRY_BOX < 0) {
                alert($NC.getDisplayMsg("JS.LOB02011P0.016", "등록BOX는 0보다 작을 수 없습니다."));
        ...
        case "ENTRY_EA":
            ...
            if (ENTRY_EA < 0) {
                alert($NC.getDisplayMsg("JS.LOB02011P0.017", "등록EA는 0보다 작을 수 없습니다."));
                rowData.ENTRY_EA = args.oldItem.ENTRY_EA;
                $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                break;
            }
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.LOB02011P0.018", "유통일자를 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocudGrid(G_GRDDETAIL, args.row, args.cell, true);
                } else {
...
function grdDetailOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        ...
        if (Number(rowData.ORDER_QTY) > Number(rowData.ENTRY_QTY) && $NC.isNull(rowData.SHORTAGE_DIV)) {
            alert($NC.getDisplayMsg("JS.LOB02011P0.023", "미출고사유를 선택하십시오."));
...
function grdDetailOnCheckPStock(row, isCheckOnly) {
    ...
    $NC.serviceCallAndWait("/LOB02010E0/getData.do", {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LOB02011P0.024", "출고가능량 정보를 가져오지 못했습니다."));
            return;
        }
        var oMsg = $NC.getOutMessage(resultData);
        if (oMsg != $ND.C_OK) {
...
function showOutwaitOverlay(e) {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOB02011P0.025", "출고대기 내역을 확인할 상품을 선택하십시오."));
        return;
    }
    ...
    if (G_GRDOUTWAIT.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LOB02011P0.026", "출고대기 내역이 존재하지 않습니다."));
        return;
    }

    $NC.showOverlay("#divOutwaitOverlay", {
        title: $NC.getDisplayMsg("JS.LOB02011P0.027", "출고대기 내역"),
        fullscreen: false,
        width: 700,
        height: 300,
        onComplete: function() {
...
function btnStockAddItemOnClick() {
    ...
    if (dsStock.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02011P0.028", "출고등록할 재고를 선택하십시오."));
        return;
    }
...
function onValidDatePopup(resultInfo) {

    var rowData = G_GRDDETAIL.data.getItem(G_GRDDETAIL.lastRow);
    if ($NC.isNull(rowData)) {
        return;
    }
    var focusCol = G_GRDDETAIL.view.getColumnIndex("VALID_DATE");
    if ($NC.isNotNull(resultInfo)) {
        rowData.VALID_DATE = resultInfo.VALID_DATE;
    } else {
        rowData.VALID_DATE = "";
    }
...
function setUserProgramPermission() {
    ...
    if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ORDER || $NC.G_VAR.ORDER_YN == $ND.C_YES) {
        ...
        // YN - 추가만 가능
        // YY - 추가/삭제 가능
        var POLICY_LO230 = $NC.G_VAR.G_PARAMETER.P_POLICY_LO230.match(/N|Y/gi);
        if ($NC.isNull(POLICY_LO230)) {
            POLICY_LO230 = [
                $ND.C_NO,
                $ND.C_NO
            ];
        ...
        $NC.setEnable("#btnEntryNew", POLICY_LO230[0] == $ND.C_YES && permission.canSave);
        $NC.setEnable("#btnStockPopup", POLICY_LO230[0] == $ND.C_YES && permission.canSave);
        $NC.setEnable("#btnEntryDelete", POLICY_LO230[1] == $ND.C_YES && permission.canDelete);
