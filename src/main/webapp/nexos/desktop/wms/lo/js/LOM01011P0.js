 ...
 *  프로그램ID         : LOM01011P0
 *  프로그램명         : 온라인출고예정등록 팝업
 *  프로그램설명       : 온라인출고예정등록 팝업 화면 Javascript
...
function _Initialize() {
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.LOM01011P0.XXX", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.LOM01011P0.XXX", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.LOM01011P0.XXX", "저장"));
}
...
function _OnLoaded() {
    ...
    if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ORDER_CREATE) {
        ...
        $NC.G_VAR.masterData = {
            ...
            ORDER_NO: "",
            INOUT_CD: "", // INOUT_CD,
            OUTBOUND_STATE: $ND.C_STATE_ORDER,
    ...
    // 예정 -> 등록, 등록 수정
        ...
        $NC.setValue("#edtInout_Date", dsMaster.INOUT_DATE);
        $NC.setValue("#edtBu_Date", dsMaster.BU_DATE);
        $NC.setValue("#edtBu_No", dsMaster.BU_NO);
        $NC.setValue("#edtSo_Date", dsMaster.SO_DATE);
        $NC.setValue("#edtSo_No", dsMaster.SO_NO);
        // $NC.setValue("#cboCar_Div", dsMaster.CAR_DIV);
        ...
        $NC.G_VAR.masterData = {
            ...
            RDELIVERY_CD: $NC.nullToDefault(dsMaster.RDELIVERY_CD, dsMaster.DELIVERY_CD),
            ...
            ORG_ORDER_DATE: dsMaster.ORG_ORDER_DATE,
            ORG_ORDER_NO: dsMaster.ORG_ORDER_NO,
        ...
        var rowData, rIndex, rCount, newRowData;
        ...
        try {
            for (rIndex = 0, rCount = dsDetail.length; rIndex < rCount; rIndex++) {
                ...
                newRowData = {
                    ...
                    ORG_ORDER_DATE: rowData.ORG_ORDER_DATE,
                    ORG_ORDER_NO: rowData.ORG_ORDER_NO,
                    ORG_BU_NO: rowData.ORG_BU_NO,
                    CSCL_REQ_YN: rowData.CSCL_REQ_YN,
        ...
        // 수정일 경우 수정불가 세팅 - 인터페이스
        if ($NC.G_VAR.masterData.DATA_DIV != "00") {
            $NC.setEnable("#cboInout_Cd", false);
            // $NC.setEnable("#cboOrder_Div", false);
            // $NC.setEnable("#cboCar_Div", false);
    ...
    $NC.setInitCombo("/WC/getDataSet.do", {
        ...
        onComplete: function() {
            if ($NC.G_VAR.G_PARAMETER.P_PROCESS_CD == $ND.C_PROCESS_ORDER_CREATE) {
                $NC.G_VAR.masterData.CAR_DIV = $NC.getValue("#cboCar_Div");
            } else {
                $NC.setValue("#cboCar_Div", $NC.G_VAR.masterData.CAR_DIV);
            }
        }
    });
...
function _OnResize(parent, viewWidth, viewHeight) {
    ...
    var $parent = $("#divStockView").parent();
...
function _Inquiry() {
    ...
    var OUTBOUND_DATE = $NC.G_VAR.masterData.ORDER_DATE; // 출고일자를 알 수 없기때문에 예정일자 기준
    ...
    // 데이터 조회
    $NC.serviceCall("/LOM01010E0/getDataSet.do", $NC.getGridParams(G_GRDSTOCK), onGetStock);
}
...
function _New() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.DELIVERY_CD)) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.002", "먼저 온라인몰 코드를 입력하십시오."));
        $NC.setFocus("#edtDelivery_Cd");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_NM)) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.003", "먼저 주문자명을 입력하십시오."));
        $NC.setFocus("#edtOrderer_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDERER_HP)) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.004", "먼저 주문자 휴대폰번호를 입력하십시오."));
        $NC.setFocus("#edtOrderer_Hp");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_NM)) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.005", "먼저 수령자명을 입력하십시오."));
        $NC.setFocus("#edtShipper_Nm");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_HP)) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.006", "먼저 수령자 휴대폰번호를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.SHIPPER_ZIP_CD) || $NC.isNull($NC.G_VAR.masterData.SHIPPER_ADDR_BASIC)) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.007", "먼저 수령자 주소를 입력하십시오."));
        $NC.setFocus("#edtShipper_Addr_Detail");
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.CAR_DIV)) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.XXX", "먼저 차량구분을 선택하십시오."));
        $NC.setFocus("#cboCar_Div");
        return;
    }
    ...
    var newRowData = {
        ...
        SO_LINE_NO: "",
        SO_KEY: "",
        ORG_ORDER_DATE: $NC.G_VAR.masterData.ORG_ORDER_DATE,
        ORG_ORDER_NO: $NC.G_VAR.masterData.ORG_ORDER_NO,
        ORG_BU_NO: $NC.G_VAR.masterData.ORG_BU_NO,
        CSCL_REQ_YN: $NC.G_VAR.masterData.CSCL_REQ_YN,
        REMARK1: "",
        SPARE1_NOTE: "",
        SPARE2_NOTE: "",
        SPARE3_NOTE: "",
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.009", "물류센터를 확인할 수 없습니다. 취소 후 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.010", "사업부를 확인할 수 없습니다. 취소 후 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDER_DATE)) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.011", "먼저 예정일자를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.012", "먼저 출고구분을 선택하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CAR_DIV)) {
        alert($NC.getDisplayMsg("JS.LOB01011P0.XXX", "먼저 차량구분을 선택하십시오."));
        $NC.setFocus("#cboCar_Div");
        return;
    }
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.008", "저장할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOM01011P0.013", "저장하시겠습니까?"))) {
        return;
    }
    ...
    for (var rIndex = 0, rCount = dsTarget.length; rIndex < rCount; rIndex++) {
        ...
        dsDetail.push({
            ...
            P_ORG_ORDER_DATE: $NC.G_VAR.masterData.ORG_ORDER_DATE,
            P_ORG_ORDER_NO: $NC.G_VAR.masterData.ORG_ORDER_NO,
            P_ORG_BU_NO: rowData.ORG_BU_NO,
            P_CSCL_REQ_YN: rowData.CSCL_REQ_YN,
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.014", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LOM01010E0/save.do", {
        P_DS_MASTER: {
            P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
            P_BU_CD: $NC.G_VAR.masterData.BU_CD,
            P_ORDER_DATE: $NC.G_VAR.masterData.ORDER_DATE,
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.015", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        ...
        case "ORDER_DIV":
            ...
            break;
        case "CAR_DIV":
            $NC.G_VAR.masterData.CAR_DIV = args.val;
            break;
        case "ORDER_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LOM01011P0.016", "예정일자를 정확히 입력하십시오."));
            $NC.G_VAR.masterData.ORDER_DATE = $NC.getValue(args.view);
            $NC.G_VAR.masterData.ORG_ORDER_DATE = $NC.getValue(args.view);
            break;
        case "DELIVERY_CD":
            $NP.onDeliveryChange(args.val, {
                ...
                title: $NC.getDisplayMsg("JS.LOM01011P0.017", "온라인몰 검색"),
                columnTitle: [
                    "온라인몰코드",
                    "온라인몰명"
                ],
                errorMessage: $NC.getDisplayMsg("JS.LOM01011P0.018", "등록되어 있지 않은 온라인몰입니다.")
            });
            return;
        case "REMARK1":
...
function grdDetailOnCellChange(e, args) {
    ...
    switch (columnId) {
        ...
        case "ORDER_QTY":
            if (Number(rowData.ORDER_QTY) < 1) {
                alert($NC.getDisplayMsg("JS.LOM01011P0.019", "예정수량은 1보다 작을 수 없습니다."));
        ...
        case "ORDER_BOX":
            if (Number(rowData.ORDER_BOX) < 0) {
                alert($NC.getDisplayMsg("JS.LOM01011P0.020", "예정BOX는 0보다 작을 수 없습니다."));
        ...
        case "ORDER_EA":
            if (Number(rowData.ORDER_EA) < 0) {
                alert($NC.getDisplayMsg("JS.LOM01011P0.021", "예정EA는 0보다 작을 수 없습니다."));
                rowData.ORDER_EA = args.oldItem.ORDER_EA;
                $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                break;
            }
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.LOM01011P0.022", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                } else {
    ...
    switch (args.column.id) {
        ...
        case "ITEM_LOT":
            $NP.showItemLotPopup({
                ...
                P_ITEM_STATE: rowData.ITEM_STATE,
                P_ITEM_LOT: $ND.C_ALL,
                // 출고일자를 알 수 없기때문에 예정일자 기준
                P_OUTBOUND_DATE: $NC.G_VAR.masterData.ORDER_DATE
...
function grdDetailOnCheckPStock(row, isCheckOnly) {
    ...
    // 데이터 조회
    $NC.serviceCallAndWait("/LOM01010E0/getData.do", {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LOM01011P0.027", "출고가능량 정보를 가져오지 못했습니다."));
            return;
        }
        var oMsg = $NC.getOutMessage(resultData);
        if (oMsg != $ND.C_OK) {
...
function btnStockAddItemOnClick() {
    ...
    if (dsStock.length == 0) {
        alert($NC.getDisplayMsg("JS.LOM01011P0.028", "예정등록할 재고를 선택하십시오."));
        return;
    }
...
function setUserProgramPermission() {
    ...
    // 수기등록일 경우
    if ($NC.G_VAR.masterData.DATA_DIV == "00") {
        if ($NC.G_VAR.G_PARAMETER.P_POLICY_LO111 != $ND.C_YES) {
    ...
    // 인터페이스 수신일 경우
        ...
        $NC.setEnable("#btnStockPopup", false);
        // 수정 삭제는 정책 기준
        if ($NC.G_VAR.G_PARAMETER.P_POLICY_LO113 != "2") {
...
function setPolicyValInfo() {
    ...
    $NC.setPolicyValInfo({
        ...
        if ($NC.G_VAR.policyVal.LO151 != "0000") {
            var deadlineTime = $NC.G_VAR.policyVal.LO151.substring(0, 2) + ":" + $NC.G_VAR.policyVal.LO151.substring(2);
            $NC.setValue("#lblOrderDeadline", $NC.getDisplayMsg("JS.LOM01011P0.029", "[발주마감시간: " + deadlineTime + "] 해당 시간 전까지만 출고예정 신규 등록 가능합니다.",
                deadlineTime));
            $NC.setVisible("#lblOrderDeadline");
        } else {
            $NC.setVisible("#lblOrderDeadline", false);
        }
    });
}