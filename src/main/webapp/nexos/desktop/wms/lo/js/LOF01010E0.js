 ...
 *  프로그램ID         : LOF01010E0
 *  프로그램명         : 출고예정작업[풀필먼트]
 *  프로그램설명       : 출고예정작업[풀필먼트] 화면 Javascript
    ...
    switch (id) {
        ...
        case "BRAND_CD":
            if ($NC.isNull($NC.getValue("#edtQBu_Cd"))) {
                alert($NC.getDisplayMsg("JS.LOF01010E0.001", "브랜드를 지정하려면 먼저 사업부를 입력하십시오."));
            ...
            return;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOF01010E0.002", "검색 시작일자를 정확히 입력하십시오."));
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOF01010E0.003", "검색 종료일자를 정확히 입력하십시오."));
            break;
        case "DELIVERY_CD":
            if ($NC.isNull($NC.getValue("#edtQBu_Cd"))) {
                alert($NC.getDisplayMsg("JS.LOF01010E0.004", "온라인몰을 지정하려면 먼저 사업부를 입력하십시오."));
            ...
            $NP.onDeliveryChange(val, {
                ...
                title: $NC.getDisplayMsg("JS.LOF01010E0.005", "온라인몰 검색"),
                columnTitle: [
                    "온라인몰코드",
                    "온라인몰명"
                ],
                errorMessage: $NC.getDisplayMsg("JS.LOF01010E0.006", "등록되어 있지 않은 온라인몰입니다.")
            });
            return;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOF01010E0.007", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE1)) {
        alert($NC.getDisplayMsg("JS.LOF01010E0.008", "검색 시작일자를 입력하십시오."));
    ...
    if ($NC.isNull(ORDER_DATE2)) {
        alert($NC.getDisplayMsg("JS.LOF01010E0.009", "검색 종료일자를 입력하십시오."));
        $NC.setFocus("#dtpQOrder_Date2");
        return;
    }

    if (ORDER_DATE1 > ORDER_DATE2) {
        alert($NC.getDisplayMsg("JS.LOF01010E0.010", "출고예정일자 범위 입력오류입니다."));
        $NC.setFocus("#dtpQOrder_Date1");
        return;
    }

    var BU_CD = $NC.getValue("#edtQBu_Cd"); // 풀필먼트만 사업부 기본값 NULL
    ...
    // 데이터 조회
    $NC.serviceCall("/LOF01010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _New() {
    ...
    if ($NC.G_VAR.policyVal.LO119 != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LOF01010E0.011", "출고예정정보 신규등록/수정이 불가능한 사업부입니다."));
        return;
    }
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "LOF01011P0",
        PROGRAM_NM: $NC.getDisplayMsg("JS.LOF01011P0.001", "출고예정등록/수정"),
        url: "lo/LOF01011P0.html",
        width: 1024,
        height: 600,
        G_PARAMETER: {
            ...
            P_CUST_CD: CUST_CD,
            P_POLICY_LO119: $NC.G_VAR.policyVal.LO119,
            P_MASTER_DS: {},
            P_DETAIL_DS: [ ],
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOF01010E0.012", "삭제할 데이터가 없습니다."));
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    if ($NC.G_VAR.stateFWBW.CANCEL != rowData.OUTBOUND_STATE) {
        alert($NC.getDisplayMsg("JS.LOF01010E0.013", "삭제가 불가능한 전표입니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOF01010E0.014", "예정전표를 삭제하시겠습니까?"))) {
        return;
    }

    // 삭제처리 호출
    $NC.serviceCall("/LOF01010E0/callLOOrderDelete.do", {
...
function grdMasterOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOF01010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail);
...
function grdMasterOnDblClick(e, args) {
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LOF01010E0.015", "출고진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        ...
        if (refRowData.OUTBOUND_STATE != resultData.O_OUTBOUND_STATE) {
            alert($NC.getDisplayMsg("JS.LOF01010E0.016", "[진행상태 : " + resultData.O_OUTBOUND_STATE + "] 데이터가 변경되었습니다.\n다시 조회 후 데이터를 확인하십시오.",
        ...
        $NC.serviceCallAndWait("/LOF01010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAIL), onGetDetail, function(subAjaxData) {
        ...
        if (dsDetail.length == 0) {
            alert($NC.getDisplayMsg("JS.LOF01010E0.017", "등록/수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function getOutboundState(params, onSuccess) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOF01010E0/getData.do", {
...
function _OnGridCheckBoxFormatterClick(e, view, args) {
    ...
    switch (columnId) {
        case "HOLD_YN":
            ...
            if (HOLD_YN == $ND.C_YES) {
                if (!confirm($NC.getDisplayMsg("JS.LOF01010E0.018", "예정일자: " + rowData.ORDER_DATE + ", 예정번호: " + rowData.ORDER_NO
                ...
                if (!confirm($NC.getDisplayMsg("JS.LOF01010E0.019", "예정일자: " + rowData.ORDER_DATE + ", 예정번호: " + rowData.ORDER_NO
            ...
            // 보류처리 호출
            $NC.serviceCall("/LOF01010E0/callLOOrderHold.do", {
                P_CENTER_CD: rowData.CENTER_CD,
                P_BU_CD: rowData.BU_CD,
                P_ORDER_DATE: rowData.ORDER_DATE,
                P_ORDER_NO: rowData.ORDER_NO,
...
function btnOrderClosingOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOF01010E0.020", "종결처리할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOF01010E0.021", "종결처리 하시겠습니까?"))) {
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    // 종결처리 호출
    $NC.serviceCall("/LOF01010E0/callLOOrderClosing.do", {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_ORDER_DATE: rowData.ORDER_DATE,
        P_ORDER_NO: rowData.ORDER_NO,
...
function btnOrderCancelOnClick() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOF01010E0.022", "조회 후 처리하십시오."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOF01010E0.023", "주문취소처리 하시겠습니까?"))) {
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    $NC.serviceCall("/LOF01010E0/callLOOrderCancel.do", {
        P_CENTER_CD: rowData.CENTER_CD,
        P_BU_CD: rowData.BU_CD,
        P_ORDER_DATE: rowData.ORDER_DATE,
        P_ORDER_NO: rowData.ORDER_NO,
        ...
        alert($NC.getDisplayMsg("JS.LOF01010E0.024", "주문취소처리 완료 하였습니다."));
        _Inquiry();
    });
}
