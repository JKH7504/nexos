...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        // return;
        case "OUTBOUND_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOF04050E0.001", "출고일자를 정확히 입력하십시오."));
            setOutboundBatchCombo();
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.002", "물류센터를 선택하십시오."), "#cboQCenter_Cd");
        return;
    }

    var OUTBOUND_DATE = $NC.getValue("#dtpQOutbound_Date");
    if ($NC.isNull(OUTBOUND_DATE)) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.003", "출고일자를 입력하십시오."), "#dtpQOutbound_Date");
        return;
    }

    var OUTBOUND_BATCH = $NC.getValue("#cboQOutbound_Batch");
    if ($NC.isNull(OUTBOUND_BATCH)) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.004", "출고차수를 선택하십시오."), "#cboQOutbound_Batch");
        return;
    }
    ...
    // 데이터 조회
    $NC.serviceCall("/LOF04050E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Print(reportInfo) {
    ...
    var CENTER_CD = $NC.getValue("#cboQCenter_Cd");
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOF04050E0.002", "물류센터를 선택하십시오."));
    ...
    // if ($NC.isNull(BU_CD)) {
    // alert($NC.getDisplayMsg("JS.LOF04050E0.005", "사업부를 입력하십시오."));
    // $NC.setFocus("#edtQBu_Cd");
    // return;
    // }
    var OUTBOUND_DATE = $NC.getValue("#dtpQOutbound_Date");
    if ($NC.isNull(OUTBOUND_DATE)) {
        alert($NC.getDisplayMsg("JS.LOF04050E0.003", "출고일자를 입력하십시오."));
    ...
    var OUTBOUND_BATCH = $NC.getValue("#cboQOutbound_Batch");
    if ($NC.isNull(OUTBOUND_BATCH)) {
        alert($NC.getDisplayMsg("JS.LOF04050E0.004", "출고차수를 선택하십시오."));
    ...
    if ($NC.getComboData("#cboQOutbound_Batch")[$NC.getComboSelectedIndex("#cboQOutbound_Batch") - 1].OUTBOUND_BATCH_DIV != "11") {
        alert($NC.getDisplayMsg("JS.LOF04050E0.006", "결품차수를 선택하십시오."));
        return;
    }
...
function grdMasterOnScan(rowData) {
    ...
    $NC.setValue("#edtBu_Cd_F", $NC.getDisplayCombo(rowData["BU_CD"], rowData["BU_NM"]));
...
function onScanItem($view, scanVal) {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.007", "조회 후 처리하십시오."), $view);
        return;
    }
    ...
    if (searchRow == -1) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.008", "해당 차수에 존재하지 않는 상품입니다."), $view);
        return;
    }
    ...
    if (searchRows.length == 0) {
        ...
        setTimeout(function() {
            onAlert($NC.getDisplayMsg("JS.LOF04050E0.009", "해당 상품은 이미 분배가 완료된 상품입니다."), $view);
        });
        return;
    }
...
function onScanItemTotal(refRowData) {
    ...
    // 상품 일괄 데이터 조회
    $NC.serviceCallAndWait("/LOF04050E0/getDataSet.do", {
        P_QUERY_ID: "LOF04050E0.RS_SUB1",
        P_QUERY_PARAMS: {
    ...
    if (dsSub.length == 0) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.010", "일괄 분배할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
        return;
    }
...
function onPickInstr(scanVal) {
    ...
    if (G_GRDMASTER.data.getLength() > 0 && $NC.G_VAR.summary.TOT_REMAIN_QTY > 0) {// 미분배 수량 존재 시
        if (!confirm($NC.getDisplayMsg("JS.LOF04050E0.011", "현재 분배 작업 중입니다.\n새로운 차수를 작업 하시겠습니까?"))) {
            return;
        }
    }
    ...
    if (orderInfo[0] != $NC.getValue("#cboQCenter_Cd") && orderInfo[0] != $ND.C_BASE_CENTER_CD) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.012", "현재 작업중인 물류센터와 다른 피킹지시서 입니다. 조회조건을 확인하십시오."));
        return;
    }
    if (orderInfo[1] != $ND.C_BASE_BU_CD) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.013", "토탈피킹지시서의 사업부를 확인 후 작업하십시오."));
        return;
    }
    if ($NC.getDate(orderInfo[2]) != $NC.getValue("#dtpQOutbound_Date")) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.014", "현재 작업중인 출고일자와 다른 피킹지시서 입니다. 조회조건을 확인하십시오."));
        return;
    }
...
function callLODistributeQtyUpdate(rowData) {
    ...
    var errorData;
    $NC.serviceCallAndWait("/LOF04050E0/callLODistributeQtyUpdate.do", {
...
function btnBatchCompleteOnClick(e) {
    ...
    if ($NC.G_VAR.summary.TOT_DISTRIBUTE_QTY == 0) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.015", "분배 작업하지 않은 차수입니다."));
        return;
    } else {
        if ($NC.G_VAR.summary.TOT_REMAIN_QTY == 0 && $NC.G_VAR.summary.TOT_DISTRIBUTE_QTY == $NC.G_VAR.summary.TOT_ENTRY_QTY) {
            onAlert($NC.getDisplayMsg("JS.LOF04050E0.016", "분배 완료 된 차수입니다."));
            return;
        }
    }

    if (!confirm($NC.getDisplayMsg("JS.LOF04050E0.017", "분배미완료 전표를 결품 처리하시겠습니까?"))) {
        return;
    }
    ...
    $NC.showProgramSubPopup({
        PROGRAM_ID: "LOF04051P0",
        PROGRAM_NM: $NC.getDisplayMsg("JS.LOF04051P0.001", "수동 DAS 결품차수 생성"),
        url: "lo/LOF04051P0.html",
        width: 920,
        height: 500,
        G_PARAMETER: {
            P_CENTER_CD: CENTER_CD,
...
function btnDistributeQtyApplyOnClick() {
    ...
    if (NEW_DISTRIBUTE_QTY <= 0) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.018", "조정할 분배수량을 정확히 입력하십시오."), "#edtL1_New_Distribute_Qty");
        return;
    }

    if ($NC.G_VAR.active.shelfData.TOT_REMAIN_QTY == NEW_DISTRIBUTE_QTY) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.019", "분배수량 조정 후 반영 처리하십시오."), "#edtL1_New_Distribute_Qty");
        return;
    }

    if ($NC.G_VAR.active.shelfData.TOT_REMAIN_QTY < NEW_DISTRIBUTE_QTY) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.020", "조정할 분배수량이 미분배수량을 초과할 수 없습니다."), "#edtL1_New_Distribute_Qty");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOF04050E0.021", "입력한 조정수량을 적용하시겠습니까?"))) {
        return;
    }
...
function btnDistributeCompleteOnClick(e) {
    ...
    if ($NC.isNotNull(e)) {
        if (!confirm($NC.getDisplayMsg("JS.LOF04050E0.022", "분배완료 처리하시겠습니까?"))) {
            return;
        }
    }
    ...
    if (dsSave.length == 0) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.023", "저장 가능한 분배 데이터가 없습니다."));
        return;
    }

    // 저장
    $NC.serviceCall("/LOF04050E0/callLODistributeQtyUpdateBT.do", {
        P_DS_MASTER: dsSave,
        P_USER_ID: $NC.G_USERINFO.USER_ID
    }, function(ajaxData) {
        // 일괄 화면 닫고
...
function btnDistributeCancelOnClick() {
    ...
    if (refRowData.DISTRIBUTE_QTY == 0) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.024", "분배되지 않은 상품입니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOF04050E0.025", "상품: " + $NC.getDisplayCombo(refRowData.ITEM_CD, refRowData.ITEM_NM) //
        + "\n\n분배취소 처리하시겠습니까?", $NC.getDisplayCombo(refRowData.ITEM_CD, refRowData.ITEM_NM)))) {
        setFocusScan();
        return;
    }

    $NC.serviceCall("/LOF04050E0/callLOProcDistributeCancel.do", {
        P_CENTER_CD: refRowData.CENTER_CD,
        P_BU_CD: refRowData.BU_CD,
        P_OUTBOUND_DATE: refRowData.OUTBOUND_DATE,
        P_OUTBOUND_BATCH: refRowData.OUTBOUND_BATCH,
...
function btnDistributeCancelBatchOnClick() {
    ...
    if ($NC.G_VAR.summary.TOT_DISTRIBUTE_QTY == 0) {
        onAlert($NC.getDisplayMsg("JS.LOF04050E0.026", "분배취소 가능한 상품이 없습니다."));
        return;
    }

    var refRowData = G_GRDMASTER.data.getItem(0);
    if (!confirm($NC.getDisplayMsg("JS.LOF04050E0.027", "출고차수: " + refRowData.OUTBOUND_BATCH //
        + "\n\n전체 분배취소 처리하시겠습니까?", refRowData.OUTBOUND_BATCH))) {
        setFocusScan();
        return;
    }

    $NC.serviceCall("/LOF04050E0/callLOProcDistributeCancelBatch.do", {
        P_CENTER_CD: refRowData.CENTER_CD,
        P_BU_CD: "",
        P_OUTBOUND_DATE: refRowData.OUTBOUND_DATE,
        P_OUTBOUND_BATCH: refRowData.OUTBOUND_BATCH,
