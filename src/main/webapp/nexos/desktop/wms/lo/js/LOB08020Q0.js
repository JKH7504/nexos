...
function btnWBDeliveryInfoOnClick() {
    ...
    if (G_GRDT1MASTER.data.getLength() == 0 || $NC.isNull(G_GRDT1MASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOB08020Q0.008", "조회 후 처리하십시오."));
        return;
    }

    var rowData = G_GRDT1MASTER.data.getItem(G_GRDT1MASTER.lastRow);
    if (rowData.SEND_STATE != "50") {
        alert($NC.getDisplayMsg("JS.LOB08020Q0.009", "아직 택배사로 송신되지 않은 운송장입니다. 배송조회할 수 없습니다."));
        return;
    }

    if ($NC.isNull(rowData.WB_NO)) {
        alert($NC.getDisplayMsg("JS.LOB08020Q0.010", "운송장번호가 없는 데이터입니다. 배송조회할 수 없습니다."));
        return;
    }
