...
function _NewB() {
    ...
    if ($NC.G_VAR.policyVal.LO219 != $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.025", "출고등록 신규처리가 불가능한 사업부입니다."));
        return;
    }
...
function onProcessPreB() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOF02010E0.028", "출고등록 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.029", "출고등록 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.030", "선택한 데이터 중 출고등록 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtB() {
    ...
    if ($NC.isNull(OUTBOUND_DATE_B)) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.031", "출고일자를 먼저 입력하십시오."));
    ...
    if ($NC.isNotNull(DELIVERY_BATCH_F)) {
        if (!confirm($NC.getDisplayMsg("JS.LOF02010E0.032", "출고등록 처리하시겠습니까?\n운송차수(" + DELIVERY_BATCH_F + ")", DELIVERY_BATCH_F))) {
            return;
        }
    } else {
        if (!confirm($NC.getDisplayMsg("JS.LOF02010E0.033", "출고등록 처리하시겠습니까?"))) {
            return;
        }
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.034", "출고등록 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.035", "선택한 데이터 중 출고등록 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterBOnDblClick(e, args) {
    ...
    if (refRowData.HOLD_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.036", "보류상태인 전표는 팝업화면에서 작업하실 수 없습니다."));
        return;
    }
    ...
    if (refRowData.CAR_SUB_CD == "2" && refRowData.ACS_ERROR_CD != $ND.C_BASE_CD_L1
        ...
        alert($NC.getDisplayMsg("JS.LOF02010E0.093", "주소 정제 후 팝업화면에서 작업하실 수 있습니다."));
        return;
    }
    ...
    function(ajaxData) {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LOF02010E0.037", "출고진행상태를 확인하지 못했습니다.\n다시 처리하십시오."));
            return;
        }
        ...
        if (refRowData.OUTBOUND_STATE != resultData.O_OUTBOUND_STATE) {
            alert($NC.getDisplayMsg("JS.LOF02010E0.038", "[진행상태 : " + resultData.O_OUTBOUND_STATE + "] 데이터가 변경되었습니다.\n다시 조회 후 데이터를 확인하십시오.",
        ...
        if (dsDetail.length == 0) {
            alert($NC.getDisplayMsg("JS.LOF02010E0.039", "등록/수정할 데이터가 존재하지 않습니다. 다시 조회 후 데이터를 확인하십시오."));
            return;
        }
...
function onSaveAcsB(ajaxData) {
    ...
    if (oMsg != $ND.C_OK) {
        ...
        if (resultData.O_ACS_ERROR_MSG == $ND.C_OK) {
            if (acsError > 0) {
                alert($NC.getDisplayMsg("JS.LOF02010E0.094", "주소정제에 실패한 데이터가 존재합니다. 데이터를 확인하십시오.\n" //
                    + "(전체건수: " + acsTotal + ", 정제호출: " + acsProcess + ", 정제오류: " + acsError + ")", acsTotal, acsProcess, acsError));
            }
        } else {
            alert($NC.getDisplayMsg("JS.LOF02010E0.XXX", "주소정제 처리 중 오류가 발생했습니다.\n" //
                + "(전체건수: " + acsTotal + ", 정제호출: " + acsProcess + ", 정제오류: " + acsError + ")", acsTotal, acsProcess, acsError) + "\n"
...
function btnAcsGetAddressOnClick() {
    ...
    if (!$NC.getProgramPermission().canSave) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.XXX", "해당 프로그램의 저장권한이 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.MAIN.002", "주소정제 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.096", "주소정제 처리할 데이터를 선택하십시오."));
        return;
    }

    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.097", "선택한 데이터 중 주소정제 가능한 데이터가 없습니다."));
        return;
    }

    $NC.serviceCall("/LOF02010E0/callAcsGetAddress.do", {
        // 출고예정으로 주소정제
        ...
        type: 2,
        message: $NC.getDisplayMsg("JS.LOF02010E0.098", "주소정제 처리 중 입니다. 잠시만 기다려 주십시오...")
    });
}
...
function btnSetCarrierCdBOnClick() {
    ...
    if ($NC.isNull(carrierData.CARRIER_CD)) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.099", "변경할 운송사를 선택하십시오."));
        $NC.setFocus("#cboCarrier_CdB");
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOF02010E0.100", "운송사를 변경하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.101", "운송사 변경할 데이터를 선택하십시오."));
        return;
    }

    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.102", "선택한 데이터 중 운송사 변경 가능한 데이터가 없습니다."));
        return;
    }

    $NC.serviceCall("/LOF02010E0/saveOrder.do", {
        P_DS_MASTER: dsMaster,
        P_USER_ID: $NC.G_USERINFO.USER_ID
