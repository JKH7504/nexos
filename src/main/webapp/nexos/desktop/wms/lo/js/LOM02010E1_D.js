...
function onProcessPreD() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOM02010E1.073", "출고확정 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.074", "출고확정취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.075", "선택한 데이터 중 출고확정취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtD() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOM02010E1.076", "출고확정 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDSUBD)) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.077", "데이터가 수정되었습니다. 저장 후 출고확정 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.078", "출고확정 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOM02010E1.079", "선택한 데이터 중 확정처리 가능한 데이터가 없습니다."));
        return;
    }

    if ($NC.G_VAR.policyVal.LO491 == $ND.C_YES && noInspectCount > 0) {
        if (!confirm($NC.getDisplayMsg("JS.LOM02010E1.080", "검수 완료 되지 않은 전표가 존재합니다. \n확정 처리하시겠습니까?"))) {
            return;
        }
    }
...
function grdMasterDInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterD", {
        ...
        queryId: "LOM02010E1.RS_T3_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
...
function grdDetailDInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailD", {
        ...
        queryId: "LOM02010E1.RS_T3_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
    ...
    var options = {
        ...
        specialRow: {
            compareFn: function(specialRow, rowData, column, row, cell, colspan) {
                ...
                if (rowData.VIRTUAL_YN == $ND.C_YES && rowData.STOCK_IN_GRP == "000") {
                    return "stySpecial";
                }
            }
        }
...
function grdSubDOnCellChange(e, args) {
    ...
    switch (G_GRDSUBD.view.getColumnId(args.cell)) {
        case "CONFIRM_QTY":
            ...
            if (CONFIRM_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LOM02010E1.081", "확정수량이 0보다 작을 수 없습니다."));
            ...
            if (Number(rowData.ENTRY_QTY) < CONFIRM_QTY) {
                alert($NC.getDisplayMsg("JS.LOM02010E1.082", "확정수량이 등록수량을 초과할 수 없습니다."));
                rowData.CONFIRM_QTY = args.oldItem.CONFIRM_QTY;
                rowData.CONFIRM_BOX = args.oldItem.CONFIRM_BOX;
                rowData.CONFIRM_EA = args.oldItem.CONFIRM_EA;
                rowData.CONFIRM_WEIGHT = args.oldItem.CONFIRM_WEIGHT;
...
function grdSubDOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONFIRM_QTY)) {
            alert($NC.getDisplayMsg("JS.LOM02010E1.083", "확정수량을 입력하십시오."));
