...
function _Initialize() {
    ...
    // 미사용기능 숨김 처리
    $("#tdQDistribute_Div").parent().hide();
    $("#divDistribute_DivB").hide();
    $("#divAdditionalViewD").hide();
    $("#btnSetDirectYn").hide();
    // 초기 숨김 처리
    $("#tdQDsp_Outbound_BatchC").hide();
    $("#divQOutbound_BatchD").hide();
    $("#tdQEquip_Div").parent().hide();
    // 초기 비활성화 처리
    $NC.setEnable("#btnSetDirectYnD", false); // 직송처리/취소버튼 비활성화
    ...
    $("#btnChangeDistributeDivB").click(btnChangeDistributeDivOnClick); // 유통구분 수정 버튼 클릭(등록)
    $("#btnChangeDistributeDivD").click(btnChangeDistributeDivOnClick); // 유통구분 수정 버튼 클릭(확정)
    $("#btnSetAdjustQty").click(window.btnSetAdjustQtyOnClick); // 조정수량일괄처리 버튼 클릭
    $("#btnSetDirectYn").click(window.btnSetDirectYnOnClick); // 직송처리/취소 버튼 클릭
    $("#btnQErp_Batch").click(function() { // ERP차수 새로고침
    ...
    // 취소/처리 버튼 툴팁 세팅
    $NC.setTooltip("#btnProcessPreB", $NC.getDisplayMsg("JS.LOB02010E1.001", $NC.getValue("#btnProcessB") + " 취소", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessNxtB", $NC.getDisplayMsg("JS.LOB02010E1.002", $NC.getValue("#btnProcessB") + " 처리", $NC.getValue("#btnProcessB")));
    $NC.setTooltip("#btnProcessPreC", $NC.getDisplayMsg("JS.LOB02010E1.001", $NC.getValue("#btnProcessC") + " 취소", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessNxtC", $NC.getDisplayMsg("JS.LOB02010E1.002", $NC.getValue("#btnProcessC") + " 처리", $NC.getValue("#btnProcessC")));
    $NC.setTooltip("#btnProcessPreD", $NC.getDisplayMsg("JS.LOB02010E1.001", $NC.getValue("#btnProcessD") + " 취소", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessNxtD", $NC.getDisplayMsg("JS.LOB02010E1.002", $NC.getValue("#btnProcessD") + " 처리", $NC.getValue("#btnProcessD")));
    $NC.setTooltip("#btnProcessPreE", $NC.getDisplayMsg("JS.LOB02010E1.001", $NC.getValue("#btnProcessE") + " 취소", $NC.getValue("#btnProcessE")));
    $NC.setTooltip("#btnProcessNxtE", $NC.getDisplayMsg("JS.LOB02010E1.002", $NC.getValue("#btnProcessE") + " 처리", $NC.getValue("#btnProcessE")));
}
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "RDELIVERY_CD":
            ...
            return;
        case "OUTBOUND_DATE":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB02010E1.003", "검색 출고일자를 정확히 입력하십시오."));
            setOutboundBatchCombo("#cboQOutbound_BatchC", false);
            setOutboundBatchCombo("#cboOutbound_BatchC", false);
            setOutboundBatchCombo("#cboQOutbound_BatchD", true);
            setErpBatchCombo($NC.G_VAR.activeView.PROCESS_CD);
            break;
        case "ORDER_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB02010E1.004", "검색 시작일자를 정확히 입력하십시오."));
            setErpBatchCombo($NC.G_VAR.activeView.PROCESS_CD);
            break;
        case "ORDER_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB02010E1.005", "검색 종료일자를 정확히 입력하십시오."));
            setErpBatchCombo($NC.G_VAR.activeView.PROCESS_CD);
            break;
        case "OUTBOUND_BATCHC":
            $NC.setEnable("#cboEquip_Div", val == $ND.C_BASE_BATCH_NO);
            _Inquiry();
            return;
    }
...
function onChangingCondition() {
    ...
    // 직송처리/취소 버튼 value 초기화, 비활성화
    $NC.setValue("#btnSetDirectYn", "직송처리");
    $NC.setEnable("#btnSetDirectYn", false);
    $NC.setEnable("#btnSetAdjustQty", false);
    window.setUserProgramPermissionB();
    // 마스터 총수량 초기화
    $NC.setValue("#edtTotal_Order_Qty" + $NC.G_VAR.activeView.PROCESS_CD);
    $NC.setValue("#edtTotal_Entry_Qty" + $NC.G_VAR.activeView.PROCESS_CD);
    $NC.setValue("#edtTotal_Confirm_Qty" + $NC.G_VAR.activeView.PROCESS_CD);
    $NC.setValue("#edtTotal_Rdelivery_Cnt" + $NC.G_VAR.activeView.PROCESS_CD);
    $NC.setValue("#edtTotal_Delivery_Qty" + $NC.G_VAR.activeView.PROCESS_CD);
    // 선택 수 초기화
...
function _OnInputChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "OUTBOUND_DATEBT":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB02010E1.006", "일괄처리생성정보의 출고일자를 정확히 입력하십시오."));
            setDeliveryBatchCombo($ND.C_PROCESS_ENTRY_BATCH);
            break;
        case "OUTBOUND_DATEB":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB02010E1.007", "출고일자를 정확히 입력하십시오."));
            setDeliveryBatchCombo($ND.C_PROCESS_ENTRY);
            break;
        ...
        case "QVIEW_DIV1":
            ...
            break;
        case "INOUT_DATED":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LOB02010E1.008", "거래명세서일자를 정확히 입력하십시오."));
            break;
...
function onSubViewChange(e, $view) {
    ...
    switch ($NC.G_VAR.activeView.PROCESS_CD) {
        ...
        case $ND.C_PROCESS_ENTRY:
            ...
            $("#tdQEquip_Div").parent().hide(); // 장비구분 비표시
        ...
        case $ND.C_PROCESS_ENTRY_BATCH:
            ...
            $("#tdQEquip_Div").parent().hide(); // 장비구분(tr) 비표시
        ...
        case $ND.C_PROCESS_DIRECTIONS:
            ...
            $("#tdQDsp_Delivery_Div").parent().hide(); // 배송처구분 비표시
            $("#tdQEquip_Div").parent().show(); // 장비구분(tr) 표시
            ...
            if ($NC.isSplitter("#divSplitterC")) {
                ...
                $NC.setInitSplitter("#divSplitterC", "v", 570, 270, 400);
            }
            break;
        // 출고 확정
        case $ND.C_PROCESS_CONFIRM:
            ...
            if ($NC.isSplitter("#divSplitterLRD")) {
                ...
                $NC.setInitSplitter("#divSplitterLRD", "v", 570, 270, 400);
...
function _Inquiry() {
    ...
    if (STATE_PRE_YN == $ND.C_NO && STATE_CUR_YN == $ND.C_NO) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.015", "검색구분을 선택하십시오."));
    ...
    switch ($NC.G_VAR.activeView.PROCESS_CD) {
        ...
        case $ND.C_PROCESS_DIRECTIONS:
            ...
            if ($NC.isNull(OUTBOUND_BATCH)) {
                alert($NC.getDisplayMsg("JS.LOB02010E1.016", "출고차수를 선택하십시오."));
        ...
        case $ND.C_PROCESS_DELIVERY:
            ...
            // PAPER_LOB04 - 분배지시서
            // LABEL_LOB01 - 피킹라벨
            // SPEC_LOB02 - 출고차수별 거래명세서
            if (reportInfo.REPORT_CD == "PAPER_LOB03" || reportInfo.REPORT_CD == "PAPER_LOB04" || reportInfo.REPORT_CD == "LABEL_LOB01"
                || reportInfo.REPORT_CD == "SPEC_LOB02") {
            ...
            break;
        default:
            alert($NC.getDisplayMsg("JS.LOB02010E1.017", "해당 프로세스는 출력물이 없습니다."));
            return;
    }
    ...
    switch (reportInfo.REPORT_CD) {
        ...
        case "PAPER_LOB07":
            ...
            queryParams = {
                ...
                P_PRINT_DIV: reportInfo.REPORT_CD == "PAPER_LOB03" ? "1" : "2"
            };
            break;
        // PAPER_LOB04 - 분배지시서
        case "PAPER_LOB04":
    ...
    $NC.setProgramReportInfo({
        ...
        if ($NC.G_VAR.activeView.PROCESS_CD == $ND.C_PROCESS_ENTRY) {
            ...
            return reportInfo.REPORT_CD != "PAPER_LOB03" //
                ...
                && reportInfo.REPORT_CD != "LABEL_LOB01";
        }
        // 출고지시 단계 이후 이전 단계 레포트 추가시 레포트 중 오더피킹지시서/토탈피킹지시서/분배지시서/피킹라벨만 표시되도록 조정
        ...
        // PAPER_LOB05 - 차량별토탈피킹지시서
        // PAPER_LOB06 - 차량별분배지시서
        return reportInfo.REPORT_CD != "PAPER_LOB05" //
            && reportInfo.REPORT_CD != "PAPER_LOB06";
    });
}
...
function onGetProcessInfo(ajaxData) {
    ...
    // alert($NC.getDisplayMsg("JS.LOB02010E1.018", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록하십시오."));
    $NC.G_VAR.activeView.PROCESS_CD = "";
    $NC.showMessage({
        message: $NC.getDisplayMsg("JS.LOB02010E1.018", "수행 가능한 프로세스가 없습니다.\n사업부별 프로세스관리에서 수행 프로세스를 등록하십시오."),
        width: 400,
...
function setMasterSummaryInfo() {
    ...
    $NC.serviceCall("/LOB02010E0/getDataSet.do", {
        P_QUERY_ID: "LOB02010E1.RS_MASTER",
        P_QUERY_PARAMS: {
            P_CENTER_CD: CENTER_CD,
            P_BU_CD: BU_CD,
            P_OUTBOUND_DATE: OUTBOUND_DATE,
            ...
            P_EQUIP_DIV: $NC.G_VAR.activeView.PROCESS_CD == $ND.C_PROCESS_DIRECTIONS ? EQUIP_DIV : null
        }
    }, onGetMasterSummary);
}
...
function btnChangeDistributeDivOnClick() {
    ...
    var grdObject, distribute, onSavFn, onSaveErrorFn;
    if ($NC.G_VAR.activeView.PROCESS_CD == $ND.C_PROCESS_ENTRY) {
        grdObject = G_GRDMASTERB;
        distribute = $ND.C_PROCESS_ENTRY;
        onSavFn = window.onSaveB;
        onSaveErrorFn = window.onSaveErrorB;
    } else {
        grdObject = G_GRDMASTERD;
        distribute = "";
        onSavFn = window.onSaveD;
        onSaveErrorFn = window.onSaveErrorD;
    }
    ...
    var DISTRIBUTE_DIV = $NC.getValue("#cboDistribute_Div" + distribute);
    if ($NC.isNull(DISTRIBUTE_DIV)) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.021", "유통구분을 선택하십시오."));
        $NC.setFocus("#cboDistribute_Div" + distribute);
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOB02010E1.022", "유통구분을 수정 하시겠습니까?"))) {
        return;
    }
    ...
    for (var rIndex = 0; rIndex < rCount; rIndex++) {
        ...
        if (rowData.CHECK_YN != $ND.C_YES || rowData.OUTBOUND_STATE < $ND.C_STATE_ENTRY) {
            continue;
        }
        dsMaster.push({
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB02010E1.023", "수정할 데이터를 선택하십시오."));
        return;
    }

    $NC.serviceCall("/LOB02010E0/saveConfirmData.do", {
        P_DS_MASTER: dsMaster,
        P_USER_ID: $NC.G_USERINFO.USER_ID
    }, onSavFn, onSaveErrorFn);
}
