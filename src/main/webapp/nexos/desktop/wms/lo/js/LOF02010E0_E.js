...
function _InquiryE(args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOF02010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTERE), onGetMasterE);
}
...
function _SaveE() {
    ...
    $NC.serviceCall("/LOF02010E0/saveDelivery.do", {
...
function onProcessPreE() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOF02010E0.082", "배송완료 취소 처리하시겠습니까?"))) {
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.083", "배송완료 취소 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.84", "선택한 데이터 중 배송완료 취소 처리 가능한 데이터가 없습니다."));
        return;
    }
...
function onProcessNxtE() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.LOF02010E0.085", "배송완료 처리하시겠습니까?"))) {
        return;
    }
    ...
    if ($NC.isGridModified(G_GRDDETAILE)) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.086", "데이터가 수정되었습니다. 저장 후 배송완료 처리하십시오."));
        return;
    }
    ...
    if (checkedCount == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.087", "배송완료 처리할 데이터를 선택하십시오."));
        return;
    }
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LOF02010E0.088", "선택한 데이터 중 배송완료처리 가능한 데이터가 없습니다."));
        return;
    }
...
function grdMasterEInitialize() {
    ...
    $NC.setInitGridObject("#grdMasterE", {
        ...
        queryId: "LOF02010E0.RS_T4_MASTER",
        sortCol: "OUTBOUND_NO",
        gridOptions: options
    });
...
function grdMasterEOnAfterScroll(e, args) {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOF02010E0/getDataSet.do", $NC.getGridParams(G_GRDDETAILE), onGetDetailE);
...
function grdDetailEInitialize() {
    ...
    $NC.setInitGridObject("#grdDetailE", {
        ...
        queryId: "LOF02010E0.RS_T4_DETAIL",
        sortCol: "LINE_NO",
        gridOptions: options
    });
...
function grdDetailEOnBeforeEditCell(e, args) {
    ...
    switch (args.column.id) {
        case "DELIVERY_QTY":
            return $NC.G_VAR.policyVal.LO519 == "1"; // 배송 완료 수량 수정 허용 기준
        case "MISSED_DIV_F":
        case "MISSED_COMMENT":
...
function grdDetailEOnCellChange(e, args) {
    ...
    switch (G_GRDDETAILE.view.getColumnId(args.cell)) {
        case "DELIVERY_QTY":
            ...
            if (DELIVERY_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LOF02010E0.089", "배송수량에 0보다 작은값을 입력할 수 없습니다."));
                ...
                alert($NC.getDisplayMsg("JS.LOF02010E0.090", "배송수량이 확정수량을 초과할 수 없습니다."));
                rowData.DELIVERY_QTY = args.oldItem.DELIVERY_QTY;
                rowData.MISSED_QTY = args.oldItem.MISSED_QTY;
                $NC.setFocusGrid(G_GRDDETAILE, args.row, args.cell, true);
                break;
