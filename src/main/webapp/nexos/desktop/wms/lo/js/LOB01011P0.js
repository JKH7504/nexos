 ...
 *  프로그램ID         : LOB01011P0
 *  프로그램명         : 출고예정등록 팝업
 *  프로그램설명       : 출고예정등록 팝업 화면 Javascript
...
function _Initialize() {
    ...
    // 신규/삭제/저장 버튼 툴팁 세팅
    $NC.setTooltip("#btnEntryNew", $NC.getDisplayMsg("JS.LOB01011P0.002", "신규"));
    $NC.setTooltip("#btnEntryDelete", $NC.getDisplayMsg("JS.LOB01011P0.003", "삭제"));
    $NC.setTooltip("#btnEntrySave", $NC.getDisplayMsg("JS.LOB01011P0.004", "저장"));
}
...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/LOB01010E0/getDataSet.do", $NC.getGridParams(G_GRDSTOCK), onGetStock);
}
...
function _New() {
    ...
    if ($NC.isNull(DELIVERY_CD)) {
        alert($NC.getDisplayMsg("JS.LOB01011P0.005", "먼저 배송처 코드를 입력하십시오."));
        $NC.setFocus("#edtDelivery_Cd");
        return;
    }
    ...
    if ($NC.isNull(RDELIVERY_CD)) {
        alert($NC.getDisplayMsg("JS.LOB01011P0.006", "먼저 실배송처 코드를 입력하십시오."));
        $NC.setFocus("#edtRDelivery_Cd");
        return;
    }

    var CAR_DIV = $NC.getValue("#cboCar_Div");
    if ($NC.isNull(CAR_DIV)) {
        alert($NC.getDisplayMsg("JS.LOB01011P0.007", "먼저 차량구분을 선택하십시오."));
...
function _Save() {
    ...
    if ($NC.isNull($NC.G_VAR.masterData.CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LOB01011P0.008", "물류센터가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.BU_CD)) {
        alert($NC.getDisplayMsg("JS.LOB01011P0.009", "사업부가 지정되어 있지 않습니다. 다시 작업하십시오."));
        return;
    }

    if ($NC.isNull($NC.G_VAR.masterData.ORDER_DATE)) {
        alert($NC.getDisplayMsg("JS.LOB01011P0.010", "먼저 예정일자를 입력하십시오."));
    ...
    if ($NC.isNull($NC.G_VAR.masterData.INOUT_CD)) {
        alert($NC.getDisplayMsg("JS.LOB01011P0.011", "먼저 출고구분을 선택하십시오."));
    ...
    if (G_GRDDETAIL.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LOB01011P0.012", "저장할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LOB01011P0.013", "저장하시겠습니까?"))) {
        return;
    }
    ...
    for (var rIndex = 0, rCount = dsTarget.length; rIndex < rCount; rIndex++) {
        ...
        dsDetail.push({
            ...
            P_OUT_UNIT_DIV_F: rowData.OUT_UNIT_DIV_F,
    ...
    if ($NC.G_VAR.masterData.CRUD == $ND.C_DV_CRUD_R && dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB01011P0.014", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LOB01010E0/save.do", {
        P_DS_MASTER: {
            P_CENTER_CD: $NC.G_VAR.masterData.CENTER_CD,
            P_BU_CD: $NC.G_VAR.masterData.BU_CD,
            P_ORDER_DATE: $NC.G_VAR.masterData.ORDER_DATE,
            ...
            P_INOUT_GRP: $NC.G_VAR.masterData.INOUT_CD_SUB,
...
function _Delete() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        alert($NC.getDisplayMsg("JS.LOB01011P0.015", "삭제할 데이터가 없습니다."));
        return;
    }
...
function masterDataOnChange(e, args) {
    ...
    switch (args.col) {
        ...
        case "CAR_DIV":
            ...
            break;
        case "ORDER_DATE":
            $NC.setValueDatePicker(args.view, args.val, $NC.getDisplayMsg("JS.LOB01011P0.027", "예정일자를 정확히 입력하십시오."));
...
function grdDetailOnCellChange(e, args) {
    ...
    switch (columnId) {
        ...
        case "ORDER_QTY":
            if (Number(rowData.ORDER_QTY) < 1) {
                alert($NC.getDisplayMsg("JS.LOB01011P0.022", "예정수량은 1보다 작을 수 없습니다."));
        ...
        case "ORDER_BOX":
            if (Number(rowData.ORDER_BOX) < 0) {
                alert($NC.getDisplayMsg("JS.LOB01011P0.023", "예정BOX는 0보다 작을 수 없습니다."));
        ...
        case "ORDER_EA":
            if (Number(rowData.ORDER_EA) < 0) {
                alert($NC.getDisplayMsg("JS.LOB01011P0.016", "예정EA는 0보다 작을 수 없습니다."));
                rowData.ORDER_EA = args.oldItem.ORDER_EA;
                $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                break;
            }
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.LOB01011P0.017", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDDETAIL, args.row, args.cell, true);
                } else {
...
function grdDetailOnCheckPStock(row, isCheckOnly) {
    ...
    // 데이터 조회
    $NC.serviceCallAndWait("/LOB01010E0/getData.do", {
        ...
        if ($NC.isEmpty(resultData)) {
            alert($NC.getDisplayMsg("JS.LOB01011P0.024", "출고가능량 정보를 가져오지 못했습니다."));
            return;
        }
        var oMsg = $NC.getOutMessage(resultData);
        if (oMsg != $ND.C_OK) {
...
function btnStockAddItemOnClick() {
    ...
    if (dsStock.length == 0) {
        alert($NC.getDisplayMsg("JS.LOB01011P0.025", "예정등록할 재고를 선택하십시오."));
        return;
    }
...
function setUserProgramPermission() {
    ...
    // 수기등록일 경우
    if ($NC.G_VAR.masterData.DATA_DIV == "00") {
        if ($NC.G_VAR.G_PARAMETER.P_POLICY_LO110 != $ND.C_YES) {
...
function setPolicyValInfo() {
    ...
    $NC.setPolicyValInfo({
        ...
        if ($NC.G_VAR.policyVal.LO150 != "0000") {
            var deadlineTime = $NC.G_VAR.policyVal.LO150.substring(0, 2) + ":" + $NC.G_VAR.policyVal.LO150.substring(2);
            $NC.setValue("#lblOrderDeadline", $NC.getDisplayMsg("JS.LOB01011P0.026", "[발주마감시간: " + deadlineTime + "] 해당 시간 전까지만 출고예정 신규 등록 가능합니다.",
                deadlineTime));
