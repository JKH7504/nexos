 ...
 *  프로그램ID         : LOF07023P0
 *  프로그램명         : 반입요청 팝업[B2CF]
 *  프로그램설명       : 반입요청 팝업[B2CF] 화면 Javascript
...
function _Initialize() {
    ...
    $NC.setTooltip("#btnReturnSave", $NC.getDisplayMsg("JS.LOF07023P0.001", "반품요청"));
...
function _Inquiry() {
    ...
    var queryParams = {
        ...
        P_POLICY_CM530: $NC.G_VAR.G_PARAMETER.P_POLICY_CM530
    };
...
function _Save() {
    ...
    if ($NC.isNull(WB_NO)) {
        alert($NC.getDisplayMsg("JS.LOF07023P0.002", "출고 택배송장이 없는 전표는 반품요청 불가능 합니다."));
        return;
    }
    var RI_ORDER_NO = $NC.G_VAR.G_PARAMETER.P_RI_ORDER_NO;
    if ($NC.isNotNull(RI_ORDER_NO)) {
        alert($NC.getDisplayMsg("JS.LOF07023P0.003", "해당 택배송장번호에 대해 반품요청 한 내역이 존재 합니다.\n신규 등록하려면, 반품예정작업[B2C]화면에서 삭제 후 다시 작업 하세요."));
        return;
    }

    if ($NC.getValueRadioGroup("rgbSend_Div") == "2") {
        if (!confirm($NC.getDisplayMsg("JS.LOF07023P0.004", "[택배사 반품요청 송신 안함]\n저장하시겠습니까?"))) {
            return;
        }
    } else {
        if (!confirm($NC.getDisplayMsg("JS.LOF07023P0.005", "저장하시겠습니까?"))) {
            return;
        }
    }
    ...
    if (dsDetail.length == 0) {
        alert($NC.getDisplayMsg("JS.LOF07023P0.006", "반품수량 입력 후 저장하십시오."));
        return;
    }
...
function grdMasterOnCellChange(e, args) {
    ...
    switch (G_GRDMASTER.view.getColumnId(args.cell)) {
        case "RETURN_QTY":
            ...
            if (RETURN_QTY < 0) {
                alert($NC.getDisplayMsg("JS.LOF07023P0.007", "반품수량이 0보다 작을 수 없습니다."));
            ...
            if (Number(rowData.CONFIRM_QTY) < RETURN_QTY) {
                alert($NC.getDisplayMsg("JS.LOF07023P0.008", "반품수량이 출고수량을 초과할 수 없습니다."));
                rowData.RETURN_QTY = args.oldItem.RETURN_QTY;
                $NC.setFocusGrid(G_GRDMASTER, args.row, args.cell, true);
                break;
            }
        ...
        case "VALID_DATE":
            if ($NC.isNotNull(rowData.VALID_DATE)) {
                if (!$NC.isDate(rowData.VALID_DATE)) {
                    alert($NC.getDisplayMsg("JS.LOF07023P0.009", "유통기한을 정확히 입력하십시오."));
                    rowData.VALID_DATE = "";
                    $NC.setFocusGrid(G_GRDMASTER, args.row, args.cell, true);
                } else {
...
function grdMasterOnBeforePost(row) {
    ...
    if ($NC.isNull(rowData.RETURN_QTY)) {
        alert($NC.getDisplayMsg("JS.LOF07023P0.010", "반품수량을 입력하십시오."));
    ...
    if ($NC.isNotNull(rowData.RETURN_QTY) && RETURN_QTY > 0 && $NC.isNull(rowData.RETURN_DIV)) {
        alert($NC.getDisplayMsg("JS.LOF07023P0.011", "반품사유구분을 입력하십시오."));
