 ...
 *  프로그램ID         : LFC03040E0
 *  프로그램명         : 월기타단가 기준관리
 *  프로그램설명       : 월기타단가 기준관리 화면 Javascript
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "CONTRACT_MONTH":
            if ($NC.isNotNull(val)) {
                $NC.setValueMonthPicker(view, val, $NC.getDisplayMsg("JS.LFC03040E0.001", "계약월을 정확히 입력하십시오."));
            }
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNotNull(CONTRACT_MONTH)) {
        if (!$NC.isMonth(CONTRACT_MONTH)) {
            alert($NC.getDisplayMsg("JS.LFC03040E0.001", "계약월을 정확히 입력하십시오."));
    ...
    // 데이터 조회
    $NC.serviceCall("/LFC03040E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LFC03040E0.004", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LFC03040E0.005", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LFC03040E0/save.do", {
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LFC03040E0.006", "삭제할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LFC03040E0.007", "삭제 하시겠습니까?"))) {
        return;
    }
        ...
        case "CONTRACT_DATE":
            if ($NC.isNotNull(rowData.CONTRACT_DATE)) {
                if (!$NC.isDate(rowData.CONTRACT_DATE)) {
                    alert($NC.getDisplayMsg("JS.LFC03040E0.008", "계약시작일자를 정확히 입력하십시오."));
...
function btnDownloadXLSFormatOnClick() {
    ...
    var COLUMN_INFO = [
        ...
        {
            P_COLUMN_NM: "ETC_MONTHLY_DIV",
            P_COLUMN_TITLE: "월기타구분",
            P_COLUMN_WIDTH: 20
        },
        {
    ...
    $NC.excelFileDownload({
        P_QUERY_ID: "LFC03040E0.RS_SUB2",
        P_SERVICE_PARAMS: {
            P_XLS_SHEET_NAME: "월기타수수료단가업로드", // Excel Sheet Title
...
function btnUploadXLSOnClick() {
    ...
    $NC.excelFileUpload({
        ...
        if ($NC.isNull(dsResultData) || dsResultData.length == 0) {
            alert($NC.getDisplayMsg("JS.LFC03040E0.XXX", "업로드할 수 있는 대상 데이터가 없습니다. 엑셀 파일을 확인하십시오."));
            return;
        }
        ...
        for (var rIndex = 0; rIndex < dsResultData.length; rIndex++) {
            dsMaster.push({
                ...
                P_ETC_MONTHLY_DIV: dsResultData[rIndex].ETC_MONTHLY_DIV,
