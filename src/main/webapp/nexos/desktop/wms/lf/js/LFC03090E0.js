 ...
 *  프로그램ID         : LFC03090E0
 *  프로그램명         : 추가작업구분 단가관리
 *  프로그램설명       : 추가작업구분 단가관리 화면 Javascript
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "CONTRACT_MONTH":
            if ($NC.isNotNull(val)) {
                $NC.setValueMonthPicker(view, val, $NC.getDisplayMsg("JS.LFC03090E0.001", "계약월을 정확히 입력하십시오."));
            }
            break;
    }
...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/LFC03090E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LFC03090E0.004", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    for (var rIndex = 0, rCount = G_GRDMASTER.data.getLength(); rIndex < rCount; rIndex++) {
        ...
        dsMaster.push({
            ...
            P_EXTRA_WORK_DIV: rowData.EXTRA_WORK_DIV,
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LFC03090E0.005", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LFC03090E0/save.do", {
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LFC03090E0.006", "삭제할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LFC03090E0.007", "삭제 하시겠습니까?"))) {
        return;
    }
...
function grdMasterOnGetColumns() {
    ...
    $NC.setGridColumn(columns, {
        id: "EXTRA_WORK_DIV_F",
        field: "EXTRA_WORK_DIV_F",
        name: "추가작업구분",
        editor: Slick.Editors.ComboBox,
        editorOptions: $NC.getGridComboEditorOptions("/WC/getDataSet.do", {
            P_QUERY_ID: "WC.POP_CMCODE",
            P_QUERY_PARAMS: {
                P_COMMON_GRP: "EXTRA_WORK_DIV",
                P_COMMON_CD: $ND.C_ALL,
                P_VIEW_DIV: "1" // 1:등록팝업, 2:조회팝업
            }
        }, {
            codeField: "EXTRA_WORK_DIV",
            dataCodeField: "COMMON_CD",
            dataFullNameField: "COMMON_CD_F",
            isKeyField: true
        })
...
function grdMasterOnContractBillPopup(resultInfo) {
    ...
    if ($NC.isNotNull(resultInfo)) {
        ...
        focusCol = G_GRDMASTER.view.getColumnIndex("EXTRA_WORK_DIV_F");
    } else {
        rowData.CONTRACT_NO = "";
        rowData.CONTRACT_NM = "";
        ...
        case "CONTRACT_DATE":
            if ($NC.isNotNull(rowData.CONTRACT_DATE)) {
                if (!$NC.isDate(rowData.CONTRACT_DATE)) {
                    alert($NC.getDisplayMsg("JS.LFC03090E0.008", "계약시작일자를 정확히 입력하십시오."));
...
function grdMasterOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONTRACT_NO)) {
            alert($NC.getDisplayMsg("JS.LFC03090E0.009", "계약번호를 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "CONTRACT_NO_F", true);
            return false;
        }
        if ($NC.isNull(rowData.EXTRA_WORK_DIV)) {
            alert($NC.getDisplayMsg("JS.LFC03090E0.010", "추가작업비구분을 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "EXTRA_WORK_DIV_F", true);
            return false;
        }
        if ($NC.isNull(rowData.UNIT_PRICE)) {
            alert($NC.getDisplayMsg("JS.LFC03090E0.012", "기준단가를 입력하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "UNIT_PRICE", true);
            return false;
        }
        if ($NC.isNull(rowData.CALC_QTY_DIV)) {
            alert($NC.getDisplayMsg("JS.LFC03090E0.013", "수량계산구분을 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "CALC_QTY_DIV_F", true);
            return false;
        }
        if ($NC.isNull(rowData.CALC_AMT_DIV)) {
            alert($NC.getDisplayMsg("JS.LFC03090E0.014", "금액계산구분을 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "CALC_AMT_DIV_F", true);
            return false;
        }
        if ($NC.isNull(rowData.CONTRACT_DATE)) {
            alert($NC.getDisplayMsg("JS.LFC03090E0.015", "계약시작일자을 선택하십시오."));
