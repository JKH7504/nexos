 ...
 *  프로그램ID         : LFC03060E0
 *  프로그램명         : 택배박스단가 기준관리
 *  프로그램설명       : 택배박스단가 기준관리 화면 Javascript
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "CONTRACT_MONTH":
            if ($NC.isNotNull(val)) {
                $NC.setValueMonthPicker(view, val, $NC.getDisplayMsg("JS.LFC03060E0.001", "계약월을 정확히 입력하십시오."));
            }
            break;
        case "CARRIER_CD":
            $NP.onCarrierChange(val, {
                P_CARRIER_CD: val,
                P_CARRIER_DIV: "1", // 1-국내택배사, 2-해외택배사, 3-운송사
                P_VIEW_DIV: "2" // 1:등록팝업, 2:조회팝업
            }, onQCarrierPopup);
            return;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNotNull(CONTRACT_MONTH)) {
        if (!$NC.isMonth(CONTRACT_MONTH)) {
            alert($NC.getDisplayMsg("JS.LFC03060E0.001", "계약월을 정확히 입력하십시오."));
    ...
    // 데이터 조회
    $NC.serviceCall("/LFC03060E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LFC03060E0.004", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LFC03060E0.005", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LFC03060E0/save.do", {
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LFC03060E0.006", "삭제할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LFC03060E0.007", "삭제 하시겠습니까?"))) {
        return;
    }
...
function grdMasterOnGetColumns() {
    ...
    $NC.setGridColumn(columns, {
        ...
        editorOptions: $NC.getGridComboEditorOptions("/WC/getDataSet.do", {
            ...
            P_QUERY_PARAMS: {
                P_COMMON_GRP: "WB_BOX_DIV",
                P_COMMON_CD: $ND.C_ALL,
                P_VIEW_DIV: "1" // 1:등록팝업, 2:조회팝업
            }
        }, {
            codeField: "WB_BOX_DIV",
            dataCodeField: "COMMON_CD",
            dataFullNameField: "COMMON_CD_F",
            isKeyField: true
        })
...
function grdMasterOnCarrierPopup(resultInfo) {
    ...
    if ($NC.isNotNull(resultInfo)) {
        ...
        focusCol = G_GRDMASTER.view.getColumnIndex("WB_BOX_DIV_F");
    } else {
        rowData.CARRIER_CD = "";
        rowData.CARRIER_NM = "";
        focusCol = G_GRDMASTER.view.getColumnIndex("CARRIER_CD");
        ...
        case "CONTRACT_DATE":
            if ($NC.isNotNull(rowData.CONTRACT_DATE)) {
                if (!$NC.isDate(rowData.CONTRACT_DATE)) {
                    alert($NC.getDisplayMsg("JS.LFC03060E0.008", "계약시작일자를 정확히 입력하십시오."));
...
function grdMasterOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONTRACT_NO)) {
            alert($NC.getDisplayMsg("JS.LFC03060E0.010", "계약번호를 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "CONTRACT_NO_F", true);
            return false;
        }
        if ($NC.isNull(rowData.CARRIER_CD)) {
            alert($NC.getDisplayMsg("JS.LFC03060E0.009", "운송사를 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "CARRIER_CD", true);
            return false;
        }
        if ($NC.isNull(rowData.WB_BOX_DIV)) {
            alert($NC.getDisplayMsg("JS.LFC03060E0.011", "택배박스구분을 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "WB_BOX_DIV_F", true);
            return false;
        }
        if ($NC.isNull(rowData.UNIT_DIV)) {
            alert($NC.getDisplayMsg("JS.LFC03060E0.012", "정산산정구분을 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "UNIT_DIV_F", true);
            return false;
        }
        if ($NC.isNull(rowData.COURIER_PRICE)) {
            alert($NC.getDisplayMsg("JS.LFC03060E0.013", "택배단가를 입력하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "COURIER_PRICE", true);
            return false;
        }
        if ($NC.isNull(rowData.CONSUMABLE_PRICE)) {
            alert($NC.getDisplayMsg("JS.LFC03060E0.013", "소모품단가를 입력하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "CONSUMABLE_PRICE", true);
            return false;
        }
        if ($NC.isNull(rowData.CALC_QTY_DIV)) {
            alert($NC.getDisplayMsg("JS.LFC03060E0.014", "수량계산구분을 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "CALC_QTY_DIV_F", true);
            return false;
        }
        if ($NC.isNull(rowData.CALC_AMT_DIV)) {
            alert($NC.getDisplayMsg("JS.LFC03060E0.015", "금액계산구분을 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "CALC_AMT_DIV_F", true);
            return false;
        }
        if ($NC.isNull(rowData.CONTRACT_DATE)) {
            alert($NC.getDisplayMsg("JS.LFC03060E0.016", "계약시작일자을 선택하십시오."));
