 ...
 *  프로그램ID         : LFC03070E0
 *  프로그램명         : 포장박스단가 기준관리
 *  프로그램설명       : 포장박스단가 기준관리 화면 Javascript
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "CONTRACT_MONTH":
            if ($NC.isNotNull(val)) {
                $NC.setValueMonthPicker(view, val, $NC.getDisplayMsg("JS.LFC03070E0.001", "계약월을 정확히 입력하십시오."));
            }
            break;
    }
...
function _Inquiry() {
    ...
    // 데이터 조회
    $NC.serviceCall("/LFC03070E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LFC03070E0.004", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LFC03070E0.005", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LFC03070E0/save.do", {
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LFC03070E0.006", "삭제할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LFC03070E0.007", "삭제 하시겠습니까?"))) {
        return;
    }
    ...
    switch (args.column.id) {
        ...
        case "BOX_SIZE_CD_F":
            $NP.showBillBoxPopup({
                ...
                P_BILL_DIV: rowData.BILL_DIV,
                P_BU_CD: $ND.C_ALL,
                P_BOX_SIZE_CD: $ND.C_ALL,
                P_VIEW_DIV: "1" // 1:등록팝업, 2:조회팝업
            }, grdMasterOnBillBoxPopup, function() {
                $NC.setFocusGrid(G_GRDMASTER, args.row, args.cell, true, true);
            });
            return;
    }
...
function grdMasterOnContractBillPopup(resultInfo) {
    ...
    if ($NC.isNotNull(resultInfo)) {
        ...
        focusCol = G_GRDMASTER.view.getColumnIndex("BOX_SIZE_CD_F");
    } else {
        rowData.CONTRACT_NO = "";
        rowData.CONTRACT_NM = "";
...
function grdMasterOnBillBoxPopup(resultInfo) {
    ...
    var focusCol;
    if ($NC.isNotNull(resultInfo)) {
        rowData.BOX_SIZE_CD = resultInfo.BOX_SIZE_CD;
        rowData.BOX_SIZE_NM = resultInfo.BOX_SIZE_NM;
        rowData.BOX_SIZE_CD_F = resultInfo.BOX_SIZE_CD_F;
...
function grdMasterOnCellChange(e, args) {
    ...
    switch (G_GRDMASTER.view.getColumnId(args.cell)) {
        case "CONTRACT_NO_F":
            ...
            return;
        case "BOX_SIZE_CD_F":
            $NP.onBillBoxChange(rowData.BOX_SIZE_CD_F, {
                P_CONTRACT_NO: rowData.CONTRACT_NO,
                P_BILL_DIV: rowData.BILL_DIV,
                P_BU_CD: $ND.C_ALL,
                P_BOX_SIZE_CD: rowData.BOX_SIZE_CD_F,
                P_VIEW_DIV: "1" // 1:등록팝업, 2:조회팝업
            }, grdMasterOnBillBoxPopup);
            return;
        case "CONTRACT_DATE":
            if ($NC.isNotNull(rowData.CONTRACT_DATE)) {
                if (!$NC.isDate(rowData.CONTRACT_DATE)) {
                    alert($NC.getDisplayMsg("JS.LFC03070E0.008", "계약시작일자를 정확히 입력하십시오."));
...
function grdMasterOnBeforeEditCell(e, args) {
    ...
    switch (args.column.id) {
        ...
        case "CONTRACT_DATE":
            return isNew;
        case "BOX_SIZE_CD_F":
            return isNew && $NC.isNotNull(rowData.CONTRACT_NO);
    }
    return true;
}
