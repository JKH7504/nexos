 ...
 *  프로그램ID         : LFC03050E0
 *  프로그램명         : 패킹단가 기준관리
 *  프로그램설명       : 패킹단가 기준관리 화면 Javascript
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "CONTRACT_MONTH":
            if ($NC.isNotNull(val)) {
                $NC.setValueMonthPicker(view, val, $NC.getDisplayMsg("JS.LFC03050E0.001", "계약월을 정확히 입력하십시오."));
            }
            break;
    }
...
function _Inquiry() {
    ...
    if ($NC.isNotNull(CONTRACT_MONTH)) {
        if (!$NC.isMonth(CONTRACT_MONTH)) {
            alert($NC.getDisplayMsg("JS.LFC03050E0.001", "계약월을 정확히 입력하십시오."));
    ...
    // 데이터 조회
    $NC.serviceCall("/LFC03050E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LFC03050E0.004", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    for (var rIndex = 0, rCount = G_GRDMASTER.data.getLength(); rIndex < rCount; rIndex++) {
        ...
        dsMaster.push({
            ...
            P_PACKING_PRICE: rowData.PACKING_PRICE,
            P_ITEM_QTY_PRICE: rowData.ITEM_QTY_PRICE,
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LFC03050E0.005", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LFC03050E0/save.do", {
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LFC03050E0.006", "삭제할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LFC03050E0.007", "삭제 하시겠습니까?"))) {
        return;
    }
        ...
        case "CONTRACT_DATE":
            if ($NC.isNotNull(rowData.CONTRACT_DATE)) {
                if (!$NC.isDate(rowData.CONTRACT_DATE)) {
                    alert($NC.getDisplayMsg("JS.LFC03050E0.008", "계약시작일자를 정확히 입력하십시오."));
...
function grdMasterOnBeforePost(row) {
    ...
    if (rowData.CRUD != $ND.C_DV_CRUD_R) {
        if ($NC.isNull(rowData.CONTRACT_NO)) {
            alert($NC.getDisplayMsg("JS.LFC03050E0.010", "계약번호를 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "CONTRACT_NO_F", true);
            return false;
        }
        if ($NC.isNull(rowData.UNIT_DIV)) {
            alert($NC.getDisplayMsg("JS.LFC03050E0.013", "단위구분을 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "UNIT_DIV_F", true);
            return false;
        }
        if ($NC.isNull(rowData.PACKING_PRICE)) {
            alert($NC.getDisplayMsg("JS.LFC03050E0.011", "패킹단가를 입력하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "PACKING_PRICE", true);
            return false;
        }
        if ($NC.isNull(rowData.ITEM_QTY_PRICE)) {
            alert($NC.getDisplayMsg("JS.LFC03050E0.012", "내품수량단가를 입력하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "ITEM_QTY_PRICE", true);
            return false;
        }
        if ($NC.isNull(rowData.CALC_QTY_DIV)) {
            alert($NC.getDisplayMsg("JS.LFC03050E0.014", "수량계산구분을 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "CALC_QTY_DIV_F", true);
            return false;
        }
        if ($NC.isNull(rowData.CALC_AMT_DIV)) {
            alert($NC.getDisplayMsg("JS.LFC03050E0.015", "금액계산구분을 선택하십시오."));
            $NC.setFocusGrid(G_GRDMASTER, row, "CALC_AMT_DIV_F", true);
            return false;
        }
        if ($NC.isNull(rowData.CONTRACT_DATE)) {
            alert($NC.getDisplayMsg("JS.LFC03050E0.016", "계약시작일자을 선택하십시오."));
