 ...
 *  프로그램ID         : LFC04010E0
 *  프로그램명         : 일기타수수료 등록
 *  프로그램설명       : 일기타수수료 등록 화면 Javascript
...
function _Initialize() {
    ...
    // 조회조건 - 정산일자 세팅
    $NC.setInitDateRangePicker("#dtpQInout_Date1", "#dtpQInout_Date2", $NC.getLastDate($NC.G_USERINFO.LOGIN_DATE), "CM");
...
function _OnConditionChange(e, view, val) {
    ...
    switch (id) {
        ...
        case "BU_CD":
            ...
            return;
        case "INOUT_DATE1":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LFC04010E0.001", "정산 시작일자를 정확히 입력하십시오."));
            break;
        case "INOUT_DATE2":
            $NC.setValueDatePicker(view, val, $NC.getDisplayMsg("JS.LFC04010E0.002", "정산 종료일자를 정확히 입력하십시오."));
            break;
        case "CONTRACT_NO":
...
function _Inquiry() {
    ...
    if ($NC.isNull(CENTER_CD)) {
        alert($NC.getDisplayMsg("JS.LFC04010E0.003", "물류센터를 선택하십시오."));
    ...
    if ($NC.isNull(BU_CD)) {
        alert($NC.getDisplayMsg("JS.LFC04010E0.004", "사업부를 입력하십시오."));
    ...
    if ($NC.isNull(INOUT_DATE1)) {
        alert($NC.getDisplayMsg("JS.LFC04010E0.005", "정산 시작일자를 입력하십시오."));
        $NC.setFocus("#dtpQInout_Date1");
        return;
    }
    var INOUT_DATE2 = $NC.getValue("#dtpQInout_Date2");
    if ($NC.isNull(INOUT_DATE2)) {
        alert($NC.getDisplayMsg("JS.LFC04010E0.006", "정산 종료일자를 입력하십시오."));
    ...
    // 데이터 조회
    $NC.serviceCall("/LFC04010E0/getDataSet.do", $NC.getGridParams(G_GRDMASTER), onGetMaster);
}
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.LFC04010E0.007", "저장할 데이터가 없습니다."));
        return;
    }
    ...
    if (dsMaster.length == 0) {
        alert($NC.getDisplayMsg("JS.LFC04010E0.008", "데이터 수정 후 저장하십시오."));
        return;
    }

    $NC.serviceCall("/LFC04010E0/save.do", {
...
function _Delete() {
    ...
    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.LFC04010E0.009", "삭제할 데이터가 없습니다."));
        return;
    }

    if (!confirm($NC.getDisplayMsg("JS.LFC04010E0.010", "삭제 하시겠습니까?"))) {
        return;
    }
    ...
    if (rowData.PROTECT_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.LFC04010E0.011", "자료 보안설정이 되어 있어 처리할 수 없습니다."));
        return;
    }
        ...
        case "INOUT_DATE":
            if ($NC.isNotNull(rowData.INOUT_DATE)) {
                if (!$NC.isDate(rowData.INOUT_DATE)) {
                    alert($NC.getDisplayMsg("JS.LFC04010E0.012", "정산일자를 정확히 입력하십시오."));
                    rowData.INOUT_DATE = "";
                    $NC.setFocusGrid(G_GRDMASTER, args.row, args.cell, true);
                } else {
                    rowData.INOUT_DATE = rowData.INOUT_DATE;
                    $NC.serviceCallAndWait("/LFC04010E0/getDataSet.do", {
                    ...
                    $NC.serviceCallAndWait("/LFC04010E0/getDataSet.do", {
                        ...
                        if (dsResult[0].EXPENSES_YN != $ND.C_YES && dsResult.length == 0) {
                            alert($NC.getDisplayMsg("JS.LFC04010E0.013", "일기타수수료 단가관리에 등록되어 있지 않습니다."));
                            rowData.INOUT_DATE = "";
                            rowData.UNIT_DIV = "";
                            rowData.UNIT_DIV_F = "";
                            rowData.UNIT_PRICE = null;
                            ...
                            $NC.setFocusGrid(G_GRDMASTER, args.row, "ETC_DAILY_DIV_F", true);
                        }
                    });
                }
            }
...
function grdMasterOnBeforeEditCell(e, args) {
    ...
    if (rowData.EXPENSES_YN == $ND.C_YES) {
        // 일실비 데이터인 경우 정산수량컬럼 입력 제한
        switch (args.column.id) {
            case "BILL_QTY":
                return false;
        }
    } else {
        // 일실비 데이터가 아닌 경우 단위구분, 정산금액 입력 제한
        switch (args.column.id) {
            case "UNIT_DIV_F":
            case "BILL_AMT":
                return false;
...
function onGetProtectInfo(ajaxData) {
    ...
    if (dsResult.length > 0) {
        ...
        if (rowData.PROTECT_YN == $ND.C_YES) {
            alert($NC.getDisplayMsg("JS.LFC04010E0.022", "자료 보안설정이 되어 있어 처리할 수 없습니다."));
...
function onSetBillComboInfo() {
    ...
    // 조회조건 - 수수료 구분
    $NC.setInitCombo("/LFC04010E0/getDataSet.do", {
        P_QUERY_ID: "LFC04010E0.RS_SUB1",
        P_QUERY_PARAMS: {
            P_CENTER_CD: $NC.getValueCombo("#cboQCenter_Cd"),
            P_BU_CD: $NC.getValue("#edtQBu_Cd"),
