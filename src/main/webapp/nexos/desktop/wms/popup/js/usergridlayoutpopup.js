...
function _Initialize() {
    ...
    $NC.setTooltip("#btnUp", $NC.getDisplayMsg("JS.USERGRIDLAYOUT.001", "위로 이동"));
    $NC.setTooltip("#btnDown", $NC.getDisplayMsg("JS.USERGRIDLAYOUT.002", "아래로 이동"));
...
function _Save() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.USERGRIDLAYOUT.003", "[그리드: " + $NC.G_VAR.G_PARAMETER.P_GRID_NM
    ...
    if (rowCount == 0) {
        alert($NC.getDisplayMsg("JS.USERGRIDLAYOUTPOPUP.004", "그리드의 항목을 모두 제거할 수 없습니다."));
        return;
    }
    ...
    // alert($NC.getDisplayMsg("JS.USERGRIDLAYOUTPOPUP.005", "고정을 마지막 항목에 지정할 수 없습니다."));
    // return;
    // }
...
function btnInitializeProgramOnClick() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.USERGRIDLAYOUT.006", "[프로그램: " + $NC.G_VAR.G_PARAMETER.P_PROGRAM_NM
...
function btnInitializeGridOnClick() {
    ...
    if (!confirm($NC.getDisplayMsg("JS.USERGRIDLAYOUT.007", "[그리드: " + $NC.G_VAR.G_PARAMETER.P_GRID_NM
...
function btnDeleteOnClick() {
    ...
    if (rowCount == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        // alert($NC.getDisplayMsg("JS.USERGRIDLAYOUTPOPUP.008", "삭제할 항목 데이터가 없습니다."));
        return;
    }

    if (rowCount == 1) {
        alert($NC.getDisplayMsg("JS.USERGRIDLAYOUTPOPUP.009", "그리드의 항목을 모두 제거할 수 없습니다."));
        return;
    }

    var rowData = G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow);
    if (rowData.REQUIRED_COL_YN == $ND.C_YES) {
        alert($NC.getDisplayMsg("JS.USERGRIDLAYOUTPOPUP.010", "필수 입력 항목입니다. 항목을 제거할 수 없습니다."));
        return;
    }
...
function btnAddOnClick() {
    ...
    if (G_GRDDETAIL.data.getLength() == 0 || $NC.isNull(G_GRDDETAIL.lastRow)) {
        // alert($NC.getDisplayMsg("JS.USERGRIDLAYOUTPOPUP.011", "추가할 항목 데이터가 없습니다."));
        return;
    }
...
function setGridColumnInfo() {
    ...
    columnNoName = $NC.getDisplayMsg("JS.USERGRIDLAYOUTPOPUP.012", "[미지정]"), //
