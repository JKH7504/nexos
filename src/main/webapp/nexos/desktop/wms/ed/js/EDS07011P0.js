 ...
 *  ==================================================================================================================================================
 *  프로그램ID         : EDS07011P0
 *  프로그램명         : 기타입출고확정미송신내역 팝업
 *  프로그램설명       : 기타입출고확정미송신내역 팝업 화면 Javascript
...
function btnSendOnClick() {
    ...
    if (!$NC.G_VAR.G_PARAMETER.P_PERMISSION.canSave) {
        alert($NC.getDisplayMsg("JS.EDS07011P0.002", "해당 프로그램의 저장권한이 없습니다."));
        return;
    }

    if (G_GRDMASTER.data.getLength() == 0 || $NC.isNull(G_GRDMASTER.lastRow)) {
        alert($NC.getDisplayMsg("JS.EDS07011P0.003", "송신처리할 기타입출고일자를 선택하십시오."));
        return;
    }

    var refRowData = $NC.getGridItemFromGroup(G_GRDMASTER.data.getItem(G_GRDMASTER.lastRow), G_GRDMASTER);
    if (!confirm($NC.getDisplayMsg("JS.EDS07011P0.004", "기타입출고일자: " + refRowData.INOUT_DATE + "\n\n해당 기타입출고일자의 미송신 데이터를 송신하시겠습니까?",
        refRowData.INOUT_DATE))) {
        return;
    }
