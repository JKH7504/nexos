 ...
 *  프로그램ID         : EDR01014P0
 *  프로그램명         : 사업부수신관리복사 팝업
 *  프로그램설명       : 사업부수신관리복사 팝업 화면 Javascript
...
function _Initialize() {
    ...
    $NC.setInitComboData({
        ...
        data: [
            {
                COMMON_CD: "1",
                COMMON_NM: $NC.getDisplayMsg("JS.EDR01014P0.002", "삭제 후 생성")
            },
            {
                COMMON_CD: "2",
                COMMON_NM: $NC.getDisplayMsg("JS.EDR01014P0.003", "미존재 수신정의만 생성")
            }
        ],
        onComplete: function() {
...
function _Save() {
    ...
    if (G_GRDMASTER.data.getLength() == 0) {
        alert($NC.getDisplayMsg("JS.EDR01014P0.004", "조회 후 처리하십시오."));
        return;
    }
    ...
    if ($NC.isNull(TO_BU_CD)) {
        alert($NC.getDisplayMsg("JS.EDR01014P0.005", "사업부를 입력하십시오."));
        $NC.setFocus("#edtTo_Bu_Cd");
        return;
    }

    if (TO_BU_CD == $NC.G_VAR.G_PARAMETER.P_BU_CD) {
        alert($NC.getDisplayMsg("JS.EDR01014P0.006", "기준 사업부와 대상 사업부가 같습니다. 다른 사업부를 선택하십시오."));
        $NC.setFocus("#edtTo_Bu_Cd");
        return;
    }
    ...
    if (COPY_DIV == 1) {
        ...
        if (checkedData.checkedCount == 0) {
            alert($NC.getDisplayMsg("JS.EDR01014P0.007", "복사할 수신정의를 선택하십시오."));
            return;
        }

        if (!confirm($NC.getDisplayMsg("JS.EDR01014P0.008", "선택한 수신정의 데이터를 복사하시겠습니까?"))) {
            return;
        }
    }
    // 번호변경 복사
        ...
        if ($NC.isNull(rowData)) {
            alert($NC.getDisplayMsg("JS.EDR01014P0.009", "복사할 정의번호를 선택하십시오."));
            return;
        }

        NEW_DEFINE_NO = $NC.getValue("#edtDefine_No");
        if ($NC.isNull(NEW_DEFINE_NO)) {
            alert($NC.getDisplayMsg("JS.EDR01014P0.010", "정의번호를 입력하십시오."));
        ...
        if ($NC.isNull(NEW_DEFINE_NM)) {
            alert($NC.getDisplayMsg("JS.EDR01014P0.011", "정의명을 입력하십시오."));
            $NC.setFocus("#edtDefine_No");
            return;
        }
        ...
        if (!confirm($NC.getDisplayMsg("JS.EDR01014P0.012", "수신구분: " + rowData.EDI_DIV + "\n수신정의번호: " + rowData.DEFINE_NO
    ...
    $NC.serviceCall("/EDR01010E0/callEMDefineRecvCopy.do", {
        P_BU_CD: $NC.G_VAR.G_PARAMETER.P_BU_CD,
        P_TO_BU_CD: TO_BU_CD,
        P_COPY_DIV: COPY_DIV,
