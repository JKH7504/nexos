...
import nexos.dao.ri.RIM01010E0DAO;
...
/**
 * Class: RIM01010E0Service<br>
 * Description: 온라인 반입예정작업(RIM01010E0) 서비스를 담당하는 Class(트랜잭션처리 담당)<br>
 ...
 */
@Service
public class RIM01010E0Service extends ServiceSupport {
    ...
    final String          SP_ID_RI_ORDER_CREATE      = "RI_ORDER_CREATE";

    @Autowired
    private RIM01010E0DAO dao;
     ...
     */
    @SuppressWarnings("unchecked")
    public String callRIOrderCreate(Map<String, Object> params) throws Exception {
        ...
        for (int rIndex = 0, rCount = dsMaster.size(); rIndex < rCount; rIndex++) {
            ...
            try {
                Map<String, Object> resultMap = callProcedure(SP_ID_RI_ORDER_CREATE, rowData);
                String oMsg = Util.getOutMessage(resultMap);
                if (!Consts.OK.equals(oMsg)) {
                    rollbackTrans(ts);
